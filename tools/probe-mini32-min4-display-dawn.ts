...
await acquireWebGPUExclusiveLock("dawn-probe", "mini32-min4-display");
const errors: string[] = [];
...
try {
  ...
  scene.fluid.refinementRegions = [{
    id: "display-min4", rule: "minimum-cell-size", minimumCellSize_cells: 4,
  ...
  for (let step = 1; step <= Number(process.env.DISPLAY_STEPS ?? 32); step++) {
    let retries = 0;
    ...
    if (![1, 8, 16, 32, Number(process.env.DISPLAY_STEPS ?? 32)].includes(step))
      continue;
    const output = `${process.env.DISPLAY_OUTPUT ?? "artifacts/mini32-min4-display"}/step-${step}`;
