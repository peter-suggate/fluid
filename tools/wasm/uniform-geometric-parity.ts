...
try {
  ...
  for(const name of ["stationary-pool","hydrostatic-pool","dam-collapse","ceiling-release","open-top","tall-pool","empty","full","mirrored-dam","embedded-ceiling","odd-pool"]){
    if(process.env.FLUID_UNIFORM_PARITY_SCENE && name!==process.env.FLUID_UNIFORM_PARITY_SCENE)continue;
    const expectedNx=name==="odd-pool"?15:16;
    const expectedNy=name==="tall-pool"?64:16;
    const scene=structuredClone(sceneDocument(getSceneDefinition("minimal-power-dam-break-32")));
    scene.container.width_m=expectedNx*0.05;scene.container.height_m=name==="tall-pool"?3.2:0.8;scene.container.depth_m=0.05;scene.container.depthBoundary="symmetry";scene.container.top=name==="open-top"?"open":"closed";scene.container.fillFraction=name==="empty"?0:name==="full"?1:name==="tall-pool"?0.125:0.5;
    ...
    try {
      ...
      if(["dam-collapse","mirrored-dam","ceiling-release","embedded-ceiling"].includes(name)){
        ...
        for(let z=0;z<nz;z++)for(let y=0;y<ny;y++)for(let x=0;x<nx;x++)v[x+nx*(y+ny*z)]=name==="ceiling-release"?(x>=4&&x<12&&y>=12?1:0):name==="embedded-ceiling"?(x>=4&&x<12&&y>=8&&y<12?1:0):(name==="mirrored-dam"?x>=10:x<6)&&y<12?1:0;
        for(let z=0;z<=nz;z++)for(let y=0;y<=ny;y++)for(let x=0;x<=nx;x++)phi[x+(nx+1)*(y+(ny+1)*z)]=(name==="ceiling-release"?Math.max(4-x,x-12,12-y):name==="embedded-ceiling"?Math.max(4-x,x-12,8-y,y-12):Math.max(name==="mirrored-dam"?10-x:x-6,y-12))*0.05;
      ...
      for(let frame=1;frame<=30;frame++){
        const selected=[1,5,30].includes(frame);
        const before: {volume:number[];phi:number[];velocity:number[];boundary:number[]}|null=selected?{
        ...
        if(![1,5,30].includes(frame))continue;
        ...
        writeFileSync(`/tmp/fluid-uniform-${name}-${frame}.json`,JSON.stringify({input:matchedInput,gpu:{volume,phi,velocity,released,advectedPhi,advectedPhi3d},rust:one,redistance}));
        ...
        if(tileSource) {
          const records=new Float32Array(await readBuffer(device,tileSource.buffer,tileSource.size!,tileSource.offset??0));
          const words=new Uint32Array(records.buffer);
          const classes=[...words].filter((_,i)=>i%4===3);
          tileMismatches=classes.filter((value,i)=>value!==one.tileClasses[i]).length;
