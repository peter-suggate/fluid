  ...
  try {
    ...
    for (let step = 1; step <= 30; step++) {
      ...
      console.log(JSON.stringify({ probe: "cm12-full-pool-advance", step,
        milliseconds: performance.now() - start, ...process.memoryUsage(),
        topologyGeneration: solver.info.topologyGenerationCount,
        topologyError: solver.info.topologyGenerationError }));
    }
