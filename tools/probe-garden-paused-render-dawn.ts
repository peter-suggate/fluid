...
const depth=Number(process.env.FLUID_PROBE_DEPTH??0), id=process.env.FLUID_PROBE_SCENE??"hero-garden-hose";
...
const steadyFrames=Number(process.env.FLUID_PROBE_STEADY_FRAMES??20);
if(steadyFrames>0){
 const elapsed:number[]=[];
 for(let frame=0;frame<steadyFrames+4;frame++){
  const start=performance.now(),encoder=device.createCommandEncoder();
  assert.ok(renderer.encode(encoder,target));device.queue.submit([encoder.finish()]);
  await device.queue.onSubmittedWorkDone();if(frame>=4)elapsed.push(performance.now()-start);
 }
 elapsed.sort((a,b)=>a-b);
 console.log(JSON.stringify({phase:"steady-paused-render",width,height,frames:steadyFrames,
  median_ms:elapsed[Math.floor(elapsed.length/2)],p95_ms:elapsed[Math.floor(elapsed.length*.95)]}));
 const encoder=device.createCommandEncoder(),recorder=new GPUPassTimestampRecorder(device,128,"Paused render stages");
 assert.ok(renderer.encode(recorder.instrument(encoder),target));recorder.resolve(encoder);
 device.queue.submit([encoder.finish()]);await device.queue.onSubmittedWorkDone();
 console.log(JSON.stringify({phase:"steady-paused-stages",reading:await recorder.read()}));
