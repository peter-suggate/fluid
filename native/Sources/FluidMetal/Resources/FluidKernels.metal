...
kernel void clearRigidExchange(device atomic_int*exchange[[buffer(0)]],constant Params&p[[buffer(1)]],uint gid[[thread_position_in_grid]]){
    if(gid>=uint(p.boundary.z))return;int base=int(gid)*8;for(int i=0;i<8;i++)atomic_store_explicit(&exchange[base+i],0,memory_order_relaxed);
}
...
kernel void integrateRigidBodies(device BodyGPU*bodies[[buffer(0)]],device atomic_int*exchange[[buffer(1)]],constant Params&p[[buffer(2)]],uint gid[[thread_position_in_grid]]){
    if(gid>=uint(p.boundary.z))return;BodyGPU body=bodies[gid];if(body.angularVelocity.w>.5f){bodies[gid]=body;return;}int base=int(gid)*8;float3 impulse=float3(atomic_exchange_explicit(&exchange[base],0,memory_order_relaxed),atomic_exchange_explicit(&exchange[base+1],0,memory_order_relaxed),atomic_exchange_explicit(&exchange[base+2],0,memory_order_relaxed))/1e6f;float3 angularImpulse=float3(atomic_exchange_explicit(&exchange[base+3],0,memory_order_relaxed),atomic_exchange_explicit(&exchange[base+4],0,memory_order_relaxed),atomic_exchange_explicit(&exchange[base+5],0,memory_order_relaxed))/1e6f;float dt=p.dimsDt.w,invMass=body.inverseMassRestitutionFriction.x,restitution=body.inverseMassRestitutionFriction.y,friction=body.inverseMassRestitutionFriction.z;
