...
const solver: any = tallCellMethod.createSolver!(device, scene, "balanced", { ...tallCellMethod.presetFor("balanced"), ...(process.env.PROBE_CYCLES ? { pressureCycles: Number(process.env.PROBE_CYCLES) } : {}) });
...
await readMax((solver as any).velocityA ?? solver.volumeTexture, (solver as any).velocityA ? 4 : 1, "velocityA-or-vol");
