...
export function losassoStepSnapshotFailures(
  ...
  const { authority, solver, fine, coarsePhi, extension, fineTransport } = record;
  const failures: string[] = [];
  const epoch = authority[0] ?? 0, rows = authority[1] ?? 0, faces = authority[2] ?? 0;
  const generation = fine[0] ?? 0;
  ...
  if (generation === 0 || generation === 0xffff_ffff || fine[1] === 0
  ...
  if (fineTransport[1] !== 0 || fineTransport[3] !== 1
...
export class WebGPUOctreeLosassoStepSnapshotRing {
  ...
  encode(encoder: GPUCommandEncoder, sources: LosassoStepSnapshotSources, step: number): boolean {
    ...
    copy(sources.fineWorklist, LAYOUT.fine, 28);
    copy(sources.coarsePhi, LAYOUT.coarsePhi, 60);
    copy(sources.extension, LAYOUT.extension, 32);
    copy(sources.fineTransport, LAYOUT.fineTransport, 32);
    slot.sequence = ++this.sequence; slot.step = step; slot.state = "encoded";
    return true;
  }
  ...
  async readLatest(): Promise<LosassoStepSnapshotRecord | undefined> {
    ...
    try {
      ...
      return Object.freeze({ step: slot.step,
