...
export class WebGPUUniformEulerianSolver {
  ...
  constructor(
    ...
    const c = scene.container, matched = createTallCellLayout(scene, quality, device.limits.maxTextureDimension3D);
    ...
    const pressureIterations = quality === "balanced" ? 64 : quality === "high" ? 80 : 96;
  ...
  private initializeVolume() {
    ...
    Object.assign(this.info, { initialVolumeCellSum: initialSum, volumeCellSum: initialSum, volumeDrift: 0, rawVolumeDrift: 0, maxSpeed_m_s: 0, front_m: this.scene.fluid.initialCondition === "dam-break" ? -c.width_m / 2 + dam.width * c.width_m : c.width_m / 2 });
  ...
  advanceTo(time_s: number, bodies: RigidBodyState[] = []) {
    ...
    if (!this.wallTimingPending) { this.wallTimingPending = true; void this.device.queue.onSubmittedWorkDone().then(() => { this.info.gpuQueueWall_ms = performance.now() - submittedAt; this.info.gpuQueueSimulation_s = delta; }).finally(() => { this.wallTimingPending = false; }); }
    ...
    if (!this.validationChecked) { this.validationChecked = true; void this.device.popErrorScope().then((error) => { if (error) console.error(`Uniform GPU validation: ${error.message}`); }); }
    return true;
  }
  ...
  async readStats() {
    ...
    this.info.volumeCellSum = words[0] / 2048; this.info.volumeDrift = (this.info.volumeCellSum - initial) / initial; this.info.rawVolumeDrift = this.info.volumeDrift;
    ...
    if (queryBytes > 0) { const times = new BigUint64Array(buffer.getMappedRange(16, queryBytes)); const timings = { advection_ms: 0, pressure_ms: 0, projection_ms: 0, rigidCoupling_ms: 0, diagnostics_ms: 0, overhead_ms: 0, total_ms: 0 }; for (const segment of querySegments) timings[segment.name] += Number(times[segment.end] - times[segment.start]) / 1e6; const categorized = timings.advection_ms + timings.pressure_ms + timings.projection_ms + timings.rigidCoupling_ms + timings.diagnostics_ms; timings.overhead_ms = Math.max(0, timings.total_ms - categorized); this.info.gpuTimings = timings; this.info.gpuStep_ms = timings.total_ms; }
