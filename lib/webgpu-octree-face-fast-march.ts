...
fn locateFinalPointVectorMeasured(initialAnchor:u32,pointGrid:vec3f)->LocatedFinalPointVector{let direct=finalPointVector(initialAnchor,pointGrid);if(velocityValid(direct)){return LocatedFinalPointVector(direct,1u,0u,0u,0u,0u);}let count=min(p.rowCapacity,min(arrayLength(&rows),arrayLength(&metrics)));var containedInvalid=vec4f(0.);var candidateRowsTested=0u;for(var candidate=0u;candidate<count;candidate+=1u){candidateRowsTested+=1u;if(candidate==initialAnchor||(rows[candidate].flags&ROW_SUPPORT3_ENDPOINT)!=0u){continue;}let value=finalPointVector(candidate,pointGrid);if(velocityValid(value)){return LocatedFinalPointVector(value,0u,1u,candidateRowsTested,0u,0u);}if(u32(round(max(0.,-value.w)))==9u&&containedInvalid.w==0.){containedInvalid=value;}}let local=surroundingOwnerDelaunayVectorMeasured(pointGrid);if(velocityValid(local.value)){return LocatedFinalPointVector(local.value,0u,1u,candidateRowsTested,1u,local.rowsTested);}return LocatedFinalPointVector(select(direct,containedInvalid,containedInvalid.w<0.),0u,1u,candidateRowsTested,1u,local.rowsTested);}
