...
export const OCTREE_FACE_BAND_OWNER_FAILURE_STAGE = Object.freeze({
  ...
  acuteGrading: 6, support4: 8, descriptor: 9,
} as const);
    ...
    switch (phase) {
      ...
      case "transition-adjacency":
        ...
        computePass("Build Section 5 local Delaunay face adjacency", (pass) => {
          ...
          0, pass, 60);
          run("resolveSupportOwners", resolveOwnerBindings, candidateWorkgroups, pass);
          run("insertSupport4", supportBindings, candidateWorkgroups, pass);
          run("recordBandPhiEdges", recordPhiEdgeBindings, candidateWorkgroups, pass);
          run("captureSupport4", [[0, this.params], [5, this.control], [32, this.transitionControl]], 1, pass);
          ...
          run("enumerateCatalogEndpoints", [[6, this.rows], [27, this.transitionMetrics],
            ...
            [43, this.guardCandidates]], Math.ceil(this.plan.rowCapacity / 64), pass);
          run("resolveSupportOwners", resolveOwnerBindings, candidateWorkgroups, pass);
          run("insertSupport5", supportBindings, candidateWorkgroups, pass);
          run("recordBandPhiEdges", recordPhiEdgeBindings, candidateWorkgroups, pass);
          run("captureSupport5", [[0, this.params], [5, this.control], [32, this.transitionControl]], 1, pass);
          ...
          run("transitionDeep", [[0, this.params], [5, this.control], [6, this.rows], [7, this.rowHash],
            [26, input.owners], [27, this.transitionMetrics], [28, tetrahedronHeaders], [29, tetrahedra],
            [30, tetrahedronVertices], [31, this.transitionAdjacency], [32, this.transitionControl]],
          Math.ceil(this.plan.metricRowCapacity / 64), pass);
          run("transitionSupport5", [[0, this.params], [5, this.control], [6, this.rows], [7, this.rowHash],
            [26, input.owners], [27, this.transitionMetrics], [28, tetrahedronHeaders], [29, tetrahedra],
            [30, tetrahedronVertices], [31, this.transitionAdjacency], [32, this.transitionControl]],
          Math.ceil(this.plan.metricRowCapacity / 64), pass);
          run("transitionSupport6", [[0, this.params], [5, this.control], [6, this.rows], [7, this.rowHash],
            [26, input.owners], [27, this.transitionMetrics], [28, tetrahedronHeaders], [29, tetrahedra],
            [30, tetrahedronVertices], [31, this.transitionAdjacency], [32, this.transitionControl]],
          Math.ceil(this.plan.metricRowCapacity / 64), pass);
          ...
          run("sampleFaceCoarsePhi", [[0, this.params], [5, this.control], [6, this.rows], [7, this.rowHash], [12, this.faces],
            [27, this.transitionMetrics], [28, tetrahedronHeaders], [29, tetrahedra],
            [30, tetrahedronVertices], [31, this.transitionAdjacency]],
...
fn strictlyObtuseCoarseMask(mask:u32)->bool{return mask==25u||mask==42u||mask==52u||mask==57u||mask==58u||mask==60u;}
...
fn recordAcuteGradingFailure(band:u32,row:Row,descriptor:u32,mask:u32){var claimed=false;loop{let result=atomicCompareExchangeWeak(&transitionControl.failureBand,INVALID,band);if(result.exchanged){claimed=true;break;}if(result.old_value!=INVALID){break;}}if(!claimed){return;}let origin=coord(row.cell);transitionControl.failureStage=OWNER_FAILURE_ACUTE_GRADING;transitionControl.failureRowCell=row.cell;transitionControl.failureRowSize=row.size;transitionControl.failureDescriptor=descriptor;transitionControl.failureTopology=INVALID;transitionControl.failureTransformFlags=0u;transitionControl.failureSelector=mask;transitionControl.failureRawX=origin.x;transitionControl.failureRawY=origin.y;transitionControl.failureRawZ=origin.z;transitionControl.failureRequestedSize=row.size;transitionControl.failureResolvedCell=INVALID;transitionControl.failureBoundaryFlips=0u;transitionControl.failureOwnerCell=INVALID;transitionControl.failureOwnerSizeValid=0u;}
...
@compute @workgroup_size(1)fn captureSupport3EndpointBoundary(){transitionControl.endpointEnd=captureBoundary(transitionControl.support7NodeEnd);if(transitionControl.endpointEnd!=transitionControl.support7NodeEnd){transitionFail(TRANSITION_CAPACITY,transitionControl.endpointEnd);return;}atomicStore(&transitionControl.hierarchyReady,select(0u,VALID,atomicLoad(&transitionControl.flags)==0u));}
...
@compute @workgroup_size(64)fn emitDeepBandFaces(@builtin(global_invocation_id)g:vec3u){let band=transitionControl.support2End+g.x;if(band>=transitionControl.endpointEnd||band>=arrayLength(&rows)||atomicLoad(&control.flags)!=0u){return;}let row=rows[band];let endpointOnly=(row.flags&ROW_SUPPORT3_ENDPOINT)!=0u;let origin=coord(row.cell);for(var axis=0u;axis<3u;axis+=1u){let boundary=origin[axis]+row.size;if(boundary>=p.dims[axis]){if(boundary==p.dims[axis]&&(p.closedBoundaryMask&positiveBoundaryBit(axis))==0u&&!endpointOnly){let slot=band*p.ownedFacesPerRow+axis*4u;if(slot>=p.faceCapacity||slot>=arrayLength(&faces)){fail(CAPACITY,row.cell);return;}var centroid=vec3f(origin)+.5*f32(row.size);centroid[axis]=f32(boundary);var keyCell=origin;keyCell[axis]=boundary-1u;let stable=3u*cell(keyCell)+axis;faces[slot]=Face(band,INVALID,axis|(row.size<<2u),stable,vec4f(0),vec4f(centroid,1.),0.,f32(row.size*row.size),LIVE,0u);appendIncidence(band,slot);atomicAdd(&control.faceCount,1u);}continue;}let ta=(axis+1u)%3u;let tb=(axis+2u)%3u;let sampleCount=select(1u,4u,row.size>1u);let half=max(1u,row.size/2u);var emitted:array<u32,4>;for(var local=0u;local<sampleCount;local+=1u){var probe=origin;probe[axis]=boundary;probe[ta]+=select(0u,half,(local&1u)!=0u);probe[tb]+=select(0u,half,(local&2u)!=0u);let neighborOwner=ownerAt(probe);if(neighborOwner.valid==0u){fail(BAD_ROW,cell(probe));return;}let neighbor=rowOf(cell(neighborOwner.origin));if(neighbor==INVALID||neighbor>=transitionControl.endpointEnd||neighbor>=arrayLength(&rows)){if(!endpointOnly){adjacencyFail(band,DETAIL_MISSING_ROW);fail(BAD_ROW,row.cell);}continue;}if(neighbor<transitionControl.support2End){continue;}let other=rows[neighbor];if(endpointOnly&&(other.flags&ROW_SUPPORT3_ENDPOINT)!=0u){continue;}var duplicate=false;for(var prior=0u;prior<local;prior+=1u){duplicate=duplicate||emitted[prior]==neighbor+1u;}if(duplicate){continue;}emitted[local]=neighbor+1u;let otherOrigin=coord(other.cell);let low=max(origin,otherOrigin);let high=min(origin+vec3u(row.size),otherOrigin+vec3u(other.size));if(high[ta]<=low[ta]||high[tb]<=low[tb]){fail(BAD_FACE,row.cell);return;}let area=f32(high[ta]-low[ta])*f32(high[tb]-low[tb]);var centroid=.5*(vec3f(low)+vec3f(high));centroid[axis]=f32(boundary);var keyCell=low;keyCell[axis]=boundary-1u;let stable=3u*cell(keyCell)+axis;let slot=band*p.ownedFacesPerRow+axis*4u+local;if(slot>=p.faceCapacity||slot>=arrayLength(&faces)){fail(CAPACITY,stable);return;}faces[slot]=Face(band,neighbor,axis|(min(row.size,other.size)<<2u),stable,vec4f(0),vec4f(centroid,1.),0.,area,LIVE,0u);appendIncidence(band,slot);appendIncidence(neighbor,slot);atomicAdd(&control.faceCount,1u);}}}
...
// the liquid and must not overwrite the projected value of a face strictly
// interior to the liquid. A band row is wet when its marched centre phi is
// negative — the same centre-sign convention the pressure solve uses — and a
// face with BOTH incident centres wet keeps the divergence-free projected
// velocity: overwriting interior faces with a cell-averaged reconstruction
// re-applies a [1 2 1]/4 smoother to the near-surface field every step and
// visibly damps sloshing. Faces with any air-centred side (including the
// interface-crossing faces) still take the marched value, which also resets
// the theta-amplified ghost gradients the projection leaves at the free
// surface; preserving those raw values is unstable.
...
fn finalSignedVector(origin:vec3i,size:u32)->vec4f{let reflected=velocityExtendedOrigin(origin,size);if(reflected.w<0){return vec4f(0);}let q=vec3u(reflected.xyz);var value=supportCellVector(q,size);
 ...
 let outside=any(origin<vec3i(0))||any(origin+vec3i(i32(size))>vec3i(p.dims));if(!velocityValid(value)&&outside){value=supportContainingVector(q);}return reflectComponents(value,reflected.w);}
...
fn surroundingOwnerDelaunayVector(pointGrid:vec3f)->vec4f{
 let low=vec3i(floor(pointGrid-vec3f(.5)));var ids:array<u32,8>;for(var i=0u;i<8u;i+=1u){ids[i]=INVALID;}var count=0u;
 let rowCount=min(p.rowCapacity,min(arrayLength(&rows),arrayLength(&rowVelocities)));for(var corner=0u;corner<8u;corner+=1u){let probe=low+vec3i(i32(corner&1u),i32((corner>>1u)&1u),i32((corner>>2u)&1u));if(any(probe<vec3i(0))||any(probe>=vec3i(p.dims))){continue;}var row=INVALID;for(var candidate=0u;candidate<rowCount;candidate+=1u){let r=rows[candidate];if(r.size==0u||(r.flags&ROW_SUPPORT3_ENDPOINT)!=0u){continue;}let origin=vec3i(coord(r.cell));if(all(probe>=origin)&&all(probe<origin+vec3i(i32(r.size)))){row=candidate;break;}}if(row==INVALID||!velocityValid(rowVelocities[row])){return invalidPointVector(15u);}var duplicate=false;var insert=count;for(var prior=0u;prior<count;prior+=1u){if(ids[prior]==row){duplicate=true;break;}if(insert==count&&row<ids[prior]){insert=prior;}}if(duplicate){continue;}if(count>=8u){return invalidPointVector(16u);}for(var shift=count;shift>insert;shift-=1u){ids[shift]=ids[shift-1u];}ids[insert]=row;count+=1u;}
 if(count<4u){return invalidPointVector(17u);}for(var ia=0u;ia+3u<count;ia+=1u){for(var ib=ia+1u;ib+2u<count;ib+=1u){for(var ic=ib+1u;ic+1u<count;ic+=1u){for(var id=ic+1u;id<count;id+=1u){let ra=rows[ids[ia]];let rb=rows[ids[ib]];let rc=rows[ids[ic]];let rd=rows[ids[id]];let a=vec3f(coord(ra.cell))+.5*f32(ra.size);let b=vec3f(coord(rb.cell))+.5*f32(rb.size);let c=vec3f(coord(rc.cell))+.5*f32(rc.size);let d=vec3f(coord(rd.cell))+.5*f32(rd.size);let u=b-a;let v=c-a;let w=d-a;let determinant=dot(u,cross(v,w));if(!finite(determinant)||abs(determinant)<=1e-8){continue;}let weights=tetraWeights(pointGrid-a,u,v,w);if(!contained(weights)){continue;}let rhs=vec3f(.5*dot(u,u),.5*dot(v,v),.5*dot(w,w));let center=(rhs.x*cross(v,w)+rhs.y*cross(w,u)+rhs.z*cross(u,v))/determinant;let radius2=dot(center,center);if(!finite(radius2)){continue;}let tolerance=2e-5*max(1.,radius2);var empty=true;for(var site=0u;site<count;site+=1u){let sr=rows[ids[site]];let delta=(vec3f(coord(sr.cell))+.5*f32(sr.size))-(a+center);if(dot(delta,delta)<radius2-tolerance){empty=false;break;}}if(!empty){continue;}let va=rowVelocities[ids[ia]];let vb=rowVelocities[ids[ib]];let vc=rowVelocities[ids[ic]];let vd=rowVelocities[ids[id]];let result=weights.x*va.xyz+weights.y*vb.xyz+weights.z*vc.xyz+weights.w*vd.xyz;return select(invalidPointVector(18u),vec4f(result,1.),finite(result.x)&&finite(result.y)&&finite(result.z));}}}}
 return invalidPointVector(19u);
}
// The owner containing a trajectory sample need not be a vertex of the
// Delaunay tetrahedron containing that continuous point. Test every published
// non-endpoint anchor for exact cube/tetra containment; only the rare
// co-spherical catalog hole then reaches the local ordinary-Delaunay repair.
fn locateFinalPointVector(initialAnchor:u32,pointGrid:vec3f)->vec4f{let direct=finalPointVector(initialAnchor,pointGrid);if(velocityValid(direct)){return direct;}let count=min(p.rowCapacity,min(arrayLength(&rows),arrayLength(&metrics)));var containedInvalid=vec4f(0.);for(var candidate=0u;candidate<count;candidate+=1u){if(candidate==initialAnchor||(rows[candidate].flags&ROW_SUPPORT3_ENDPOINT)!=0u){continue;}let value=finalPointVector(candidate,pointGrid);if(velocityValid(value)){return value;}if(u32(round(max(0.,-value.w)))==9u&&containedInvalid.w==0.){containedInvalid=value;}}if(containedInvalid.w<0.){return containedInvalid;}let local=surroundingOwnerDelaunayVector(pointGrid);return select(direct,local,velocityValid(local));}
