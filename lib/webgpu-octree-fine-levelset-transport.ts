...
var<workgroup>pageChanged:array<u32,64>;var<workgroup>pageOldInterface:array<u32,64>;var<workgroup>pageInterface:array<u32,64>;
...
fn neighborSample(id:u32,local:u32,direction:u32)->u32{let q=localCoord(local);var inside=true;var next=q;if(direction==0u){inside=q.x>0u;next.x=select(params.brickResolution-1u,q.x-1u,inside);}else if(direction==1u){inside=q.x+1u<params.brickResolution;next.x=select(0u,q.x+1u,inside);}else if(direction==2u){inside=q.y>0u;next.y=select(params.brickResolution-1u,q.y-1u,inside);}else if(direction==3u){inside=q.y+1u<params.brickResolution;next.y=select(0u,q.y+1u,inside);}else if(direction==4u){inside=q.z>0u;next.z=select(params.brickResolution-1u,q.z-1u,inside);}else{inside=q.z+1u<params.brickResolution;next.z=select(0u,q.z+1u,inside);}if(inside){return id*params.samplesPerBrick+localIndex(next);}let neighbor=metadata[id*10u+4u+direction];if(neighbor>=params.pageCapacity||metadata[neighbor*10u+2u]!=params.generation){return INVALID;}return neighbor*params.samplesPerBrick+localIndex(next);}
@compute @workgroup_size(64)fn commitFineTransport(@builtin(workgroup_id)w:vec3u,@builtin(local_invocation_index)lid:u32,@builtin(num_workgroups)n:vec3u){let work=fineLinearWorkgroup(w,n);var pageLive=control.committed!=0u&&work<worklist[0];var changed=0u;var hadInterface=0u;var hasInterface=0u;var id=INVALID;if(pageLive){id=worklist[5u+work];if(id>=params.pageCapacity||metadata[id*10u+2u]!=params.generation){pageLive=false;}}if(pageLive){for(var local=lid;local<params.samplesPerBrick;local+=64u){let index=id*params.samplesPerBrick+local;let valid=(sampleFlags[index]&VALID)!=0u;let oldValue=phi[index];let newValue=workA[index];if(valid&&finiteScalar(oldValue)&&finiteScalar(newValue)){changed|=select(0u,1u,(oldValue<0.0)!=(newValue<0.0));for(var direction=0u;direction<6u;direction+=1u){let neighbor=neighborSample(id,local,direction);if(neighbor!=INVALID&&(sampleFlags[neighbor]&VALID)!=0u){let oldOther=phi[neighbor];let other=workA[neighbor];hadInterface|=select(0u,1u,finiteScalar(oldOther)&&(oldOther<0.0)!=(oldValue<0.0));hasInterface|=select(0u,1u,finiteScalar(other)&&(other<0.0)!=(newValue<0.0));}}}else if(valid){changed=1u;} }}pageChanged[lid]=changed;pageOldInterface[lid]=hadInterface;pageInterface[lid]=hasInterface;workgroupBarrier();var width=32u;loop{if(width==0u){break;}if(lid<width){pageChanged[lid]|=pageChanged[lid+width];pageOldInterface[lid]|=pageOldInterface[lid+width];pageInterface[lid]|=pageInterface[lid+width];}workgroupBarrier();width>>=1u;}if(pageLive&&lid==0u){let previous=metadata[id*10u+3u];let interfaceBefore=(previous&PAGE_INTERFACE)!=0u||pageOldInterface[0]!=0u;let interfaceNow=pageInterface[0]!=0u;
...
@compute @workgroup_size(1)fn publishFineTransportTopologyDelta(){let capacity=params.pageCapacity;topologyDelta[0]=0u;topologyDelta[1]=params.generation;topologyDelta[2]=0u;topologyDelta[3]=0u;topologyDelta[4]=0u;topologyDelta[5]=1u;topologyDelta[6]=1u;topologyDelta[7]=0u;if(control.committed==0u||arrayLength(&worklist)<5u||worklist[1]!=params.generation||worklist[0]>capacity||arrayLength(&topologyDelta)<8u+2u*capacity){return;}let livePages=worklist[0];var count=0u;var previous=0u;for(var work=0u;work<livePages;work+=1u){let id=worklist[5u+work];if(id>=capacity||metadata[id*10u+2u]!=params.generation){return;}let candidate=topologyDelta[8u+work];if(candidate!=INVALID){if(count>0u&&candidate<=previous){return;}topologyDelta[8u+capacity+count]=candidate;previous=candidate;count+=1u;}}topologyDelta[0]=count;topologyDelta[2]=1u;topologyDelta[3]=livePages;topologyDelta[4]=count;topologyDelta[5]=1u;topologyDelta[6]=1u;}
`;
