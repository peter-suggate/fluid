...
@compute @workgroup_size(1)fn prepareSurfaceGraph(){
 ...
 for(var i=0u;i<32u;i+=1u){atomicStore(&candidateControl[i],0u);}
 ...
 atomicStore(&candidateControl[14],p.capacities.x);atomicStore(&candidateControl[15],p.capacities.y);atomicStore(&candidateControl[16],p.capacities.y);setDispatch(17u,vec3u(0u,1u,1u));setDispatch(20u,vec3u(0u,1u,1u));
 ...
 atomicStore(&candidateLeafLocator[0],p.radixLayout.y);atomicStore(&candidateLeafLocator[1],select(0u,epoch,valid));atomicStore(&candidateLeafLocator[2],select(0u,pageCount,valid));atomicStore(&candidateLeafLocator[3],select(0u,supportCount,valid));atomicStore(&candidateLeafLocator[4],0u);atomicStore(&candidateLeafLocator[5],0u);atomicStore(&candidateLeafLocator[6],0u);atomicStore(&candidateLeafLocator[7],select(ERROR_TOPOLOGY,0u,valid));atomicStore(&candidateLeafLocator[8],p.ownerLayout.x);atomicStore(&candidateLeafLocator[9],p.ownerLayout.y);atomicStore(&candidateLeafLocator[10],p.ownerLayout.z);atomicStore(&candidateLeafLocator[11],p.radixLayout.z);atomicStore(&candidateLeafLocator[12],0u);atomicStore(&candidateLeafLocator[13],0u);atomicStore(&candidateLeafLocator[14],0u);atomicStore(&candidateLeafLocator[15],0u);
...
@compute @workgroup_size(1)fn prepareSurfaceGraphRecords(){
 ...
 let nodeDispatch=groups(count);setDispatch(20u,nodeDispatch);commitDispatch[3]=nodeDispatch.x;commitDispatch[4]=nodeDispatch.y;commitDispatch[5]=nodeDispatch.z;
}
...
@compute @workgroup_size(1)fn finishSurfaceGraphCommit(){let epoch=commitDispatch[6];if(epoch==0u){return;}for(var i=0u;i<16u;i+=1u){if(i!=6u){atomicStore(&acceptedLeafLocator[i],atomicLoad(&candidateLeafLocator[i]));}}for(var i=0u;i<32u;i+=1u){if(i!=3u){atomicStore(&acceptedControl[i],atomicLoad(&candidateControl[i]));}}atomicStore(&acceptedLeafLocator[6],epoch);atomicStore(&acceptedControl[3],epoch);}
`;
