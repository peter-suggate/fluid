...
fn interpolationOwner(x:vec3f)->u32{return owner(x);}
...
export function makePowerVelocityPrepassBuilderWGSL(): string {
 return buildPowerTrajectoryQueriesWGSL
  .replace("let x=positions[i];var row=interpolationOwner(x.xyz);let inactive=x.w<=0.;if(row==0xffffffffu){row=0u;}let qb=i*12u;let vb=i*76u;",
   "let x=positions[i];let qb=i*12u;let vb=i*76u;let inactive=false;if(x.w<=0.0){word(qb+4u,0x20000000u);word(qb+5u,vb);word(qb+6u,76u);word(qb+7u,i);return;}let row=interpolationOwner(x.xyz);if(row==0xffffffffu){word(qb,0u);word(qb+1u,75u);word(qb+2u,0u);word(qb+3u,0u);word(qb+4u,1u);word(qb+5u,vb);word(qb+6u,76u);word(qb+7u,i);word(qb+8u,0u);word(qb+9u,0u);word(qb+10u,0u);word(qb+11u,0u);for(var c=0u;c<8u;c+=1u){storeVelocity(vb+c,vec4f(0.0));}return;}")
  .replace("word(qb+4u,th.flags|((m.flags&63u)<<8u));",
   "word(qb+4u,th.flags|((m.flags&63u)<<8u));");
}
