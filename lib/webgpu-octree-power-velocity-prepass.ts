...
export function planOctreePowerVelocityPrepass(queryCapacityValue: number,
  ...
  const samplerBytes = queryCapacity * 20 + OCTREE_POWER_PREPASS_STATUS_COUNTER_BYTES
...
export class WebGPUOctreePowerVelocityPrepass {
  ...
  constructor(private readonly device: GPUDevice, queryCapacityValue: number,
    ...
    this.statuses = device.createBuffer({ label: "Octree power sample statuses",
      size: this.queryCapacity * 4 + OCTREE_POWER_PREPASS_STATUS_COUNTER_BYTES, usage: storage });
    ...
    this.preparePipeline = pipeline("prepareDirectPowerVelocitySamples", "Prepare direct power velocity samples");
  ...
  encodeFromPositions(encoder: GPUCommandEncoder, positions: GPUBuffer | GPUBufferBinding,
    ...
    let pass = encoder.beginComputePass({ label: "Prepare direct power velocity samples" });
    ...
    pass.setBindGroup(0, this.device.createBindGroup({ layout: this.preparePipeline.getBindGroupLayout(0), entries: [
      params, { binding: 10, resource: { buffer: this.control } },
    ] }));
    ...
    pass.setBindGroup(0, this.device.createBindGroup({ layout: this.publishPipeline.getBindGroupLayout(0), entries: [
      params, { binding: 1, resource: bind(positions) }, { binding: 8, resource: { buffer: this.results } },
      { binding: 9, resource: { buffer: this.statuses } }, { binding: 10, resource: { buffer: this.control } },
    ] }));
    pass.dispatchWorkgroups(1);
    pass.end();
  }
...
struct R{cell:u32,size:u32,topology:u32,flags:u32}
struct SI{cellPlusOne:atomic<u32>,size:u32,row:u32,pad:u32}
...
@group(0)@binding(3)var<storage,read_write>sites:array<SI>;
...
const VALID:u32=0x80000000u;const CAPACITY:u32=1u;const INVALID_QUERY:u32=2u;const NONFINITE:u32=4u;const NO_CONTAINING_SIMPLEX:u32=8u;const INACTIVE:u32=0x20000000u;const UNIFORM:u32=${OCTREE_POWER_CATALOG_ENTRY_UNIFORM}u;
...
fn fail(i:u32,flag:u32){atomicOr(&control.flags,flag);atomicMin(&control.firstError,i);if(flag==NO_CONTAINING_SIMPLEX){atomicAdd(&control.noContainingSimplex,1u);}if(i<arrayLength(&results)){results[i]=vec4f(0.);}if(i<arrayLength(&statuses)){statuses[i]=flag;}}
