...
@compute @workgroup_size(4,4,4) fn bakeCoefficients(@builtin(global_invocation_id) gid:vec3u){let id=vec3i(gid);if(!validPacked(id)){return;}
  ...
  else if(id.y==1){let distance=max(h.y,f32(base-1)*h.y);let n0=vec3i(id.x,0,id.z);let t0=termCoefficients(ownPhi,textureLoad(phiIn,n0,0).x,textureLoad(solidIn,n0,0).x,1.0/(distance*h.y));vertical.x=t0.x;diag+=t0.y;if(activeSample(vec3i(id.x,2,id.z))){let n=vec3i(id.x,2,id.z);let t=termCoefficients(ownPhi,textureLoad(phiIn,n,0).x,textureLoad(solidIn,n,0).x,1.0/(h.y*h.y));vertical.y=t.x;diag+=t.y;}}
