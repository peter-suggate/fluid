...
fn stencil(id:vec3i)->vec2f{let ownPhi=textureLoad(phiIn,id,0).x;let ownP=textureLoad(pressureIn,id,0).x;let q=vec3i(id.x,i32(floor(sampleY(id))),id.z);let h=level.cell.xyz;var s=vec2f(0.0);let base=baseAt(id.x,id.z);
  // Paper Eq 15/16 row at the bottom sub-cell: solid below (Neumann), Eq 5
  // interpolated pressure above, so the top endpoint couples at
  // 1/(distance*h) — matching the fine jacobi in tall-cell-kernels.ts.
  if(id.y==0&&base>0){
    ...
    if(id.x>0){let n=vec3i(id.x-1,0,id.z);s+=term(ownPhi,textureLoad(phiIn,n,0).x,textureLoad(pressureIn,n,0).x,textureLoad(solidIn,n,0).x,1.0/(h.x*h.x));}
    if(id.x+1<dims().x){let n=vec3i(id.x+1,0,id.z);s+=term(ownPhi,textureLoad(phiIn,n,0).x,textureLoad(pressureIn,n,0).x,textureLoad(solidIn,n,0).x,1.0/(h.x*h.x));}
    if(id.z>0){let n=vec3i(id.x,0,id.z-1);s+=term(ownPhi,textureLoad(phiIn,n,0).x,textureLoad(pressureIn,n,0).x,textureLoad(solidIn,n,0).x,1.0/(h.z*h.z));}
    if(id.z+1<dims().z){let n=vec3i(id.x,0,id.z+1);s+=term(ownPhi,textureLoad(phiIn,n,0).x,textureLoad(pressureIn,n,0).x,textureLoad(solidIn,n,0).x,1.0/(h.z*h.z));}
  } else {
  ...
  if(id.y==0){let n=vec3i(id.x,1,id.z);let distance=max(h.y,f32(base-1)*h.y);s+=term(ownPhi,textureLoad(phiIn,n,0).x,textureLoad(pressureIn,n,0).x,textureLoad(solidIn,n,0).x,1.0/(distance*h.y));}
...
fn topStencil(id:vec3i)->vec2f{let ownPhi=textureLoad(phiIn,id,0).x;let ownP=topStored(id);let q=vec3i(id.x,i32(floor(sampleY(id))),id.z);let h=level.cell.xyz;var s=vec2f(0.0);let base=baseAt(id.x,id.z);
  if(id.y==0&&base>0){
    if(id.x>0){let n=vec3i(id.x-1,0,id.z);s+=term(ownPhi,textureLoad(phiIn,n,0).x,topStored(n),textureLoad(solidIn,n,0).x,1.0/(h.x*h.x));}
    if(id.x+1<dims().x){let n=vec3i(id.x+1,0,id.z);s+=term(ownPhi,textureLoad(phiIn,n,0).x,topStored(n),textureLoad(solidIn,n,0).x,1.0/(h.x*h.x));}
    if(id.z>0){let n=vec3i(id.x,0,id.z-1);s+=term(ownPhi,textureLoad(phiIn,n,0).x,topStored(n),textureLoad(solidIn,n,0).x,1.0/(h.z*h.z));}
    if(id.z+1<dims().z){let n=vec3i(id.x,0,id.z+1);s+=term(ownPhi,textureLoad(phiIn,n,0).x,topStored(n),textureLoad(solidIn,n,0).x,1.0/(h.z*h.z));}
  } else {
  ...
  if(id.y==0){let n=vec3i(id.x,1,id.z);let distance=max(h.y,f32(base-1)*h.y);s+=term(ownPhi,textureLoad(phiIn,n,0).x,topStored(n),textureLoad(solidIn,n,0).x,1.0/(distance*h.y));}
...
fn sourceStencil(id:vec3i)->vec2f{if(!sourceActive(id)||textureLoad(sourcePhi,id,0).x>0.0||textureLoad(sourceSolid,id,0).x>0.9){return vec2f(1.0,0.0);}let ownPhi=textureLoad(sourcePhi,id,0).x;let ownP=textureLoad(sourcePressure,id,0).x;let q=vec3i(id.x,i32(floor(sourceSampleY(id))),id.z);let h=sourceLevel.cell.xyz;var s=vec2f(0.0);
  let base=sourceBaseAt(id.x,id.z);
  if(id.y==0&&base>0){
    if(id.x>0){let n=vec3i(id.x-1,0,id.z);s+=sourceTerm(ownPhi,textureLoad(sourcePhi,n,0).x,textureLoad(sourcePressure,n,0).x,textureLoad(sourceSolid,n,0).x,1.0/(h.x*h.x));}
    if(id.x+1<sourceDims().x){let n=vec3i(id.x+1,0,id.z);s+=sourceTerm(ownPhi,textureLoad(sourcePhi,n,0).x,textureLoad(sourcePressure,n,0).x,textureLoad(sourceSolid,n,0).x,1.0/(h.x*h.x));}
    if(id.z>0){let n=vec3i(id.x,0,id.z-1);s+=sourceTerm(ownPhi,textureLoad(sourcePhi,n,0).x,textureLoad(sourcePressure,n,0).x,textureLoad(sourceSolid,n,0).x,1.0/(h.z*h.z));}
    if(id.z+1<sourceDims().z){let n=vec3i(id.x,0,id.z+1);s+=sourceTerm(ownPhi,textureLoad(sourcePhi,n,0).x,textureLoad(sourcePressure,n,0).x,textureLoad(sourceSolid,n,0).x,1.0/(h.z*h.z));}
  } else {
...
// The packed bottom texel is the tall-cell column-average VOF. Reconstruct
// the settled point-sample occupancy (fill height alpha*base above terrain)
// so a flooded floor reads as deep liquid and a drained upper tall region
// reads as air — matching the paper's Eq 4 point samples.
// A tall cell below a wet band bottom is liquid (paper invariant: the
// interface never lies inside a tall cell); the settled fill applies only to
// genuinely drained columns whose band bottom is dry.
fn fineSourceAlpha(q:vec3i)->f32{if(any(q<vec3i(0))||q.x>=dims().x||q.y>=fineY()||q.z>=dims().z){return 0.0;}let b=baseAt(q.x,q.z);if(q.y<b&&b>0){let a=clamp(textureLoad(sourcePhi,vec3i(q.x,0,q.z),0).x,0.0,1.0);if(a<0.5&&dims().y>2&&textureLoad(sourcePhi,vec3i(q.x,2,q.z),0).x>=0.5){return 0.5;}return a;}let py=2+q.y-b;if(py<2||py>=dims().y){return 0.0;}return textureLoad(sourcePhi,vec3i(q.x,py,q.z),0).x;}
@compute @workgroup_size(4,4,4) fn buildFinePhi(@builtin(global_invocation_id) gid:vec3u){let id=vec3i(gid);if(!validPacked(id)){return;}let h=min(level.cell.x,min(level.cell.y,level.cell.z));if(!activeSample(id)){textureStore(phiOut,id,vec4f(5.0*h));return;}let q=vec3i(id.x,i32(floor(sampleY(id))),id.z);let alpha=fineSourceAlpha(q);let liquid=alpha>=0.5;let offsets=array<vec3i,6>(vec3i(-1,0,0),vec3i(1,0,0),vec3i(0,-1,0),vec3i(0,1,0),vec3i(0,0,-1),vec3i(0,0,1));var interfaceCell=false;for(var index=0;index<6;index+=1){if((fineSourceAlpha(q+offsets[index])>=0.5)!=liquid){interfaceCell=true;}}let distance=select(5.0*h,max(0.05*h,abs(0.5-alpha)*h),interfaceCell);textureStore(phiOut,id,vec4f(select(distance,-distance,liquid),0.0,0.0,0.0));}
...
interface Level {
  ...
  cell:{x:number;y:number;z:number};pressure:[GPUTexture,GPUTexture];rhs:GPUTexture;phi:GPUTexture;solid:GPUTexture;base:GPUTexture;params:GPUBuffer;correctionParams:GPUBuffer;owned:boolean;
}
...
export class TallCellMultigrid {
  ...
  constructor(private device:GPUDevice,geometry:TallCellLayout,fine:TallCellMultigridFineResources,readonly refinementCycles=2,deferPipelineCompilation=false){
    ...
    this.shaderModule=device.createShaderModule({label:"Restricted tall-cell multigrid",code:multigridShader});void this.shaderModule.getCompilationInfo().then(info=>{for(const message of info.messages)if(message.type==="error")console.error(`Tall multigrid WGSL ${message.lineNum}:${message.linePos} ${message.message}`);}).catch(()=>{/* Device loss is reported by the renderer. */});this.pipelineLayout=device.createPipelineLayout({bindGroupLayouts:[this.layout]});if(!deferPipelineCompilation){this.createPipelinesSync();void device.popErrorScope().then(error=>{if(error)console.error(`Tall multigrid pipeline creation: ${error.message}`);}).catch(()=>{/* Device loss is reported by the renderer. */});}
  ...
  private pipelineDescriptor(entryPoint:string):GPUComputePipelineDescriptor{return{label:`Tall multigrid ${entryPoint}`,layout:this.pipelineLayout,compute:{module:this.shaderModule,entryPoint}};}
  private createPipelinesSync(){const pipeline=(entryPoint:string)=>this.device.createComputePipeline(this.pipelineDescriptor(entryPoint));this.clearPipeline=pipeline("clearPressure");this.smoothPipelines=[pipeline("smoothRed"),pipeline("smoothBlack")];this.topPipeline=pipeline("solveTop");this.basePipeline=pipeline("downsampleBase");this.finePhiPipeline=pipeline("buildFinePhi");this.reinitializePhiPipeline=pipeline("reinitializePhi");this.phiPipeline=pipeline("downsamplePhi");this.solidPipeline=pipeline("downsampleSolid");this.restrictPipeline=pipeline("restrictResidual");this.prolongatePipeline=pipeline("prolongateCorrection");this.residualPipeline=pipeline("reducePressureDiagnostics");}
  async initializePipelines(onProgress:(label:string,completed:number,total:number)=>void,offset=0,total=12){
    const definitions=[
      ["Clear pressure","clearPressure"],["Smooth red cells","smoothRed"],["Smooth black cells","smoothBlack"],
      ["Solve coarsest level","solveTop"],["Downsample column bases","downsampleBase"],["Build level set","buildFinePhi"],
      ["Reinitialize level set","reinitializePhi"],["Downsample level set","downsamplePhi"],["Downsample solids","downsampleSolid"],
    ...
    this.clearPipeline=compiled[0];this.smoothPipelines=[compiled[1],compiled[2]];this.topPipeline=compiled[3];this.basePipeline=compiled[4];this.finePhiPipeline=compiled[5];this.reinitializePhiPipeline=compiled[6];this.phiPipeline=compiled[7];this.solidPipeline=compiled[8];this.restrictPipeline=compiled[9];this.prolongatePipeline=compiled[10];this.residualPipeline=compiled[11];
  }
