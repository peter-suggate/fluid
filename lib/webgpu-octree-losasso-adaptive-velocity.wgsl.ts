...
@compute @workgroup_size(1)fn finishAdaptiveVelocity(){let errors=atomicLoad(&finishReceipt[avp.receiptBase+2u]);let ready=select(0u,1u,errors==0u);atomicStore(&finishReceipt[avp.receiptBase+7u],ready);let generation=atomicLoad(&finishGraph[5u]);if(avp.bankOffset==0u&&avp.receiptBase==0u&&avp.pad0==0u){let accepted=generation!=0u&&ready==1u&&atomicLoad(&finishReceipt[avp.receiptBase])==generation;let predictorBase=avp.receiptBase+12u;let both=accepted&&atomicLoad(&finishReceipt[predictorBase+7u])==1u&&atomicLoad(&finishReceipt[predictorBase])==generation;let published=select(0u,generation,accepted);atomicStore(&finishGraph[6u],select(0u,generation,both));atomicStore(&finishLeafLocator[13u],published);atomicStore(&finishLeafLocator[14u],published);}else if(avp.bankOffset==1u){let prior=avp.receiptBase-12u;let both=generation!=0u&&ready==1u&&atomicLoad(&finishReceipt[prior+7u])==1u&&atomicLoad(&finishReceipt[prior])==generation&&atomicLoad(&finishReceipt[avp.receiptBase])==generation;let published=select(0u,generation,both);atomicStore(&finishGraph[6u],published);atomicStore(&finishLeafLocator[13u],published);atomicStore(&finishLeafLocator[14u],published);}}
