...
@compute @workgroup_size(64)fn emitGlobalFineTetra${tetraIndex}(@builtin(global_invocation_id)g:vec3u){let i=g.x;let count=min(atomicLoad(&args.activeCubeCount),min(arrayLength(&cubes),arrayLength(&offsets)/6u));if(i>=count||i*2u+1u>=arrayLength(&values)){return;}let packed=cubes[i];let descriptor=packed.y>>16u;let scale=f32(max(1u,descriptor&255u));let base=vec3f(f32(packed.x&0xffffu),f32(packed.y&0xffffu),f32(packed.x>>16u));let lo=values[i*2u];let hi=values[i*2u+1u];let v0=lo.x;let v1=lo.y;let v2=lo.z;let v3=lo.w;let v4=hi.x;let v5=hi.y;let v6=hi.z;let v7=hi.w;let gx=.25*((v1+v2+v5+v6)-(v0+v3+v4+v7));let gy=.25*((v2+v3+v6+v7)-(v0+v1+v4+v5));let gz=.25*((v4+v5+v6+v7)-(v0+v1+v2+v3));let size=vec3f(p.sample.xyz);let gradient=vec3f(gx*size.x/u.container.x,gy*size.y/u.container.y,gz*size.z/u.container.z);var n=vec3f(0,1,0);if(length(gradient)>1e-5){n=-normalize(gradient);}let mode=(descriptor>>8u)&3u;let filterEnabled=mode==0u&&base.x>0.0&&base.y>0.0&&base.z>0.0&&base.x<f32(p.sample.x)&&base.y<f32(p.sample.y)&&base.z<f32(p.sample.z);var cursor=offsets[i*6u+${tetraIndex}u];tet(&cursor,clipped(base,scale,${cornerPosition[a]},descriptor),clipped(base,scale,${cornerPosition[b]},descriptor),clipped(base,scale,${cornerPosition[c]},descriptor),clipped(base,scale,${cornerPosition[d]},descriptor),v${a},v${b},v${c},v${d},n,filterEnabled);}
`;
}
