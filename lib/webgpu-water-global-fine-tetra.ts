...
export function cubeContourEdgePoint(edge: number, values: readonly number[]): readonly [number, number, number] {
  ...
  const t = .5 + centred;
  return [0, 1, 2].map(axis => corners[a]![axis]! + (corners[b]![axis]! - corners[a]![axis]!) * t)
...
fn emitWallLane(cursor:ptr<function,u32>,base:vec3f,descriptor:u32,values:array<f32,8>,lane:u32){
  let face=wallFaceValues(values,descriptor);let mask=wallMask(face);let axis=(descriptor>>14u)&3u;let side=(descriptor&255u)-252u;let plane=f32(side);let ambiguous=mask==5u||mask==10u;let connected=wallAmbiguousConnected(face);let count=wallBoundaryCount(face);let triangles=wallTriangleCount(values,descriptor);if(lane>=triangles){return;}
  ...
  var normal=vec3f(0);normal[axis]=select(-1.,1.,side!=0u);let shift=vec3f(.5);tri(cursor,base+wallPoint3(a,axis,plane)+shift,base+wallPoint3(b,axis,plane)+shift,base+wallPoint3(c,axis,plane)+shift,normal,false);
}
`;
