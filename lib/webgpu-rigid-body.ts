...
fn integrate(@builtin(global_invocation_id) id: vec3u) {
  ...
  for(var index=0u;index<12u;index++){if(index<count){publish(index);}else{renderBodies[index]=RenderBody(vec4f(0),vec4f(0),vec4f(1,0,0,0),vec4f(0));}}
}
`;
...
export class WebGPURigidBodySystem {
  ...
  syncBodies(bodies: readonly RigidBodyState[]) {
    ...
    const state = new Float32Array(GPU_RIGID_BODY_CAPACITY * GPU_RIGID_STATE_FLOATS);
    ...
    this.bodyIds=ids;this.structuralSignatures=structural;this.authoredTransformSignatures=authored;this.commandSignatures=commands;
  }
  ...
  encode(encoder: GPUCommandEncoder, dt_s: number, cellVolume_m3: number, snapshotCount = 1, cellHeight_m = 1) {
    ...
    this.device.queue.writeBuffer(this.paramsBuffer,0,new Float32Array([dt_s,this.scene.fluid.density_kg_m3,this.bodyCount,Math.max(1,snapshotCount),g.x,g.y,g.z,0,c.width_m,c.height_m,c.depth_m,sceneHasTerrain(this.scene)?1:0,cellHeight_m,c.top === "closed"?1:0,0,0,cellVolume_m3,.9,.5,0]));
  ...
  destroy() { this.stateBuffer.destroy();this.renderBuffer.destroy();this.stateScratch.destroy();this.renderScratch.destroy();this.paramsBuffer.destroy();this.pickParamsBuffer.destroy();this.pickResultBuffer.destroy(); }
}
