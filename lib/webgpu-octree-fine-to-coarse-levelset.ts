...
fn finePage(key:u32)->u32{
 if(arrayLength(&worklist)<5u||worklist[1]!=p.generation||worklist[3]!=1u||worklist[4]!=1u){return INVALID;}
 let count=min(worklist[0],p.pageCapacity);let logicalCount=p.brickDims.x*p.brickDims.y*p.brickDims.z;
 let directoryBase=5u+p.pageCapacity;
 if(5u+count>arrayLength(&worklist)||key>=logicalCount||directoryBase+key>=arrayLength(&worklist)){return INVALID;}
