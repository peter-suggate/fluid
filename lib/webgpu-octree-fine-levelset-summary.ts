...
bandWidth: number): FineLevelSetSummaryRefinementSignal {
  if (!summary.published || summary.directoryFlags !== 0 || !summary.found || summary.entryFlags !== 0
  ...
  if (summary.brickCount !== lookup.expectedBrickCount || summary.sampleCount !== lookup.expectedSampleCount) {
    return "fallback";
  }
...
fn merge(key:u32,minPhi:f32,maxPhi:f32,minAbs:f32,samples:u32,bricks:u32,flags:u32){let start=hash(key)&(p.hashCapacity-1u);
