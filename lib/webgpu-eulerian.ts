...
export class WebGPUEulerianSolver {
  ...
  /** Adopt scalar-only scene revisions; per-step params pick them up directly. */
  applySceneUniforms(scene: SceneDescription) { this.scene = scene; }

  // `scene` is not readonly so applySceneUniforms can swap scalar revisions.
  constructor(private device: GPUDevice, public scene: SceneDescription, quality: GPUQuality, private onRigidLoads?: (loads: GPURigidLoad[]) => void, private readonly options:WebGPUEulerianSolverOptions={}) {
  ...
  advanceTo(time_s:number,bodies:RigidBodyState[]=[]){
    ...
    if(!this.validationChecked)this.device.pushErrorScope("validation");const measurementInstrumentationEnabled=usePerformanceInstrumentationStore.getState().enabled;const shouldTrace=measurementInstrumentationEnabled&&!this.physicsTracePending&&performance.now()-this.lastPhysicsTraceAt_ms>=100;const traceSampleId=shouldTrace?++this.physicsTraceSampleId:0;const physicsTrace=shouldTrace&&GPUStageTimestampRecorder.supported(this.device)?new GPUStageTimestampRecorder(this.device,traceSampleId,"physics",`tall-cell:sim-${this.lastTime.toFixed(6)}`,64):undefined;const physicsQueueTrace=shouldTrace?new GPUQueueWallPerformanceTraceRecorder(traceSampleId,"physics",`tall-cell:sim-${this.lastTime.toFixed(6)}`):undefined;const encoder=physicsTrace?.instrument(this.device.createCommandEncoder({label:"GPU fluid step"}))??this.device.createCommandEncoder({label:"GPU fluid step"});physicsTrace?.begin();{const plan=encoder.beginComputePass({label:"Plan phi substeps"});plan.setPipeline(this.planSubstepsPipeline);plan.setBindGroup(0,this.planSubstepsGroup);plan.dispatchWorkgroups(1);plan.end();}encoder.clearBuffer(this.rigidExchangeBuffer);encoder.clearBuffer(this.reductionBuffer);physicsTrace?.completePhase(encoder,{id:"other",label:"Substep planning + advance setup"});
    ...
    const submittedAt=measurementInstrumentationEnabled?performance.now():0;physicsQueueTrace?.begin();this.device.queue.submit([encoder.finish()]);const physicsQueueTraceRead=physicsQueueTrace?.read(this.device.queue);const physicsTraceRead=physicsTrace?physicsTrace.read().then(trace=>trace??physicsQueueTraceRead).catch(()=>physicsQueueTraceRead):physicsQueueTraceRead;if(physicsTraceRead){this.lastPhysicsTraceAt_ms=submittedAt;this.physicsTracePending=true;void physicsTraceRead.then(trace=>{const instrumentation=usePerformanceInstrumentationStore.getState();if(trace&&instrumentation.enabled&&instrumentation.enabledAt_ms<=submittedAt)this.info.physicsTrace=trace;}).catch(()=>physicsTrace?.destroy()).finally(()=>{this.physicsTracePending=false;});}
