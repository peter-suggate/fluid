...
const RESIDENT=1u;const CORE=2u;const HALO=4u;const ACTIVATED=8u;const WAS_RESIDENT=32u;const HEADER=16u;const INVALID=0xffffffffu;
...
@compute @workgroup_size(64) fn markSurfaceCandidates(@builtin(global_invocation_id) gid:vec3u){let i=gid.x;if(!producerAccepted()||i>=candidateControl[0]){return;}if(i>=arrayLength(&candidates)){atomicStore(&transaction[0],1u);return;}let c=candidates[i];if(c.row>=arrayLength(&leaves)){atomicStore(&transaction[0],2u);return;}let leaf=leaves[c.row];if(leaf.size==0u){atomicStore(&transaction[0],3u);return;}let a=origin(leaf.packedOrigin);if(any(a>=params.dimsBrick.xyz)){atomicStore(&transaction[0],4u);return;}let first=a/params.dimsBrick.w;let last=min(params.brickDimsCapacity.xyz-vec3u(1u),(a+vec3u(leaf.size-1u))/params.dimsBrick.w);let bits=select(HALO,CORE,(c.flags&CORE)!=0u);for(var z=first.z;z<=last.z;z++){for(var y=first.y;y<=last.y;y++){for(var x=first.x;x<=last.x;x++){let b=x+params.brickDimsCapacity.x*(y+params.brickDimsCapacity.y*z);atomicOr(&states[b],bits);}}}}
...
@compute @workgroup_size(64) fn markSurfaceCandidates(@builtin(global_invocation_id) gid:vec3u){let i=gid.x;if(!producerAccepted()||i>=candidateControl[0]){return;}if(i>=arrayLength(&candidates)){atomicStore(&transaction[0],1u);return;}let c=candidates[i];if(c.row>=arrayLength(&leaves)){atomicStore(&transaction[0],2u);return;}let leaf=leaves[c.row];if(leaf.size==0u){atomicStore(&transaction[0],3u);return;}let a=origin(leaf.packedOrigin);if(any(a>=params.dimsBrick.xyz)){atomicStore(&transaction[0],4u);return;}let first=a/params.dimsBrick.w;let last=min(params.brickDimsCapacity.xyz-vec3u(1u),(a+vec3u(leaf.size-1u))/params.dimsBrick.w);let bits=select(HALO,CORE,(c.flags&CORE)!=0u);for(var z=first.z;z<=last.z;z++){for(var y=first.y;y<=last.y;y++){for(var x=first.x;x<=last.x;x++){let logical=x+params.brickDimsCapacity.x*(y+params.brickDimsCapacity.y*z);let slot=claimBrick(logical);if(slot!=0xffffffffu){atomicOr(&states[slot*2u+1u],bits);}}}}}
