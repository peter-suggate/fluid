...
} from "../core/webgpu-water-pipeline";
...
export async function readGlobalFineGenerationDiagnostics(
  ...
  const transportControl = debug.globalFineTransportControl;
  const redistanceControl = debug.globalFineRedistanceControl;
  const volumeControl = debug.globalFineVolumeControl;
  const pageDeltaDebug = debug.globalFinePageDeltaDebug;
...
export async function smokeRenderHybridPresentation(
  ...
  packed.set([solver.info.nx, solver.info.ny, solver.info.nz, solver.info.gridKind === "octree" ? 3 : 1], 20);
...
export async function readCompactOctreeVelocityField3D(
  ...
  if (!controlBuffer || !headerBuffer || !velocityBuffer) {
    ...
    const sample = (axis: number, x: number, y: number, z: number): number | undefined => {
      ...
      // the quiescent air complement, so a missing record means "this face is
      // outside the published band", never "this face is zero". Zero-filling it
      // made an absent face indistinguishable from a still one: a one-sided
      // support hole read out as a full-magnitude value error, and coveredCells
      // reported the whole lattice no matter how little the band actually
      // covered. Report absence the way the Power branch does and the way
      // fluid-symmetry-diagnostic's non-finite contract expects.
      return Number.NaN;
    };
...
export async function readCompactOctreePressureState3D(
  ...
  const controlBuffer = structured.structuredVelocityControl;
  const headerBuffer = debug.powerLeafHeaders;
  const pressureBuffer = debug.powerPressureBuffer;
  const pressureDebug = debug.losassoPressureDebug;
  if (!controlBuffer && headerBuffer && pressureBuffer && pressureDebug) {
    ...
    if (control[3] !== 1 || rowCount === 0 || rowCount * 48 > headerBuffer.size
      || rowCount * 4 > pressureBuffer.size || rowCount * 4 > pressureDebug.rightHandSide.size
      || rowCount * 4 > pressureDebug.diagonal.size) return undefined;
    ...
    const [headerBytes, pressureBytes, rhsBytes, diagonalBytes, ...stageBytes] = await Promise.all([
      ...
      readBufferBinding(device, { buffer: pressureDebug.rightHandSide }, rowCount * 4),
      readBufferBinding(device, { buffer: pressureDebug.diagonal }, rowCount * 4),
...
export async function auditOwnerLatticeConsistency(
  ...
  const debug = octreeDebugSources(solver).ownerLatticeDebug;
  if (!debug) return [];
  const [nx, ny, nz] = dims;
  ...
  if (compactPaged) {
    ...
    if (!source?.coarsePhiDirectory || !source.coarsePhiRowCapacity) {
      ...
      return { field: new Float32Array(0), summary: {
        ...
        enclosedAirComponentCount: 0, enclosedAirCells: 0,
        // No spatial field was materialized, so there is no occupied extent to
        // report. Null is the same "no wet cells" representation
        // `summarizeScalarField` publishes, never a degenerate zero box.
        wetBounds: null, centroidCells: null,
      } };
    }
    ...
    const [metadataBytes, sampleBytes, worklistBytes, coarseBytes, coarseControlBytes,
      ...
      debug.globalFineCoarseLevelSetControl
        ? readBufferBinding(device, { buffer: debug.globalFineCoarseLevelSetControl }, 64)
        : Promise.resolve(undefined),
      debug.globalFineRestrictionControl
        ? readBufferBinding(device, { buffer: debug.globalFineRestrictionControl }, 32)
        : Promise.resolve(undefined),
      source.topologyControl
        ...
        ? readBufferBinding(device, { buffer: source.topologyControl }, 48)
        : Promise.resolve(undefined),
      debug.globalFineTransportControl
        ? readBufferBinding(device, { buffer: debug.globalFineTransportControl }, 64)
        : Promise.resolve(undefined),
      debug.globalFineRedistanceControl
        ? readBufferBinding(device, { buffer: debug.globalFineRedistanceControl },
          FINE_LEVELSET_REDISTANCE_CONTROL_BYTES)
        : Promise.resolve(undefined),
      debug.globalFineVolumeControl
        ? readBufferBinding(device, { buffer: debug.globalFineVolumeControl }, 64)
        : Promise.resolve(undefined),
...
export async function dumpFineRedistancePageDeltaForensics(
  ...
  const debug = octreeDebugSources(solver).globalFinePageDeltaDebug;
