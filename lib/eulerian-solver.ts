...
import { damBreakBoxContains, inflowStrength, initialLiquidContainsCell, sceneDamBreakBox } from "./initial-fluid";
import {
...
export class EulerianFluidSolver {
  ...
  private initializeOccupancy() {
    ...
    for (let k = 0; k < this.nz; k += 1) for (let j = 0; j < this.ny; j += 1) for (let i = 0; i < this.nx; i += 1) {
      const occupied = initialLiquidContainsCell(this.scene, i, j, k, [this.nx, this.ny, this.nz],
        this.scene.fluid.initialCondition === "dam-break"
          ? damBreakBoxContains(dam, (i + 0.5) / this.nx, (j + 0.5) / this.ny, (k + 0.5) / this.nz)
          : (j + 0.5) / this.ny <= fill);
