...
@compute @workgroup_size(1)fn prepareAdvance(){for(var i=0u;i<=12u;i+=1u){atomicStore(&receipts[i],0u);}for(var i=13u;i<=15u;i+=1u){atomicStore(&receipts[i],0u);}for(var i=32u;i<36u;i+=1u){atomicStore(&receipts[i],0u);}atomicStore(&control[12],0u);atomicStore(&control[13],0u);atomicStore(&control[14],0u);atomicStore(&control[15],0u);atomicStore(&control[17],1u);atomicStore(&control[18],0u);atomicStore(&control[19],0u);
 let reach=bitcast<f32>(velocityReceipt[1]);let reason=select(0u,1u,atomicLoad(&control[7])!=1u)|select(0u,2u,graph[0]==0u||graph[3]!=graph[0])|select(0u,4u,graph[0]!=atomicLoad(&control[1]))|select(0u,8u,graph[5]!=atomicLoad(&control[2]))|select(0u,16u,velocityControl[0]!=graph[0])|select(0u,32u,velocityControl[2]<nodeCount()||velocityControl[3]!=velocityControl[0]||velocityControl[4]!=0u)|select(0u,64u,velocityControl[5]!=graph[5])|select(0u,128u,velocityControl[6]!=graph[5])|select(0u,256u,velocityReceipt[0]!=graph[5]||velocityReceipt[7]!=1u)|select(0u,512u,!finite1(reach)||reach<0.);atomicStore(&receipts[54],reason);if(reason!=0u){atomicOr(&control[12],ERR_GRAPH);}}
...
@compute @workgroup_size(1)fn captureTransportReceipt(){let maximumDelta=atomicLoad(&control[13]);atomicStore(&receipts[0],bitcast<u32>(p.numerics.x));atomicStore(&receipts[1],maximumDelta);atomicStore(&receipts[2],atomicLoad(&control[14]));atomicStore(&receipts[3],atomicLoad(&control[15]));atomicStore(&receipts[4],atomicLoad(&control[18]));atomicStore(&receipts[5],atomicLoad(&control[19]));let failure=atomicLoad(&control[12]);if(failure!=0u){atomicStore(&receipts[54],0x10000u|failure);}let retain=maximumDelta==0u&&atomicLoad(&receipts[12])==0u&&p.inflowVelocityStrength.w==0.;atomicStore(&control[17],select(1u,0u,retain));atomicStore(&control[13],0u);atomicStore(&control[14],0u);atomicStore(&control[15],0u);}
...
fn leafFractionAtBank(leaf:u32,bank:u32)->Sample{if(leaf>=leafCount()){return Sample(0.,0u);}var values:array<f32,8>;var mn=3.402823e38;var mx=-3.402823e38;for(var c=0u;c<8u;c+=1u){let value=constrainedValue(cornerSlot(leaf,c),bank);if(value.valid==0u){return Sample(0.,0u);}values[c]=value.value;mn=min(mn,value.value);mx=max(mx,value.value);}if(mn>0.){return Sample(0.,1u);}if(mx<0.){return Sample(1.,1u);}
 // Use the same regularized Heaviside as conservative-mass bootstrap. The hard
 // 27-sample sign count changes a whole surface slab at once and feeds lumpy,
 // discontinuous liquid fractions to pressure when phi crosses a sample.
 let width=max(.25*f32(leaves[leaf].originSpan.w)*p.originCell.w,1e-8);var liquid=0.;for(var z=0u;z<3u;z+=1u){for(var y=0u;y<3u;y+=1u){for(var x=0u;x<3u;x+=1u){let value=trilinear(values,(vec3f(f32(x),f32(y),f32(z))+vec3f(.5))/3.);
  liquid+=clamp(.5-value/(2.*width),0.,1.);}}}let fraction=liquid/27.;return Sample(fraction,select(0u,1u,finite1(fraction)));}
...
@compute @workgroup_size(1)fn finalizeAccepted(){atomicStore(&receipts[8],atomicLoad(&control[14]));atomicStore(&receipts[9],atomicLoad(&control[15]));atomicStore(&receipts[13],atomicLoad(&control[16]));
 let residual=bitcast<f32>(atomicLoad(&receipts[6]));if(residual>p.numerics.z){atomicOr(&control[12],ERR_REDISTANCE);}if(atomicLoad(&receipts[43])==0u){atomicOr(&control[12],ERR_VOLUME);}let failure=atomicLoad(&control[12]);if(failure==0u||atomicLoad(&receipts[54])==0u){atomicStore(&receipts[54],failure);}let valid=failure==0u;
...
@compute @workgroup_size(1)fn prepareVolumeEvidence(){let base=volumeReceiptBase();for(var i=0u;i<8u;i+=1u){atomicStore(&receipts[base+i],0u);}atomicStore(&receipts[base],graph[0]);atomicStore(&receipts[base+1u],select(graph[5],atomicLoad(&state[2]),candidate()));let epoch=graph[0];var matches=epoch!=0u&&graph[3]==epoch&&graph[4]==0u&&graph[1]<=p.limits.y&&graph[2]<=p.limits.x&&graph[2]<=arrayLength(&constraints)&&graph[2]<=arrayLength(&phi)&&graph[1]<=arrayLength(&physicalVolumes)&&graph[1]<=arrayLength(&preRedistanceVolumes);if(candidate()){matches=matches&&atomicLoad(&state[0])==MAGIC&&atomicLoad(&state[8])==epoch&&atomicLoad(&state[10])==graph[2]&&atomicLoad(&state[11])==1u&&atomicLoad(&state[2])!=0u;}else{let reason=select(0u,1u,epoch==0u)|select(0u,2u,graph[3]!=epoch||graph[4]!=0u)|select(0u,4u,graph[1]>p.limits.y||graph[2]>p.limits.x||graph[2]>arrayLength(&constraints)||graph[2]>arrayLength(&phi)||graph[1]>arrayLength(&physicalVolumes)||graph[1]>arrayLength(&preRedistanceVolumes))|select(0u,8u,atomicLoad(&state[0])!=MAGIC)|select(0u,16u,atomicLoad(&state[1])!=epoch)|select(0u,32u,atomicLoad(&state[2])!=graph[5])|select(0u,64u,atomicLoad(&state[3])!=graph[6])|select(0u,128u,atomicLoad(&state[4])!=graph[2])|select(0u,256u,atomicLoad(&state[5])!=graph[1])|select(0u,512u,atomicLoad(&state[7])!=1u);if(reason!=0u){atomicStore(&receipts[54],0x20000u|reason);}matches=matches&&reason==0u;}if(!matches){atomicOr(&state[12],ERR_GRAPH);}}
@compute @workgroup_size(64)fn validateVolumeEvidence(@builtin(global_invocation_id)gid:vec3u){let base=volumeReceiptBase();let i=gid.x;if(i==0u){let epoch=graph[0];let generation=select(graph[5],atomicLoad(&state[2]),candidate());var matches=atomicLoad(&receipts[base])==epoch&&atomicLoad(&receipts[base+1u])==generation&&epoch!=0u&&graph[3]==epoch&&graph[4]==0u&&graph[1]==leafCount()&&graph[2]==nodeCount();if(candidate()){matches=matches&&atomicLoad(&state[8])==epoch&&atomicLoad(&state[10])==graph[2]&&atomicLoad(&state[11])==1u;}else{let reason=select(0u,1u,atomicLoad(&receipts[base])!=epoch)|select(0u,2u,atomicLoad(&receipts[base+1u])!=generation)|select(0u,4u,epoch==0u)|select(0u,8u,graph[3]!=epoch||graph[4]!=0u)|select(0u,16u,graph[1]!=leafCount())|select(0u,32u,graph[2]!=nodeCount())|select(0u,64u,atomicLoad(&state[1])!=epoch)|select(0u,128u,atomicLoad(&state[2])!=generation)|select(0u,256u,atomicLoad(&state[3])!=graph[6])|select(0u,512u,atomicLoad(&state[4])!=graph[2])|select(0u,1024u,atomicLoad(&state[5])!=graph[1])|select(0u,2048u,atomicLoad(&state[7])!=1u);if(reason!=0u){atomicStore(&receipts[54],0x30000u|reason);}matches=matches&&reason==0u;}if(!matches){atomicOr(&state[12],ERR_GRAPH);}atomicStore(&receipts[base+2u],nodeCount());if(atomicLoad(&receipts[base+4u])!=leafCount()){atomicOr(&state[12],ERR_VOLUME);}}if(i>=nodeCount()){return;}let value=phi[i][bank()];if(!finite(value)){atomicOr(&state[12],ERR_VOLUME|ERR_FINITE);}if(constraints[i].header.y!=0u){atomicAdd(&receipts[base+3u],1u);let expected=projected(i,bank());if(expected.valid==0u||abs(value-expected.value)>p.volume.y){atomicOr(&state[12],ERR_VOLUME|ERR_CONSTRAINT);}}}
...
@compute @workgroup_size(64)fn markTransportReach(@builtin(global_invocation_id)gid:vec3u){let i=gid.x;if(i>=nodeCount()){return;}let reach=bitcast<f32>(atomicLoad(&arena[11]));let current=atomicLoad(&control[6])&1u;let redistance=atomicLoad(&arena[12])==1u;let bank=select(current,1u-current,redistance);let centre=phi[i][bank];if(!finite(centre)){atomicOr(&control[12],ERR_GRAPH);bandMask[i]=0u;return;}var scheduled=(redistance&&p.policy.y!=0u)||abs(centre)<reach;var mixed=false;var frozen=false;var memberships=0u;if(8u*i+7u>=arrayLength(&incidentLeaves)){atomicOr(&control[12],ERR_GRAPH);}else{for(var incident=0u;incident<8u;incident+=1u){let leaf=incidentLeaves[8u*i+incident];if(leaf==INVALID){continue;}if(leaf>=leafCount()){atomicOr(&control[12],ERR_GRAPH);continue;}let record=leaves[leaf];var minimumPhi=3.402823e38;var maximumPhi=-3.402823e38;var localCorner=INVALID;var valid=true;for(var corner=0u;corner<8u;corner+=1u){let slot=cornerSlot(record,corner);if(slot>=nodeCount()){atomicOr(&control[12],ERR_GRAPH);valid=false;continue;}let value=phi[slot][bank];if(!finite(value)){atomicOr(&control[12],ERR_GRAPH);valid=false;continue;}scheduled=scheduled||abs(value)<reach;minimumPhi=min(minimumPhi,value);maximumPhi=max(maximumPhi,value);if(slot==i){localCorner=corner;}}if(!valid){continue;}if(localCorner==INVALID){continue;}memberships+=1u;mixed=mixed||(minimumPhi<=0.&&maximumPhi>=0.);for(var axis=0u;axis<3u;axis+=1u){let other=cornerSlot(record,localCorner^(1u<<axis));if(other>=nodeCount()){atomicOr(&control[12],ERR_GRAPH);continue;}let otherPhi=phi[other][bank];if(!finite(otherPhi)){atomicOr(&control[12],ERR_GRAPH);continue;}frozen=frozen||changesSign(centre,otherPhi);}}}if(memberships==0u){atomicOr(&control[12],ERR_GRAPH);}scheduled=scheduled||mixed||frozen;bandMask[i]=select(0u,1u,scheduled)|select(0u,2u,frozen);}
`;
...
const INVALID:u32=0xffffffffu;const ERR_GRAPH:u32=1u;struct Params{dims:vec4u,originCell:vec4f,limits:vec4u,numerics:vec4f,policy:vec4u,volume:vec4f,inflow0:vec4f,inflow1:vec4f}struct Constraint{header:vec4u,masters:vec4u,numerators:vec4u}struct LeafRecord{originSpan:vec4u,info:vec4u,corners0:vec4u,corners1:vec4u}
@group(0)@binding(0)var<uniform>p:Params;@group(0)@binding(1)var<storage,read>graph:array<u32>;@group(0)@binding(2)var<storage,read>constraints:array<Constraint>;@group(0)@binding(3)var<storage,read_write>control:array<atomic<u32>>;@group(0)@binding(4)var<storage,read_write>phi:array<vec2f>;@group(0)@binding(5)var<storage,read_write>transportControl:array<atomic<u32>>;@group(0)@binding(6)var<storage,read_write>worklist:array<u32>;@group(0)@binding(7)var<storage,read>bandMask:array<u32>;@group(0)@binding(8)var<storage,read>leaves:array<LeafRecord>;@group(0)@binding(9)var<storage,read>incidentLeaves:array<u32>;@group(0)@binding(10)var<storage,read>velocity:array<vec4u>;
fn nodeCount()->u32{return min(min(min(graph[2],p.limits.x),arrayLength(&constraints)),arrayLength(&bandMask));}fn bank()->u32{return atomicLoad(&control[6])&1u;}fn capacity()->u32{return min(p.limits.x,arrayLength(&worklist)/2u);}fn cornerSlot(r:LeafRecord,c:u32)->u32{if(c<4u){return r.corners0[c];}return r.corners1[c-4u];}fn velocityValid(i:u32)->bool{return 2u*i<arrayLength(&velocity)&&(velocity[2u*i].w&7u)==7u;}fn characteristicSupport(i:u32)->bool{if(!velocityValid(i)||8u*i+7u>=arrayLength(&incidentLeaves)){return false;}for(var incident=0u;incident<8u;incident+=1u){let leaf=incidentLeaves[8u*i+incident];if(leaf==INVALID){continue;}if(leaf>=arrayLength(&leaves)){return false;}let r=leaves[leaf];for(var corner=0u;corner<8u;corner+=1u){if(!velocityValid(cornerSlot(r,corner))){return false;}}}return true;}@compute @workgroup_size(64)fn publishTransportIndependent(@builtin(global_invocation_id)gid:vec3u){let i=gid.x;if(i>=nodeCount()||constraints[i].header.y!=0u){return;}var marked=bandMask[i];if(marked>3u){atomicOr(&control[12],ERR_GRAPH);return;}if(atomicLoad(&transportControl[12])==0u&&!characteristicSupport(i)){marked=0u;}if((marked&1u)==0u){if(atomicLoad(&transportControl[12])==0u){let current=bank();phi[i][1u-current]=phi[i][current];}atomicAdd(&transportControl[1],1u);return;}let rank=atomicAdd(&transportControl[0],1u);if(rank>=capacity()){atomicOr(&control[12],ERR_GRAPH);atomicStore(&transportControl[4],1u);return;}worklist[rank]=i|select(0u,0x80000000u,(marked&2u)!=0u&&atomicLoad(&transportControl[12])==1u);}
`;
...
fn exactEdgeSeed(i:u32,b:u32)->EdgeSeed{if(i>=count()||8u*i+7u>=arrayLength(&incidentLeaves)){atomicOr(&control[12],ERR_GRAPH);return EdgeSeed(FAR,0u);}let centre=phi[i][b];if(!finite(centre)){atomicOr(&control[12],ERR_GRAPH);return EdgeSeed(FAR,0u);}let centreMagnitude=abs(centre);var seed=FAR;var frozen=0u;var memberships=0u;for(var incident=0u;incident<8u;incident+=1u){let leaf=incidentLeaves[8u*i+incident];if(leaf==INVALID){continue;}if(leaf>=leafCount()){atomicOr(&control[12],ERR_GRAPH);continue;}let r=leaves[leaf];var localCorner=INVALID;for(var c=0u;c<8u;c+=1u){let slot=cornerSlot(r,c);if(slot>=count()){atomicOr(&control[12],ERR_GRAPH);continue;}if(slot==i){localCorner=c;}}if(localCorner==INVALID){continue;}memberships+=1u;let edgeLength=f32(r.originSpan.w)*p.originCell.w;for(var axis=0u;axis<3u;axis+=1u){let other=cornerSlot(r,localCorner^(1u<<axis));if(other>=count()){atomicOr(&control[12],ERR_GRAPH);continue;}let otherPhi=phi[other][b];if(!finite(otherPhi)){atomicOr(&control[12],ERR_GRAPH);continue;}if(changesSign(centre,otherPhi)){let denominator=centreMagnitude+abs(otherPhi);var crossing=0.;if(denominator>0.){crossing=edgeLength*centreMagnitude/denominator;}if(!finite(crossing)||crossing<0.){atomicOr(&control[12],ERR_GRAPH);continue;}seed=min(seed,crossing);frozen=1u;}}}if(memberships==0u){atomicOr(&control[12],ERR_GRAPH);}return EdgeSeed(seed,frozen);}
...
const ADAPTIVE_PHI_MAGIC:u32=0x41504849u;
...
// Publish the retained/prolongated phi adopted by a topology handoff without
// manufacturing another scalar generation.
@compute @workgroup_size(1)fn publishTopologyHandoff(){let epoch=atomicLoad(&candidateGraph[0]);let generation=atomicLoad(&candidateGraph[5]);let ready=epoch!=0u&&atomicLoad(&candidateGraph[3])==epoch&&atomicLoad(&candidateGraph[4])==0u&&generation!=0u&&atomicLoad(&state[0])==ADAPTIVE_PHI_MAGIC&&atomicLoad(&state[1])==epoch&&atomicLoad(&state[2])==generation&&atomicLoad(&state[7])==1u&&atomicLoad(&state[12])==0u;
 // The ready topology commit adopts the candidate phi already transferred and
 // projected by the handoff.  It is an identity publication, not an external
 // mass-to-phi reconstruction and not another scalar generation.
 if(ready){atomicStore(&receipts[1],0u);atomicStore(&receipts[2],0u);atomicStore(&receipts[3],0u);atomicStore(&receipts[4],0u);atomicStore(&receipts[5],0u);atomicStore(&receipts[7],0u);atomicStore(&receipts[12],0u);atomicStore(&receipts[14],1u);atomicStore(&receipts[15],atomicLoad(&candidateGraph[1]));atomicStore(&receipts[22],1u);atomicStore(&receipts[23],generation);atomicStore(&receipts[32],0u);atomicStore(&receipts[33],atomicLoad(&candidateGraph[2]));atomicStore(&receipts[34],0u);atomicStore(&receipts[35],0u);atomicStore(&receipts[54],0u);}else if(epoch!=0u){let reason=select(0u,1u,atomicLoad(&candidateGraph[3])!=epoch)|select(0u,2u,atomicLoad(&candidateGraph[4])!=0u)|select(0u,4u,generation==0u)|select(0u,8u,atomicLoad(&state[1])!=epoch)|select(0u,16u,atomicLoad(&state[2])!=generation)|(atomicLoad(&state[12])<<8u);atomicStore(&receipts[54],reason);atomicStore(&receipts[14],0u);atomicStore(&receipts[22],0u);}}
`;
...
@group(0)@binding(11)var<storage,read> surfaceMass:array<f32>;
...
fn densityLeafSample(leaf:u32)->Sample{if(leaf==INVALID||leaf>=leafCount()||leaf>=arrayLength(&surfaceMass)){return Sample(0.,0u);}let span=f32(leaves[leaf].originSpan.w);let width=span*p.originCell.w;let volume=width*width*width;let rho=surfaceMass[leaf]/max(volume,1e-30);let value=(.5-rho)*width;return Sample(value,select(0u,1u,finite(rho)&&rho>=0.&&finite(value)));}
fn sampleDensity(q:vec3f)->Sample{if(q.y>f32(p.dims.y)&&p.policy.x!=0u){return Sample(max(p.numerics.w,.5*p.originCell.w),1u);}let bounded=clamp(q,vec3f(0),vec3f(p.dims.xyz));let ownerPoint=min(bounded,vec3f(p.dims.xyz)-vec3f(.00001));return densityLeafSample(findLeafHalfOpen(ownerPoint));}
...
@compute @workgroup_size(64)fn deriveGhosts(@builtin(global_invocation_id)gid:vec3u){
 ...
 var negativePoint=vec3f(0);var positivePoint=vec3f(0);var negativeSample=Sample(0.,0u);var positiveSample=Sample(0.,0u);if(negativeLeaf!=INVALID){negativePoint=leafCentre(negativeLeaf);negativeSample=densityLeafSample(negativeLeaf);}if(positiveLeaf!=INVALID){positivePoint=leafCentre(positiveLeaf);positiveSample=densityLeafSample(positiveLeaf);}
 if(negativeLeaf==INVALID){negativePoint=positivePoint;negativePoint[f.axis]-=f32(leaves[positiveLeaf].originSpan.w);negativeSample=sampleDensity(negativePoint);}if(positiveLeaf==INVALID){let wetSpan=f32(leaves[negativeLeaf].originSpan.w);let direction=select(1.,-1.,(f.reserved&FACE_ROW_ON_POSITIVE_SIDE)!=0u);var probe=negativePoint;probe[f.axis]+=direction*(.5*wetSpan+.0001);let airLeaf=findLeafHalfOpen(probe);if(airLeaf!=INVALID&&airLeaf<leafCount()){positivePoint=leafCentre(airLeaf);positiveSample=densityLeafSample(airLeaf);}else{positivePoint=negativePoint;positivePoint[f.axis]+=direction*wetSpan;positiveSample=sampleDensity(positivePoint);}}
...
const MAGIC:u32=0x4c504849u;const RENDERER_CELL_CENTERED:u32=0x08000000u;const ADAPTIVE_CORNERS:u32=0x10000000u;const MASS_EVIDENCE:u32=0x40000000u;struct Params{dims:vec4u,originCell:vec4f,limits:vec4u,numerics:vec4f,policy:vec4u,volume:vec4f}
struct LeafRecord{originSpan:vec4u,info:vec4u,corners0:vec4u,corners1:vec4u}
struct LeafLookup{cell:u32,span:u32,slot:u32,flags:u32}
struct Constraint{header:vec4u,masters:vec4u,numerators:vec4u}
...
// Mirror adaptive-mass's 65536-unit finest-cell ABI once, at publication.
// Downstream topology combines only integer units and integer overlap counts;
// it never averages rho in floating-point traversal order.
fn evidenceMassUnits(slot:u32)->vec2u{if(slot>=graph[1]||slot>=arrayLength(&leaves)||slot>=arrayLength(&surfaceMass)){return vec2u(0u);}let h=p.originCell.w;let quantum=(h*h*h)/65536.;let raw=surfaceMass[slot]/max(quantum,1e-30);if(!finite(raw)||raw<0.||raw>4294967040.){return vec2u(0u);}return vec2u(u32(floor(raw+.5)),1u);}
fn evidenceRho(slot:u32)->f32{if(slot>=graph[1]||slot>=arrayLength(&leaves)){return -1.;}let fixed=evidenceMassUnits(slot);if(fixed.y==0u){return -1.;}let span=leaves[slot].originSpan.w;let cells=span*span*span;let fullUnits=cells*65536u;if(cells==0u||fullUnits==0u){return -1.;}return f32(fixed.x)/f32(fullUnits);}
...
struct RendererNodeEvidence{rho:f32,occupiedOctants:u32,liquidOctants:u32}
// One raw conservative value per geometric tree node, shared by all incident
// leaves. The occupied-octant count distinguishes a broad sheet (four spatial
// octants at one side of a node) from an isolated one/two-octant finger. A
// true sub-grid sheet additionally has no incident rho>=.5 cell; otherwise the
// same four-octant pattern is merely an ordinary bulk front.
fn rendererNodeEvidence(node:vec3u)->RendererNodeEvidence{var total=0.;var count=0u;var occupied=0u;var liquid=0u;for(var mask=0u;mask<8u;mask+=1u){let q=vec3i(node)+vec3i(select(-1,0,(mask&1u)!=0u),select(-1,0,(mask&2u)!=0u),select(-1,0,(mask&4u)!=0u));if(any(q<vec3i(0))||any(q>=vec3i(p.dims.xyz))){continue;}let leaf=evidenceLocate(vec3u(q));if(leaf==0xffffffffu){continue;}let rho=evidenceRho(leaf);if(rho>=0.){total+=rho;count+=1u;occupied+=select(0u,1u,rho>1e-5);liquid+=select(0u,1u,rho>=.5);}}if(count==0u){return RendererNodeEvidence(-1.,0u,0u);}return RendererNodeEvidence(total/f32(count),occupied,liquid);}
...
fn topologyHashMatches(rows:u32,cap:u32)->bool{let epoch=graph[0];return atomicLoad(&arena[0])==MAGIC&&epoch!=0u&&atomicLoad(&arena[1])==epoch&&atomicLoad(&arena[2])==rows&&atomicLoad(&arena[3])==0u&&atomicLoad(&arena[4])==p.dims.w&&atomicLoad(&arena[5])==p.dims.x&&atomicLoad(&arena[6])==p.dims.y&&atomicLoad(&arena[7])==p.dims.z&&atomicLoad(&arena[9])==20u&&atomicLoad(&arena[10])==20u+8u*p.limits.y&&atomicLoad(&arena[11])==cap&&atomicLoad(&arena[13])==0u&&atomicLoad(&arena[18])==20u+8u*p.limits.y+4u*cap&&atomicLoad(&arena[19])==epoch;}
...
@compute @workgroup_size(64)fn buildTopologyEvidenceHash(@builtin(global_invocation_id)gid:vec3u){let row=gid.x;let rows=min(graph[1],p.limits.y);let cap=p.policy.w;if(row>=rows||atomicLoad(&arena[13])!=0u||atomicLoad(&arena[19])==graph[0]){return;}let entry=leafDirectory[row];let wanted=hash(entry.cell,entry.span);var installed=false;for(var probe=0u;probe<32u;probe+=1u){let at=20u+8u*p.limits.y+4u*((wanted+probe)&(cap-1u));var claim=atomicCompareExchangeWeak(&arena[at],0u,entry.cell+1u);loop{if(claim.exchanged||claim.old_value!=0u){break;}claim=atomicCompareExchangeWeak(&arena[at],0u,entry.cell+1u);}if(claim.exchanged){atomicStore(&arena[at+1u],entry.span);atomicStore(&arena[at+2u],row+1u);atomicStore(&arena[at+3u],wanted);installed=true;break;}}if(!installed){atomicOr(&arena[13],4u);}}
...
fn publishTopologyEvidenceRows(@builtin(global_invocation_id)gid:vec3u){
 ...
 let fixedMass=evidenceMassUnits(entry.slot);
 if(fixedMass.y==0u){atomicOr(&arena[13],32u);return;}
 ...
 let flags=3u|ADAPTIVE_CORNERS|MASS_EVIDENCE|select(0u,4u,mn<=0.&&mx>=0.)
 ...
 // Topology word +7 is exact mass evidence. Renderer word +7 deliberately
 // retains its established geometric-volume ABI.
 atomicStore(&arena[base+7u],fixedMass.x);
 let rb=8u+8u*row;renderer[rb]=entry.cell+1u;renderer[rb+1u]=entry.span;
 // Conserved mass supplies the nodal sign; the redistanced SDF contributes
 // metric curvature only when it agrees. Retaining a negative SDF in an
 // air-side mass node creates renderer-only sheets and disconnected fragments.
 let rendererRho=evidenceRho(entry.slot);
 ...
 for(var corner=0u;corner<8u;corner+=1u){let highX=(corner&1u)!=0u;let highZ=(corner&4u)!=0u;let node=leaf.originSpan.xyz+vec3u(select(0u,entry.span,highX),select(0u,entry.span,(corner&2u)!=0u),select(0u,entry.span,highZ));let evidence=rendererNodeEvidence(node);if(evidence.rho<0.){atomicOr(&arena[13],32u);return;}let massPhi=canonicalPublishedZero((.5-evidence.rho)*p.originCell.w);let sdfPhi=bitcast<f32>(renderer[aux+corner]);let signsAgree=(massPhi<=0.&&sdfPhi<=0.)||(massPhi>=0.&&sdfPhi>=0.);let value=canonicalPublishedZero(select(massPhi,sdfPhi,signsAgree));rendererTerms[corner]=value;rendererMinimum=min(rendererMinimum,value);rendererMaximum=max(rendererMaximum,value);renderer[aux+corner]=bitcast<u32>(value);}
...
@compute @workgroup_size(1)fn finishTopologyEvidence(){let rows=min(graph[1],p.limits.y);let cap=p.policy.w;let epoch=graph[0];let publicationMatches=evidenceValid(rows,cap)&&atomicLoad(&arena[1])==epoch&&atomicLoad(&arena[2])==rows&&atomicLoad(&arena[12])==state[2]&&atomicLoad(&arena[14])==graph[5];if(!publicationMatches){atomicOr(&arena[13],16u);}if(atomicLoad(&arena[13])==0u){renderer[0]=0x80000000u;renderer[1]=state[2];renderer[2]=rows;renderer[3]=p.dims.w;renderer[4]=p.dims.x;renderer[5]=p.dims.y;renderer[6]=p.dims.z;renderer[7]=bitcast<u32>(p.originCell.w);atomicStore(&arena[19],epoch);atomicStore(&arena[0],MAGIC);atomicStore(&receipts[15],rows);atomicStore(&receipts[23],state[2]);}else{atomicStore(&arena[0],0u);atomicStore(&arena[19],0u);}}
`;
