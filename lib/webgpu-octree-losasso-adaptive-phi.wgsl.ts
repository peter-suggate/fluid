...
@compute @workgroup_size(64)fn prepareTopologyEvidence(@builtin(global_invocation_id)gid:vec3u){let item=gid.x;let rows=min(graph[1],p.limits.y);let cap=p.policy.w;if(item<arrayLength(&renderer)){renderer[item]=0u;}if(item<cap){let at=20u+8u*p.limits.y+4u*item;atomicStore(&arena[at],0u);atomicStore(&arena[at+1u],0u);atomicStore(&arena[at+2u],0u);atomicStore(&arena[at+3u],0u);}if(item!=0u){return;}atomicStore(&receipts[15],0u);atomicStore(&arena[0],0u);atomicStore(&arena[13],0u);if(!evidenceValid(rows,cap)){atomicStore(&arena[13],1u);return;}
 renderer[0]=0x80000000u;renderer[1]=state[2];renderer[2]=rows;renderer[3]=p.dims.w;renderer[4]=p.dims.x;renderer[5]=p.dims.y;renderer[6]=p.dims.z;renderer[7]=bitcast<u32>(p.originCell.w);
 atomicStore(&arena[1],graph[0]);atomicStore(&arena[2],rows);atomicStore(&arena[3],0u);atomicStore(&arena[4],p.dims.w);atomicStore(&arena[5],p.dims.x);atomicStore(&arena[6],p.dims.y);atomicStore(&arena[7],p.dims.z);atomicStore(&arena[8],bitcast<u32>(p.originCell.w));atomicStore(&arena[9],20u);atomicStore(&arena[10],20u+8u*p.limits.y);atomicStore(&arena[11],cap);atomicStore(&arena[12],state[2]);atomicStore(&arena[14],graph[5]);atomicStore(&arena[15],bitcast<u32>(p.originCell.x));atomicStore(&arena[16],bitcast<u32>(p.originCell.y));atomicStore(&arena[17],bitcast<u32>(p.originCell.z));atomicStore(&arena[18],20u+8u*p.limits.y+4u*cap);}
...
@compute @workgroup_size(1)fn finishTopologyEvidence(){let rows=min(graph[1],p.limits.y);let cap=p.policy.w;if(!evidenceValid(rows,cap)||atomicLoad(&arena[13])!=0u){return;}for(var row=0u;row<rows;row+=1u){let entry=leafDirectory[row];let wanted=hash(entry.cell,entry.span);var installed=false;for(var probe=0u;probe<32u;probe+=1u){let at=20u+8u*p.limits.y+4u*((wanted+probe)&(cap-1u));if(atomicLoad(&arena[at])==0u){atomicStore(&arena[at],entry.cell+1u);atomicStore(&arena[at+1u],entry.span);atomicStore(&arena[at+2u],row+1u);atomicStore(&arena[at+3u],wanted);installed=true;break;}}if(!installed){atomicOr(&arena[13],4u);break;}}if(atomicLoad(&arena[13])==0u){atomicStore(&arena[0],MAGIC);atomicStore(&receipts[15],rows);atomicStore(&receipts[23],state[2]);}}
`;
