...
export function resolveSvoPrimaryTraversal(
  ...
  void scale;
  const primaryTraversal = SVO_PRIMARY_TRAVERSAL_OVERRIDE ?? (requested === "raster" ? "mesh" : requested);
  return resolveSvoPipelineComposition({ primaryTraversal }).primaryTraversal;
}
