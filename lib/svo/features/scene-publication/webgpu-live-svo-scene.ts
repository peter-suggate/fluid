...
} from "../construction/webgpu-svo-sparse-bricks";
import {
...
export class WebGPULiveSvoScene implements GPUSolverInstance {
  ...
  private static async buildWorld(
    ...
    const gpuSelection = options.cpuBrickSelection ? undefined
      : (await import("../primary-visibility/webgpu-svo-brick-selection")).selectSvoBrickOccupancyGpu;
    const gpuClassification = options.cpuBrickSelection ? undefined
      : (await import("../construction/webgpu-svo-node-classification")).classifySvoNodesGpu;
