...
export class WebGPUOctreeLosassoVelocityMigration {
  ...
  async initialize(): Promise<void> { this.assertLive(); if (this.pipeline) return;
    ...
    this.pipeline = await this.device.createComputePipelineAsync({ label: "Migrate Losasso lagged wet velocity",
  ...
  encodeMigration(broker: PassBroker, target: WebGPUOctreeLosassoVelocityMigrationSource): void {
    this.assertReady(); const pipeline = this.pipeline!;
    const buffers = [this.params, this.oldControl, this.oldGeometry, this.oldDirectory,
      this.oldVelocity, target.control, target.faceGeometry, target.extendedVelocity];
    ...
    pass.setPipeline(pipeline); pass.setBindGroup(0, this.device.createBindGroup({
      layout: pipeline.getBindGroupLayout(0), entries: buffers.map((buffer, binding) => ({
  ...
  destroy(): void { if (this.destroyed) return; this.destroyed = true;
    for (const buffer of [this.params, this.oldControl, this.oldGeometry,
      this.oldDirectory, this.oldVelocity]) buffer.destroy(); this.pipeline = undefined; }
  private assertReady(): void { this.assertLive(); if (!this.pipeline) throw new Error("Losasso velocity migration is not initialized"); }
