...
const INVALID:u32=0xffffffffu;const LIMBS:u32=36u;
...
@group(0)@binding(2)var<storage,read>oldGeometry:array<vec4u>;
@group(0)@binding(3)var<storage,read>oldDirectory:array<vec2u>;
@group(0)@binding(4)var<storage,read>oldVelocity:array<f32>;
@group(0)@binding(5)var<storage,read>newControl:array<u32>;
@group(0)@binding(6)var<storage,read>newGeometry:array<vec4u>;
@group(0)@binding(7)var<storage,read_write>newVelocity:array<f32>;
...
fn exactOld(packed:u32,q:vec3u)->u32{if(p.directoryCapacity==0u||(p.directoryCapacity&(p.directoryCapacity-1u))!=0u){return INVALID;}
 let hash=hashFace(packed,q);let mask=p.directoryCapacity-1u;for(var probe=0u;probe<32u;probe+=1u){
  ...
  if(record.y==hash&&face<oldControl[2]&&face<arrayLength(&oldGeometry)
   &&all(oldGeometry[face]==vec4u(packed,q))){return face;}}return INVALID;}
...
fn migrateLosassoLaggedVelocity(@builtin(global_invocation_id)invocation:vec3u){let face=invocation.x;
 ...
 var exact:array<i32,36>;var count=0u;for(var b=0u;b<span;b+=1u){for(var a=0u;a<span;a+=1u){
  ...
  if(old!=INVALID&&old<arrayLength(&oldVelocity)){let value=oldVelocity[old];if(finite(value)){addExact(&exact,value);count+=1u;}}
