...
/**
 * Signed radix-256 row fold, the same superaccumulator `applyLosassoOperator`
 * uses.
 *
 * Face-id and child-row order is not D4-canonical: a row and its mirror image
 * walk the same multiset in different sequences, and a compensated fold parts
 * them by an ulp. The smoother divides by its own coefficient sum and the
 * restriction sums a child list, so both have to survive the mirror, and
 * integer limbs are the only accumulation that does. Only the decode rounds.
 */
const rowFoldWGSL = /* wgsl */ `
...
fn image(row:u32)->vec2f{
 ...
 let centre=x[row];var imageSum:array<i32,36>;
 ...
 for(var cursor=begin;cursor<end;cursor+=1u){let faceId=rowFaces[cursor];if(faceId>=arrayLength(&faces)){valid=false;continue;}
  ...
  addExact(&diagonalSum,coefficient);
  addExact(&imageSum,term);
 }
 let value=exactValue(&imageSum);let diagonal=exactValue(&diagonalSum);
...
fn image(row:u32)->vec2f{if(row+1u>=arrayLength(&offsets)){return vec2f(3.402823e38,-1.);}
 ...
 let centre=x[row];var imageSum:array<i32,36>;var diagonalSum:array<i32,36>;var valid=true;
 for(var cursor=begin;cursor<end;cursor+=1u){let edgeId=rowEdges[cursor];if(edgeId>=arrayLength(&edges)){valid=false;continue;}
  ...
  let term=coefficient*difference;if(!finite(coefficient)||coefficient<0.||!finite(term)){valid=false;continue;}
  addExact(&diagonalSum,coefficient);addExact(&imageSum,term);
 }
 let value=exactValue(&imageSum);let diagonal=exactValue(&diagonalSum);
 return select(vec2f(3.402823e38,-1.),vec2f(value,diagonal),valid&&finite(value)&&finite(diagonal)&&diagonal>0.);
}
...
@compute @workgroup_size(64)fn restrictLevel(@builtin(global_invocation_id)g:vec3u){
 ...
 for(var cursor=begin;cursor<end;cursor+=1u){let child=childList[cursor];
  if(child<fineControl[1]){addExact(&sum,input[child]);}
 }
 output[row]=exactValue(&sum);
}
...
fn restrictInto(coarseLevel:u32,lane:u32,enabled:bool){
 ...
 for(var row=lane;row<rows;row+=256u){let begin=hierarchy[base+CHILD_OFFSETS[coarseLevel]+row];
  let end=hierarchy[base+CHILD_OFFSETS[coarseLevel]+row+1u];var sum:array<i32,36>;
  for(var cursor=begin;cursor<end;cursor+=1u){let child=hierarchy[base+CHILD_LIST_OFFSETS[coarseLevel]+cursor];
   addExact(&sum,residualVectors[vectorIndex(coarseLevel-1u,child)]);
  }
  rhsVectors[vectorIndex(coarseLevel,row)]=exactValue(&sum);
