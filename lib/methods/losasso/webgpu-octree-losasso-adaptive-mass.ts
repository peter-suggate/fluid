...
import { OCTREE_LOSASSO_ADAPTIVE_MASS_RECEIPT_WORDS } from "../octree-shared/octree-losasso-receipt-abi";

export const OCTREE_LOSASSO_ADAPTIVE_MASS_CONTROL_WORDS = 32;

/** The receipt is the consumer half of this producer: the shared engine copies
 * it at the step tail and decodes it for the mass statistics it reports. Its
 * format lives in `octree-shared/octree-losasso-receipt-abi.ts` and is
 * re-exported here so this producer, the harness and their tests keep naming
 * it in one place. */
export {
  OCTREE_LOSASSO_ADAPTIVE_MASS_MAGIC,
  OCTREE_LOSASSO_ADAPTIVE_MASS_RECEIPT_WORDS,
  unpackAdaptiveMassReceipt,
} from "../octree-shared/octree-losasso-receipt-abi";
