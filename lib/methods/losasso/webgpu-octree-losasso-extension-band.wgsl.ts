...
import { makeFineLevelSetSortedWorklistLookupWGSL } from
  "../octree-shared/webgpu-octree-fine-levelset-bricks";
...
fn publishLosassoAirBandFaces(@builtin(workgroup_id)group:vec3u,@builtin(local_invocation_index)lane:u32){
 ...
 if(any(cell>=p.velocityDimensions)){return;}
 // Whether this page may carry outermost W7 faces is a property of the page,
 // not of one surviving sample; the membership comment below names the D4
 // failure that distinction prevents.
 var pageWithinBand=false;
 for(var local=0u;local<p.samplesPerBrick;local+=1u){
  let sampleQ=brick*p.brickResolution+vec3u(local%p.brickResolution,
   (local/p.brickResolution)%p.brickResolution,local/(p.brickResolution*p.brickResolution));
  if(any(sampleQ>=p.sampleDimensions)){continue;}
  let at=page*p.samplesPerBrick+local;
  if(at>=arrayLength(&samples)||(bandFinePackedFlags(at)&VALID)==0u){continue;}
  if(!finite(bandFinePackedPhi(at))){continue;}
  pageWithinBand=true;break;}
 for(var local=0u;local<facesPerBrick;local+=1u){let axis=local/facesPerAxis;let faceLocal=local%facesPerAxis;
 ...
 let sample=finePhiCells(centre);
 let faceProbeResolved=sample.valid!=0u;
 ...
 // coarse face past that sample's B4 page, so at a diagonal sparse-topology
 // edge every phi corner of a face can be unresident even though the page that
 // needs the face is squarely inside the band. Such a face is admitted on the
 // page's authority, never on a second phi probe: the redistanced fine band
 // ends at a JFA level set whose radius is only accurate to a fraction of a
 // fine cell, so which of the outermost samples it keeps is decided by that
 // error rather than by geometry. Every kept sample is inside the band by
 // construction, which is why the magnitude test cannot reject a resolved face
 // either -- the whole outer boundary of this publication *is* the fine band
 // mask. The previous fallback re-probed one page-clamped point and let that
 // single sample's survival publish the face, so a mirrored pair whose JFA
 // distances straddled the mask edge by a tenth of a fine cell published on
 // the high side only, and the resulting one-sided W7 ring drove the extension
 // asymmetric. Page residency is the granularity the halo argument was always
 // made at, and it survives the mask's own edge noise.
 if(faceProbeResolved){if(abs(sample.value)>width){continue;}}
 else if(!pageWithinBand){continue;}
 // Every page whose halo reaches this face proposes it, and an atomic claim
 // picks one arbitrary proposer to write the record. Membership survives that
 // race because the published set is their union, and the stamp must be
 // proposer-independent for the same reason: an earlier page-anchored fallback
 // let two pages disagree about a face by up to a coarse cell of phi, and
 // whichever won the claim decided whether the final dilation re-expanded it,
 // so under a mirror the outermost ring came out D4-asymmetric and run-to-run
 // unstable. Only the face-local probe is a pure function of the face, so only
 // it may set the ring. Where it found no resident support the face sits
 // exactly where fine topology has run out: stamp it at the band edge, which
 // is both proposer-independent and the correct place to stop.
 let distance=select(width,abs(sample.value),faceProbeResolved);
 let layer=select(p.band.w,
  min(p.band.w,max(1u,u32(ceil(abs(sample.value)/p.velocityOriginCell.w)))),faceProbeResolved);
 appendAirFace(vec4u(axis,faceQ),vec4u(axis,ACTIVE,bitcast<u32>(distance),layer));}}
