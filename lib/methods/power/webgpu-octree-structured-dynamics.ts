...
import { octreeAlgorithmDiagnosticsEnabled } from "../octree-shared/octree-algorithm-diagnostics";
import { GPU_RIGID_BODY_CAPACITY } from "../../core/webgpu-rigid-body";
import type { SurfaceInflowState } from "../octree-shared/surface-state";
import { STRUCTURED_PROJECTION_ENERGY_WORDS } from "../octree-shared/octree-structured-receipt-abi";

/** The four-stage energy reduction is decoded by the shared engine, so its
 * word count and decoder live in
 * `octree-shared/octree-structured-receipt-abi.ts`; re-exported here so this
 * producer, the harness and its tests keep naming the format in one place. */
export {
  STRUCTURED_PROJECTION_ENERGY_WORDS,
  decodeStructuredProjectionEnergy,
  type StructuredProjectionEnergyDecode,
  type StructuredProjectionEnergySample,
} from "../octree-shared/octree-structured-receipt-abi";
