...
} from "../octree-shared/octree-air-velocity-support-abi";

/**
 * The arena ABI now lives in `octree-shared/octree-air-velocity-support-abi.ts`
 * because the shared coarse-summary and fine-transport stages read the same
 * arena and may not import a lane. Re-exported here so every existing consumer
 * of this module keeps one import.
 */
export {
  OCTREE_AIR_SUPPORT_LAYOUT_VERSION,
  OCTREE_AIR_SUPPORT_VALID,
...
export type { OctreeAirVelocitySupportLayout } from "../octree-shared/octree-air-velocity-support-abi";
...
export function planOctreeAirVelocitySupport(
  ...
  const maximumSupportCapacity = u32(maximumSupportCapacityValue,
    "Air-support configured capacity");
  ...
  // owners are dropped before a candidate exists, and compact identity claims
  // merge duplicate cells before stable ordering. The domain's finest-cell
  // count is therefore a hard upper bound on the number of support records,
  // independent of the row count, and the smaller of the two bounds remains
  // provably >= the true maximum. (Both are conservative: rows are themselves
  // disjoint leaves, so the real bound is cells minus rows.)
  const supportCapacity = Math.min(
    ...
    maximumSupportCapacity > 0 ? maximumSupportCapacity : Number.POSITIVE_INFINITY);
  ...
  // At most rowCapacity + supportCapacity adaptive identities can be
  // published. A <= 0.5 load factor makes bounded linear probing complete;
  // aligning the terminal allocation keeps the capacity derivable in WGSL
  // from arrayLength without spending another uniform word.
  const ownerDirectorySlotCapacity = Math.ceil(checkedProduct("Air-support adaptive owner hash", 2,
    rowCapacity + supportCapacity) * 16 / alignment) * alignment / 16;
  const ownerDirectoryBytes = checkedProduct("Air-support adaptive owner directory",
    ownerDirectorySlotCapacity, 16);
  ...
  return Object.freeze({
    ...
    ownerDirectoryBytes, ownerDirectorySlotCapacity, ownerDirectoryCellCapacity,
    totalBytes,
  });
}
