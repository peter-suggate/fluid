...
import type { OctreePipelinedWorksetLinearOperator } from "../octree-shared/webgpu-octree-pipelined-mgpcg";
...
implements OctreeFirstOrderSPDVCycle {
  ...
  private pipelines!: Readonly<Record<HybridStage, GPUComputePipeline>>;
  ...
  constructor(
    ...
    this.shaderCode = octreeSection43HybridPreconditionerShader;
  ...
  async initializePipelines(): Promise<void> {
    ...
    this.pipelineInitialization = (async () => {
      ...
      const entries = Object.keys(HYBRID_BINDINGS) as HybridStage[];
...
fn diagonalAt(row: u32) -> f32 {
  return section63Coefficients[(accepted[5] & 1u) * section63.hierarchy.w + row * 19u];
}
fn validDiagonal(row: u32) -> bool {
  let diagonal = diagonalAt(row);
  return finite(diagonal) && diagonal > 0.0;
}
fn coefficientBase(row:u32)->u32{return (accepted[5]&1u)*section63.hierarchy.w+row*19u;}
...
fn bandWorksetBase(cls: u32) -> u32 {
  return (accepted[5] & 1u) * params.worksetBankStrideWords
    + cls * params.worksetStrideWords;
}
fn bandRow(item: u32) -> u32 {
  let base = bandWorksetBase(4u);
  if (atomicLoad(&bandWorksets[base]) != accepted[4]
...
fn prepareCorrectionDispatches() {
  ...
  let unionValid = atomicLoad(&bandWorksets[unionBase]) == accepted[4]
  ...
  for (var index = 0u; index < 2u; index += 1u) {
    ...
    transitionValid = transitionValid
      && atomicLoad(&bandWorksets[base]) == accepted[4]
