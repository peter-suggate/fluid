...
/** Binding-free exact IBO1 -> DCA1 TRA compiler and direct gamma resolver. */
...
fn itr1ApplyCanonical(packet:u32,mask:vec2u,slot:u32){
  ...
  for(var axis=0u;axis<3u;axis+=1u){
    ...
    dca1MarkRowMask(packet,axis,own.x|within.x,own.y|within.y);
    if(origin[axis]+4u<dims[axis]){let cross=itr1ShiftCross(positive&itr1Plane(axis,3u),axis);
      ...
      dca1MarkRowMask(packet+stride,axis,cross.x,cross.y);}
  }
}
    ...
    for(var side=0u;side<6u;side+=1u){let axis=side>>1u;
      ...
      for(var refLocal=0u;refLocal<refCount;refLocal+=1u){let faceRef=${p}IBORef(slot,leaf,side,refLocal);
        ...
        for(var at=begin;at<end;at+=1u){let entry=itr1Load(ITR1_BASE+directory.y+at);
          ...
          dca1MarkRowMask(owner.x,rowAxis,itr1Bit(owner.y).x,itr1Bit(owner.y).y);
        }} }
  }
}
fn ${p}DynamicClosureApplyTRA(packet:u32,mask:vec2u,worker16:u32){
  let slot=${p}IBOAcceptedSlot();if(worker16==0u){itr1ApplyCanonical(packet,mask,slot);}
  itr1ApplyBoundary(packet,mask,worker16,slot);
}
fn itr1StableRowForOwner(packet:u32,axis:u32,lane:u32)->u32{
  ...
  if(address.w==ITR1_INVALID){return ITR1_INVALID;}let local=address.xyz;let leaf=address.w;
  let descriptor=${p}IBOLeafDescriptorId(slot,leaf);
  if(local[axis]>0u){let dims=${p}IBOLeafDimensions(slot,leaf);
    if(axis==0u){return ${p}IBOCanonicalRowBase(descriptor,0u)+(local.x-1u)
      +(dims.x-1u)*(local.y+dims.y*local.z);}
    if(axis==1u){return ${p}IBOCanonicalRowBase(descriptor,1u)+local.x
      +dims.x*((local.y-1u)+(dims.y-1u)*local.z);}
    return ${p}IBOCanonicalRowBase(descriptor,2u)+local.x
      +dims.x*(local.y+dims.y*(local.z-1u));}
  ...
  for(var side=0u;side<6u;side+=1u){if((side>>1u)!=axis
    ...
    for(var refLocal=0u;refLocal<count;refLocal+=1u){let faceRef=${p}IBORef(slot,leaf,side,refLocal);
      ...
      let localRow=itr1Load(ITR1_BASE+directory.z+boundary);if(localRow!=ITR1_INVALID){
        return faceRef.z+${p}IBOTemplateRowWord(faceRef.x,localRow,0u);}}
  }return ITR1_INVALID;
}
fn ${p}DynamicClosureScatterGammaSnapshotRow(packet:u32,axis:u32,lane:u32){
  let row=itr1StableRowForOwner(packet,axis,lane);if(row!=ITR1_INVALID){${p}IBOTRAScatterSnapshot(row);}}
fn ${p}DynamicClosureScatterGammaRefinementRow(packet:u32,axis:u32,lane:u32){
  let row=itr1StableRowForOwner(packet,axis,lane);if(row!=ITR1_INVALID){${p}IBOTRAScatterRefinement(row);}}
`;
}
