...
export class AdaptiveMassFrameCapture {
  ...
  instrument(
    ...
    if (this.encoder) throw new Error("Sparse Geometric (CM12) frame capture already instrumented");
    this.gpu = gpu;
  ...
  finish(queue: Pick<GPUQueue, "onSubmittedWorkDone">): AdaptiveMassFrameCaptureResult {
    if (this.closed) throw new Error("Sparse Geometric (CM12) frame capture already closed");
    if (this.queueStartedAt_ms === undefined) {
      throw new Error("Sparse Geometric (CM12) queue capture was not started");
    }
    this.closed = true;
