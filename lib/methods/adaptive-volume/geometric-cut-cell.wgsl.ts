/**
 * Bounded convex clipping for a tetrahedron and two linear half-spaces.
 ...
 *
 * A tetrahedron clipped by two planes has at most 6 faces, 8 vertices and 12
 * edges. Storage allows 8 faces, 8 vertices per face and 16 cap candidates.
...
fn geometricClipPolyhedron(input:GeometricCutPolyhedron,normal:vec3f,
 offset:f32,solid:bool,tolerance:f32)->GeometricCutPolyhedron{
  ...
  var cap:array<GeometricCutVertex,16>;var capCount=0u;
  ...
  for(var face=0u;face<input.faces;face+=1u){
    ...
    var polygon:array<GeometricCutVertex,8>;var written=0u;
    var previous=input.vertices[face*8u+count-1u];
    var previousSigned=geometricCutSigned(previous,normal,offset,solid);
    for(var index=0u;index<count;index+=1u){
      let current=input.vertices[face*8u+index];
      ...
      if((previousSigned>=0.0)!=(currentSigned>=0.0)){
        ...
        if(written<8u){polygon[written]=point;written+=1u;}
        var unique=true;
        ...
        if(unique&&capCount<16u){cap[capCount]=point;capCount+=1u;}
      }
      if(currentSigned>=0.0&&written<8u){polygon[written]=current;written+=1u;}
      previous=current;previousSigned=currentSigned;
    }
    if(written>=3u&&output.faces<8u){
      let destination=output.faces*8u;
  ...
  if(capCount>=3u&&capCount<=8u&&output.faces<8u){
...
fn geometricCutPolyhedronVolume(poly:GeometricCutPolyhedron)->f32{
  ...
  for(var face=0u;face<poly.faces;face+=1u){
    for(var index=0u;index<poly.counts[face];index+=1u){
      center+=poly.vertices[face*8u+index].position;samples+=1u;
    }
  }
  if(samples==0u){return 0.0;}center/=f32(samples);
  var volume=0.0;
  for(var face=0u;face<poly.faces;face+=1u){
    let a=poly.vertices[face*8u].position;
    for(var index=1u;index+1u<poly.counts[face];index+=1u){
      volume+=geometricTetrahedronVolume(center,a,
        poly.vertices[face*8u+index].position,poly.vertices[face*8u+index+1u].position);
    }
  }
  return volume;
}
 ...
 solidSDF:vec4f,fluidNormal:vec3f,fluidOffset:f32)->GeometricCutVolumes{
  ...
  for(var face=0u;face<4u;face+=1u){
    ...
    for(var corner=0u;corner<3u;corner+=1u){let index=faces[face][corner];
      poly.vertices[face*8u+corner]=GeometricCutVertex(positions[index],solidSDF[index]);
    }
  }
  var capacity=whole;
  ...
  for(var face=0u;face<poly.faces;face+=1u){
    for(var index=0u;index<poly.counts[face];index+=1u){
      let value=dot(fluidNormal,poly.vertices[face*8u+index].position)-fluidOffset;
...
fn geometricClippedBoxVolumes(widths:vec3f,solidSDF:array<f32,8>,
 fluidNormal:vec3f,fluidOffset:f32)->GeometricCutVolumes{
  ...
  for(var tetra=0u;tetra<6u;tetra+=1u){
    ...
    let volume=geometricClippedTetrahedronVolumes(positions,values,fluidNormal,fluidOffset);
