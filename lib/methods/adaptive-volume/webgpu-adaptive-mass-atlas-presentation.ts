/**
 * Browser-safe presentation bridge for adaptive-volume sparse brick state.
 *
  ...
  for (let axis = 0; axis < 3; axis += 1) {
    ...
    if (!Number.isSafeInteger(value) || value < 1
      ...
      throw new RangeError(
        `adaptive-volume presentation dimension ${axis} must be in [1, `
...
export class WebGPUAdaptiveMassSparsePresentation {
  ...
  constructor(device: GPUDevice) {
    ...
    this.densityTexture = device.createTexture({
      label: "Sparse Geometric (CM12) compact-presentation fallback",
      size: [1, 1, 1],
      dimension: "3d",
      format: "r32float",
      usage: scalarUsage,
    });
    this.levelSetTexture = device.createTexture({ label: "Sparse Geometric (CM12) level-set fallback",
      size: [1, 1, 1], dimension: "3d", format: "r32float", usage: scalarUsage });
    this.gridCellTexture = device.createTexture({ label: "Sparse Geometric (CM12) owner fallback",
      size: [1, 1, 1], dimension: "3d", format: "rg32uint",
      usage: GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_DST
        | GPUTextureUsage.COPY_SRC
        | GPUTextureUsage.STORAGE_BINDING });
    this.velocityTexture = device.createTexture({ label: "Sparse Geometric (CM12) velocity fallback",
      size: [1, 1, 1], dimension: "3d", format: "rgba32float", usage: scalarUsage });
    this.pressureTexture = device.createTexture({ label: "Sparse Geometric (CM12) pressure fallback",
      size: [1, 1, 1], dimension: "3d", format: "r32float", usage: scalarUsage });
    this.divergenceTexture = device.createTexture({ label: "Sparse Geometric (CM12) divergence fallback",
      size: [1, 1, 1], dimension: "3d", format: "r32float", usage: scalarUsage });
  }
