...
export interface SparseCM12ResidentStageSeams {
   ...
   */
  readonly close: (stage: SparseCM12ResidentStageId, encoder: GPUCommandEncoder) => void;
  /**
...
export const planSparseCM12LinearDispatch = (
  ...
  if (!Number.isSafeInteger(workgroups) || workgroups < 0) {
    throw new RangeError(`Invalid Sparse Geometric (CM12) workgroup count: ${workgroups}`);
  }
...
fn retireSparseCM12PresentationPages(@builtin(global_invocation_id)gid:vec3u){
  ...
  ${worldDirectoryLayout ? /* wgsl */ `if(brick>=CM12_WDR_INITIAL_LEAVES){
    ...
    // Re-arm cached neighbour ownership at the actual directory deletion.
    // Clearing it at simulation retirement is too early: presentation can
    // retain this key until a later frame, allowing allocation to cache the
    // still-present owner again before this release removes it.
    for(var bit=0u;bit<27u;bit+=1u){
      ...
      if(neighbor!=INVALID){
        atomicAnd(&activity[ACTIVITY_HEADER+ACTIVITY_RECORD_WORDS*neighbor+45u],
          ~(1u<<(26u-bit)));
      }
    }
...
SparseCM12DeviceCompilationCache {
  ...
  const bindGroupLayout = device.createBindGroupLayout({
    label: "Sparse Geometric (CM12) resident layout",
    entries: [
  ...
  const cache: SparseCM12DeviceCompilationCache = {
    ...
    pipelineLayout: device.createPipelineLayout({
      label: "Sparse Geometric (CM12) resident pipeline layout",
...
const generationDeviceRoots = new WeakMap<GPUDevice, GPUDevice>();

/** Keep allocation accounting outside a resident factory's closure context.
 * A generation must never retain its predecessor through a device wrapper. */
function boundedGenerationDevice(input: GPUDevice, maximumBytes: number) {
  const root = generationDeviceRoots.get(input) ?? input;
  let preparing = true, reserved = 0;
  const allocated: GPUBuffer[] = [];
  const device = new Proxy(root, { get(target, property) {
    if (property === "createBuffer") return (descriptor: GPUBufferDescriptor) => {
      if (preparing && reserved + descriptor.size > maximumBytes) {
        throw new SparseCM12GenerationBudgetDeferred(reserved + descriptor.size, maximumBytes);
      }
      const buffer = target.createBuffer(descriptor);
      if (preparing) { reserved += descriptor.size; allocated.push(buffer); }
      return buffer;
    };
    const value = Reflect.get(target, property, target);
    return typeof value === "function" ? value.bind(target) : value;
  } });
  generationDeviceRoots.set(device, root);
  return { device,
    rollback() { for (const buffer of allocated) buffer.destroy(); },
    finish() { preparing = false; allocated.length = 0; },
  };
}
...
const ACTIVITY_RECORD_WORDS = 48;
...
/** Ordinary-scene physical residency budget. Callers may raise this for a
...
export function sparseCM12OwnershipTablePlan(
  ...
  if (!Number.isSafeInteger(residentBrickCount) || residentBrickCount < 0
    ...
    throw new RangeError(`Sparse Geometric (CM12) resident brick count ${residentBrickCount} is invalid`);
  }
...
export function sparseCM12TopologyPagePoolPlan(
  ...
  if (!Number.isSafeInteger(mutableFrontierBricks) || mutableFrontierBricks < 0) {
    throw new RangeError("Sparse Geometric (CM12) topology page demand must be a nonnegative safe integer");
  }
  const pageWords = gpuTopologyCellPageWords(brickFineResolution);
  if (!enabled || mutableFrontierBricks <= 0) return {
    requestedPageCapacity: 0, pageBudget: 0,
    pageCapacity: 0, freeListWords: 0,
    pageWords, descriptorWords: 0,
  };
  const requestedPageCapacity = Math.max(GPU_TOPOLOGY_PAGE_POOL_MINIMUM,
    Math.ceil(mutableFrontierBricks));
  const boundedPageBudget = Math.max(0, Math.floor(pageBudget));
  const pageCapacity = Math.min(boundedPageBudget, requestedPageCapacity);
  return {
    requestedPageCapacity, pageBudget: boundedPageBudget,
...
function packedTemplateCellMetadata(brick: number, resolution: number): number {
  if (brick < 0 || brick >= 2 ** (32 - TEMPLATE_CELL_RESOLUTION_BITS)
    ...
    throw new Error(`Sparse Geometric (CM12) cell metadata cannot be packed: ${brick}/${resolution}`);
  }
...
function packedTemplateRowTerms(offset: number, count: number): number {
  if (offset > TEMPLATE_ROW_TERM_OFFSET_MASK || count > TEMPLATE_ROW_TERM_COUNT_MASK) {
    throw new Error(`Sparse Geometric (CM12) row term range cannot be packed: ${offset}+${count}`);
  }
...
function packedTemplateRowMetadata(
  ...
  if (requirementOffset > TEMPLATE_ROW_METADATA_OFFSET_MASK || axis > 3) {
    throw new Error(`Sparse Geometric (CM12) row metadata cannot be packed: ${requirementOffset},${axis}`);
  }
...
interface PackedResidentTopologyTemplates {
  ...
  /** Internal sparse-air rows needed only while every covering neighbour is inactive. */
  readonly initialInactiveNeighbourFallbacks?: readonly Readonly<{
    rowId: number;
    neighbourBricks: readonly number[];
  }>[];
...
export interface SparseCM12TopologyPreparationBudget {
  readonly maximumCells: number;
  readonly maximumRows: number;
  readonly maximumBytes: number;
}

export type SparseCM12TopologyPreparation =
  | { readonly status: "deferred"; readonly resource: "cells" | "rows" | "bytes";
      readonly requiredAtLeast: number; readonly limit: number }
  | { readonly status: "ready"; readonly acceptedGeneration: number;
      readonly candidateGeneration: number; readonly changedBrickKeys: readonly number[];
      readonly words: Uint32Array; readonly cellCount: number; readonly rowCount: number;
      readonly acceptedCellWorklist: Uint32Array; readonly acceptedRowWorklist: Uint32Array;
      readonly candidateCellWorklist: Uint32Array; readonly candidateRowWorklist: Uint32Array;
      readonly initialDensity: Float32Array; readonly initialGamma: Float32Array };

class SparseCM12TopologyPreparationCapacity extends Error {
  constructor(readonly resource: "cells" | "rows" | "bytes",
    readonly requiredAtLeast: number, readonly limit: number) {
    super(`Sparse CM12 candidate ${resource} require ${requiredAtLeast}, budget ${limit}`);
  }
}

/** Compile a complete accepted/candidate union, without an all-rung catalogue.
 * Requests must already include their physical 2:1 closure. No accepted object
 * is mutated, and no partial packet escapes when any budget is exhausted.
 * The returned packet is preparation only: publishing it requires the resident
 * generation/field-transfer transaction, never merely granting a candidate slot. */
export function prepareSparseCM12TopologyWorkingSet(
  acceptedGrid: SparseAtlasCompositeGrid,
  requestedResolutions: ReadonlyMap<number, SparseBrickResolution>,
  budget: SparseCM12TopologyPreparationBudget,
): SparseCM12TopologyPreparation {
  for (const name of ["maximumCells", "maximumRows", "maximumBytes"] as const) {
    const limit = budget[name];
    if (!Number.isSafeInteger(limit) || limit < 0) {
      throw new RangeError(`Sparse Geometric (CM12) topology ${name} must be a nonnegative safe integer`);
    }
  }
  const atlas = acceptedGrid.atlas;
  const changed = new Map<number, SparseBrickResolution>();
  for (const [key, resolution] of requestedResolutions) {
    const brick = atlas.directory.get(key);
    if (!brick || !isSparseBrickResolution(resolution, atlas.brickFineResolution)) {
      throw new RangeError(`Sparse Geometric (CM12) candidate request ${key}/${resolution} is invalid`);
    }
    if (brick.resolution !== resolution) changed.set(key, resolution);
  }
  const neighbors = compileSparseCM12StableLeafFaceNeighbors({
    coordinates: atlas.bricks.map((brick) => brick.coordinate),
    spans: atlas.bricks.map(sparseBrickSpan),
  });
  for (let leaf = 0; leaf < atlas.bricks.length; leaf += 1) {
    ...
    const width = atlas.brickFineResolution * sparseBrickSpan(brick)
      / (changed.get(brick.key) ?? brick.resolution);
    for (const other of neighbors[leaf]!) {
      const neighbor = atlas.bricks[other]!;
      const otherWidth = atlas.brickFineResolution * sparseBrickSpan(neighbor)
        / (changed.get(neighbor.key) ?? neighbor.resolution);
      if (Math.max(width, otherWidth) > 2 * Math.min(width, otherWidth)) {
        throw new RangeError(`Sparse Geometric (CM12) candidate ${brick.key}/${neighbor.key} lacks physical 2:1 closure`);
      }
    }
  }
  try {
    const cells = acceptedGrid.cells.length + atlas.bricks.reduce((sum, brick) => {
      const resolution = changed.get(brick.key);
      return sum + (resolution === undefined ? 0
        : sparseCM12BrickLiveCellCount(atlas, brick, resolution));
    }, 0);
    if (cells > budget.maximumCells) {
      throw new SparseCM12TopologyPreparationCapacity("cells", cells, budget.maximumCells);
    }
    if (acceptedGrid.gradientRows.length > budget.maximumRows) {
      throw new SparseCM12TopologyPreparationCapacity("rows",
        acceptedGrid.gradientRows.length, budget.maximumRows);
    }
    const packet = packResidentTopologyTemplates(atlas, acceptedGrid,
      { requested: changed, neighbors, budget });
    return { status: "ready", acceptedGeneration: atlas.generation,
      candidateGeneration: atlas.generation + 1,
      changedBrickKeys: atlas.bricks.filter((brick) => changed.has(brick.key)).map((brick) => brick.key),
      words: packet.words, cellCount: packet.cellCount, rowCount: packet.rowCount,
      acceptedCellWorklist: packet.initialCellWorklist,
      acceptedRowWorklist: packet.initialRowWorklist,
      candidateCellWorklist: packet.candidateCellWorklist!,
      candidateRowWorklist: packet.candidateRowWorklist!,
      initialDensity: packet.initialDensity, initialGamma: packet.initialGamma };
  } catch (error) {
    if (!(error instanceof SparseCM12TopologyPreparationCapacity)) throw error;
    return { status: "deferred", resource: error.resource,
      requiredAtLeast: error.requiredAtLeast, limit: error.limit };
  }
}
   ...
   for (const row of rows) {
    for (const requirement of rowRequirements[row.id]!) {
      ...
      if (Math.abs(row.centerFine[axis]! - origin[axis]!) <= 1e-4) side = 2 * axis;
        ...
        - sparseBrickMaximumFine(atlas, sourceBrick, axis)) <= 1e-4) side = 2 * axis + 1;
      if (side < 0) continue;
...
function sparseCM12ContiguousRowOwnership(
  ...
  if (rows.length !== rowRequirements.length) {
    throw new Error("Sparse Geometric (CM12) row ownership input lengths differ");
  }
  ...
  rowRequirements.forEach((requirements, oldRow) => {
    if (requirements.length === 0) {
      throw new Error(`Sparse Geometric (CM12) row ${oldRow} has no owner`);
    }
    ...
    if (brick >= brickCount || level < 0) {
      throw new Error(`Sparse Geometric (CM12) row ${oldRow} has invalid owner ${brick}/${resolution}`);
    }
  ...
  if (orderedRows.length !== rowRequirements.length) {
    throw new Error("Sparse Geometric (CM12) row-owner catalog does not partition template rows");
  }
...
function resampleBrick(brick: SparseAdaptiveMassBrick,
  ...
  geometry?: Pick<SparseAdaptiveMassAtlas, "dimensions" | "brickFineResolution">,
    ...
    for (let dz = 0; dz < factor; dz += 1) for (let dy = 0; dy < factor; dy += 1) {
      for (let dx = 0; dx < factor; dx += 1) {
        ...
        const width = geometry
          ? geometry.brickFineResolution * sparseBrickSpan(brick) / brick.resolution : 1;
        const volume = geometry ? [sx, sy, sz].reduce((product, q, axis) => product
          * Math.max(0, Math.min(width, geometry.dimensions[axis]!
            - geometry.brickFineResolution * brick.coordinate[axis]! - q * width)), 1) : 1;
        sum += volume * values[sx + brick.resolution * (sy + brick.resolution * sz)]!;
        weight += volume;
      }
    }
...
function sparseCM12BrickLiveCellCount(atlas: SparseAdaptiveMassAtlas,
  ...
  return ([0, 1, 2] as const).reduce<number>((count, axis) => {
    ...
    const extent = Math.max(0, sparseBrickMaximumFine(atlas, brick, axis) - origin);
...
function packResidentTopologyTemplates(atlas: SparseAdaptiveMassAtlas,
  ...
  preparation?: Readonly<{
    requested: ReadonlyMap<number, SparseBrickResolution>;
  ...
  const localBricksFor = (core: readonly SparseAdaptiveMassBrick[]) => {
    ...
    for (const brick of core) {
      if (preparation) {
        for (const leaf of preparation.neighbors[leafByKey.get(brick.key)!]!) {
          keys.add(atlas.bricks[leaf]!.key);
        }
        continue;
      }
  ...
  const cachedResampledBrick = (brick: SparseAdaptiveMassBrick,
    ...
    if (!sampled) {
      sampled = resampleBrick(brick, resolution, preparation ? atlas : undefined);
  ...
  const compactBrickCellRange = (grid: SparseAtlasCompositeGrid,
    ...
    if (first === undefined) {
      throw new Error(`Sparse Geometric (CM12) ${context} brick ${key}/${resolution} has no compact base`);
    }
    let end = first;
    ...
    if (count !== expected || count === 0) {
      throw new Error(`Sparse Geometric (CM12) ${context} brick ${key}/${resolution} has ${count} `
    ...
    for (let cell = first; cell < end; cell += 1) {
      ...
      if (!source || source.brickKey !== key || source.brickResolution !== resolution) {
        throw new Error(`Sparse Geometric (CM12) ${context} brick ${key}/${resolution} compact range `
  ...
  for (const brick of atlas.bricks) for (let levelIndex = 0;
    ...
    if (preparation && templateLevels[levelIndex] !== brick.resolution) continue;
  ...
  const inactiveNeighbourFallbacks: Array<{
    oldRowId: number;
    neighbourBricks: readonly number[];
  }> = [];
  ...
  const appendRows = (grid: SparseAtlasCompositeGrid,
    ...
    for (const source of grid.gradientRows) {
      ...
      const terms = source.terms.map((term) => {
        ...
        if (!sourceCell) {
          throw new Error(`Sparse Geometric (CM12) template row ${source.id} references cell ${
        ...
        if (id === undefined) throw new Error("Sparse Geometric (CM12) template cell remap failed");
  ...
  if (preparation) {
    const buildCandidateChunk = (localBricks: readonly SparseAdaptiveMassBrick[]) =>
      buildSparseAtlasCompositeGrid(variantAtlasAtLevels((brick) =>
        preparation.requested.get(brick.key) ?? brick.resolution, localBricks),
      0.5, variantWorkspace);
    // First establish every changed leaf's contiguous cell range, including
    // changed halo leaves. A second bounded pass can then remap all seam terms
    // without retaining the transient chunk grids or borrowing their objects.
    for (const { localBricks } of chunkContexts) {
      const grid = buildCandidateChunk(localBricks);
      for (const brick of localBricks) {
        const resolution = preparation.requested.get(brick.key);
        if (resolution === undefined) continue;
        const range = 2 * (templateLevels.length * brickIndex.get(brick.key)!
          + templateLevels.indexOf(resolution));
        if (cellRanges[range + 1] !== 0) continue;
        const sourceRange = compactBrickCellRange(grid, brick, resolution, "candidate");
        ...
        for (let offset = 0; offset < sourceRange.count; offset += 1) {
          ...
          cellId.set(templateCellKey(brick.key, resolution, source.localIndex), cells.length);
          cells.push(snapshotTemplateCell(source, cells.length));
        }
      }
    }
  ...
  // The all-rung catalogue must retain a one-sided pressure/transport row
  // when an authored neighbour is temporarily inactive. Composite-grid
  // construction quite correctly omits sparse-air on an internal face, but
  // that leaves no selected row after runtime retirement disables every
  // two-sided variant. Publish a full-cell fallback only when one catalogue
  // neighbour physically covers the complete face. The runtime
  // hostExteriorRowSupersededAt proof removes it whenever that neighbour and
  // its matching two-sided interface are selected.
  const negativeFaces = ([0, 1, 2] as const).map(() =>
    new Map<number, SparseAdaptiveMassBrick[]>());
  const positiveFaces = ([0, 1, 2] as const).map(() =>
    new Map<number, SparseAdaptiveMassBrick[]>());
  for (const brick of atlas.bricks) for (const axis of [0, 1, 2] as const) {
    const negative = brick.coordinate[axis] * atlas.brickFineResolution;
    const positive = sparseBrickMaximumFine(atlas, brick, axis);
    let negativeBucket = negativeFaces[axis].get(negative);
    if (!negativeBucket) negativeFaces[axis].set(negative, negativeBucket = []);
    negativeBucket.push(brick);
    let positiveBucket = positiveFaces[axis].get(positive);
    if (!positiveBucket) positiveFaces[axis].set(positive, positiveBucket = []);
    positiveBucket.push(brick);
  }
  const appendInactiveNeighbourFallback = (cell: SparseAtlasCompositeCell,
    axis: SparseAtlasAxis, side: -1 | 1): void => {
    const owner = atlas.bricks[brickIndex.get(cell.brickKey)!]!;
    const plane = side < 0 ? cell.minimumFine[axis] : cell.maximumFine[axis];
    const ownerBoundary = side < 0
      ? owner.coordinate[axis] * atlas.brickFineResolution
      : sparseBrickMaximumFine(atlas, owner, axis);
    if (plane !== ownerBoundary) return;
    const tangent0 = ((axis + 1) % 3) as SparseAtlasAxis;
    const tangent1 = ((axis + 2) % 3) as SparseAtlasAxis;
    const candidates = (side < 0 ? positiveFaces : negativeFaces)[axis].get(plane) ?? [];
    const coveringNeighbours = candidates.filter((candidate) => candidate.key !== owner.key
      && candidate.coordinate[tangent0] * atlas.brickFineResolution
        <= cell.minimumFine[tangent0]
      && sparseBrickMaximumFine(atlas, candidate, tangent0)
        >= cell.maximumFine[tangent0]
      && candidate.coordinate[tangent1] * atlas.brickFineResolution
        <= cell.minimumFine[tangent1]
      && sparseBrickMaximumFine(atlas, candidate, tangent1)
        >= cell.maximumFine[tangent1]);
    if (coveringNeighbours.length === 0) return;
    const distance = cell.widthsFine[axis];
    const coefficient = side < 0 ? 1 / distance : -1 / distance;
    const centerFine = [...cell.centerFine] as [number, number, number];
    centerFine[axis] = plane;
    const rowKey = `${axis}/${centerFine.join("/")}/${cell.id}:${coefficient}`;
    ...
    rowKeys.add(rowKey);
    const area = cell.widthsFine[tangent0] * cell.widthsFine[tangent1];
    const oldRowId = rows.length;
    rows.push({ id: oldRowId, kind: "sparse-air", axis, centerFine,
      area, distance, areaFineCells2: area, centerDistanceFine: distance,
      dualWeight: area * distance,
      terms: [{ cellId: cell.id, coefficient }],
      ...(side > 0 ? { negativeBrickKey: owner.key } : { positiveBrickKey: owner.key }),
      exteriorPhi: 0.5 });
    rowRequirements.push([
      packedTemplateCellMetadata(brickIndex.get(owner.key)!, cell.brickResolution),
    ]);
    inactiveNeighbourFallbacks.push({ oldRowId,
      neighbourBricks: coveringNeighbours.map((brick) => brickIndex.get(brick.key)!) });
  };
  for (const cell of cells) for (const axis of [0, 1, 2] as const) {
    appendInactiveNeighbourFallback(cell, axis, -1);
    appendInactiveNeighbourFallback(cell, axis, 1);
  }
  ...
  const initialInactiveNeighbourFallbacks = inactiveNeighbourFallbacks.map((fallback) => ({
    rowId: ownership.oldToNew[fallback.oldRowId]!,
    neighbourBricks: fallback.neighbourBricks,
  }));
  const selected = preparation ? atlas.bricks.map((brick) =>
    preparation.requested.get(brick.key) ?? brick.resolution) : undefined;
  const candidateCellWorklist = selected ? Uint32Array.from(cells.flatMap((cell) =>
    cell.brickResolution === selected[brickIndex.get(cell.brickKey)!] ? [cell.id] : [])) : undefined;
  const candidateRowWorklist = selected ? Uint32Array.from(rows.flatMap((row) =>
  ...
  return { words, cellCount: cells.length, rowCount: rows.length,
    initialCellWorklist, initialRowWorklist, initialInactiveNeighbourFallbacks,
    candidateCellWorklist, candidateRowWorklist,
    initialDensity: Float32Array.from(cells, (cell) => cell.density),
    initialGamma: Float32Array.from(cells, (cell) => cell.gamma),
    maximumOwnedRowCount: ownership.maximumOwnedRowCount };
}
...
const LEVELSET_VOLUME_ENTRY_POINTS = [
  ...
  "lsvValidateTopology", "lsvSealTopology", "lsvPublishTopology",
  "lsvPrepareVertexDispatch", "lsvAdvectPhi", "lsvBeginConstraintProjection", "lsvApplyConstraints",
  "lsvAdvanceConstraintProjection", "lsvCommitPhi",
  "lsvUnionPhi", "lsvrBegin", "lsvrCaptureOriginal", "lsvrSeedClosestPoints",
  "lsvrRelaxClosestPoints", "lsvrAdvance", "lsvrResolve", "lsvrAuditContour",
] as const;
...
type GeometricVolumeResidentLayout = SparseGeometricVolumeLayout;
...
export function encodeSparseCM12SignedPresentationKey(
  ...
  if (![x, y, z].every(Number.isSafeInteger)
    ...
    throw new RangeError(`Sparse Geometric (CM12) signed presentation coordinate ${coordinate.join(",")} is not representable`);
  }
...
export function decodeSparseCM12SignedPresentationKey(
  ...
  if (!Number.isSafeInteger(key) || key < 0 || key > 0xffff_ffff || key === INVALID) {
    throw new RangeError(`Sparse Geometric (CM12) signed presentation key ${key} is invalid`);
  }
...
export function encodeSparseCM12FinePresentationSource(
  ...
  if (!Number.isInteger(brick) || brick < 0
    ...
    throw new RangeError(`Sparse Geometric (CM12) presentation brick ${brick} exceeds the 24-bit ABI`);
  }
  ...
  if (!Number.isInteger(octant) || octant < 0 || octant >= pageCount) {
    throw new RangeError(`Sparse Geometric (CM12) presentation octant ${octant} is invalid`);
  }
  if (!Number.isInteger(spanLog) || spanLog < 0
    || spanLog > SPARSE_CM12_PRESENTATION_MAX_SPAN_LOG) {
    throw new RangeError(`Sparse Geometric (CM12) presentation span ${spanBricks} is not representable`);
  }
  if (pagesPerAxis < 4) {
...
export function sparseCM12FinePresentationPlan(
  ...
  if (presentationPageResolution > brickFineResolution
    ...
    throw new RangeError(`Sparse Geometric (CM12) presentation page ${presentationPageResolution} does not divide brick ladder ${brickFineResolution}`);
  }
  const pagesPerAxis = brickFineResolution / presentationPageResolution;
  ...
  if (signedSparseAddressing
    ...
    throw new RangeError("Signed Sparse Geometric (CM12) presentation cannot advertise a padded address lattice");
  }
  if (coordinateOffsetPages.some((value) => !Number.isSafeInteger(value) || value < 0)
    ...
    throw new RangeError("Sparse Geometric (CM12) presentation address lattice is invalid");
  }
    ...
    for (let oz = 0; oz < pagesPerAxis; oz += 1)
      for (let oy = 0; oy < pagesPerAxis; oy += 1)
        for (let ox = 0; ox < pagesPerAxis; ox += 1) {
          ...
          if (!signedSparseAddressing && coordinate.some((value, axis) => value >= brickDimensions[axis])) continue;
          pages.push({
            key: signedSparseAddressing
              ? encodeSparseCM12SignedPresentationKey(coordinate)
              : coordinate[0] + brickDimensions[0]
  ...
  for (let page = 1; page < pages.length; page += 1) {
    if (pages[page - 1]!.key === pages[page]!.key) {
      throw new Error(`Sparse Geometric (CM12) presentation page ${pages[page]!.key} has two owners`);
    }
  }
  const pageCount = pages.length;
  const pageCapacity = options.pageCapacity ?? pageCount;
  if (!Number.isSafeInteger(pageCapacity) || pageCapacity < pageCount) {
    throw new RangeError(`Sparse Geometric (CM12) presentation capacity ${pageCapacity} cannot hold ${pageCount} resident pages`);
  }
...
export interface SparseCM12GPUActivityRecord {
  ...
  readonly generationRequestedResolution: SparseBrickResolution;
...
export interface SparseCM12GPUActivitySnapshot {
  ...
  readonly sourceTopologyLeaseHeld?: boolean;
  readonly sourceTopologyLeaseRevoked?: boolean;
  ...
  /** Capacity and cumulative unfulfilled allocation requests for WDR pages. */
  readonly topologyPageAllocator: SparseCM12TopologyPageAllocatorReceipt;
  readonly records: readonly SparseCM12GPUActivityRecord[];
}

export interface SparseCM12TopologyPageAllocatorReceipt {
...
export interface SparseCM12WorldGrowthReceipt {
  ...
  readonly capacity: number;
  /** Peak number of physical dynamic leaf/page identities ever issued. */
  readonly allocatedTopologyPageHighWaterMark: number;
...
/** VEX2 numerical receipt and topology-cached dispatch observability. */
...
function residentStateLayout(
  ...
  return {
    ...
    volumeTransport: {
      ...
      transportEdgeCapacity,
      transportEdgeMetadata: words(4 * transportEdgeCapacity),
      transportEdgeWeightsA: words(transportEdgeCapacity),
      transportEdgeWeightsB: words(transportEdgeCapacity),
      wholeFrameControlBaseWords: Math.max(7 * cellCount, cellCount + rowCount + 8) + 100 + 4 * cellCount,
      transportReceiverHeadsBaseWords: Math.max(7 * cellCount, cellCount + rowCount + 8) + 164 + 4 * cellCount,
      transportDonorHeadsBaseWords: Math.max(7 * cellCount, cellCount + rowCount + 8) + 164 + 5 * cellCount,
    },
    sourceLedger: {
...
function packResidentTopology(
  ...
  if (!Number.isInteger(maximumSpanLog) || maximumSpanLog < 0
    ...
    throw new RangeError(`Sparse Geometric (CM12) maximum span ${atlas.maximumSpanBricks} is invalid`);
  }
  ...
  for (let brick = 0; brick < atlas.bricks.length; brick += 1) {
    ...
    words[record] = (source.unclipped ? 0x8000_0000 : 0) | Math.log2(sparseBrickSpan(source))
...
export class WebGPUSparseCM12Resident {
  ...
  get leafCapacity() { return this.constructionAtlas.bricks.length + this.topologyPageCapacity; }
  private generationPlanningRequired = false;
  private generationPlanningGate?: Promise<SparseCM12GenerationPlanningGate>;
  get needsGenerationPlanning() { return this.generationPlanningRequired; }
  async needsDetailedGenerationPlanning(maximumSpan: number, demoteEpochs: number,
    finestTravel: number, frozenFrontierOnly = false): Promise<boolean> {
    this.assertLive();
    if (!this.generationPlanningGate) {
      const metadata = new Uint32Array(this.leafCapacity).fill(1 << 8);
      this.constructionAtlas.bricks.forEach((brick, leaf) => {
        const packedOwner = this.lastPacked!.words[this.lastPacked!.brickOffset + 2 * leaf]!;
        const backed = ((packedOwner & 0x7fff_ffff) >>> 5) !== 0;
        metadata[leaf] = ((brick.spanBricks ?? 1) << 8) | Number(backed) | (Number(!!brick.unclipped) << 1);
      });
      this.generationPlanningGate = SparseCM12GenerationPlanningGate.create(this.device,
        this.activity, metadata, ACTIVITY_HEADER_WORDS, ACTIVITY_RECORD_WORDS).then(gate => {
          this.generationPlanningGateBytes = gate.allocatedBytes;
          return gate;
        });
    }
    const gate = await this.generationPlanningGate;
    this.assertLive();
    return gate.needed(maximumSpan, demoteEpochs, finestTravel, frozenFrontierOnly,
      Math.floor(0.8 * this.levelSetVolumeLayout.activeCellCapacity));
  }
  private initialGenerationCellIds!: Uint32Array;
  private initialGenerationRowIds!: Uint32Array;
  ...
  private sourceTopologyLeaseHeld = false;

  /** Freeze optional topology work, never numerical frame advancement. GPU
   * frontier demand or a backed promotion revokes this lease immediately. */
  acquireGenerationSourceTopologyLease(): () => void {
    this.assertLive();
    if (this.sourceTopologyLeaseHeld) throw new Error("CM12 source topology is already leased");
    this.sourceTopologyLeaseHeld = true;
    this.device.queue.writeBuffer(this.activity, 4 * SOURCE_TOPOLOGY_LEASE_WORD, new Uint32Array([1, 0]));
    let released = false;
    return () => {
      if (released) return;
      released = true; this.sourceTopologyLeaseHeld = false;
      if (!this.destroyed) this.device.queue.writeBuffer(this.activity,
        4 * SOURCE_TOPOLOGY_LEASE_WORD, new Uint32Array([0]));
    };
  }
    ...
    const device = generationDeviceRoots.get(this.device) ?? this.device;
      ...
      try {
        if (typeof Worker !== "undefined") {
          const worker = new Worker(new URL("./sparse-cm12-preparation-worker.ts", import.meta.url), { type: "module" });
          let recipe: CM12ResourceRecipe;
          try {
            recipe = await new Promise<CM12ResourceRecipe>((resolve, reject) => {
              ...
              worker.postMessage({ atlas: nextAtlas, active, finestCellSize_m, newAirCoverage, candidateKeys,
                ...
                source: source ? { geometry: source.geometry, geometryRecipe:source.geometryRecipe, cellIds: source.cellIds, rowIds: source.rowIds,
                  ...
                  levelSetVolume: source.levelSetVolume
                    ? { layout: source.levelSetVolume.layout } : undefined,
            ...
            [...(rigid ? [rigid.bodies, rigid.exchange] : []), ...(source ? [this.state, this.topologyArena] : [])], {signal, compilationDevice: device,
              onSlice: (milliseconds, operation) => {
                if (milliseconds > this.generationPreparationMaximumSliceMs) {
                  ...
                  this.generationPreparationMaximumSliceOperation = operation;
                }
              } });
          ...
          if (data.transfer) next.preparedGenerationTransfer = Object.assign(
  ...
  private candidatePhaseLimitForQA?: (typeof SPARSE_CM12_RESIDENT_STAGE_SUBSTAGES)["candidate-transfer"][number];
  ...
  private constructor(
    ...
    private readonly presentationBindGroup: GPUBindGroup,
    ...
    private readonly levelSetVolumeLayout: LevelSetVolumeLayout,
  ...
  get fieldSnapshotSourceForQA() {
    return { state: this.state, conditioning: this.conditioning, topologyArena: this.topologyArena,
      ...
      faceParityWord: SPARSE_CM12_FRAME_CONTROL_HEADER.faceParity };
  }
  ...
  setCandidatePhaseLimitForQA(
    phase: (typeof SPARSE_CM12_RESIDENT_STAGE_SUBSTAGES)["candidate-transfer"][number] | undefined,
  ): void {
    this.candidatePhaseLimitForQA = phase;
  }
  ...
  static create(
    ...
    return this.createConfigured(device, atlas, grid, finestCellSize_m,
      ...
      topologyPageCapacityMaximum, false, undefined, initialVelocity_m_s, undefined, initialScene);
  }
  ...
  private static async createConfigured(
    ...
    if (signedWorldGrowth) {
      ...
      if (!sparseCM12SignedPresentationInitialWorldFits(
        ...
        throw new RangeError("Sparse Geometric (CM12) dynamic world exceeds the signed presentation address ABI");
      }
    }
    ...
    const maximumStateWords = Math.floor(Math.min(device.limits.maxStorageBufferBindingSize,
    ...
    const layout = residentStateLayout(physicsCellCapacity, physicsRowCapacity,
      baseLayout.volumeTransport.subfaceCapacity, worldLeafCapacity,
      Boolean(rigid) || Boolean(initialSolidWorld), Boolean(initialSolidWorld),
      tracerLattice.count, journal?.iterationCapacity ? {
        iterationCapacity: journal.iterationCapacity + 1,
        snapshotCapacity: journal.snapshotCapacity ?? SPARSE_CM12_PRESSURE_JOURNAL_SNAPSHOTS,
      } : {}, transportEdgeCapacity);
    const cellWorkgroups = Math.ceil(physicsCellCapacity / WORKGROUP_SIZE);
    const storage = GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC;
    const parameters = device.createBuffer({ label: "Sparse Geometric (CM12) resident parameters",
    ...
    const state = device.createBuffer({ label: "Sparse Geometric (CM12) resident state",
    ...
    const partials = device.createBuffer({ label: "Sparse Geometric (CM12) resident reductions",
    ...
    const effectiveTransportVelocity = effectiveTransportVelocityLayout
      ? device.createBuffer({
        label: "Sparse Geometric (CM12) accepted effective transport velocity",
        size: effectiveTransportVelocityLayout.byteLength,
        usage: storage,
      })
      : undefined;
    const scalars = device.createBuffer({ label: "Sparse Geometric (CM12) resident scalar reductions",
      size: SPARSE_CM12_PRESSURE_SCALAR_BYTES, usage: storage });
    const conditioning = device.createBuffer({
      label: "Sparse Geometric (CM12) conservative transport and conditioning accumulators",
      ...
      size: 4 * (layout.volumeTransport.transportDonorHeadsBaseWords + physicsCellCapacity),
    ...
    const initialRowRequirementsSelected = (row: number) => {
    ...
    const initialRowIds = Array.from(templates.initialRowWorklist).filter(
      initialRowRequirementsSelected,
    );
    for (const fallback of templates.initialInactiveNeighbourFallbacks ?? []) {
      if (!initialRowRequirementsSelected(fallback.rowId)) continue;
      const coveringNeighbourActive = fallback.neighbourBricks.some((brick) =>
        initialActivity[ACTIVITY_HEADER_WORDS + ACTIVITY_RECORD_WORDS * brick + 10] !== 0);
      if (!coveringNeighbourActive) initialRowIds.push(fallback.rowId);
    }
      ...
      ? compileSparseCM12FactoredAEIPackedTemplateCatalog({
        ...
        validDimensions: (leaf, resolution) => {
          ...
          return ([0, 1, 2] as const).map((axis) => {
            const origin = atlas.brickFineResolution * brick.coordinate[axis]!;
            return Math.min(resolution, Math.ceil(Math.max(0,
              sparseBrickMaximumFine(atlas, brick, axis) - origin) / scale));
    ...
    if (internedBoundaryCompilation && !internedBoundaryCompilation.exactStableRowSet) {
      throw new Error("Sparse Geometric (CM12) IBO1 production image failed its exact stable-row proof: "
    ...
    if (iboGeometryNeighbors && internedBoundaryCatalog) {
      for (let leaf = 0; leaf < packed.brickCount; leaf += 1) {
        ...
        if (actual.length !== expected.length
          ...
          throw new Error("Sparse Geometric (CM12) IGN1 geometry closure differs from the canonical "
      ...
      if (iboGeometryNeighbors.maximumFanout !== expectedMaximumFanout) {
        throw new Error("Sparse Geometric (CM12) IGN1 maximum fanout receipt differs from catalog: "
    ...
    if (internedBoundaryImage) {
      ...
      if (!initialSemantics.exact || initialSemantics.duplicateCandidateRows !== 0) {
        throw new Error("Sparse Geometric (CM12) ISA1 initial slot semantic receipt failed: "
    ...
    if (internedBoundaryImage
      ...
      throw new Error("Sparse Geometric (CM12) IBO1 production image exceeds its memory plan: "
    ...
    if (4 * iboSemanticAuthorityWords
      ...
      throw new Error("Sparse Geometric (CM12) ISA1 dynamic authority exceeds its memory plan: "
    ...
    if (topologyEffectsAuthorityLayout
      ...
      throw new Error("Sparse Geometric (CM12) TFX1 candidate-effects authority exceeds its memory plan: "
    ...
    if (!faceAddressProgram) {
      throw new Error("Sparse Geometric (CM12) production requires the all-rung BFA1 face program");
    }
    ...
    const levelSetVolumeInitialWords = createLevelSetVolumeInitialWords(levelSetVolumeLayout);
    writeGPUBufferBytes(device.queue, topologyArena,
      4 * levelSetVolumeLayout.headerBaseWords,
      levelSetVolumeInitialWords.buffer as ArrayBuffer,
      levelSetVolumeInitialWords.byteOffset, levelSetVolumeInitialWords.byteLength);
    ...
    // dispatches them.
    // Last triplet is the active adaptive-phi vertex dispatch snapshot.
    const acceptedAndShadowIndirect = new Uint32Array(36);
    ...
    const transportPacketIndirectArguments = transportPacketAuthorityLayout
      ? device.createBuffer({
        label: "Sparse Geometric (CM12) transport and cached VEX indirect dispatches",
    ...
    const coarseTransportIndirectArguments = coarseTransportCellPacking
      ? device.createBuffer({
        label: "Sparse Geometric (CM12) packed coarse transport indirect dispatch",
        size: 12,
        usage: GPUBufferUsage.INDIRECT | GPUBufferUsage.COPY_DST,
      })
      : undefined;
    const refinementPolicyIndirectArguments = refinementPolicyLeaderLayout
      ? device.createBuffer({
        label: "Sparse Geometric (CM12) refinement-policy leader indirect dispatch",
        size: 12,
        usage: GPUBufferUsage.INDIRECT | GPUBufferUsage.COPY_DST,
      })
      : undefined;
    const framePlanIndirectArguments = device.createBuffer({
      label: "Sparse Geometric (CM12) FPL1 fixed packet indirect dispatches",
      size: 4 * framePlanLayout.indirectSnapshotWords,
      usage: GPUBufferUsage.INDIRECT | GPUBufferUsage.COPY_DST,
    });
    const presentationIndirectArguments = device.createBuffer({
      label: "Sparse Geometric (CM12) FPP1 compact page indirect dispatch",
      size: 12,
      usage: GPUBufferUsage.INDIRECT | GPUBufferUsage.COPY_DST,
    });
    const frameControlIndirectArguments = device.createBuffer({
      label: "Sparse Geometric (CM12) FCA1 fixed authority indirect dispatches",
      size: 12 * SPARSE_CM12_FRAME_CONTROL_FAMILY_COUNT,
      usage: GPUBufferUsage.INDIRECT | GPUBufferUsage.COPY_DST
        | GPUBufferUsage.COPY_SRC,
    });
    const fineParams = device.createBuffer({
      label: "Sparse Geometric (CM12) compact fine presentation parameters",
      size: 16,
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    ...
    const fineSamples = device.createBuffer({
      label: "Sparse Geometric (CM12) accepted/candidate fine presentation samples",
    ...
    const fineWorkA = device.createBuffer({ label: "Sparse Geometric (CM12) fine presentation work A",
      size: 4, usage: storage });
    const fineWorkB = device.createBuffer({ label: "Sparse Geometric (CM12) fine presentation work B",
      size: 4, usage: storage });
    const fineRollback = device.createBuffer({ label: "Sparse Geometric (CM12) fine presentation rollback",
      size: 4, usage: storage });
    const presentationAllocatorBindGroupLayout = device.createBindGroupLayout({
      label: "Sparse Geometric (CM12) presentation page allocator layout",
      entries: [
    ...
    const presentationAllocatorPipelineLayout = device.createPipelineLayout({
      label: "Sparse Geometric (CM12) presentation page allocator pipeline layout",
      bindGroupLayouts: [presentationAllocatorBindGroupLayout],
    });
    const presentationAllocatorBindGroup = device.createBindGroup({
      label: "Sparse Geometric (CM12) presentation page allocator bindings",
    ...
    const diagnosticsReadback = device.createBuffer({
      label: "Sparse Geometric (CM12) resident diagnostic readback",
    ...
    const velocityExtensionDepths = device.createBuffer({
      label: "Sparse Geometric (CM12) velocity-extension depth records",
    ...
    const bindGroup = device.createBindGroup({ label: "Sparse Geometric (CM12) resident bindings",
    ...
    const presentationBindGroup = device.createBindGroup({ label: "Sparse Geometric (CM12) presentation compiled-topology bindings",
      layout: bindGroupLayout, entries: [
        { binding: 0, resource: { buffer: parameters } },
        { binding: 1, resource: { buffer: topology } },
        { binding: 2, resource: { buffer: state } },
    ...
    const pressureBindGroup = device.createBindGroup({
      label: "Sparse Geometric (CM12) read-only pressure bindings",
    ...
    const transportDepthBindGroups = Array.from(
      { length: SPARSE_CM12_VELOCITY_EXTENSION_DEPTH }, (_, index) =>
        device.createBindGroup({
          label: `Sparse Geometric (CM12) Phase-1 transport execution bindings depth ${index + 1}`,
          layout: bindGroupLayout,
          entries: [
          { binding: 0, resource: { buffer: parameters } },
          { binding: 1, resource: { buffer: topology } },
    ...
    const effectiveVelocityPressureBindGroup = effectiveTransportVelocity
      ? device.createBindGroup({
        label: "Sparse Geometric (CM12) VEX/collocation effective-velocity bindings",
        layout: bindGroupLayout,
        entries: [
          { binding: 0, resource: { buffer: parameters } },
          { binding: 1, resource: { buffer: topology } },
...
`,
    ...
    const createResidentShaderSource = (velocityExtensionFixedRecurrenceDepth?: number) =>
      createWebgpuSparseCM12ResidentWGSL(
        ...
        initialScene
          ? createInitialLevelSetGeometryWGSL(initialScene, atlas.dimensions, finestCellSize_m)
          : /* wgsl */ `
fn lsvAuthoredPhi(positionFine:vec3f)->f32{
  let owner=compactOwnerCellAt(vec3i(floor(positionFine)));
  if(owner.x==INVALID){return 1e6;}
  ...
  return select(0.5,-0.5,fill>=0.5)*cellMinimumWidth(owner.x);
    ...
    const names = [...COMPILED_TOPOLOGY_ENTRY_POINTS, ...LEVELSET_VOLUME_ENTRY_POINTS,
                        "seedGeometricVolumeDestination", ...WHOLE_FRAME_VOLUME_ENTRY_POINTS,
    ...
    for (const name of LEVELSET_VOLUME_ENTRY_POINTS) presentationEntryNames.add(name);
    ...
    if (solidOccupancyLayout) {
      const initialization = device.createCommandEncoder({
        label: "Sparse Geometric (CM12) SolidWorld aperture initialization",
      });
    ...
    // Adaptive phi has a generation-local bounded arena. Even a fully backed
    // CM12 topology can outgrow it through ordinary GPU rerungs, so retain the
    // cheap planning receipt cadence and grow by resident replacement before
    // an oversized shadow topology is admitted.
    result.generationPlanningRequired = true;
  ...
  /** CPU-only resource compiler invoked in a separate preparation worker.
   * Its GPU-shaped recorder emits data/commands, never a live device. */
  static async recordPreparedGeneration(input: {
    ...
    candidateKeys?: ReadonlySet<number>;
    journal?: SparseCM12PressureJournalCapacityRequest;
    source?: Omit<SparseCM12GenerationFields, "state" | "liveControl" | "levelSetVolume"> & {
      ...
      levelSetVolume?: Readonly<{ layout: LevelSetVolumeLayout }>;
    ...
    const transfer = input.source ? await prepareSparseCM12GenerationTransfer(recorder.device,
      input.source.geometry!, grid, { ...input.source,
        ...
        levelSetVolume: input.source.levelSetVolume ? {
          arena: recorder.externalResources[firstSource + 1] as GPUBuffer,
    ...
    // data and symbolic resource references are transferred.
    return recorder.finish({resident,transfer});
  }
  ...
  async waitForSimulationPipelines(): Promise<void> {
    ...
    if (this.simulationPipelineFailure) {
      throw new Error(`Sparse Geometric (CM12) simulation compilation failed: ${
        this.simulationPipelineFailure}`);
    }
    if (!this.destroyed && !this.simulationPipelinesReady) {
      throw new Error("Sparse Geometric (CM12) simulation compilation did not publish pipelines");
    }
  }
  ...
  encode(
    ...
    const dispatchTransportPacket = (name: string) => {
      if (!this.transportPacketIndirectArguments) {
        throw new Error("Sparse Geometric (CM12) transport packet authority is unavailable");
      }
      const activePass = openPass();
      activePass.setPipeline(this.pipelines[name]!);
    ...
    const candidatePhaseLimitForQA = this.candidatePhaseLimitForQA;
    ...
    stage("body-forces", () => {
      ...
      if (inflow) {
        ...
        // The finalized rate is the admission authority for this frame's
        // source event. Publish the same swept plug into phi before pressure;
        // conservative transport credits dt*rate to V once later in the frame.
        dispatch("lsvUnionPhi", Math.ceil(
        ...
        for (let level = 0; level < phiConstraintLevels; level += 1) {
          dispatch("lsvApplyConstraints", Math.ceil(
            this.levelSetVolumeLayout.vertexCapacity / WORKGROUP_SIZE));
    ...
    stage("pressure-topology", ({ closeSubstage }) => {
      // Final scalar conditioning is complete. Pressure and adaptivity now
      // read the accepted adaptive phi field directly.
    ...
    stage("conservative-transport", ({ closeSubstage }) => {
      selectBindGroup(this.transportBindGroup);
      dispatch("lsvPrepareVertexDispatch", 1);
      closePass();
      encoder.copyBufferToBuffer(this.topologyArena,
        4 * (this.levelSetVolumeLayout.headerBaseWords
          + LSV_GLOBAL_HEADER.vertexDispatch),
        this.acceptedIndirectArguments, 132, 12);
      const dispatchPhi = (name: string) => {
        const activePass = openPass();
        activePass.setPipeline(this.pipelines[name]!);
        activePass.dispatchWorkgroupsIndirect(this.acceptedIndirectArguments, 132);
      };
      dispatchPhi("lsvAdvectPhi");
      // Constraints can depend on constrained masters at the next coarser
      // rung. Descending widths put a dispatch boundary between dependencies.
      dispatch("lsvBeginConstraintProjection", 1);
      const phiConstraintLevels = Math.ceil(Math.log2(
        this.constructionAtlas.maximumSpanBricks * this.brickFineResolution)) + 1;
      for (let level = 0; level < phiConstraintLevels; level += 1) {
      ...
      dispatch("lsvCommitPhi", 1);
      dispatch("lsvrBegin", 1);
      dispatchPhi("lsvrCaptureOriginal");
      dispatchPhi("lsvrSeedClosestPoints");
      dispatch("lsvrAdvance", 1);
      // A four-fine-cell metric band crosses at most sixteen adaptive graph
      // edges under the supported 2:1 balance. Unreached vertices retain an
      // honest deep-phase classification in lsvrResolve.
      for (let pass = 0; pass < 16; pass += 1) {
        dispatchPhi("lsvrRelaxClosestPoints");
        dispatch("lsvrAdvance", 1);
      }
      dispatchPhi("lsvrResolve");
      dispatch("lsvBeginConstraintProjection", 1);
      for (let level = 0; level < phiConstraintLevels; level += 1) {
        dispatchPhi("lsvApplyConstraints");
        dispatch("lsvAdvanceConstraintProjection", 1);
      }
      dispatchPhi("lsvrAuditContour");
      dispatch("beginWholeFrameVolumeTransport", 1);
      dispatchAccepted("initializeWholeFrameVolumeCells", "cell");
      dispatchAccepted("buildWholeFrameVolumeCoupling", "cell");
      dispatchAccepted("addWholeFrameUncoveredDonorFallbacks", "cell");
      ...
      dispatchAccepted("validateWholeFrameVolume", "cell");
      dispatchAccepted("commitWholeFrameVolume", "cell");
      dispatch("finishWholeFrameVolumeTransport", 1);
      dispatchAccepted("prepareWholeFrameVolumeSharpening", "cell");
  ...
  async readPressureJournal(): Promise<SparseCM12PressureJournal | undefined> {
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) pressure journal readback",
      size: 4 * floats,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
  ...
  private encodeCompiledTopologyGeneration(encoder: GPUCommandEncoder): void {
    ...
    pass.end();
    const phi = encoder.beginComputePass({
      label: "Sparse CM12 adaptive level-set topology and transfer",
    });
  ...
  private encodeFramePlanPresentation(
    ...
    plan.setBindGroup(0, this.presentationBindGroup);
    ...
    execute.setBindGroup(0, this.presentationBindGroup);
  ...
  setRefinementRegionParameters(parameters: ArrayBuffer): void {
    ...
    if (parameters.byteLength !== SPARSE_CM12_REFINEMENT_REGION_BYTES) {
      throw new RangeError(`Sparse Geometric (CM12) refinement-region parameters must occupy ${
  ...
  setSolidWorld(solidWorld: SolidWorld): void {
    ...
    const clear = this.device.createCommandEncoder({
      label: "Sparse Geometric (CM12) replace SolidWorld occupancy",
    });
    ...
    const refresh = this.device.createCommandEncoder({
      label: "Sparse Geometric (CM12) refresh SolidWorld apertures",
    });
  ...
  private encodeSolidWorldApertureRefresh(encoder: GPUCommandEncoder): void {
    ...
    const pass = encoder.beginComputePass({
      label: "Sparse Geometric (CM12) derive SolidWorld apertures",
    });
    pass.setBindGroup(0, this.bindGroup);
  ...
  encodeInitialPresentation(encoder: GPUCommandEncoder, finestCellSize_m: number,
    columnHeightMode: boolean | "off" | "auto" | "on" = this.presentationColumnHeightMode,
    surfaceMode: "rdf" | "plic" = this.presentationSurfaceMode): void {
    this.presentationColumnHeightMode = typeof columnHeightMode === "boolean"
      ? columnHeightMode ? "on" : "off" : columnHeightMode;
    this.presentationSurfaceMode = surfaceMode;
    this.assertLive();
    this.writeParameters(this.lastPacked!, 0.004, finestCellSize_m, 1, [0, 0, 0]);
    this.encodeFailureGate(encoder);
    const pass = encoder.beginComputePass({ label: "Sparse Geometric (CM12) resident initial presentation" });
    pass.setBindGroup(0, this.bindGroup);
  ...
  /** Republish an unchanged accepted scalar field after a live presentation
   * control changes. FPP scheduling is normally density/topology driven, so a
   * paused RDF/PLIC switch needs this explicit publication transaction rather
   * than waiting for an unrelated physics step to dirty its pages. */
  async refreshPresentationConfiguration(
    finestCellSize_m: number,
    sharpening?: SharpeningTrace,
    activityPolicy?: SparseCM12ActivityPolicy,
    pressureControl?: SparseCM12PressureControl,
    ...
    this.assertLive();
    this.writeParameters(this.lastPacked!, 0.004, finestCellSize_m, 1,
      [0, 0, 0], sharpening, activityPolicy, pressureControl, 0,
      worldDimensions_m, this.lastInflow, false, true);
    const encoder = this.device.createCommandEncoder({
      label: "Sparse Geometric (CM12) live surface-mode publication",
    });
    this.encodeFramePlanPresentation(encoder,
      "Sparse CM12 live surface-mode presentation");
    this.device.queue.submit([encoder.finish()]);
    await this.assertSimulationHealthy();
    // The submitted publication consumed the nonce. Clear only after its
    // health boundary so a later ordinary encoder cannot erase it early.
    this.device.queue.writeBuffer(this.parameters,
      SPARSE_CM12_FAILURE_PARAMETER_OFFSET - 8,
      new Float32Array([0]));
  }
  ...
  async refreshRefinementRegions(finestCellSize_m: number,
    ...
    const receipt = this.device.createBuffer({ size: 4,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ });
    try {
      const encoder = this.device.createCommandEncoder({ label: "Sparse Geometric (CM12) live region update" });
  ...
  private encodeTopologyEditTransaction(
      ...
      const openTopologyPass = () => {
        if (!topologyPass) {
          topologyPass = encoder.beginComputePass({
            label: mode === 0 ? "Sparse Geometric (CM12) region edit topology" : "Sparse Geometric (CM12) resident liquid injection topology",
          });
    ...
    if (mode === 2) {
      ...
      injectionPass.dispatchWorkgroups(1);
      injectionPass.setPipeline(this.pipelines.completeGeometricSourceLedger!);
      injectionPass.dispatchWorkgroups(1);
    } else {
    ...
    // The accepted adaptive phi field is the surface authority. Apply the
    // exact live insertion geometry to both banks in the same transaction as
    // its conservative volume dose, then propagate dependent seam vertices.
    injectionPass.setPipeline(this.pipelines.lsvUnionPhi!);
    injectionPass.dispatchWorkgroups(Math.ceil(
      this.levelSetVolumeLayout.vertexCapacity / WORKGROUP_SIZE));
    injectionPass.setPipeline(this.pipelines.lsvBeginConstraintProjection!);
    injectionPass.dispatchWorkgroups(1);
    const phiConstraintLevels = Math.ceil(Math.log2(
      this.constructionAtlas.maximumSpanBricks * this.brickFineResolution)) + 1;
    for (let level = 0; level < phiConstraintLevels; level += 1) {
      injectionPass.setPipeline(this.pipelines.lsvApplyConstraints!);
      injectionPass.dispatchWorkgroups(Math.ceil(
        this.levelSetVolumeLayout.vertexCapacity / WORKGROUP_SIZE));
      injectionPass.setPipeline(this.pipelines.lsvAdvanceConstraintProjection!);
      injectionPass.dispatchWorkgroups(1);
    }
  ...
  private presentationColumnHeightMode: "off" | "auto" | "on" = "auto";
  private presentationSurfaceMode: "rdf" | "plic" = "rdf";
  ...
  private writeParameters(
    ...
    forcePresentationPublication = false,
  ): void {
    ...
    // Volume-only sharpening follows the advected adaptive level set.
    u.set([policy.prepareBricksPerFrame, 0, 0,
    ...
    this.coarseFirstPolicySignature = packAdaptivitySurfaceParameters(
    ...
    if (sharpening?.presentationColumnHeightEnabled !== undefined) {
      this.presentationColumnHeightMode = sharpening.presentationColumnHeightEnabled ? "on" : "off";
    }
    if (sharpening?.presentationColumnHeightMode !== undefined) {
      this.presentationColumnHeightMode = sharpening.presentationColumnHeightMode;
    }
    if (sharpening?.presentationSurfaceMode !== undefined) {
      this.presentationSurfaceMode = sharpening.presentationSurfaceMode;
    }
    ...
    f.set([
      ...
      this.presentationColumnHeightMode === "on" ? 0
        : this.presentationColumnHeightMode === "off" ? 1 : 2,
      this.presentationSurfaceMode === "plic" ? 1 : 0,
      forcePresentationPublication ? 1 : 0, 0,
  ...
  async readDiagnostics(): Promise<{
    ...
    await this.assertSimulationHealthy();
    const encoder = this.device.createCommandEncoder({
      label: "Sparse Geometric (CM12) diagnostic scalar readback",
    });
    encoder.copyBufferToBuffer(this.scalars, 0, this.diagnosticsReadback, 0,
      SPARSE_CM12_PRESSURE_SCALAR_BYTES);
    const allocatorDiagnosticOffset = this.diagnosticsReadback.size - 16;
    encoder.copyBufferToBuffer(this.topologyArena,
      4 * (this.worldDirectoryLayout.baseWords + SPARSE_CM12_WORLD_DIRECTORY_HEADER.leafCapacity),
      this.diagnosticsReadback, allocatorDiagnosticOffset, 12);
    encoder.copyBufferToBuffer(this.topologyArena,
      4 * (this.worldDirectoryLayout.baseWords + SPARSE_CM12_WORLD_DIRECTORY_HEADER.freeCount),
      this.diagnosticsReadback, allocatorDiagnosticOffset + 12, 4);
    ...
    const allocator = new Uint32Array(mapped, allocatorDiagnosticOffset, 4);
    ...
    const result = {
      ...
      topologyPageAllocator: {
        freePages: allocator[0]! - allocator[1]! + allocator[3]!,
        capacity: allocator[0]! - this.initialWorldLeafCount,
        allocationCancellations: allocator[2]!,
      },
  ...
  async readTracers(): Promise<Float32Array> {
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) marker readback",
      size: bytes,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) marker copy",
      });
  ...
  /** Compact topology metadata and GPU field addresses captured while the
   * generation owner has suspended frame encoding. Field values stay on device. */
  async prepareTransferredGeneration(
    source: Awaited<ReturnType<WebGPUSparseCM12Resident["captureGenerationTransferSource"]>>,
    atlas: SparseAdaptiveMassAtlas, active: ReadonlySet<number>, finestCellSize_m: number,
    maximumBytes = Number.POSITIVE_INFINITY,
  ): Promise<WebGPUSparseCM12Resident> {
    const prepared = await this.prepareGenerationReplacement(source, atlas, active, finestCellSize_m, maximumBytes);
    try { await prepared.commit(); return prepared.resident; }
    catch (error) { prepared.resident.destroy(); throw error; }
    finally { prepared.disposePreparation(); }
  }

  private generationTransferControlDescription() {
    return {
      scalarParityWord: this.frameControlLayout.baseWords + SPARSE_CM12_FRAME_CONTROL_HEADER.scalarParity,
      faceParityWord: this.frameControlLayout.baseWords + SPARSE_CM12_FRAME_CONTROL_HEADER.faceParity,
      densityOffsets: [this.layout.densityA, this.layout.densityB] as const,
      gammaOffsets: [this.layout.gammaA, this.layout.gammaB] as const,
      velocityOffsets: [this.layout.cellVelocityA, this.layout.cellVelocityB] as const,
  ...
  private generationTransferTarget(activeBrickKeys?: ReadonlySet<number>) {
    return { state: this.state, activeBrickKeys,
      ...
      levelSetVolume: { arena: this.topologyArena, layout: this.levelSetVolumeLayout },
    ...
    try {
      ...
      return { resident: next, disposePreparation: () => transfer.destroy(), commit: async () => {
        ...
        if (latest.sourceTopologyLeaseRevoked
          ...
          || latest.records.length !== source.activity.records.length
          || latest.records.some((record, index) => {
            const before = source.activity.records[index]!;
            return record.leafId !== before.leafId || record.active !== before.active
              || record.acceptedResolution !== before.acceptedResolution || record.topologyPage !== before.topologyPage;
          })) throw new SparseCM12GenerationStale();
        next.refinementRegionParameters.set(this.refinementRegionParameters);
  ...
  async captureGenerationTransferSource(
    planning?: Awaited<ReturnType<WebGPUSparseCM12Resident["captureGenerationPlanningSource"]>>,
  ) {
    const release = this.sourceTopologyLeaseHeld ? undefined : this.acquireGenerationSourceTopologyLease();
    try { return await this.captureGenerationTransferSourceWhileLeased(planning); }
    finally { release?.(); }
  }

  /** Planning needs leaf intent only. Defer row lists and transfer geometry
   * until a replacement has actually been admitted. */
  async captureGenerationPlanningSource() {
    const release = this.sourceTopologyLeaseHeld ? undefined : this.acquireGenerationSourceTopologyLease();
    try { return await this.captureGenerationPlanningSourceWhileLeased(); }
    finally { release?.(); }
  }

  private async captureGenerationPlanningSourceWhileLeased() {
    this.assertLive();
    const activity = await this.readActivitySnapshot(true);
    const original = this.constructionAtlas;
    const brickDimensions = original.brickDimensions;
    const active = new Set<number>();
    const sourceFirst = new Map<number, number>();
    const sourcePageCoordinates = new Map<number, readonly [number, number, number]>();
    const planned = new Map<number, SparseBrickResolution>();
    const backedPlanned = new Map<number, SparseBrickResolution>();
    let predictedLevelSetCells = 0;
    const frozenFrontier = new Set<number>();
    const recordsByKey = new Map<number, SparseCM12GPUActivityRecord>();
    const dynamicKeys = new Set<number>();
    const bricks: SparseAdaptiveMassBrick[] = [];
    for (const record of activity.records) {
      const authored = original.bricks[record.leafId];
      if (!authored && record.topologyPage === undefined) continue;
      const coordinate = authored?.coordinate ?? record.coordinate!;
      const key = sparseAtlasBrickKey(coordinate, { brickDimensions, signedCoordinates: true });
      const prior = recordsByKey.get(key);
      if (prior) throw new Error(`CM12 duplicate captured leaf at ${coordinate}: `
        + JSON.stringify([prior, record].map(r => ({ leaf: r.leafId, coordinate: r.coordinate,
          active: r.active, page: r.topologyPage, resolution: r.acceptedResolution }))));
      ...
      const requestedResolution = record.generationRequestedResolution;
      if (record.active || demandedActivation) {
        predictedLevelSetCells += requestedResolution ** 3;
      }
      if ((record.planReasons & 0x00020000) !== 0) {
        frozenFrontier.add(key);
      ...
      if ((record.active || demandedActivation) && !hasCandidateSlot && record.generationRequestedResolution !== resolution)
        planned.set(key, record.generationRequestedResolution);
      if ((record.active || demandedActivation) && hasCandidateSlot
        && record.generationRequestedResolution !== resolution) {
        backedPlanned.set(key, record.generationRequestedResolution);
      }
    ...
    // Normally backed rerungs remain GPU-local. When their already-published
    // requests would consume the phi arena's safety margin, route those exact
    // intents through resident replacement before the candidate can overflow.
    if (predictedLevelSetCells
      > Math.floor(0.8 * this.levelSetVolumeLayout.activeCellCapacity)) {
      for (const [key, resolution] of backedPlanned) planned.set(key, resolution);
    }
    const atlas = createSparseAdaptiveMassAtlas(original.dimensions, bricks,
      activity.acceptedTopologyGeneration, original.brickFineResolution, true, false);
    return { atlas, active, sourceFirst, sourcePageCoordinates, dynamicKeys,
      planned, frozenFrontier, activity, recordsByKey };
  }

  private async captureGenerationTransferSourceWhileLeased(
    planning?: Awaited<ReturnType<WebGPUSparseCM12Resident["captureGenerationPlanningSource"]>>,
  ) {
    ...
    try {
      const encoder = this.device.createCommandEncoder();
      encoder.copyBufferToBuffer(this.topologyArena, this.topologyWorklistBaseBytes, header, 0, 128);
    ...
    // cannot silently combine an activity epoch with another row-list epoch.
    const check = this.device.createBuffer({ label: "CM12 source topology lease check",
      size: 12, usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ });
    try {
      const encoder = this.device.createCommandEncoder();
      encoder.copyBufferToBuffer(this.activity, 4 * SOURCE_TOPOLOGY_LEASE_WORD, check, 0, 8);
      encoder.copyBufferToBuffer(this.topologyArena, this.topologyWorklistBaseBytes, check, 8, 4);
      this.device.queue.submit([encoder.finish()]); await check.mapAsync(GPUMapMode.READ);
      const observed = new Uint32Array(check.getMappedRange());
  ...
  async readDiagnosticFields(
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) QA field readback",
      size: readbackBytes,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) QA field copy",
      });
      ...
      for (const record of activitySnapshot.records) {
        ...
        const [brickX, brickY, brickZ] = this.constructionAtlas.bricks[brick]!.coordinate;
        ...
        if (lower[0] < this.brickFineResolution * brickX
          ...
          throw new Error(`Sparse Geometric (CM12) active brick ${brick} at ${brickX},${brickY},${brickZ}`
        ...
        for (let dz = Math.max(0, -lower[2]!); dz < span[2] && lower[2] + dz < nz; dz += 1)
          for (let dy = Math.max(0, -lower[1]!); dy < span[1] && lower[1] + dy < ny; dy += 1)
            for (let dx = Math.max(0, -lower[0]!); dx < span[0] && lower[0] + dx < nx; dx += 1) {
  ...
  async readPhase1TransportHashesQA() {
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) Phase-1 hash-only QA readback",
      size: 4 * receiptWords,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    let raw: Uint32Array;
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) Phase-1 hash-only QA copy",
      });
    ...
    if (raw[h.magic] !== SPARSE_CM12_PHASE1_TRANSPORT_QA_MAGIC
      ...
      throw new Error("Sparse Geometric (CM12) Phase-1 hash-only QA header is incompatible");
    }
  ...
  async readPhase1TransportReceiptQA(
    ...
    const receiptWords = layout.totalWords - layout.baseWords;
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) Phase-1 raw transport QA readback",
      size: 4 * receiptWords,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    let raw: Uint32Array;
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) Phase-1 raw transport QA copy",
      });
      encoder.copyBufferToBuffer(this.activity, 4 * layout.baseWords,
        readback, 0, 4 * receiptWords);
      this.device.queue.submit([encoder.finish()]);
    ...
    if (raw[h.magic] !== SPARSE_CM12_PHASE1_TRANSPORT_QA_MAGIC
      || raw[h.version] !== SPARSE_CM12_PHASE1_TRANSPORT_QA_VERSION
      || raw[h.cellCapacity] !== layout.cellCapacity) {
      throw new Error("Sparse Geometric (CM12) Phase-1 raw transport QA header is incompatible");
    }
    const relative = (absolute: number) => absolute - layout.baseWords;
    const plane = (base: number, count: number) => raw.slice(relative(base),
      relative(base) + count);
    ...
    for (let brick = 0; brick < activity.records.length; brick += 1) {
      ...
      const authored = this.constructionAtlas.bricks[brick]!;
      const spanBricks = sparseBrickSpan(authored);
      const origin = authored.coordinate.map(value => value * this.brickFineResolution);
      const spanFine = this.brickFineResolution * spanBricks;
      const scale = spanFine / record.acceptedResolution;
      const valid = origin.map((value, axis) => Math.ceil(Math.max(0,
        sparseBrickMaximumFine(this.constructionAtlas, authored, axis) - value) / scale));
    ...
    if (!allowStageLimitedCandidate && this.transportExecutionImage
      ...
      const validationReadback = this.device.createBuffer({
        label: "Sparse Geometric (CM12) Phase-1 TEI/packet-authority QA readback",
        size: 4 * (imageWords + authorityWords + 1),
        usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
      });
      try {
        const encoder = this.device.createCommandEncoder({
          label: "Sparse Geometric (CM12) Phase-1 TEI/packet-authority QA copy",
        });
  ...
  async readFramePlanPresentationFaultRecordQA() {
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) FPP1 fault-record QA readback",
      size: 4 * SPARSE_CM12_FRAME_PLAN_PRESENTATION_PAGE_WORDS,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) FPP1 fault-record QA copy",
      });
      encoder.copyBufferToBuffer(this.activity,
  ...
  async readPhase1TransportProfileQA() {
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) Phase-1 transport profile readback",
      size: 4 * SPARSE_CM12_PHASE1_TRANSPORT_PROFILE_WORDS,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) Phase-1 transport profile copy",
      });
  ...
  async readAdaptiveRepresentationQA() {
    ...
    const headerReadback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) accepted-topology header QA",
      size: 4 * headerWords,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    ...
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) accepted-topology header QA copy",
      });
    ...
    const listReadback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) accepted-topology lists QA",
    ...
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) accepted-topology lists QA copy",
      });
    ...
    const leafReadback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) accepted-leaf manifest QA",
    ...
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) accepted-leaf manifest QA copy",
      });
      encoder.copyBufferToBuffer(this.topologyArena,
  ...
  async readCandidateEffectsTransactionQA() {
    ...
    const totalWords = SPARSE_CM12_TOPOLOGY_EFFECTS_HEADER_WORDS
      ...
      + isaWords + iboWords;
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) candidate-effects transaction QA readback",
      size: 4 * totalWords,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) candidate-effects transaction QA copy",
      });
  ...
  async readAdaptiveLevelSetQA(includeVertices = false) {
    this.assertLive();
    const layout = this.levelSetVolumeLayout;
    const readback = this.device.createBuffer({ label: "Adaptive phi header QA", size: 384,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ });
    try {
      const encoder = this.device.createCommandEncoder();
      encoder.copyBufferToBuffer(this.topologyArena, 4 * layout.headerBaseWords, readback, 0, 128);
      for (let slot = 0; slot < 2; slot++) encoder.copyBufferToBuffer(this.topologyArena,
        4 * layout.slots[slot]!.headerBaseWords, readback, 128 * (slot + 1), 128);
      this.device.queue.submit([encoder.finish()]); await readback.mapAsync(GPUMapMode.READ);
      const words = new Uint32Array(readback.getMappedRange());
      const slot = words[LSV_GLOBAL_HEADER.acceptedSlot]!;
      const accepted = slot < 2 ? words.subarray(32 * (slot + 1), 32 * (slot + 2)) : undefined;
      const named = accepted ? Object.fromEntries(Object.entries(LSV_SLOT_HEADER)
        .map(([name, offset]) => [name, accepted[offset]!])) : undefined;
      const vertices: Array<{ positionFine: readonly [number, number, number];
        phiFine: number; support: number; constrained: boolean }> = [];
      const count = Math.min(layout.vertexCapacity,
        accepted?.[LSV_SLOT_HEADER.vertexCount] ?? 0);
      if (includeVertices && count > 0 && slot < 2) {
        const slotLayout = layout.slots[slot]!;
        const bank = accepted![LSV_SLOT_HEADER.sourceBank]!;
        const samples = this.device.createBuffer({ label: "Adaptive phi vertex QA",
          size: count * 24, usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ });
        try {
          const sampleEncoder = this.device.createCommandEncoder();
          for (const [source, destination, size] of [
            [slotLayout.vertexRecordsBaseWords, 0, count * 16],
            [bank ? slotLayout.phi1BaseWords : slotLayout.phi0BaseWords, count * 16, count * 4],
            [bank ? slotLayout.support1BaseWords : slotLayout.support0BaseWords, count * 20, count * 4],
          ]) sampleEncoder.copyBufferToBuffer(this.topologyArena, 4 * source!,
            samples, destination!, size!);
          this.device.queue.submit([sampleEncoder.finish()]);
          await samples.mapAsync(GPUMapMode.READ);
          const raw = samples.getMappedRange();
          const coordinates = new Int32Array(raw);
          const scalars = new Float32Array(raw);
          const metadata = new Uint32Array(raw);
          for (let vertex = 0; vertex < count; vertex++) vertices.push({
            positionFine: [coordinates[4 * vertex]!, coordinates[4 * vertex + 1]!,
              coordinates[4 * vertex + 2]!],
            phiFine: scalars[4 * count + vertex]!,
            support: metadata[5 * count + vertex]!,
            constrained: (metadata[4 * vertex + 3]! & 7) !== 0,
          });
        } finally { if (samples.mapState === "mapped") samples.unmap(); samples.destroy(); }
      }
      return {
        storage: "adaptive shared vertices with constrained hanging vertices" as const,
        allocatedBytes: 4 * (layout.totalWords - layout.baseWords),
        vertexCapacity: layout.vertexCapacity, cellCapacity: layout.activeCellCapacity,
        acceptedSlot: slot, acceptedGeneration: words[LSV_GLOBAL_HEADER.acceptedGeneration]!,
        activeVertices: accepted?.[LSV_SLOT_HEADER.vertexCount] ?? 0,
        activeCells: accepted?.[LSV_SLOT_HEADER.activeCellCount] ?? 0,
  ...
  async readAcceptedGeometricVolumeQA() {
    ...
    const size = Math.max(80, count * 80);
    ...
    try {
      ...
      for (let brick = 0; brick < count; brick++) {
        ...
        maximumExcessFine3 = Math.max(maximumExcessFine3, values[at + 18]!);
      ...
      return {
        ...
        boundContract: "finite nonnegative V; positive V requires positive open capacity; excess is measured separately" as const,
        excessVolumeFine3: sums[16]!, excessCells: sums[17]!, maximumExcessFine3,
  ...
  async readGeometricVolumeTransportReceiptQA() {
    ...
    try {
      ...
      const solidWords = words.subarray(56 + GEOMETRIC_SOURCE_LEDGER_FLOATS);
      const solidFloats = floats.subarray(56 + GEOMETRIC_SOURCE_LEDGER_FLOATS);
      const supportWords = words.subarray(60 + GEOMETRIC_SOURCE_LEDGER_FLOATS);
      const supportFloats = floats.subarray(60 + GEOMETRIC_SOURCE_LEDGER_FLOATS);
      const couplingWords = words.subarray(98 + GEOMETRIC_SOURCE_LEDGER_FLOATS);
      const couplingFloats = floats.subarray(98 + GEOMETRIC_SOURCE_LEDGER_FLOATS);
      ...
      return Object.freeze({
        algorithm: "adaptive-levelset-volume" as const,
        scope: "current-resident-latest-transport" as const,
        hoseSourceLedger: sourceLedger,
        movingSolid: { active: solidWords[0] !== 0, transportFraction: solidFloats[1]!,
          rowReexpressionPending: solidWords[2] !== 0 },
        supportedTransportBoundary: {
          originalMaterialLowerFine: Array.from(supportFloats.slice(0, 3)),
          originalMaterialUpperFine: Array.from(supportFloats.slice(3, 6)),
          minimumVelocityFinePerSecond: Array.from(supportFloats.slice(6, 9)),
          maximumVelocityFinePerSecond: Array.from(supportFloats.slice(9, 12)),
          envelopeLowerFine: Array.from(supportFloats.slice(12, 15)),
          envelopeUpperFine: Array.from(supportFloats.slice(15, 18)),
          closedSweepEvents: supportWords[18]!, maximumPreventedEstimateFine3: supportFloats[22]!,
          totalPreventedEstimateFine3: supportFloats[23]!,
          firstClosure: supportWords[18]! > 0 ? { rowId: supportWords[19]!, donorCell: supportWords[20]!,
            receiverCell: supportWords[21]!, originalSweepFine3: supportFloats[24]!,
            originalLowFine3: supportFloats[25]!, originalHighFine3: supportFloats[26]! } : null,
        ...
        coupling: Object.freeze({
          ...
          translatedBoxCount: couplingWords[5]!,
          maximumBalancingRowResidual: couplingFloats[6]!,
          maximumBalancingDonorResidual: couplingFloats[7]!,
          excessCellCount: couplingWords[8]!, maximumExcessFine3: couplingFloats[9]!,
          finalRowResidual: couplingFloats[10]!, finalDonorResidual: couplingFloats[11]!,
          missingSupportReceiverCount: couplingWords[12]!, clampedTraceCount: couplingWords[13]!,
          sharpeningMissingPhiCount: couplingWords[14]!, sharpeningMovedCellCount: couplingWords[15]!,
          // Estimator disagreement is not a bound on error against exact H.
          sharpeningQuadratureDisagreementFine3: couplingFloats[16]!,
          sharpeningIntegrationSamples: couplingWords[17]!,
          maximumTraceDisplacementFine: couplingFloats[18]!,
  ...
  /** One consolidated readback for an offline moving-volume feasibility audit. */
  async readGeometricMovingDualSnapshotQA() {
    this.assertLive();
    const volume = this.layout.volumeTransport;
    const header = this.device.createBuffer({ label: "Geometric moving-dual header QA",
      size: 260, usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ });
    let snapshot: GPUBuffer | undefined;
    try {
      const first = this.device.createCommandEncoder();
      first.copyBufferToBuffer(this.topologyArena, this.topologyWorklistBaseBytes, header, 0, 128);
      first.copyBufferToBuffer(this.conditioning, 4 * volume.controlBaseWords, header, 128, 128);
      first.copyBufferToBuffer(this.topologyArena, 4 * (this.frameControlLayout.baseWords
        + SPARSE_CM12_FRAME_CONTROL_HEADER.scalarParity), header, 256, 4);
      this.device.queue.submit([first.finish()]); await header.mapAsync(GPUMapMode.READ);
      const h = new Uint32Array(header.getMappedRange()).slice(); header.unmap();
      const acceptedCount = h[4]!, acceptedSlot = h[2]! & 1, faceCount = h[32]!;
      if (acceptedCount > this.cellCount || faceCount > volume.subfaceCapacity) {
        throw new Error(`Invalid moving-dual QA sizes ${acceptedCount}/${faceCount}`);
      }
      const templateCellCount = this.templateWords[2]!;
      const fields = [this.layout.movingSolid.oldCapacityFloats,
        this.layout.movingSolid.newCapacityFloats, volume.currentVolume, volume.lowVolume,
        volume.positiveLimiter, volume.negativeLimiter, this.layout.liquid, this.layout.rhs,
        this.layout.diagonal, this.layout.residual,
        this.layout.sourceLedger.sourceRateBaseFloats!, this.layout.densityA,
        this.layout.densityB] as const;
      const counts = [acceptedCount, ...fields.map(() => this.cellCount), 2 * this.cellCount,
        2 * volume.subfaceCapacity, 4 * faceCount, 4 * faceCount, faceCount,
        8 * templateCellCount];
      const totalWords = counts.reduce((sum, count) => sum + count, 0);
      snapshot = this.device.createBuffer({ label: "Geometric moving-dual snapshot QA",
        size: Math.max(4, 4 * totalWords), usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ });
      const encoder = this.device.createCommandEncoder(); let at = 0;
      if (acceptedCount) encoder.copyBufferToBuffer(this.topologyArena,
        this.topologyWorklistBaseBytes + 4 * h[14 + acceptedSlot]!, snapshot, 0, 4 * acceptedCount);
      at += acceptedCount;
      for (const base of fields) {
        encoder.copyBufferToBuffer(this.state, 4 * base, snapshot, 4 * at, 4 * this.cellCount);
        at += this.cellCount;
      }
      encoder.copyBufferToBuffer(this.state, 4 * volume.cellSubfaceRanges,
        snapshot, 4 * at, 8 * this.cellCount); at += 2 * this.cellCount;
      encoder.copyBufferToBuffer(this.state, 4 * volume.cellSubfaceEntries,
        snapshot, 4 * at, 8 * volume.subfaceCapacity); at += 2 * volume.subfaceCapacity;
      encoder.copyBufferToBuffer(this.state, 4 * volume.subfaceMetadata,
        snapshot, 4 * at, 16 * faceCount); at += 4 * faceCount;
      encoder.copyBufferToBuffer(this.state, 4 * volume.subfaceFluxes,
        snapshot, 4 * at, 16 * faceCount); at += 4 * faceCount;
      encoder.copyBufferToBuffer(this.state, 4 * volume.subfaceRoundoff,
        snapshot, 4 * at, 4 * faceCount); at += faceCount;
      encoder.copyBufferToBuffer(this.topologyArena, 4 * this.templateWords[6]!,
        snapshot, 4 * at, 32 * templateCellCount);
      this.device.queue.submit([encoder.finish()]); await snapshot.mapAsync(GPUMapMode.READ);
      const words = new Uint32Array(snapshot.getMappedRange()).slice();
      const floats = new Float32Array(words.buffer); at = 0;
      const acceptedCells = Array.from(words.slice(at, at += acceptedCount));
      const cellFields = fields.map(() => Array.from(floats.slice(at, at += this.cellCount)));
      const cellFaceRanges = Array.from(words.slice(at, at += 2 * this.cellCount));
      const cellFaceEntries = Array.from(words.slice(at, at += 2 * volume.subfaceCapacity));
      const metadata = Array.from(words.slice(at, at += 4 * faceCount));
      const faceState = Array.from(floats.slice(at, at += 4 * faceCount));
      const roundoff = Array.from(floats.slice(at, at += faceCount));
      const descriptors = Array.from(floats.slice(at, at + 8 * templateCellCount));
      return { acceptedSlot, acceptedCells, cellCount: this.cellCount, templateCellCount,
        controlWords: Array.from(h.slice(32, 64)), acceptedScalarParity: h[64]! & 1,
        faceCount, fields: { oldCapacity: cellFields[0]!, newCapacity: cellFields[1]!,
          currentVolume: cellFields[2]!, previousProximal: cellFields[3]!,
          currentDual: cellFields[4]!, proposedProximal: cellFields[5]!,
          pressureMembership: cellFields[6]!, rhs: cellFields[7]!, diagonal: cellFields[8]!,
          residual: cellFields[9]!, sourceRate: cellFields[10]!,
          densityA: cellFields[11]!, densityB: cellFields[12]! },
        cellFaceRanges, cellFaceEntries, metadata, faceState, roundoff, descriptors };
    } finally {
      if (header.mapState === "mapped") header.unmap(); header.destroy();
      if (snapshot?.mapState === "mapped") snapshot.unmap(); snapshot?.destroy();
    }
  }
  ...
  async readFramePlanPresentationHeaderQA() {
    this.assertLive();
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) FPP1 header QA readback",
      size: 4 * SPARSE_CM12_FRAME_PLAN_PRESENTATION_HEADER_WORDS,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) FPP1 header QA copy",
      });
      encoder.copyBufferToBuffer(this.activity,
  ...
  async readFrameControlQA(): Promise<SparseCM12FrameControlQA> {
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) FCA1 QA readback",
      size: bytes,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({ label: "Sparse Geometric (CM12) FCA1 QA copy" });
      encoder.copyBufferToBuffer(this.topologyArena, 4 * this.frameControlLayout.baseWords,
        readback, 0, bytes);
      this.device.queue.submit([encoder.finish()]);
      await readback.mapAsync(GPUMapMode.READ);
      ...
      if (words[SPARSE_CM12_FRAME_CONTROL_HEADER.magic] !== SPARSE_CM12_FRAME_CONTROL_MAGIC) {
        throw new Error("Sparse Geometric (CM12) FCA1 QA header is unavailable or incompatible");
      }
  ...
  async readFinalScalarMaskHeaderQA() {
    this.assertLive();
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) FSM1 header QA readback",
      size: 4 * SPARSE_CM12_FINAL_SCALAR_MASK_HEADER_WORDS,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) FSM1 header QA copy",
      });
      encoder.copyBufferToBuffer(this.topologyArena,
  ...
  async readTransportPacketIndirectQA(): Promise<readonly number[]> {
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) transport packet indirect QA readback", size: 12,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) transport packet indirect QA copy",
      });
  ...
  async readCoarseTransportScheduleQA() {
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) coarse transport schedule QA readback",
      size: 4 * words,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) coarse transport schedule QA copy",
      });
  ...
  async readDynamicTransportPacketsQA() {
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) dynamic TEI QA readback",
      size: 4 * (1 + 2 * slotWords),
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) dynamic TEI QA copy",
      });
      encoder.copyBufferToBuffer(this.topologyArena,
  ...
  async readAcceptedIndirectQA(): Promise<readonly number[]> {
    ...
    const bytes = 33 * 4;
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) accepted-indirect QA readback", size: bytes,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) accepted-indirect QA copy",
      });
  ...
  async readFrameControlIndirectQA(): Promise<readonly number[]> {
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) FCA1 indirect QA readback", size: bytes,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) FCA1 indirect QA copy",
      });
  ...
  Promise<SparseCM12PresentationPageAllocatorReceipt> {
    this.assertLive();
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) presentation allocator QA readback", size: 16,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) presentation allocator QA copy",
      });
      encoder.copyBufferToBuffer(this.activity,
  ...
  async readWorldGrowthReceiptQA(): Promise<SparseCM12WorldGrowthReceipt> {
    ...
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) world-growth QA copy",
      });
      encoder.copyBufferToBuffer(this.topologyArena,
      ...
      if (dynamicCellOffset < 0) {
        throw new Error("Sparse Geometric (CM12) world-growth QA dynamic cell slab is invalid");
      }
      ...
      return Object.freeze({
        ...
        allocatedTopologyPageHighWaterMark: Math.max(0,
          words[h.nextLeaf]! - this.initialWorldLeafCount),
  ...
  async readVelocityExtensionHeaderQA(): Promise<SparseCM12VelocityExtensionHeaderQA> {
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) VEX2 header QA readback", size: bytes,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) VEX2 header QA copy",
      });
      ...
      const schedule = words.subarray(headerWords + maskWords);
      ...
      if (words[h.magic] !== SPARSE_CM12_VELOCITY_EXTENSION_MAGIC
        ...
        throw new Error("Sparse Geometric (CM12) VEX2 header QA receipt is unavailable or incompatible");
      }
      ...
      return {
        ...
        acceptedPacketCount: schedule[2]!,
  ...
  async readVelocityExtensionQA() {
    ...
    if (!this.effectiveTransportVelocity) {
      throw new Error("Sparse Geometric (CM12) VEX2 effective-velocity plane is unavailable");
    }
    const layout = this.velocityExtensionLayout;
    const capacity = layout.cellCapacity;
    const headerWords = SPARSE_CM12_VELOCITY_EXTENSION_HEADER_WORDS;
    ...
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) VEX2 QA readback",
      size: 4 * totalWords,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({ label: "Sparse Geometric (CM12) VEX2 QA copy" });
      encoder.copyBufferToBuffer(this.activity, 4 * layout.headerBaseWords,
        readback, 0, 4 * headerWords);
      ...
      if (header[SPARSE_CM12_VELOCITY_EXTENSION_HEADER.magic]
          !== SPARSE_CM12_VELOCITY_EXTENSION_MAGIC) {
        throw new Error("Sparse Geometric (CM12) VEX2 QA header is unavailable or incompatible");
      }
  ...
  async readActivitySnapshot(
    ...
    const topologyPageAllocatorAt = wordsToRead;
    const worldHeaderAt = topologyPageAllocatorAt + 4;
    ...
    const totalWords = includeWorldLeaves
      ...
      : worldHeaderAt;
    const readback = this.device.createBuffer({
      label: "Sparse Geometric (CM12) activity QA readback",
      size: 4 * totalWords,
      usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
    });
    try {
      const encoder = this.device.createCommandEncoder({
        label: "Sparse Geometric (CM12) activity QA copy",
      });
      encoder.copyBufferToBuffer(this.activity, 0, readback, 0, 4 * wordsToRead);
      encoder.copyBufferToBuffer(this.topologyArena,
        4 * (this.worldDirectoryLayout.baseWords + SPARSE_CM12_WORLD_DIRECTORY_HEADER.leafCapacity),
        readback, 4 * topologyPageAllocatorAt, 12);
      encoder.copyBufferToBuffer(this.topologyArena,
        4 * (this.worldDirectoryLayout.baseWords + SPARSE_CM12_WORLD_DIRECTORY_HEADER.freeCount),
        readback, 4 * topologyPageAllocatorAt + 12, 4);
      if (includeWorldLeaves) {
        encoder.copyBufferToBuffer(this.topologyArena,
        ...
        return {
          ...
          densityMoments: [5, 6, 7].map((offset) =>
            new DataView(words.buffer).getFloat32(4 * (at + offset), true)) as
              [number, number, number],
          generationRequestedResolution: (words[at + 47] || words[at + 12]) as SparseBrickResolution,
      ...
      return {
        ...
        sourceTopologyLeaseHeld: words[SOURCE_TOPOLOGY_LEASE_WORD] === 1,
        sourceTopologyLeaseRevoked: words[SOURCE_TOPOLOGY_LEASE_WORD + 1] !== 0,
        ...
        commitFailed: words[21] !== 0,
        topologyPageAllocator: {
          freePages: words[topologyPageAllocatorAt]! - words[topologyPageAllocatorAt + 1]!
            + words[topologyPageAllocatorAt + 3]!,
          capacity: words[topologyPageAllocatorAt]! - this.initialWorldLeafCount,
          allocationCancellations: words[topologyPageAllocatorAt + 2]!,
        },
        records,
      };
    } finally {
  ...
  private assertLive(): void {
    if (this.destroyed) throw new Error("Sparse Geometric (CM12) resident pipeline is destroyed");
  }
}
