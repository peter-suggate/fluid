...
export const ADAPTIVITY_PARAMS: MethodParamSpec[] = [
  ...
  },
  {
    ...
    tier: "coarse",
    options: [
      { value: "4", label: "1³ / 2³ / 4³ · production" },
      { value: "8", label: "1³ / 2³ / 4³ / 8³ · comparison" },
    ],
    hint: "Selects both the adaptive ladder maximum and the matching presentation-page resolution. B4 is the production default: at most 64 cells per brick. Macro pages retain larger physical coverage; B8 is available for comparison.",
  },
  {
    kind: "select",
  ...
  {
    kind: "number", key: "finestTravelCells", label: "Finest surface travel",
    ...
    hint: "Surface/front displacement threshold for finest-rung lookahead. Fully flooded bulk does not refine merely because it translates uniformly.",
  },
  {
    kind: "number", key: "fourTravelCells", label: "Half-resolution surface travel",
    ...
    hint: "Surface/front displacement threshold for a half-resolution destination minimum. Normalization keeps it no higher than the finest threshold.",
  },
  {
    kind: "number", key: "twoTravelCells", label: "Quarter-resolution surface travel",
    ...
    hint: "Displacement threshold for a quarter-resolution minimum; slower non-surface bulk may target 1³.",
  },
  {
  ...
  },
  {
    ...
    hint: "Liquid exposed on both sides of an axis and thinner than this represented width targets the finest rung.",
  },
  {
  ...
  },
  {
    ...
    step: 0.25, digits: 2,
    hint: "Minimum integrated liquid mass needed to keep a brick populated. The default rejects fragments smaller than one full finest cell.",
  },
  {
...
export const adaptiveMassAdaptivityFeature: FeatureDefinition = {
  id: "simulation.adaptive-volume.adaptivity", label: "Fluid adaptivity",
  ...
  variants: ADAPTIVITY_MODES.map(option => ({id:option.value, point:"simulation.adaptive-volume.adaptivity", update:"live" as const, default:option.value === "coarse-first"})),
};
