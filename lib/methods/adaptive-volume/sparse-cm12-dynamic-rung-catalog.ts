export type DynamicPageRung = 1 | 2 | 4 | 8;
...
function buildDynamicRungCatalogue(fineResolution: 4 | 8) {
  /** Relocatable topology prepared once for the selected dyadic page ladder.
   ...
   */
  const DYNAMIC_PAGE_RUNGS: readonly DynamicPageRung[] = fineResolution === 4 ? [4, 2, 1] : [8, 4, 2, 1];
  let cells = 0, rows = 0, terms = 0, boundaries = 0;
  const DYNAMIC_RUNG_LAYOUTS: readonly DynamicRungLayout[] = DYNAMIC_PAGE_RUNGS.map(resolution => {
    ...
    const layout = Object.freeze({ resolution, width: fineResolution / resolution,
  ...
  const DYNAMIC_PAGE_CELL_COUNT = cells;
  const DYNAMIC_PAGE_ROW_COUNT = rows;
  const DYNAMIC_PAGE_TERM_COUNT = terms;
  const DYNAMIC_PAGE_BOUNDARY_COUNT = boundaries;
  function dynamicRungLayout(resolution: number): DynamicRungLayout {
  ...
  function dynamicRowTermOffset(resolution: DynamicPageRung, row: number): number {
  ...
  function compileDynamicSeamVariant(own: DynamicPageRung,
    ...
    const patchWidth = fineResolution / patchResolution;
    const distance = neighbor === 0 ? ownLayout.width : (ownLayout.width + fineResolution / neighbor) / 2;
    ...
    for (let v = 0; v < patchResolution; v++) for (let u = 0; u < patchResolution; u++) {
      ...
      center[axis] = positive ? fineResolution : 0;
  ...
  function preparedDynamicSeamCatalogue(): ReadonlyMap<string, DynamicSeamVariant> {
  ...
  /** The physical 2:1 floor for a newly admitted ordinary selected-ladder page.
   ...
   */
  function coarseDynamicAdmissionRung(neighborWidths: readonly number[],
    minimumResolution: DynamicPageRung = 1, maximumResolution: DynamicPageRung = fineResolution): DynamicPageRung {
    ...
    for (const width of neighborWidths) {
      ...
      while (fineResolution / rung > 2 * width && rung < fineResolution) rung *= 2;
      if (fineResolution / rung > 2 * width) throw new RangeError("no prepared rung satisfies 2:1 grading");
    }
    if (rung > maximumResolution) throw new RangeError("authored cap conflicts with 2:1 admission");
    return rung as DynamicPageRung;
  }

  function dynamicRungLayoutWGSL(): string {
    return `
  const CM12_DYNAMIC_RUNG_COUNT:u32=${DYNAMIC_PAGE_RUNGS.length}u;
  ...
  fn cm12DynamicRungIndex(resolution:u32)->u32{
    return ${Math.log2(fineResolution)}u-firstLeadingBit(resolution);
  }
  fn cm12DynamicCellOffset(resolution:u32)->u32{
    return array<u32,${DYNAMIC_PAGE_RUNGS.length}>(${DYNAMIC_RUNG_LAYOUTS.map(l => `${l.cellOffset}u`).join(",")})[cm12DynamicRungIndex(resolution)];
  }
  fn cm12DynamicRowOffset(resolution:u32)->u32{
    return array<u32,${DYNAMIC_PAGE_RUNGS.length}>(${DYNAMIC_RUNG_LAYOUTS.map(l => `${l.rowOffset}u`).join(",")})[cm12DynamicRungIndex(resolution)];
  }
  fn cm12DynamicTermOffset(resolution:u32)->u32{
    return array<u32,${DYNAMIC_PAGE_RUNGS.length}>(${DYNAMIC_RUNG_LAYOUTS.map(l => `${l.termOffset}u`).join(",")})[cm12DynamicRungIndex(resolution)];
  }
  fn cm12DynamicBoundaryOffset(resolution:u32)->u32{
    return array<u32,${DYNAMIC_PAGE_RUNGS.length}>(${DYNAMIC_RUNG_LAYOUTS.map(l => `${l.boundaryOffset}u`).join(",")})[cm12DynamicRungIndex(resolution)];
  }
  fn cm12DynamicCellRung(within:u32)->u32{
    ${DYNAMIC_RUNG_LAYOUTS.map(l => `if(within<${l.cellOffset+l.cellCount}u){return ${l.resolution}u;}`).join("")}
    return 1u;
  }
  fn cm12DynamicRowRung(within:u32)->u32{
    ${DYNAMIC_RUNG_LAYOUTS.map(l => `if(within<${l.rowOffset+l.rowCount}u){return ${l.resolution}u;}`).join("")}
    return 1u;
  }
  ...
  const DYNAMIC_PAGE_HEADER_WORDS = 16;
  const DYNAMIC_PAGE_ROW_BASE = DYNAMIC_PAGE_HEADER_WORDS;
  const DYNAMIC_PAGE_TERM_BASE = DYNAMIC_PAGE_ROW_BASE + 8 * DYNAMIC_PAGE_ROW_COUNT;
  const DYNAMIC_PAGE_INCIDENCE_BASE = DYNAMIC_PAGE_TERM_BASE + 2 * DYNAMIC_PAGE_TERM_COUNT;
  const DYNAMIC_PAGE_WORDS = DYNAMIC_PAGE_INCIDENCE_BASE + 2 * DYNAMIC_PAGE_BOUNDARY_COUNT;
   ...
   */
  function prepareDynamicPageImage(cellBase: number, rowBase: number, termBase: number): Uint32Array {
  ...
  const DYNAMIC_SEAM_HEADER_WORDS = 4;
  const DYNAMIC_SEAM_ROW_WORDS = 23;
  /** Fixed lookup table followed by immutable relative row/term/incidence data. */
  function packDynamicSeamCatalogue(): Uint32Array {
    const variants = preparedDynamicSeamCatalogue();
    const headerCount = DYNAMIC_PAGE_RUNGS.length * (DYNAMIC_PAGE_RUNGS.length + 1) * 6;
    ...
    for (const variant of variants.values()) {
      ...
      const header = ((own * (DYNAMIC_PAGE_RUNGS.length + 1) + neighbor) * 6 + variant.side) * DYNAMIC_SEAM_HEADER_WORDS;
  ...
  return {
    DYNAMIC_PAGE_RUNGS, DYNAMIC_RUNG_LAYOUTS, DYNAMIC_PAGE_CELL_COUNT, DYNAMIC_PAGE_ROW_COUNT,
    DYNAMIC_PAGE_TERM_COUNT, DYNAMIC_PAGE_BOUNDARY_COUNT, dynamicRungLayout, dynamicRowTermOffset,
    compileDynamicSeamVariant, preparedDynamicSeamCatalogue, coarseDynamicAdmissionRung,
    dynamicRungLayoutWGSL, DYNAMIC_PAGE_HEADER_WORDS, DYNAMIC_PAGE_ROW_BASE, DYNAMIC_PAGE_TERM_BASE,
    DYNAMIC_PAGE_INCIDENCE_BASE, DYNAMIC_PAGE_WORDS, prepareDynamicPageImage,
    DYNAMIC_SEAM_HEADER_WORDS, DYNAMIC_SEAM_ROW_WORDS, packDynamicSeamCatalogue,
  };
}
const catalogues = new Map<number, ReturnType<typeof buildDynamicRungCatalogue>>();
export function dynamicRungCatalogue(fineResolution: number = 8) {
  if (fineResolution !== 4 && fineResolution !== 8) throw new RangeError("dynamic pages require B4 or B8");
  let result = catalogues.get(fineResolution);
  if (!result) { result = buildDynamicRungCatalogue(fineResolution); catalogues.set(fineResolution, result); }
  return result;
}
/** B8 compatibility oracle; production selects its catalogue per atlas. */
export const {
  DYNAMIC_PAGE_RUNGS, DYNAMIC_RUNG_LAYOUTS, DYNAMIC_PAGE_CELL_COUNT, DYNAMIC_PAGE_ROW_COUNT,
  DYNAMIC_PAGE_TERM_COUNT, DYNAMIC_PAGE_BOUNDARY_COUNT, dynamicRungLayout, dynamicRowTermOffset,
  compileDynamicSeamVariant, preparedDynamicSeamCatalogue, coarseDynamicAdmissionRung,
