...
} from "./sparse-cm12-compiled-topology.wgsl";
import {
  createGeometricSourceWGSL,
  type GeometricSourceLayout,
} from "./geometric-source.wgsl";
import {
  createGeometricVolumeResidentWGSL,
...
import {
  ...
  PRESSURE_JOURNAL_CAPTURE_WGSL,
} from "./features/pressure-inspection/capture.wgsl";
import {
  cm12SimulationFailureWGSL,
  guardCM12SimulationDispatches,
} from "./sparse-cm12-simulation-failure.wgsl";
import {
  SPARSE_CM12_COMMON_HEIGHT_ENABLED,
  sparseCM12HeightReconstructionWGSL,
} from "./sparse-cm12-height-reconstruction.wgsl";
import { coarseFirstWGSL } from "./features/adaptivity/coarse-first.wgsl";
...
import type { SparseCM12IncrementalActivityLayout } from "./features/adaptivity/sparse-cm12-incremental-activity";
import { createSparseCM12IncrementalActivityWGSL } from "./features/adaptivity/sparse-cm12-incremental-activity.wgsl";
...
import { createSparseCM12FramePlanPresentationWGSL } from "./sparse-cm12-frame-plan-presentation.wgsl";
import {
...
import type { SparseCM12PressureTopologyRepairLayout } from "./sparse-cm12-pressure-topology-repair";
import { createSparseCM12PressureTopologyRepairWGSL } from "./sparse-cm12-pressure-topology-repair.wgsl";
import { SPARSE_CM12_PRESSURE_TOPOLOGY_REPAIR_CAUSE } from "./sparse-cm12-pressure-topology-repair";
import type {
  SparseCM12VelocityExtensionLayout,
  SparseCM12VelocityExtensionStateLayout,
} from "./sparse-cm12-velocity-extension";
import { createSparseCM12VelocityExtensionWGSL } from "./sparse-cm12-velocity-extension.wgsl";
import type { SparseCM12BrickTileFaceAddressLayout } from "./sparse-cm12-brick-tile-face-address-program";
import { createSparseCM12BrickTileFaceAddressWGSL } from "./sparse-cm12-brick-tile-face-address-program.wgsl";
import type { SparseCM12PressureExecutionImageLayout } from "./sparse-cm12-pressure-execution-image";
import { createSparseCM12PressureExecutionImageWGSL } from "./sparse-cm12-pressure-execution-image.wgsl";
import type { SparseCM12LogicalOwnerDirectoryLayout } from "./sparse-cm12-logical-owner-directory";
import { createSparseCM12LogicalOwnerDirectoryWGSL } from "./sparse-cm12-logical-owner-directory.wgsl";
import type { SparseCM12TransportExecutionImageLayout } from "./sparse-cm12-transport-execution-image";
import { createSparseCM12TransportExecutionImageWGSL } from "./sparse-cm12-transport-execution-image.wgsl";
import type { SparseCM12EffectiveTransportVelocityLayout } from "./sparse-cm12-effective-transport-velocity";
import { createSparseCM12EffectiveTransportVelocityWGSL } from "./sparse-cm12-effective-transport-velocity.wgsl";
import type { SparseCM12TransportPacketAuthorityLayout } from "./sparse-cm12-transport-packet-authority";
import { createSparseCM12TransportPacketAuthorityWGSL } from "./sparse-cm12-transport-packet-authority.wgsl";
import type { SparseCM12FinalScalarPacketMaskLayout } from "./sparse-cm12-final-scalar-packet-masks";

import { createSparseCM12FinalScalarPacketMaskWGSL } from "./sparse-cm12-final-scalar-packet-masks.wgsl";
import type { SparseCM12Phase1TransportQALayout } from "./sparse-cm12-phase1-transport-receipt";
import {
  createSparseCM12Phase1TransportQAWGSL,
  SPARSE_CM12_PHASE1_TRANSPORT_QA_WGSL_STUBS,
} from "./sparse-cm12-phase1-transport-receipt.wgsl";
import type { SparseCM12InternedBoundaryLayout } from "./sparse-cm12-interned-boundary-operators";
import { createSparseCM12InternedBoundaryImageWGSL } from "./sparse-cm12-interned-boundary-image.wgsl";
import type { SparseCM12InternedRefLookupLayout } from "./sparse-cm12-interned-ref-lookup";
import { createSparseCM12InternedRefLookupWGSL } from "./sparse-cm12-interned-ref-lookup.wgsl";
import type { SparseCM12IboTRASupplementLayout } from "./sparse-cm12-ibo-tra-supplement";
import { createSparseCM12IboTRASupplementWGSL } from "./sparse-cm12-ibo-tra-supplement.wgsl";
import { createSparseCM12IboTRAResidentHooksWGSL } from "./sparse-cm12-ibo-tra-resident-hooks.wgsl";
import {
  createSparseCM12GeometryFaceNeighborsWGSL,
  createSparseCM12IBOSemanticAuthorityWGSL,
} from "./sparse-cm12-ibo-semantic-authority.wgsl";
import {
...
import { createSparseCM12TopologyEffectsAuthorityWGSL } from "./sparse-cm12-topology-effects-authority.wgsl";
import {
...
// The standalone oracle above deliberately retains the generic row API used
// by its focused Dawn tests. Production pressure traverses CNX's contiguous
// term plane after one generation fence and addresses a row by dense ordinal,
// avoiding the dynamic-page divisions in termCell/termCoefficient.
const SPARSE_CM12_COMPILED_PRESSURE_ROW_GRADIENT_WGSL =
  SPARSE_CM12_PRESSURE_ROW_GRADIENT_WGSL.replace(
    "fn pressureRowGradient(row:u32,inputOffset:u32)->f32{",
    "fn pressureRowGradientCompiled(rowOrdinal:u32,inputOffset:u32)->f32{",
  )
    .replace(
      "rowTermRange(row)",
      "cnxRowTermRangeByOrdinalUnchecked(rowOrdinal)",
    )
    .replaceAll("termCell(", "cnxRowTermCellUnchecked(")
    .replaceAll("termCoefficient(", "cnxRowTermCoefficientUnchecked(");

/** Diagnostic A/B toggle. `FLUID_CM12_PHI_BAND=0` makes the phi lattice follow
 * the accepted solver rung in every brick, which is the pre-band phi domain
 * exactly. Read once at shader generation and at arena planning: the off arm
 * emits a constant `false`, so it costs no runtime branch and no extra pass. */
export const FINE_PHI_BAND_ENABLED = (globalThis as {
  process?: { env?: Record<string, string | undefined> };
}).process?.env?.FLUID_CM12_PHI_BAND !== "0";
...
export function createWebgpuSparseCM12ResidentWGSL(
  ...
  dynamicSeamCatalogueBaseWords = 0,
): string {
  if (
    ...
    throw new RangeError(
      `presentation page ${presentationPageResolution} does not divide brick ladder ${brickFineResolution}`,
    );
  }
  if (!internedBoundaryImage || !faceAddressLayout) {
    throw new Error(
      "Sparse Geometric (CM12) production WGSL requires compiled ITR and BFA layouts",
    );
  }
  if (!transportExecutionImageLayout || !transportPacketAuthorityLayout) {
    throw new Error(
      "Sparse Geometric (CM12) production composition requires TEI2 and TPA1",
    );
  }
  if (immutableHostIncidenceBaseWords === undefined) {
    throw new Error(
      "Sparse Geometric (CM12) production requires an immutable host-incidence image",
    );
  }
  if (
  ...
  const presentationPagesPerAxis =
    brickFineResolution / presentationPageResolution;
  const transportCellCapacity =
    velocityExtensionLayouts?.activity.cellCapacity ?? 1;
  ...
  const incrementalActivityEntries = incrementalActivityLayout
    ? createSparseCM12IncrementalActivityWGSL(
...
fn incrementalActivityReplaceCensus(brick:u32,score:u32,reasons:u32){
  ...
  if(f32(oldScore)/255.0>=p.activityTiming.y){atomicSub(&activity[3],1u);}
  if(f32(oldScore)/255.0<=p.activityTiming.w){atomicSub(&activity[4],1u);}
  if((reasons&1u)!=0u){atomicAdd(&activity[2],1u);}
  if(f32(score)/255.0>=p.activityTiming.y){atomicAdd(&activity[3],1u);}
  if(f32(score)/255.0<=p.activityTiming.w){atomicAdd(&activity[4],1u);}
}
`;
  const framePlanEntries = framePlanLayout
    ? createSparseCM12FramePlanWGSL({
        layout: framePlanLayout,
        arenaName: "activity",
      })
    : "";
  const framePlanPresentationEntries =
...
fn cm12PublishTransferredEffectiveVelocity(cell:u32,velocity:vec3f){
  ...
  const phase1TransportQAEntries = phase1TransportQALayout
    ? createSparseCM12Phase1TransportQAWGSL({
        layout: phase1TransportQALayout,
        publishEffectiveVelocity:
          effectiveTransportVelocityLayout !== undefined,
  ...
  const frameControlEntries = frameControlLayout
    ? createSparseCM12FrameControlWGSL({
        layout: frameControlLayout,
        controlName: "topologyArena",
...
`;
  const pressureExecutionImageEntries =
    createSparseCM12PressureExecutionImageWGSL({
      layout: pressureExecutionImageLayout,
      arenaName: "fineSamples",
      sourcePrefix: "peiSource",
      fullRebuild: true,
      publishFailure: (fault, owner) =>
        `cm12RecordFailure(6u,${owner},bitcast<vec4u>(vec4f(f32(${fault}),0.0,0.0,0.0)));`,
    });
  const compiledTopologyEntries = createSparseCM12CompiledTopologyWGSL({
    layout: compiledTopologyLayout,
    // A rejected candidate leaves the previously accepted topology image
    // byte-identical and available. The broader edit/injection lifecycle
    // predicate deliberately rejects that transaction state, so CNX uses the
    // narrower retained-source predicate below.
    source: { lifecycleAccepted: "sparseCM12CompiledTopologySourceAccepted()" },
    publishFailure: (fault, owner) =>
      `cm12RecordFailure(6u,${owner},bitcast<vec4u>(vec4f(f32(${fault}),0.0,0.0,0.0)));`,
  });
  const compiledTopologyTransportAccessEntries =
    compiledTopologyLayout && geometricVolumeLayout
      ? createSparseCM12CompiledTopologyTransportAccessWGSL({
          rowSubfaceRanges: geometricVolumeLayout.rowSubfaceRanges,
          cellSubfaceRanges: geometricVolumeLayout.cellSubfaceRanges,
          cellSubfaceEntries: geometricVolumeLayout.cellSubfaceEntries,
          subfaceMetadata: geometricVolumeLayout.subfaceMetadata,
        })
      : "";
  const pressureTopologyRepairEntries =
    createSparseCM12PressureTopologyRepairWGSL({
      layout: pressureTopologyRepairLayout,
      arenaName: "topologyArena",
      prefix: "ptr",
      workgroupSize: 64,
      fullImageAccepted: "peiFullImageAccepted()",
      publishFailure: (fault, owner) =>
        `cm12RecordFailure(6u,${owner},bitcast<vec4u>(vec4f(f32(${fault}),0.0,0.0,0.0)));` +
        "peiPublishPressureSolveDispatchGate(false);",
    }) +
    /* wgsl */ `
...
fn residentTopologyEffectsPreflightReady()->bool{
 return tfxPreflightReady(
  atomicLoad(&topologyArena[${
    topologyEffectsAuthorityLayout.baseWords +
    SPARSE_CM12_TOPOLOGY_EFFECTS_HEADER.generation
  }u]),
  atomicLoad(&topologyArena[${
    topologyEffectsAuthorityLayout.baseWords +
    SPARSE_CM12_TOPOLOGY_EFFECTS_HEADER.ptrTargetGeneration
  }u]),
  atomicLoad(&topologyArena[${
    topologyEffectsAuthorityLayout.baseWords +
    SPARSE_CM12_TOPOLOGY_EFFECTS_HEADER.ptrCount
  }u]),
  atomicLoad(&topologyArena[${
    topologyEffectsAuthorityLayout.baseWords +
    SPARSE_CM12_TOPOLOGY_EFFECTS_HEADER.ptrHash
  }u]));}
...
`;
  ...
  const logicalOwnerEntries =
    ...
    logicalOwnerDirectory
      ...
      : "";
  const solidOccupancyEntries =
    sparseCM12SolidOccupancyWGSL(solidOccupancyLayout);
  const transportExecutionImageEntries =
    createSparseCM12TransportExecutionImageWGSL({
      layout: transportExecutionImageLayout,
    });
  const transportPacketAuthorityEntries =
...
${createSparseCM12GeometryFaceNeighborsWGSL({
  ...
  offsetBaseWords:
    internedBoundaryImage.semanticAuthority.geometryOffsetBaseWords,
  neighborBaseWords:
    internedBoundaryImage.semanticAuthority.geometryNeighborBaseWords,
  arenaName: "topologyArena",
  hookPrefix: "cm12",
})}
...
const ISA1_AUTHORITY_LEAF_CAPACITY:u32=${
        internedBoundaryImage.semanticAuthority.leafCapacity
      }u;
const ISA1_IMMUTABLE_CONTENT_HASH:u32=${
        internedBoundaryImage.semanticAuthority.immutableContentHash
      }u;
const ISA1_IMMUTABLE_CERTIFICATE_HASH:u32=${
        internedBoundaryImage.semanticAuthority.immutableCertificateHash
      }u;
...
// Phase one of a scheduled-leaf compile: claim, then reset the leaf header and
// its whole ref block.  The claim is a compare-exchange on the leaf's delta
// generation word, so across the entire dispatch exactly one invocation owns a
// leaf even when several delta leaves list it as a geometry neighbour; every
// other claimant returns without touching a byte.  Result .x is 0 for unowned
// or faulted, 1 for owned-and-inactive (seal only) and 2 for owned-and-active
// (compile the six sides, then seal); result .y is the scheduled descriptor.
fn cm12IBOCompileClaimLeaf(leaf:u32)->vec2u{
  if(leaf>=IBO1_LEAF_CAPACITY){return vec2u(0u,IBO1_INVALID);}
  let slot=cm12IBOShadowSlot();
  let generation=cm12IBOCandidateGeneration();
  if(!cm12IBOTryClaimLeaf(slot,leaf,generation)){return vec2u(0u,IBO1_INVALID);}
  ...
  if(begun.x!=0u){cm12IBORecordFault(begun);return vec2u(0u,descriptor);}
  return vec2u(select(1u,2u,scheduledActive),descriptor);
}
// Phase two: one side of one owned leaf.  The output ordinal is already local to
// a side (record = leaf*24 + side*4 + output), so a lane owns its own compact
// ref range and no counter is shared between lanes.  The per-side selection scan
// stays serial because cm12IBOSealDeltaLeaf requires refs packed from local 0.
fn cm12IBOCompileLeafSide(slot:u32,leaf:u32,descriptor:u32,side:u32){
  var output=0u;
...
// One workgroup per topology-delta leaf.  The geometry closure of that leaf
// (itself plus up to 26 immutable geometry neighbours) is the task set; lanes
// fan out over it and, inside phase two, over the 6 sides of every owned leaf,
// so a frame that commits a handful of bricks no longer walks tens of thousands
// of dependent indirect loads on a single lane.  The changed-set receipt and the
// geometry-closure append stay on lane 0, once per root leaf, exactly as before;
// both are commutative folds (count/xor/sum) so the receipt is bit-identical.
const CM12_IBO_DELTA_LANES:u32=64u;
var<workgroup> cm12IBODeltaRootLeaf:u32;
var<workgroup> cm12IBODeltaTaskCount:u32;
var<workgroup> cm12IBODeltaLeaf:array<u32,64>;
var<workgroup> cm12IBODeltaState:array<u32,64>;
var<workgroup> cm12IBODeltaDescriptor:array<u32,64>;
fn cm12IBODeltaClosureLeaf(root:u32,index:u32)->u32{
  if(index==0u){return root;}
  return cm12ISAGeometryNeighbor(root,index-1u);
}
@compute @workgroup_size(64) fn compileSparseCM12InternedBoundaryDelta(
 @builtin(local_invocation_id)lid:vec3u,@builtin(workgroup_id)wid:vec3u){
  if(lid.x==0u){let listed=topologyDeltaLeafInvocation(wid.x);
    cm12IBODeltaRootLeaf=listed;cm12IBODeltaTaskCount=0u;
    if(listed!=INVALID){cm12ISARecordChangedLeaf(listed);
      // Dynamic roots remain in the exact changed-set receipt. Their faces
      // are certified against the prepared dynamic catalogue, not SCMT/IBO.
      if(listed<CM12_WDR_INITIAL_LEAVES){
        cm12ISAAppendGeometryClosure(listed,cm12IBOCandidateGeneration());
        cm12IBODeltaTaskCount=1u+cm12ISAGeometryNeighborCount(listed);}}}
  let root=workgroupUniformLoad(&cm12IBODeltaRootLeaf);
  let taskCount=workgroupUniformLoad(&cm12IBODeltaTaskCount);
  let slot=cm12IBOShadowSlot();
  var base=0u;
  loop{
    if(base>=taskCount){break;}
    let chunk=min(taskCount-base,CM12_IBO_DELTA_LANES);
    if(lid.x<chunk){let leaf=cm12IBODeltaClosureLeaf(root,base+lid.x);
      let claim=cm12IBOCompileClaimLeaf(leaf);
      cm12IBODeltaLeaf[lid.x]=leaf;cm12IBODeltaState[lid.x]=claim.x;
      cm12IBODeltaDescriptor[lid.x]=claim.y;}
    // Publish the phase-one resets before any lane writes a ref into them.
    workgroupBarrier();storageBarrier();
    for(var task=lid.x;task<chunk*6u;task+=CM12_IBO_DELTA_LANES){
      let local=task/6u;
      if(cm12IBODeltaState[local]==2u){cm12IBOCompileLeafSide(slot,
        cm12IBODeltaLeaf[local],cm12IBODeltaDescriptor[local],task-local*6u);}}
    // Publish every ref of a leaf before its owner lane hashes and seals it.
    workgroupBarrier();storageBarrier();
    if(lid.x<chunk&&cm12IBODeltaState[lid.x]!=0u){
      cm12IBORecordFault(cm12IBOSealDeltaLeaf(slot,cm12IBODeltaLeaf[lid.x]));}
    workgroupBarrier();
    base+=CM12_IBO_DELTA_LANES;
  }}
...
@compute @workgroup_size(64) fn validateSparseCM12InternedBoundaryDeltaPackets(
 @builtin(local_invocation_id)lid:vec3u,@builtin(workgroup_id)wid:vec3u){
  if(lid.x==0u){let listed=topologyDeltaLeafInvocation(wid.x);
    ...
    if(listed!=INVALID&&listed<CM12_WDR_INITIAL_LEAVES){
...
// Same closure, one lane per leaf.  cm12IBOReplayDeltaLeaf is a verbatim
// slot-to-slot copy, so two invocations replaying a leaf shared by two delta
// leaves write identical words - it was already concurrent across workgroups
// and stays idempotent across lanes, needing no claim and no barrier.
@compute @workgroup_size(64) fn replaySparseCM12InternedBoundaryDelta(
 @builtin(local_invocation_id)lid:vec3u,@builtin(workgroup_id)wid:vec3u){
  if(lid.x==0u){let listed=topologyDeltaLeafInvocation(wid.x);
    cm12IBODeltaRootLeaf=listed;cm12IBODeltaTaskCount=0u;
    if(listed!=INVALID&&listed<CM12_WDR_INITIAL_LEAVES){
  ...
  let root=workgroupUniformLoad(&cm12IBODeltaRootLeaf);
  let taskCount=workgroupUniformLoad(&cm12IBODeltaTaskCount);
  let source=acceptedTopologySlot();let retiredSlot=1u-source;
  for(var index=lid.x;index<taskCount;index+=CM12_IBO_DELTA_LANES){
    cm12IBOReplayDeltaLeaf(source,retiredSlot,
      cm12IBODeltaClosureLeaf(root,index),cm12IBOAcceptedGeneration());}
  if(wid.x==0u&&lid.x==0u){cm12IBOStore(IBO1_BASE+2u,source);
...
`;
  const faceAddressEntries = createSparseCM12BrickTileFaceAddressWGSL({
    layout: faceAddressLayout,
    arenaName: "topologyArena",
  });
  const internedBoundaryCommitReceipt = internedBoundaryImage
    ? /* wgsl */ `
  if(deltaCount!=0u){
    ...
    atomicStore(&topologyArena[${
      topologyEffectsAuthorityLayout.baseWords +
      SPARSE_CM12_TOPOLOGY_EFFECTS_HEADER.reservedBase +
      11
    }u],select(0u,1u,iboReady));
...
fn lsvBuildSlot()->u32{return select(0u,1u-lsvAcceptedSlot(),lsvAcceptedSlot()<2u);}
// --- Fine-phi band -------------------------------------------------------
// Phi lives on a vertex lattice. On a rung seam the shared vertices are
// hanging: they are slaved to the coarse cell's trilinear patch, and every
// rung change re-samples them trilinearly. Both are information losses at
// exactly the place the interface sits. Inside the band the phi lattice is the
// brick's finest template rung whatever the accepted solver rung is, so an
// interface-plane vertex is independent and a rung change carries it
// bit-exactly. Outside the band phi cells still follow the accepted rung.
// The band is the 26-neighbour presentation apron of surface/thin/cut bricks,
// eight fine cells wide against a four-cell metric band, so any hanging
// constraint that survives sits where |phi| is already saturated.
fn cm12PhiBandBrick(brick:u32)->bool{
${FINE_PHI_BAND_ENABLED ? `  let reasons=atomicLoad(&activity[activityRecord(brick)+1u]);
  return (reasons&(1u|256u|512u))!=0u||brickHasPresentationSurfaceSupport(brick);`
  : `  _=brick;
  return false;`}
}
// vec2u(phi cell count, phi resolution). A zero count removes the brick from
// the phi domain entirely, exactly as an inactive leaf contributes no cells to
// the accepted solver worklist.
fn cm12PhiBrickPlan(brick:u32,banded:bool)->vec2u{
  if(brick>=p.dispatch.w||!brickActive(brick)){return vec2u(0u,0u);}
...
// compactOwnerCellAt over the planned phi rung instead of the accepted solver
// rung. The clipped-leaf aperture is reproduced exactly, so a phi ordinal and
// the stable template cell it names stay in bijection.
fn cm12PhiOwnerCellAtSlot(slot:u32,q:vec3i)->u32{
  ...
  if(brick==INVALID){return INVALID;}
  let resolution=lsvBrickPhiResolution(slot,brick);
  if(resolution==0u){return INVALID;}
  ...
  if(any(local>=valid)){return INVALID;}
  let localOffset=local.x+valid.x*(local.y+valid.y*local.z);
  if(localOffset>=count){return INVALID;}
  return first+localOffset;
}
// The source phi plan covers every represented solver page. An absent page
// is air by sparse-world residency, rather than a failed phi hash lookup.
// Eight finest-cube queries certify the half-spacing ball at a departure;
// existing pages (including broken/missing cell backing) refuse this proof.
// This is reached only when ordinary sampling and phase continuation fail.
fn cm12UnrepresentedAirClearance(position:vec3f)->f32{
  let lower=vec3i(floor(position-vec3f(0.5)));
  let upper=vec3i(floor(position+vec3f(0.5)));
  let slot=lsvAcceptedSlot();var clearance=0.5;
  for(var corner=0u;corner<8u;corner+=1u){
    let q=select(lower,upper,vec3<bool>((corner&1u)!=0u,(corner&2u)!=0u,(corner&4u)!=0u));
    let coordinate=vec3i(floor(vec3f(q)/f32(BRICK_FINE_RESOLUTION)));
    let brick=brickDirectoryLookupAtSignedCoordinate(coordinate);
    if(brick!=INVALID&&lsvBrickPhiResolution(slot,brick)!=0u){
      let nearest=clamp(position,vec3f(q),vec3f(q)+vec3f(1.0));
      clearance=min(clearance,length(position-nearest));
    }
  }
  return clearance;
}
// O(1) inverse of the plan: template cells are contiguous per (brick, rung),
// so the per-brick prefix base is the only state an ordinal lookup needs.
fn cm12PhiCellOrdinalInSlot(slot:u32,cell:u32)->u32{
  if(cell==INVALID||cell>=p.counts.x){return INVALID;}
  let brick=cellBrick(cell);
  let resolution=lsvBrickPhiResolution(slot,brick);
  if(resolution==0u||cellResolution(cell)!=resolution){return INVALID;}
  let range=templateBrickCellRange(brick,resolution);
  if(range.y==0u||cell<range.x||cell-range.x>=range.y){return INVALID;}
  let base=lsvBrickPhiBase(slot,brick);if(base==INVALID){return INVALID;}
  return base+(cell-range.x);
}
// Cell-record word 7 stays the owning SOLVER cell: transport spans and wall
// clipping must read the coarse cell even where phi is finer.
fn cm12PhiOwnerSolverCell(phiCell:u32)->u32{
  let owner=compactOwnerCellAt(vec3i(floor(cellCenter(phiCell))));
  return select(phiCell,owner.x,owner.x!=INVALID);
}
// Published samples carry log2 of the width of the field that produced them.
// In the band that is the phi width, not the accepted cell width.
fn cm12PhiPresentationScale(brick:u32,span:u32,resolution:u32)->u32{
  let phiResolution=max(resolution,lsvBrickPhiResolution(lsvAcceptedSlot(),brick));
  return BRICK_FINE_RESOLUTION*span/max(1u,phiResolution);
}
fn cm12PhiWidthAt(positionFine:vec3f)->f32{
  let stencil=lsvStencilAtPosition(positionFine);
  if(!stencil.resolved){return 0.0;}
  return min(stencil.widths.x,min(stencil.widths.y,stencil.widths.z));
}
...
` + createLevelSetVolumeWGSL({
      ...
      // The phi domain is the planned phi-cell space, not the solver cell
      // space. Every one of these five hooks moves with it together; mixing
      // the two would address a phi record by a solver ordinal.
      buildCellCountExpression: "lsvPlannedCellCount()",
      buildCellAtOrdinal: ordinal =>
        `lsvPhiCellAtOrdinalInSlot(lsvBuildSlot(),${ordinal})`,
      acceptedCellOrdinal: cell =>
        `cm12PhiCellOrdinalInSlot(lsvAcceptedSlot(),${cell})`,
      buildCellMemberOrdinal: cell =>
        `cm12PhiCellOrdinalInSlot(lsvBuildSlot(),${cell})`,
      buildOwnerSolverCell: cell => `cm12PhiOwnerSolverCell(${cell})`,
      acceptedOwnerCellAt: lattice =>
        `cm12PhiOwnerCellAtSlot(lsvAcceptedSlot(),${lattice})`,
      buildOwnerCellAt: lattice =>
        `cm12PhiOwnerCellAtSlot(lsvBuildSlot(),${lattice})`,
      phiCellPlan: {
        brickCountExpression: "p.dispatch.w",
        bandedBrickPlan: brick => `cm12PhiBrickPlan(${brick},true)`,
        acceptedBrickPlan: brick => `cm12PhiBrickPlan(${brick},false)`,
        brickCellRange: (brick, resolution) =>
      ...
      unrepresentedAirClearance: position => `cm12UnrepresentedAirClearance(${position})`,
      ...
      publishAdvectionFailure: (fault, owner, samplePosition) =>
        `cm12RecordFailure(7u,${owner},vec4u(${fault},bitcast<u32>(${samplePosition}.x),` +
        `bitcast<u32>(${samplePosition}.y),bitcast<u32>(${samplePosition}.z)));`,
...
struct LsvCellStencil{resolved:bool}
fn lsvCellStencil(_cell:u32)->LsvCellStencil{return LsvCellStencil(false);}
fn lsvStencilAtPosition(_position:vec3f)->LsvCellStencil{return LsvCellStencil(false);}
...
const EXP_TRANSPORT_PROFILE:bool=${
    phase1TransportProfileBaseWords === undefined ? "false" : "true"
  };
...
// A fine world page beside a frozen coarse host needs the ordinary compiled
// mixed-ratio graph before it can become active. This is construction demand,
// not permission to refine the already accepted host.
const ACTIVITY_FROZEN_FRONTIER_GENERATION:u32=0x00020000u;
// Prephysics-only reason: this dry candidate lies in the swept AABB of a
// nonzero donor. A later dispatch may add its six nonrecursive face supports.
const ACTIVITY_GEOMETRIC_SWEEP_RECEIVER:u32=0x00080000u;
...
const CM12_DYNAMIC_SEAM_CATALOGUE:u32=${dynamicSeamCatalogueBaseWords}u;
...
fn sparseCM12CompiledTopologySourceAccepted()->bool{
  let base=topologyWorklistBase();let phase=atomicLoad(&topologyArena[base+3u]);
  let generation=atomicLoad(&topologyArena[base]);
  // Phase 3 rejects only the isolated candidate. Publication never entered
  // its authorized no-fail tail, so generation, slot, lists and incidence
  // still describe the retained accepted source mirrored by activity[12].
  let retainedRejected=phase==3u
    &&atomicLoad(&topologyArena[base+1u])!=generation
    &&atomicLoad(&activity[12])==generation;
  let stable=phase==0u&&atomicLoad(&topologyArena[base+1u])==generation
    &&atomicLoad(&activity[12])==generation;
  return stable||retainedRejected;
}
...
${
  sourceLayout
    ? createGeometricSourceWGSL(sourceLayout)
    : `fn geometricSourceRate(cell:u32)->f32{return 0.0;}
fn geometricSourceCommitMicrostep(dt:f32){}
fn geometricSourceFinishStagedCompensation(){}`
}

fn physicalCellOpenFraction(id:u32)->f32{
...
fn cellOpenFraction(id:u32)->f32{
  if(!geometricSolidMotionActive()){return physicalCellOpenFraction(id);}
  return geometricSolidCurrentCapacity(id)/cellVolume(id);
}
fn cellOpenVolume(id:u32)->f32{return geometricSolidCurrentCapacity(id);}
// The copied rigid cell/face samplers use different quadrature points.
// A sampled open face can therefore touch an exactly closed sampled cell.
// Use one compatible closure in both pressure and physical transport. This
// conservative whole-row mask may overblock a partially open mixed row; exact
// cut-subface geometry is required to recover that finer aperture.
fn physicalRowHasClosedEndpoint(id:u32)->bool{
  if(!hasSolidBoundaries()){return false;}
  let range=rowTermRange(id);
  for(var term=range.x;term<range.y;term+=1u){
    // Read the geometry currently voxelized, not the old/new snapshot planes:
    // old-row snapshots run after old-pose voxelization, new rows after new.
    if(physicalCellOpenFraction(termCell(term))==0.0){return true;}
  }
  return false;
}
fn physicalRowOpenFraction(id:u32)->f32{
  let solid=select(1.0,state[p.solidOffsets.y+3u*id],hasSolidBoundaries());
  let sampled=solid*solidVoxelRowOpenFraction(id);
  if(sampled==0.0||physicalRowHasClosedEndpoint(id)){return 0.0;}
  return sampled;
}
fn rowOpenFraction(id:u32)->f32{return geometricSolidRowOpen(id);}
fn rowSolidVelocity(id:u32)->f32{
  if(!hasSolidBoundaries()){return 0.0;}return state[p.solidOffsets.y+3u*id+1u];
}
fn physicalRowPressureOpenFraction(id:u32)->f32{
...
fn rowPressureOpenFraction(id:u32)->f32{
  let open=geometricSolidRowPressureOpen(id);
  if(geometricSolidMotionActive()){return open;}
...
fn rowSeparatingFromClosedWorldState(row:u32,densityOffset:u32,
 velocityOffset:u32)->bool{
  ...
  if(!cellTransportActive(cell)||!lsvCellLiquid(cell)){return false;}
  ...
  let travelDeadband=select(1e-4,5e-5,pressureAcceptedRowMember(row));
...
// Form capacity from integer open occupancy. Subtracting a rounded solid
// fraction from one can make a fully closed voxel slightly negative on GPU.
fn geometricOpenQ8Fraction(openQ8:u32, samples:u32)->f32{
  if(samples==0u||openQ8==0u){return 0.0;}
  let total=255u*samples;
  if(openQ8==total){return 1.0;}
  return f32(openQ8)/f32(total);
}
fn geometricVoxelOpenFraction(solidQ8:u32)->f32{
  return geometricOpenQ8Fraction(255u-solidQ8,1u);
}
fn refreshSparseCM12SolidWorldCell(cell:u32){
  ...
  state[solidVoxelCellOpenOffset()+cell]=geometricOpenQ8Fraction(openQ8,volume);
}
...
fn refreshSparseCM12SolidWorldRow(row:u32){
  ...
  state[solidVoxelRowOpenOffset()+row]=geometricOpenQ8Fraction(openQ8,area);
}
@compute @workgroup_size(64)
...
fn candidateFaceBoundaryRowRange(brick:u32,accepted:u32,
 side:u32,boundary:u32)->vec2u{
  if(brick>=CM12_WDR_INITIAL_LEAVES){
    let axis=side/2u;let qU=boundary%accepted;let qV=boundary/accepted;
    var q=vec3u(0u);q[axis]=select(0u,accepted-1u,(side&1u)!=0u);
    q[(axis+1u)%3u]=qU;q[(axis+2u)%3u]=qV;
...
fn candidateFaceRow(index:u32)->u32{if((index&0x80000000u)!=0u){return index&0x7fffffffu;}return ta(index);}
fn candidatePhysicalFaceRow(index:u32)->u32{
  let row=candidateFaceRow(index);
  if(row>=ta(3u)||rowKind(row)!=3u){return row;}
  let cell=termCell(rowTermRange(row).x);
  for(var incidence=incidenceBegin(cell);incidence<incidenceEnd(cell);incidence+=1u){
    if(ta(IMMUTABLE_HOST_INCIDENCE_BASE+2u*incidence)==row){return incidenceRow(incidence);}}
  return row;
}
...
fn topologyFreezeEnabled()->bool{return (p.surfaceProof.w&0x80000000u)!=0u;}
fn brickResolutionFrozen(brick:u32)->bool{
  return topologyFreezeEnabled()&&brickActive(brick);
}
...
fn brickPackedCandidateSlot(brick:u32)->u32{
  if(brick>=CM12_WDR_INITIAL_LEAVES){return select(INVALID,brick,cm12PreparedPageReady(brick));}
...
fn brickCandidateTopologyComplete(brick:u32)->bool{
  ...
  // needed by transfer, PTR, CNX, and publication.
...
// membership-only; rerung transitions still require a complete packed
// candidate. Same-rung retirement separately needs no target field.
...
fn templateBrickCellRange(brick:u32,resolution:u32)->vec2u{
  if(brick>=CM12_WDR_INITIAL_LEAVES){
    ...
    return vec2u(ta(2u)+(brick-CM12_WDR_INITIAL_LEAVES)*CM12_DYNAMIC_CELLS
      +cm12DynamicCellOffset(resolution),count);
  }
...
fn templateRowOwnerRange(brick:u32,resolution:u32)->vec2u{
  if(brick>=CM12_WDR_INITIAL_LEAVES){
    let count=3u*(resolution+1u)*resolution*resolution;
    return vec2u(ta(3u)+(brick-CM12_WDR_INITIAL_LEAVES)*CM12_DYNAMIC_ROWS
      +cm12DynamicRowOffset(resolution),count);
  }
...
fn scheduledBrickActive(brick:u32)->bool{
  // An unscheduled intent cannot change the membership of a committed image.
  // Use the same publication boundary as scheduledBrickResolution.
  return select(brickActive(brick),candidateBrickActive(brick),
...
fn rowAccepted(row:u32)->bool{
  if(row>=ta(3u)){
    ...
    let leaf=CM12_WDR_INITIAL_LEAVES+page;let resolution=cm12DynamicRowRung(within);
    if(!brickActive(leaf)||acceptedBrickResolution(leaf)!=resolution){return false;}
    let local=within-cm12DynamicRowOffset(resolution);
    let perAxis=(resolution+1u)*resolution*resolution;let axis=local/perAxis;
    let face=local%(resolution+1u);if(face!=0u&&face!=resolution){return true;}
    let uv=(local%perAxis)/(resolution+1u);var q=vec3u(0u);
    q[axis]=select(0u,resolution-1u,face==resolution);
    q[(axis+1u)%3u]=uv%resolution;q[(axis+2u)%3u]=uv/resolution;
    let cell=cm12DynamicCellOffset(resolution)+q.x+resolution*(q.y+resolution*q.z);
    let at=dynamicIncidenceOverrideAt(candidateTopologyPageBase(page),cell,
      2u*axis+select(0u,1u,face==resolution));
    return ta(at)==row;
  }
  if(hostExteriorRowSupersededAt(row,false)){return false;}
  let requirements=rowRequirementOffset(row);let count=ta(requirements);
  for(var at=0u;at<count;at+=1u){let metadata=ta(requirements+1u+at);
    let brick=metadata>>TEMPLATE_CELL_RESOLUTION_BITS;
    let resolution=metadata&TEMPLATE_CELL_RESOLUTION_MASK;
...
fn compactOwnerCellAt(q:vec3i)->vec3u{
  ...
  // A clipped leaf has fewer cells per axis. A coordinate beyond one axis
  // can still have an in-range flattened index and alias the next row.
  if(any(local>=valid)){return vec3u(INVALID);}
  let localOffset=local.x+valid.x*(local.y+valid.y*local.z);
...
fn presentationCanonicalCoarseCoordinate(coarse:vec3i,cellScale:u32,brick:u32)->vec3i{
  let dimensions=max(vec3i(1),vec3i(p.dimensions.xyz)/i32(cellScale));
  if(all(coarse>=vec3i(0))&&all(coarse<dimensions)){return coarse;}
  if(brickHasUnclippedWorldGeometry(brick)){return coarse;}
  let fine=coarse*i32(cellScale);let edge=i32(BRICK_FINE_RESOLUTION);
  let coordinate=vec3i(cm12WorldFloorToSpan(fine.x,edge),
    cm12WorldFloorToSpan(fine.y,edge),cm12WorldFloorToSpan(fine.z,edge))/edge;
...
// Presentation consumes the same accepted TEI leaf descriptors as transport:
// first cell, native scale and valid strides are compiled at topology commit.
// Page/proof workgroups stage the nearby directory once. Queries outside that
// window still load a compiled leaf, never reconstruct its template address.
var<workgroup>presentationTopologyStaged:u32;
// Binding 14 remains renderer metadata during publication. Binding 3 carries
// the existing TEI buffer in the presentation bind group (no copy or rebuild).
fn presentationTeiWord(index:u32)->u32{
  return bitcast<vec4u>(partials[index/4u])[index%4u];
}
fn presentationLoadLeaf(slot:u32,brick:u32)->CM12TransportLeaf{
  if(brick>=CM12_TEI_LEAF_CAPACITY){return CM12TransportLeaf(
    0u,0u,INVALID,0u,INVALID,vec3u(0u),0u,0u);}
  let at=cm12TeiLeafBase(slot)+CM12_TEI_LEAF_WORDS*brick;
  let generation=presentationTeiWord(at);
  if(generation==0u){return CM12TransportLeaf(
    0u,0u,INVALID,0u,INVALID,vec3u(0u),0u,0u);}
  let packed=presentationTeiWord(at+5u);let scale=presentationTeiWord(at+6u);
  return CM12TransportLeaf(generation,presentationTeiWord(at+1u),
    presentationTeiWord(at+2u),presentationTeiWord(at+3u),brick,
    vec3u(packed&31u,(packed>>5u)&31u,(packed>>10u)&31u),scale,
    cm12TeiScaleLog2(scale,presentationTeiWord(at+7u)));
}
fn presentationStageTopology(brick:u32,lane:u32){
  let center=cm12WorldLeafCoordinate(brick);let slot=acceptedTopologySlot();
  if(lane==0u){cm12TeiCacheCenter=center;cm12TeiCacheSlot=slot;}
  if(lane<27u){
    let coordinate=center+vec3i(i32(lane%3u)-1,i32((lane/3u)%3u)-1,i32(lane/9u)-1);
    let leaf=presentationLoadLeaf(slot,cm12WorldOwnerAt(coordinate));
  ...
  if(lane==0u){presentationTopologyStaged=1u;}workgroupBarrier();
}
fn presentationLeafAt(lower:vec3i)->CM12TransportLeaf{
  let edge=i32(BRICK_FINE_RESOLUTION);
  let tile=vec3i(cm12WorldFloorToSpan(lower.x,edge),
    cm12WorldFloorToSpan(lower.y,edge),cm12WorldFloorToSpan(lower.z,edge))/edge;
  let delta=tile-cm12TeiCacheCenter;
  if(presentationTopologyStaged!=0u&&all(delta>=vec3i(-1))&&all(delta<=vec3i(1))){
    let index=u32(delta.x+1)+3u*u32(delta.y+1)+9u*u32(delta.z+1);
    let a=cm12TeiCache0[index];let b=cm12TeiCache1[index];
    return CM12TransportLeaf(a.x,a.y,a.z,a.w,b.x,
      vec3u(b.y&31u,(b.y>>5u)&31u,(b.y>>10u)&31u),b.z,b.w);
  }
  return presentationLoadLeaf(acceptedTopologySlot(),cm12WorldOwnerAt(tile));
}
fn presentationCompiledOwnerCellAt(q:vec3i)->vec3u{
  let leaf=presentationLeafAt(q);
  if((leaf.flags&0x80000000u)==0u||leaf.scale==0u){return vec3u(INVALID);}
  let relative=q-cm12WorldLeafCoordinate(leaf.owner)*i32(BRICK_FINE_RESOLUTION);
  if(any(relative<vec3i(0))){return vec3u(INVALID);}
  let local=vec3u(relative)>>vec3u(leaf.scaleLog2);
  if(any(local>=leaf.valid)){return vec3u(INVALID);}
  let offset=local.x+leaf.valid.x*(local.y+leaf.valid.y*local.z);
  if(offset>=leaf.count){return vec3u(INVALID);}
  return vec3u(leaf.first+offset,leaf.owner,leaf.flags&31u);
}
// Integrate each native donor once, weighted by its exact overlap volume.
// A logical tile has one accepted leaf even at mixed-rung and macro seams.
fn presentationLeafMass(leaf:CM12TransportLeaf,lower:vec3i,extent:vec3i,
 densityOffset:u32)->f32{
  if((leaf.flags&0x80000000u)==0u||leaf.scale==0u){return 0.0;}
  if((atomicLoad(&activity[activityRecord(leaf.owner)+1u])&64u)==0u){return 0.0;}
    ...
    loop{if(y>=extent.y){break;}
      ...
      loop{if(x>=extent.x){break;}
        ...
        if(all(local<leaf.valid)&&offset<leaf.count){
          ...
          mass+=f32(dx*dy*dz)*clamp(state[densityOffset+cell]
            /max(cellOpenFraction(cell),1e-6),0.0,1.0);
        }
        x+=dx;
      }y+=dy;
...
fn presentationCompiledDensityAt(lower:vec3i,cellScale:i32,densityOffset:u32)->f32{
  ...
  if((leaf.flags&0x80000000u)!=0u&&leaf.scale!=0u){
    ...
    if(all(local<leaf.valid)&&offset<leaf.count&&leaf.scale>=u32(cellScale)){
      return clamp(state[densityOffset+leaf.first+offset]
        /max(cellOpenFraction(leaf.first+offset),1e-6),0.0,1.0);
    }
    ...
    if(all(lower>=origin)&&all(lower+vec3i(cellScale)<=upper)){
      return presentationLeafMass(leaf,lower,vec3i(cellScale),densityOffset)
  ...
  loop{if(z>=cellScale){break;}
    ...
    loop{if(y>=cellScale){break;}
      ...
      loop{if(x>=cellScale){break;}
        ...
        mass+=presentationLeafMass(presentationLeafAt(q),q,vec3i(dx,dy,dz),densityOffset);
        x+=dx;
      }y+=dy;
    }z+=dz;
  }
...
fn presentationStencilDensityAt(coarse:vec3i,cellScale:u32,
 ...
 densityOffset:u32)->f32{
  let canonical=presentationCanonicalCoarseCoordinate(coarse,cellScale,cm12PresentationBrick);
  if(cacheFits){
 ...
 densityOffset:u32)->vec3f{
  ...
  while(y<upper){
    ...
    if(owner.x==INVALID||!brickActive(owner.y)){
      ...
      open=geometricVoxelOpenFraction(cm12SolidVoxelFractionQ8(q));
...
// intervals without treating a coarse value as a point sample. Auto requires
// liquid in the accepted floor owner, so a completely dry or elevated column
// rejects itself, as does a cavity, overhang, or cut column. A monotone
// floor-connected column may cross any number of bricks because its summed
// volume remains the authoritative height.
fn presentationIntegratedAdaptiveFloorHeight(x:i32,z:i32,
 densityOffset:u32)->vec2f{
  let floorOwner=compactOwnerCellAt(vec3i(x,0,z));
  if(floorOwner.x==INVALID||!brickActive(floorOwner.y)
    ||state[densityOffset+floorOwner.x]
      /max(cellOpenFraction(floorOwner.x),1e-6)<=1e-6){
...
// A coarse column is a volume average over its X/Z footprint. Repeating its
// height at every fine sample creates one flat terrace per authority cell.
// Interpolate neighboring column receipts at their native centers instead;
// all vertical pages evaluate the same horizontal field. This changes only
// presentation and preserves constant/off-grid waterline receipts exactly.
fn presentationContinuousColumnHeight(brick:u32,x:i32,z:i32,
 densityOffset:u32,floorColumn:bool)->vec2f{
  if(brick>=p.dispatch.w||!brickActive(brick)){return vec2f(0.0);}
  let scale=BRICK_FINE_RESOLUTION*brickSpan(brick)/acceptedBrickResolution(brick);
  if(scale==1u){
    if(floorColumn){return presentationIntegratedAdaptiveFloorHeight(x,z,densityOffset);}
    return presentationIntegratedColumnHeight(brick,x,z,densityOffset);
  }
  let position=(vec2f(f32(x),f32(z))+vec2f(0.5))/f32(scale)-vec2f(0.5);
  let lower=vec2i(floor(position));let t=fract(position);var height=0.0;
  for(var dz=0;dz<2;dz+=1){for(var dx=0;dx<2;dx+=1){
    let center=vec2i((vec2f(lower+vec2i(dx,dz))+vec2f(0.5))*f32(scale));
    let y=cm12WorldLeafCoordinate(brick).y*i32(BRICK_FINE_RESOLUTION);
    let sample=presentationCanonicalCoarseCoordinate(
      vec3i(center.x,y,center.y),1u,brick).xz;
    var receipt=vec2f(0.0);
    if(floorColumn){
      receipt=presentationIntegratedAdaptiveFloorHeight(sample.x,sample.y,densityOffset);
    }else{
      let owner=compactOwnerCellAt(vec3i(sample.x,y,sample.y));
      if(owner.x==INVALID){return vec2f(0.0);}
      receipt=presentationIntegratedColumnHeight(owner.y,sample.x,sample.y,densityOffset);
    }
    if(receipt.y!=1.0){return vec2f(0.0);}
    let weight=select(1.0-t.x,t.x,dx==1)*select(1.0-t.y,t.y,dz==1);
    height+=weight*receipt.x;
  }}
  return vec2f(height,1.0);
}
...
fn preparePresentationColumnHeights(lane:u32,brick:u32,pageOrigin:vec3i,
 densityOffset:u32,halo:bool){
  ...
  // Ordinary adaptivity needs no authored-region permission. A surface page
  // may sit above the floor (the min-8 B2/B1 waterline is in brick y=1), so
  // its accepted surface/thin evidence requests the world-column walk. The
  // receipt itself starts at the physical floor and rejects a dry gap,
  // overhang, cavity, or non-monotone column.
  let adaptiveFloorColumn=((activityReasons&(1u|64u|256u))!=0u
  ...
  if(uniformWorldColumn){
    if(lane==0u){
      ...
      for(var neighbour=0u;neighbour<9u;neighbour+=1u){
        ...
        let coordinate=presentationCanonicalCoarseCoordinate(brickOrigin
          +vec3i(dx,0,dz)*i32(BRICK_FINE_RESOLUTION),1u,brick);
        var receipt=presentationIntegratedWorldColumnHeight(
          coordinate.x,coordinate.z,densityOffset);
        if(p.correctionReserved.x>1.5){
          // Auto uses the complete accepted-owner walk. Unlike the specialised
          // B1 shortcut, it proves monotonicity from the physical floor through
          // the whole authored column and therefore rejects floating sheets.
          receipt=presentationIntegratedAdaptiveFloorHeight(
            coordinate.x,coordinate.z,densityOffset);
        }
      ...
      if(worldCenter>=0.0){
      ...
      }else if(p.correctionReserved.x<0.5){
        // Explicit On retains the local-column fallback. Auto must reject a
        // world column whose floor-connected receipt failed: a local bracket
        // can describe the top of a floating slab while missing the dry gap
        // below it.
        ...
        for(var column=0u;column<count;column+=1u){
          presentationHeightCache[column]=-1.0;
        }
      }
    }
    ...
    for(var column=lane;column<count;column+=64u){
      let localZ=column/axis;let localX=column-localZ*axis;
      let coordinate=presentationCanonicalCoarseCoordinate(pageOrigin
        +vec3i(i32(localX)-offset,0,i32(localZ)-offset),1u,brick);
      let x=coordinate.x;let z=coordinate.z;
      let receipt=presentationContinuousColumnHeight(brick,x,z,densityOffset,true);
    ...
    for(var column=lane;column<count;column+=64u){
      ...
      let receipt=presentationContinuousColumnHeight(columnBrick,x,z,densityOffset,false);
...
fn presentationHeightPolicyEnabled(brick:u32)->bool{
  // Surface reconstruction is an authority choice. Shared RDF must reach
  // every published interface, including a calm coarse waterline; otherwise
  // the nominal RDF mode silently becomes a height/RDF hybrid. The legacy
  // PLIC mode retains its independent exact-column presentation option.
  if(p.correctionReserved.y<0.5){return false;}
  ...
  let requested=p.correctionReserved.x<0.5;
  let automatic=p.correctionReserved.x>1.5&&brick<p.dispatch.w
    &&acceptedBrickResolution(brick)<BRICK_FINE_RESOLUTION*brickSpan(brick);
  return (requested||automatic)&&brick<p.dispatch.w&&p.injectionCenter.w<=0.5;
}
...
// Display a continuous scalar between accepted cell-center densities. The
// conservative per-cell linear polynomial used for refinement has independent
// face traces and therefore creates terraces when contoured. Interpolation is
// presentation-only; directSmoothedPresentationDensityAt retains conservative
// transfer of the authoritative finite-volume means.
fn smoothedPresentationDensityAt(q:vec3i,cellScale:u32,
 patchFirst:vec3i,patchDimensions:vec3u)->f32{
  return interpolatedPresentationDensityAt(q,cellScale,patchFirst-vec3i(1,3,1),
    patchDimensions+vec3u(2u,6u,2u),true,0u);
}
...
fn directSmoothedPresentationDensityAt(q:vec3i,cellScale:u32,
 densityOffset:u32)->f32{
  ...
  let owner=compactOwnerCellAt(q);
  let canonical=presentationCanonicalCoarseCoordinate(lower,cellScale,owner.y);
  ...
  for(var axis=0u;axis<3u;axis+=1u){
    ...
    backCoordinate[axis]-=1;forwardCoordinate[axis]+=1;
    backCoordinate=presentationCanonicalCoarseCoordinate(backCoordinate,cellScale,owner.y);
    forwardCoordinate=presentationCanonicalCoarseCoordinate(forwardCoordinate,cellScale,owner.y);
...
fn cm12TransportOwnerAtFine(q:vec3i,direct:bool)->CM12TransportOwner{
  if(direct){
    let cell=${
      implicitTransportOwnerArithmeticForQA
        ? "cm12ImplicitAuthoredOwnerAtFine(q)"
        : "ownerCellAt(q)"
    };
...
const TRANSPORT_CHARACTERISTIC_CLEARANCE:u32=${
    velocityExtensionLayouts?.state.characteristicSupportFloatBase ?? 0
  }u;
...
fn cm12FinalScalarCellFacts(cell:u32)->vec4u{
  ...
  let flip=pressureCellMembershipPredicate(cell)!=pressureAcceptedCellMember(cell);
  if(bulk){
...
fn transportDepartureCharacteristicClearance(stencil:TransportStencil)->f32{
  ...
  var minimumVelocity=vec3f(1e30);var maximumVelocity=vec3f(-1e30);
  for(var corner=0u;corner<8u;corner+=1u){let cell=stencil.cells[corner];
    ...
    minimumVelocity=min(minimumVelocity,velocity);
    maximumVelocity=max(maximumVelocity,velocity);
  }
  // A first-corner reference changes under stencil permutations. The
  // velocity bounding-box diagonal conservatively bounds every pair's
  // separation and is invariant under axis reflections and permutations.
  let maximumDelta=length(maximumVelocity-minimumVelocity);
...
fn prepareTransportFaceRow(row:u32,rowOrdinal:u32){
  if(rowArea(row)<=1e-8){
  ...
  let supportBeginRange=cnxRowTermRangeByOrdinalUnchecked(rowOrdinal);
  let supportBegin=supportBeginRange.x;let supportEnd=supportBeginRange.y;
  for(var term=supportBegin;term<supportEnd;term+=1u){
    let cell=cnxRowTermCellUnchecked(term);
    let packed=state[FACE_VELOCITY_SUPPORT+4u*cell+3u];
  ...
  if(p.refinementRegionControl.x>0u){
    let beginRange=cnxRowTermRangeByOrdinalUnchecked(rowOrdinal);
    let begin=beginRange.x;let end=beginRange.y;
    for(var term=begin;term<end;term+=1u){
      let cell=cnxRowTermCellUnchecked(term);
...
fn prepareSparseCM12AcceptedFaceRows(@builtin(global_invocation_id)gid:vec3u){
  ...
  prepareTransportFaceRow(row,gid.x);
}
@compute @workgroup_size(64)
...
fn injectLiquidFaces(@builtin(global_invocation_id)gid:vec3u){
  ...
  // A pending external dose has not entered the fluid. Do not apply a jet
  // impulse when the bounded source admitted no volume or its edit rejected.
  ${sourceLayout ? "if(!sparseCM12TopologyLifecycleAccepted()||state[GEOMETRIC_SOURCE_LEDGER+6u]<=0.0){return;}" : "return;"}
...
fn validateDensityDeficitSupport(donor:u32,visible:f32,deficit:f32,density:f32){
  if(density!=0.0){cm12RecordFailure(2u,donor,
    vec4u(bitcast<u32>(visible),bitcast<u32>(deficit),bitcast<u32>(density),0u));}
}
...
// A composite pressure row is a bipartite physical face, not a clique.
// Each side's signed gradient coefficients sum to +/- 1 / distance. Their
// product therefore gives the subface fraction, including unequal patches.
fn cm12PhysicalSubfaceArea(row:u32,ownCoefficient:f32,otherCoefficient:f32)->f32{
  if(ownCoefficient*otherCoefficient>=0.0){return 0.0;}
  let distance=rowDistance(row);
  // Recover each side's area fraction before multiplying. On dyadic faces
  // these fractions are exact powers of two; multiplying the two reciprocal
  // distances first needlessly rounds an exact unit subface away from one.
  return rowArea(row)*(abs(ownCoefficient)*distance)*(abs(otherCoefficient)*distance);
}
...
fn densityCapacityRepairShare(moved:i32,area:f32,totalArea:f32)->i32{
  if(moved<=0||area<=0.0||totalArea<=0.0){return 0;}
  // Preserve exact integer division whenever a recipient is one of an exact
  // number of equal-area portions. Converting a large receipt to f32 before
  // division can round the quotient upward and send more than that portion.
  let portions=round(totalArea/area);
  if(portions>=1.0&&portions<1073741824.0&&area*portions==totalArea){
    return moved/i32(portions);
  }
  return i32(floor(f32(moved)*(area/totalArea)));
}
fn scatterDensityCapacityRepairCellAtPlane(cell:u32,plane:u32){
  if(cell==INVALID||!cellTransportActive(cell)){return;}
  let moved=densityCapacityRepairMass(cell);if(moved<=0){return;}
  let totalArea=densityCapacityRepairArea(cell);if(totalArea<=1e-8){return;}
  var distributed=0i;
  for(var at=incidenceBegin(cell);at<incidenceEnd(cell);at+=1u){let row=incidenceRow(at);
    if(!rowAccepted(row)){continue;}
    let own=termCoefficient(incidenceTerm(at));
    ...
    for(var term=begin;term<end;term+=1u){let neighbor=termCell(term);
      ...
      let share=densityCapacityRepairShare(moved,area,totalArea);
      if(share<=0){continue;}
      distributed+=share;atomicAdd(&conditioning[plane*p.counts.x+neighbor],share);
    }
  }
  // Debit exactly the credited integer receipts. Fractional residue stays at
  // the source, so a catalogue's last term never gets a privileged direction.
...
fn prepareDensityCapacityRepairGather(
 @builtin(global_invocation_id)gid:vec3u){
  let cell=acceptedTemplateCellInvocation(gid.x);if(cell==INVALID){return;}
  atomicStore(&conditioning[5u*p.counts.x+cell],bitcast<i32>(densityCapacityRepairArea(cell)));
  atomicStore(&conditioning[6u*p.counts.x+cell],densityCapacityRepairMass(cell));
}
@compute @workgroup_size(64)
...
fn gatherDensityCapacityRepair(
 @builtin(global_invocation_id)gid:vec3u){
  ...
  let ownMass=atomicLoad(&conditioning[6u*p.counts.x+cell]);
  let ownArea=bitcast<f32>(atomicLoad(&conditioning[5u*p.counts.x+cell]));
  var receipt=0i;
  for(var at=incidenceBegin(cell);at<incidenceEnd(cell);at+=1u){let row=incidenceRow(at);
    if(!rowAccepted(row)){continue;}
    let own=termCoefficient(incidenceTerm(at));
    ...
    for(var term=begin;term<end;term+=1u){let neighbor=termCell(term);
      ...
      receipt-=densityCapacityRepairShare(ownMass,area,ownArea);
      let neighborMass=atomicLoad(&conditioning[6u*p.counts.x+neighbor]);
      let neighborArea=bitcast<f32>(atomicLoad(&conditioning[5u*p.counts.x+neighbor]));
      receipt+=densityCapacityRepairShare(neighborMass,area,neighborArea);
    }
  }
  state[destinationDensity()+cell]=max(0.0,before
    +f32(receipt)/cm12PhysicalMassFixedScale()/cellVolume(cell));
}
`
    : ""
}
...
fn forceFaces(@builtin(global_invocation_id)gid:vec3u){
  ...
  if(!cnxAccepted()||cnxStableRowUnchecked(gid.x)!=row){return;}
  let metadata=cnxRowPackedMetadataByOrdinal(gid.x);let axis=metadata&3u;
  if(rowSeparatingFromClosedWorld(row)){
    let range=cnxRowTermRangeByOrdinalUnchecked(gid.x);
    let cell=cnxRowTermCellUnchecked(range.x);
...
fn enforceSparseCM12InflowFaces(@builtin(global_invocation_id)gid:vec3u){
  let row=cnxAcceptedRowInvocation(gid.x);
  if(row==INVALID||rowArea(row)<=1e-8){return;}
  let coverage=sparseCM12InflowFaceCoverage(row);if(coverage<=0.0){return;}
  let axis=cnxRowPackedMetadataByOrdinal(gid.x)&3u;
...
fn movingPressurePredictedFill(id:u32)->bool{
  ...
  // Classification precedes this frame's pressure projection. Predict in
  // extensive fine-cell volume with the same incidence weights and
  // wall-relative stored velocity used by preparePressure. This anticipates a
  // Dirichlet free-surface cell potentially filling during frozen-frame
  // transport; the postprojection transport audit remains authoritative.
  var equation=0.0;var correction=0.0;
  ...
  for(var at=incidenceRangeForCell.x;at<incidenceRangeForCell.y;at+=1u){
    ...
    if(hasSolidBoundaries()){
      velocity-=(1.0-rowOpenFraction(row))*rowSolidVelocity(row);
    }
    let ownCoefficient=cnxIncidenceOwnCoefficientUnchecked(at);
    let weight=select(rowDualWeight(row),rowStaticDualWeight(row),hasSolidBoundaries());
    var value=ownCoefficient*weight*velocity;
    if(value>0.0){
      var supported=false;let range=cnxRowTermRangeByOrdinalUnchecked(rowOrdinal);
      for(var term=range.x;term<range.y;term+=1u){
        let other=cnxRowTermCellUnchecked(term);
        if(other==id||cnxRowTermCoefficientUnchecked(term)*ownCoefficient>=0.0){continue;}
        let otherVolume=state[destinationDensity()+other]*cellVolume(other);
        supported=supported||otherVolume>9.5367431640625e-7*cellOpenVolume(other)
          ||geometricSourceRate(other)>0.0;
      }
  ...
  let predictedVolume=state[destinationDensity()+id]*cellVolume(id)
    +p.frame.x*equation;
...
// Full-image pressure publication is owned by PEI2.
fn ptrTopologyGeneration()->u32{return atomicLoad(&topologyArena[topologyWorklistBase()]);}
fn pressureAcceptedCellMember(cell:u32)->bool{return peiPressureCellMember(cell);}
fn pressureCandidateCellMember(cell:u32)->bool{return peiBuildingPressureCellMember(cell);}
fn pressureAcceptedRowMember(row:u32)->bool{return peiPressureRowMember(row);}
fn pressureCandidateRowMember(row:u32)->bool{return peiBuildingPressureRowMember(row);}
fn peiSourceTopologyGeneration()->u32{return ptrTopologyGeneration();}
fn peiSourceFullTopologyAccepted()->bool{return cnxAccepted();}
fn peiSourceFullCellOrdinal(cell:u32)->u32{return cnxCellOrdinalUnchecked(cell);}
...
fn peiSourceFullBeginBuild(){
  atomicStore(&activity[ACCEPTED_COARSE_ROW_COUNT],0u);
  atomicStore(&activity[ACCEPTED_MIXED_ROW_COUNT],0u);
  atomicStore(&activity[PRESSURE_ACTIVE_ROW_COUNT],0u);
}
fn peiSourceFullBuildAccepted()->bool{
  return atomicLoad(&topologyArena[cm12FailureBase()])==0u;}
fn peiSourceFullPublishCoefficient(cell:u32){publishFullPressureCoefficientCell(cell);}
// PEI2 rebuilds the only coefficient consumed by the production Jacobi solve
// directly from the sealed CNX graph.  The incidence and axis accumulation
// order matches the established generic diagonal tail above, while avoiding
// directed-edge exchanges and dirty aggregate propagation with no consumer.
fn publishFullPressureCoefficientCell(cell:u32){
  var diagonalAxes=vec3f(0.0);
  if(peiBuildingPressureCellMember(cell)){
    let range=cnxCellIncidenceRangeUnchecked(cell);
    // Preserve the production authored-interior diagonal tree. CNX incidences
    // retain source order, so the two-term off-diagonal recurrence is the same
    // -x,+x,-y,+y,-z,+z sequence formerly read from the PCF edge catalogue.
    var interior=cell<ta(2u)&&!cm12WorldHasDynamicLeaves()
      &&!hasSolidBoundaries()&&range.y-range.x==6u;
    var interiorDiagonal=0.0;
    for(var at=range.x;at<range.y&&interior;at+=1u){
      let rowOrdinal=cnxIncidenceRowOrdinalUnchecked(at);
      let row=cnxStableRowUnchecked(rowOrdinal);
      let terms=cnxRowTermRangeByOrdinalUnchecked(rowOrdinal);
      interior=terms.y-terms.x==2u;
      let own=cnxIncidenceOwnCoefficientUnchecked(at);var other=0.0;
      var otherCell=PEI_INVALID;
      for(var term=terms.x;term<terms.y;term+=1u){
        let termCellId=cnxRowTermCellUnchecked(term);
        if(termCellId!=cell){otherCell=termCellId;
          other=cnxRowTermCoefficientUnchecked(term);}
      }
      interior=interior&&otherCell!=PEI_INVALID
        &&peiBuildingPressureCellMember(otherCell);
      interiorDiagonal-=rowDualWeight(row)*own*other;
    }
    if(interior){
      if(!(interiorDiagonal==interiorDiagonal&&abs(interiorDiagonal)<=3.402823466e38)){
        cm12RecordFailure(6u,cell,
          bitcast<vec4u>(vec4f(10.0,interiorDiagonal,0.0,0.0)));return;}
      state[p.stateOffsets2.z+cell]=interiorDiagonal;return;
    }
    for(var at=range.x;at<range.y;at+=1u){
      let rowOrdinal=cnxIncidenceRowOrdinalUnchecked(at);
      let row=cnxStableRowUnchecked(rowOrdinal);
      if(row==PEI_INVALID){
        cm12RecordFailure(6u,cell,bitcast<vec4u>(vec4f(6.0,0.0,0.0,0.0)));return;}
      let theta=state[p.stateOffsets3.x+row];
      if(!peiBuildingPressureRowMember(row)||theta<=0.0){continue;}
      let coefficient=cnxIncidenceOwnCoefficientUnchecked(at);
      let axis=cnxRowPackedMetadataByOrdinal(rowOrdinal)&3u;
      diagonalAxes[axis]+=rowDualWeight(row)*coefficient*coefficient/theta;
    }
  }
  let diagonal=(diagonalAxes.x+diagonalAxes.y)+diagonalAxes.z;
  if(!(diagonal==diagonal&&abs(diagonal)<=3.402823466e38)){
    cm12RecordFailure(6u,cell,bitcast<vec4u>(vec4f(10.0,diagonal,0.0,0.0)));return;}
...
fn classifyPressureRow(row:u32)->bool{
  ...
  if(cut&&rowKind(row)==2u){
    ...
    // vanish. PEI membership distinguishes this from an excluded face.
...
${SPARSE_CM12_COMPILED_PRESSURE_ROW_GRADIENT_WGSL}
fn applyOperator(cell:u32,inputOffset:u32)->f32{
  ...
  if(!cnxAccepted()){return 0.0;}
  var negative=vec3f(0.0);var positive=vec3f(0.0);
  let incidenceRangeForCell=cnxCellIncidenceRangeUnchecked(cell);
  for(var incidence=incidenceRangeForCell.x;incidence<incidenceRangeForCell.y;incidence+=1u){
    let rowOrdinal=cnxIncidenceRowOrdinalUnchecked(incidence);
    let row=cnxStableRowUnchecked(rowOrdinal);let theta=state[p.stateOffsets3.x+row];
    if(!pressureAcceptedRowMember(row)||theta<=0.0){continue;}
    let ownCoefficient=cnxIncidenceOwnCoefficientUnchecked(incidence);
    let jump=pressureRowGradientCompiled(rowOrdinal,inputOffset);
    let contribution=rowDualWeight(row)*ownCoefficient*jump/theta;
    let axis=cnxRowPackedMetadataByOrdinal(rowOrdinal)&3u;
    if(ownCoefficient>0.0){negative[axis]+=contribution;}
    else{positive[axis]+=contribution;}
  }
...
fn preparePressure(@builtin(global_invocation_id)gid:vec3u){
  ...
  if(!cnxAccepted()){return;}
  var negative=vec3f(0.0);var positive=vec3f(0.0);let diagonal=state[p.stateOffsets2.z+id];
  let incidenceRangeForCell=cnxCellIncidenceRangeUnchecked(id);
  for(var at=incidenceRangeForCell.x;at<incidenceRangeForCell.y;at+=1u){
    let rowOrdinal=cnxIncidenceRowOrdinalUnchecked(at);
    let row=cnxStableRowUnchecked(rowOrdinal);
    if(!pressureAcceptedRowMember(row)){continue;}
    let coefficient=cnxIncidenceOwnCoefficientUnchecked(at);
    let fluxWeight=select(rowDualWeight(row),rowStaticDualWeight(row),
      hasSolidBoundaries());
    var fluidVelocity=state[destinationFaceVelocity()+row];
    if(hasSolidBoundaries()){
      fluidVelocity-=(1.0-rowOpenFraction(row))*rowSolidVelocity(row);
    }
    let value=coefficient*fluxWeight*fluidVelocity;
    let axis=cnxRowPackedMetadataByOrdinal(rowOrdinal)&3u;
...
var<workgroup>reduceD:array<f32,64>;
...
fn staticSolidRestrictionError(origin:vec3i,rung:u32,lane:u32)->f32{
  ...
  for(var group=lane;group<faceGroups;group+=64u){
    ...
    for(var dv=0u;dv<span;dv+=1u){for(var du=0u;du<span;du+=1u){
      ...
      let value=geometricVoxelOpenFraction(max(cm12SolidVoxelFractionQ8(negative),
        cm12SolidVoxelFractionQ8(positive)));
      sum+=value;squareSum+=value*value;count+=1.0;
    }}
    let mean=sum/count;
...
// The seed residual is already the exact b-Ap residual used by the initial
// convergence receipt. Reduce its Jacobi and residual facts together so PCG
// initialization evaluates the pressure operator only once.
fn reducePressureSeed(lane:u32,group:u32,value:vec4f){
  reduceA[lane]=value.x;reduceB[lane]=value.y;
  reduceC[lane]=value.z;reduceD[lane]=value.w;workgroupBarrier();
  var width=32u;loop{if(lane<width){
      reduceA[lane]+=reduceA[lane+width];reduceB[lane]+=reduceB[lane+width];
      reduceC[lane]+=reduceC[lane+width];reduceD[lane]=max(reduceD[lane],reduceD[lane+width]);}
    workgroupBarrier();if(width==1u){break;}width/=2u;}
  if(lane==0u){partials[group]=vec4f(
    reduceA[0],reduceB[0],reduceC[0],reduceD[0]);}
}
...
fn initializePCG(@builtin(global_invocation_id)gid:vec3u,
 @builtin(local_invocation_id)lid:vec3u,@builtin(workgroup_id)wid:vec3u){
  let id=pressureCellInvocation(gid.x);var receipt=vec4f(0.0);
  if(id!=INVALID){
    ...
    state[p.stateOffsets3.w+id]=z;
    let rhs=state[p.stateOffsets2.y+id];
    receipt=vec4f(residual*z,rhs*rhs,residual*residual,abs(residual));
  }
  reducePressureSeed(lid.x,wid.x,receipt);
}

@compute @workgroup_size(64)
fn reduceInitialize(@builtin(local_invocation_id)lid:vec3u){
  var seed=vec4f(0.0);for(var at=lid.x;at<pressureCellWorkgroups();at+=64u){
    let value=partials[at];seed.x+=value.x;seed.y+=value.y;seed.z+=value.z;
    seed.w=max(seed.w,value.w);}
  reduceA[lid.x]=seed.x;reduceB[lid.x]=seed.y;
  reduceC[lid.x]=seed.z;reduceD[lid.x]=seed.w;workgroupBarrier();var width=32u;loop{
    if(lid.x<width){reduceA[lid.x]+=reduceA[lid.x+width];reduceB[lid.x]+=reduceB[lid.x+width];
      reduceC[lid.x]+=reduceC[lid.x+width];reduceD[lid.x]=max(reduceD[lid.x],reduceD[lid.x+width]);}
    workgroupBarrier();if(width==1u){break;}width/=2u;}
  if(lid.x==0u){scalars[0]=reduceA[0];scalars[1]=reduceB[0];scalars[2]=0.0;
    ...
    scalars[8]=reduceC[0];scalars[9]=reduceD[0];
    let tolerance=pressureRelativeTolerance();
    if(tolerance>0.0&&reduceC[0]<=tolerance*tolerance*reduceB[0]){
...
fn projectPressureRowOrdinal(row:u32,rowOrdinal:u32){
  let theta=state[p.stateOffsets3.x+row];
  if(theta<=0.0){return;}
  let separating=rowSeparatingFromClosedWorld(row);
  if(!separating&&rowArea(row)<=1e-8){
    state[destinationFaceVelocity()+row]=select(0.0,rowSolidVelocity(row),hasSolidBoundaries());return;}
  let jump=pressureRowGradientCompiled(rowOrdinal,p.stateOffsets2.x);
...
fn projectPressureRow(row:u32){
  let rowOrdinal=cnxRowOrdinal(row);if(rowOrdinal==INVALID){return;}
  projectPressureRowOrdinal(row,rowOrdinal);
}

@compute @workgroup_size(64)
fn projectSparseCM12DynamicFaceRows(@builtin(global_invocation_id)gid:vec3u){
  let row=cnxAcceptedRowInvocation(gid.x);
  if(row==INVALID||row<ta(3u)||!rowAccepted(row)||!pressureAcceptedRowMember(row)){return;}
  projectPressureRowOrdinal(row,gid.x);
...
fn collocateAndDiagnose(@builtin(global_invocation_id)gid:vec3u,
 @builtin(local_invocation_id)lid:vec3u,@builtin(workgroup_id)wid:vec3u){
  let id=cnxAcceptedCellInvocation(gid.x);
  ...
  if(id!=INVALID&&!cellTransportActive(id)){
    ...
    for(var at=incidenceRangeForCell.x;at<incidenceRangeForCell.y;at+=1u){
      let rowOrdinal=cnxIncidenceRowOrdinalUnchecked(at);
      let row=cnxStableRowUnchecked(rowOrdinal);
      let metadata=cnxRowPackedMetadataByOrdinal(rowOrdinal);
      touchesMixed=touchesMixed||((metadata>>2u)&3u)==2u;
      let term=cnxIncidenceOwnTermUnchecked(at);let axis=metadata&3u;
      let fluxWeight=select(rowDualWeight(row),rowStaticDualWeight(row),
        hasSolidBoundaries());
      let coefficient=cnxRowTermCoefficientUnchecked(term);
      let w=abs(coefficient)*fluxWeight;
      ...
      if(pressureAcceptedCellMember(id)){
    ...
    // Pressure, VEX and phi advection must share one frozen liquid set. A
    // conservative cut-cell volume can be below the density isovalue while
    // its level-set centre is liquid; retaining the pre-projection VEX value
    // there advects phi with a different velocity from the pressure solve.
    cm12PublishCollocatedWetEffectiveVelocity(id,velocity,
      pressureAcceptedCellMember(id));
    let targetDivergence=0.0;
    let controlVolume=cellOpenVolume(id);
    let divergence=select(0.0,-equation/max(controlVolume,1e-8)
      -targetDivergence,pressureAcceptedCellMember(id));
  ...
  var width=32u;loop{if(lid.x<width){
    ...
    reduceB[lid.x]=max(reduceB[lid.x],reduceB[lid.x+width]);}
    workgroupBarrier();if(width==1u){break;}width/=2u;}
  // Binding 3 is the persistent effective-velocity plane during collocation.
  // Writing diagnostic partials there races with cell publication and erases
  // low-index velocities. The sharpening/pressure-image scratch is dead after
  // projection; retain the two group maxima there until the next dispatch.
  if(lid.x==0u){
    state[p.stateOffsets5.x+2u*wid.x]=reduceA[0];
    state[p.stateOffsets5.x+2u*wid.x+1u]=reduceB[0];
  }
}
...
fn reduceDivergenceDiagnostics(@builtin(local_invocation_id)lid:vec3u){
  ...
  for(var at=lid.x;at<acceptedTemplateCellWorkgroups();at+=64u){
    globalMaximum=max(globalMaximum,state[p.stateOffsets5.x+2u*at]);
    mixedMaximum=max(mixedMaximum,state[p.stateOffsets5.x+2u*at+1u]);
  }
  reduceA[lid.x]=globalMaximum;reduceB[lid.x]=mixedMaximum;workgroupBarrier();
...
// A brick is enclosed liquid when EVERY accepted cell in it is submerged,
// where a cell of widths w is submerged if -phi(cell centre) >= |w|/2 (the
// ball of radius -phi about the centre then contains the cell). The census
// AND-reduces that per-cell test over the brick once and caches it as reason
// bit 15; the six/26-neighbour lifecycle consumers here read the receipt, so
// this stays one load rather than up to 64 phi walks per query.
fn brickPhiDeepLiquid(brick:u32)->bool{
  if(brick==INVALID||!brickActive(brick)){return false;}
  return (atomicLoad(&activity[activityRecord(brick)+1u])&32768u)!=0u;
}
 ...
 @builtin(local_invocation_index)lane:u32){
  ...
  if(lane<26u){
    ...
    if(neighbor!=INVALID&&neighbor!=brick&&brickActive(neighbor)){
      ...
      if((atomicLoad(&activity[neighborOutput+3u])
      ...
      if((neighborReasons&(1u|256u|512u))!=0u
        ||((neighborReasons&64u)!=0u&&activityF32(neighborOutput+4u)<1.0-1e-3)){
...
fn forcedSurfaceResolutionForQA()->u32{return p.surfaceProof.w&31u;}
...
${coarseFirstWGSL}// Flags occupy the unused high bits of two 27-bit support masks. Mask before
// shifting flags back: swept directions 22..26 are not surface/occupancy bits.
fn cm12UnpackActivityFlags(packed:vec2u)->u32{
  return (packed.x>>27u)|((packed.y>>27u)<<5u);
}

// A swept characteristic needs the pages between its source and predicted
// endpoint, including face/edge subsets of diagonal travel. A wall may block
// the diagonal endpoint while leaving a tangential component open; rejecting
// that endpoint must not remove the open part of the swept support box.
fn cm12SweptPageSupportMask(offset:vec3i)->u32{
  var mask=0u;
  for(var z=min(0,offset.z);z<=max(0,offset.z);z+=1){
    for(var y=min(0,offset.y);y<=max(0,offset.y);y+=1){
      for(var x=min(0,offset.x);x<=max(0,offset.x);x+=1){
        if(x!=0||y!=0||z!=0){
          mask|=1u<<u32(x+1+3*(y+1)+9*(z+1));
        }
      }
    }
  }
...
fn measureBrickActivity(@builtin(local_invocation_id)lid:vec3u,
 @builtin(workgroup_id)wid:vec3u){
  let lane=lid.x;
  if(lane==0u){activityRequestedBrick=incrementalActivityBrickInvocation(wid.x);}
  let requestedBrick=workgroupUniformLoad(&activityRequestedBrick);
  // Only clean/invalid work exits here. Dirty retired leaves still remove
  // their old census below. The uniform load proves the exit safe for barriers.
  if(requestedBrick==INVALID){return;}
  let brick=requestedBrick;
  ...
  if(lane==0u){activityCoarseNormalSupport=vec4f(0.0);}
  ...
  for(var localCell=lane;localCell<measuredCount;localCell+=64u){
    ...
    // (0.866*w for a cube). A non-metric sample carries no usable distance,
    // so its sign is all the evidence there is. Testing this per accepted
    // cell instead of once at the brick centre keeps the sufficient condition
    // rigorous while letting a shallow pool actually qualify as enclosed: the
    // single centre sample demanded half the BRICK diagonal (~6.9 fine cells
    // at B8), which no mini32 brick ever reaches.
    let cellDeepLiquid=ownWet&&(!ownPhiSample.metric
      ||-ownPhiSample.phi>=0.5*length(cellWidths(cell)));
    shallowLiquidCell=shallowLiquidCell||!cellDeepLiquid;
    ...
    for(var incidence=firstIncidence;incidence<incidenceLimit;incidence+=1u){
      ...
      let transportArea=rowArea(row);
      if(aperture>0.0&&transportArea>1e-8){
        var transportVelocity=state[destinationFaceVelocity()+row];
        if(hasSolidBoundaries()){
          transportVelocity-=(1.0-aperture)*rowSolidVelocity(row);
        }
        transportVelocity/=aperture;
        // Keep activity compilation independent of the optional geometric-
        // volume module. This is the same eight-f32-ulp capacity tolerance
        // used by gvRoundoff when that module is present.
        let transportRoundoff=9.5367431640625e-7*cellOpenVolume(cell);
        // Membership follows the represented face velocity only. The former
        // v+dt*a widening pre-staged the page under EVERY resting liquid cell
        // because gravity alone makes the forced value nonzero on every
        // vertical face, so a settled pool activated and retired a dry page
        // every frame. The acceleration this step is about to be cancelled by
        // pressure; the flux transport will actually use is measured
        // post-projection by markProjectedGeometricTransportReceivers, whose
        // transaction runs before transport. The conservative envelope in
        // gatherGeometricPreflightVelocityBounds still carries v+dt*a.
        if(p.frame.x*transportArea*abs(transportVelocity)>transportRoundoff){
          transportMinimumVelocity[axis]=min(transportMinimumVelocity[axis],transportVelocity);
          transportMaximumVelocity[axis]=max(transportMaximumVelocity[axis],transportVelocity);
        }
      }
      ...
      for(var term=begin;term<end;term+=1u){
        ...
        if(crossesIsovalue&&(!ownWet||cellBrick(neighbor)==brick||coarseFirstEnabled())){
          interfaceCell=true;ownDensityInterface=true;densityInterfaceCell=true;surfaceAxes|=1u<<axis;
        }
        if(crossesIsovalue){
    ...
    // the configured thin-feature width: it must carry liquid at its centre
    ...
    // bricks at B8 forever. Measure the represented slab against the authored
    // physical thin-feature width, exactly as the density form did.
    let thinFeatureWidth=p.activityThresholds.w;
    var cellIsThinFluid=false;
    if(ownWet&&ownPhiSample.metric&&-ownPhiSample.phi<=0.5*thinFeatureWidth){
      for(var axis=0u;axis<3u;axis+=1u){
    ...
    // A nonzero donor also needs the adjacent interpolation support, even
    // below the rendered contour. Keeping only its swept endpoint misses the
    // side pages of a diagonal stencil and newly advected dilute edge cells.
    // Reuse this census closure; no characteristic or transport pass is added.
    // This geometric stencil is half of cm12BrickFrontierDemandMask: it is the
    // velocity-free term that lets a resting surface hold its dry apron.
    if(interfaceCell||cellIsThinFluid||rho>0.0){
    ...
    // Forward CM12 deficit carries every nonzero donor, including the dilute
    // edge ahead of rho=.5. Residency must follow that mass characteristic;
    // the surface classifier above remains the authority for refinement.
    if(rho>0.0){
      // The center bit is an exact material receipt. Neighbour-direction
      // consumers skip bit13, so dilute nonzero donors can author this without
      // turning quantized census density into a residency authority.
      sweptSupportMask|=1u<<13u;
      let minimum=cellMinimum(cell);let widths=cellWidths(cell);
      let brickWidth=i32(BRICK_FINE_RESOLUTION);
      ...
      // Membership follows the next physical transport, including the
      // acceleration applied before its projection. A zero-displacement cell
      // whose closed AABB merely touches a page boundary does not request that
      // dry page; ceil(upper)-1 turns the upper bound into a half-open range.
      let sweptLower=vec3f(minimum)+p.frame.x*transportMinimumVelocity;
      let sweptUpper=vec3f(minimum)+widths+p.frame.x*transportMaximumVelocity;
      let lowerBrick=vec3i(floor(sweptLower/f32(BRICK_FINE_RESOLUTION)));
      let upperBrick=vec3i(ceil(sweptUpper/f32(BRICK_FINE_RESOLUTION)))-vec3i(1);
      let lowerOffset=clamp(lowerBrick-sourceBrick,vec3i(-1),vec3i(1));
      let upperOffset=clamp(upperBrick-sourceBrick,vec3i(-1),vec3i(1));
      for(var dz=lowerOffset.z;dz<=upperOffset.z;dz+=1){
        for(var dy=lowerOffset.y;dy<=upperOffset.y;dy+=1){
          for(var dx=lowerOffset.x;dx<=upperOffset.x;dx+=1){
            if(dx!=0||dy!=0||dz!=0){
              let bit=u32(dx+1)+3u*u32(dy+1)+9u*u32(dz+1);
              sweptSupportMask|=1u<<bit;
            }
      }}}
      // Keep the configured longer lookahead as a resolution hint. It does
      // not enlarge membership beyond the physical step above.
      let lookaheadBrick=vec3i(floor((center
        +p.activityTiming.x*p.frame.x*ownVelocity)/f32(BRICK_FINE_RESOLUTION)));
      let lookaheadOffset=clamp(lookaheadBrick-sourceBrick,vec3i(-1),vec3i(1));
      if(any(lookaheadOffset!=vec3i(0))){
        supportMask|=cm12SweptPageSupportMask(lookaheadOffset);
      }
    }
  ...
  let reducedSurfaceAxes=cm12UnpackActivityFlags(reducedPackedMasks);
  ...
  // Cached whole-brick enclosure receipt (bit 15), read back by
  // brickPhiDeepLiquid. A brick with no measured cells proves nothing.
  if(measuredCount>0u&&(reducedSurfaceAxes&256u)==0u){reasons|=32768u;}
  ...
  // Geometric frontier demand. Zero for any brick that is not occupied, and
  // zero again as soon as an occupied brick stops holding an interface cell
  // against a shared face, so cm12BrickFrontierDemandMask releases an apron the
  // moment its supporting surface leaves.
  atomicStore(&activity[output+32u],select(0u,reducedSupportMask,occupied));
  atomicStore(&activity[output+3u],reducedSweptSupportMask);
...
// The one 3^3 neighbour-demand predicate shared by every frontier consumer:
// SparseWorld allocation, swept activation and empty-brick retirement all read
// this exact word so a claim and its retirement can never disagree.
//
// It is the union of two independently published masks on the immutable
// activity snapshot:
//   activity[record+3]  swept transport demand, from the represented face
//                       velocity of every nonzero donor cell (word 3).
//   activity[record+32] geometric interface support, the 26-neighbour stencil
//                       of every interface/thin-fluid cell touching a brick
//                       face, gated on the occupied classifier (word 32).
// Velocity alone cannot hold a dry presentation-support page: a resting pool
// sweeps nowhere, so a swept-only predicate could keep an apron only by
// re-claiming it every frame, and the symmetric retirement handed it straight
// back. Word 32 is published as zero for any brick that is not occupied and
// for any occupied brick that no longer holds an interface cell against the
// shared face, so an apron whose supporting surface leaves is released on the
// next census.
//
// Note what this mask CANNOT do on its own: the interface stencil is measured
// against RESIDENT opposing cells, so a surface page whose apron is already
// gone reports no interface at all and can never ask for it back. Bootstrapping
// and holding that apron is brickTouchesAcceptedLiquid, the boundary-liquid-face
// receipt classifyAcceptedLiquidFrontier publishes into word 38 from a sparse-air
// row - it needs no resident neighbour. Both activation and retirement test it.
fn cm12BrickFrontierDemandMask(brick:u32)->u32{
  let record=activityRecord(brick);
  return (atomicLoad(&activity[record+3u])
    |atomicLoad(&activity[record+32u]))&0x07ffffffu;
}
...
fn brickHasTransportDemand(brick:u32)->bool{
  return cm12WorldLeafAllocated(brick)
    &&(injectionReachesBrick(brick)
      ||atomicLoad(&activity[activityRecord(brick)+3u])!=0u);
}
...
fn closeRefinementPolicyTileResolution(@builtin(workgroup_id)wid:vec3u,
 @builtin(local_invocation_index)lane:u32){
  ...
  for(var linear=lane;linear<count;linear+=64u){
    ...
    if(sibling!=INVALID&&brickActive(sibling)
      &&cachedRefinementPolicyTileScale(sibling)==scale){
      if(!brickResolutionFrozen(sibling)){
...
fn brickTouchesDemandedMissingWorldPage(brick:u32)->bool{
  ...
  let support=atomicLoad(&activity[output+3u])&0x07ffffffu;
...
fn planBrickResolution(@builtin(global_invocation_id)gid:vec3u){
  ...
  // physical transport demand admits a graded candidate; this removes
  ...
  if(coarseFirstEnabled()&&!validForcedSurfaceRung){
    ...
    else if(coarseRequired<current&&(reasons&32768u)!=0u&&!policySurface){
      // The accepted-cell submerged-ball receipt certifies bulk now. Waiting a
      // surface-settle epoch here makes a rapidly translating core traverse
      // the page before it can recover its coarse rung. Transfer and 2:1
      // closure still certify the requested bulk level in this transaction.
      requested=coarseRequired;planReasons=2048u;proofEpochs=0u;
    }
...
fn closePlannedResolution(@builtin(global_invocation_id)gid:vec3u){
  let brick=gid.x;if(brick>=p.dispatch.w){return;}
  if(brickResolutionFrozen(brick)){return;}
  ...
  for(var side=0u;side<6u*brickSpan(brick)*brickSpan(brick);side+=1u){let neighborCoordinate=candidateFaceNeighborCoordinate(brick,side);
    ...
    required=max(required,max(1u,select(max(neighborResolution,neighborAccepted),neighborResolution,coarseFirstEnabled())
...
fn validateCandidateResolution(@builtin(global_invocation_id)gid:vec3u){
  ...
  // Prepared dynamic pages use the same candidate and lifecycle transaction
  // as host templates, with separate stable storage for every rung.
  // Retirement has no candidate field to store. Fixed-resolution authored
  // leaves can therefore retire their existing geometry without a rerung slot,
  // just like prepared dynamic pages. They must enter the shared transaction.
  let lifecycleRetirement=brickActive(brick)&&!candidateBrickActive(brick)
    &&candidate==accepted&&(brick<CM12_WDR_INITIAL_LEAVES
      ||atomicLoad(&activity[output+37u])!=INVALID);
...
// Coarse-first publishes the complete graded plan atomically, including bulk
// demotions. Legacy budgeted planning retains its accepted-neighbour floor.
...
// A backed cell rung does not by itself prove that every simultaneously
// scheduled neighbour combination has a mortar in the immutable catalogue.
// Validate physical faces against the exact scheduled image before preparing
// candidate fields. A refusal halts execution and leaves the old image live.
// This planner is also reachable from the presentation-only shader family,
// where resident-volume's private GV_SUPPORT constant is intentionally pruned.
const CM12_GEOMETRIC_SUPPORT_BASE:u32=${geometricVolumeLayout?.supportControlBaseWords ?? 0}u;
const GEOMETRIC_TOPOLOGY_BACKING_MISSING:u32=CM12_GEOMETRIC_SUPPORT_BASE+34u;
var<workgroup> geometricBrickBackingMissing:atomic<u32>;
var<workgroup> geometricBrickCertificationEnabled:u32;
fn geometricScheduledCellHasMaterial(cell:u32,brick:u32)->bool{
  if(!brickActive(brick)){return false;}
  let resolution=acceptedBrickResolution(brick);
  let range=templateBrickCellRange(brick,resolution);
  let origin=vec3f(cm12WorldLeafCoordinate(brick))*f32(BRICK_FINE_RESOLUTION);
  let width=f32(BRICK_FINE_RESOLUTION*brickSpan(brick))/f32(resolution);
  let lower=cellCenter(cell)-0.5*cellWidths(cell);
  let upper=cellCenter(cell)+0.5*cellWidths(cell);
  let first=vec3u(clamp(floor((lower-origin)/width),vec3f(0.0),vec3f(f32(resolution))));
  let end=vec3u(clamp(ceil((upper-origin)/width),vec3f(0.0),vec3f(f32(resolution))));
  for(var z=first.z;z<end.z;z+=1u){for(var y=first.y;y<end.y;y+=1u){for(var x=first.x;x<end.x;x+=1u){
    let local=x+resolution*(y+resolution*z);
    if(local<range.y&&state[destinationDensity()+range.x+local]!=0.0){return true;}
  }}}
  return false;
}
${SPARSE_CM12_DYNAMIC_SEAM_BINDING_WGSL}

fn geometricScheduledCellMissingFace(cell:u32)->u32{
  var negative=vec3f(0.0);var positive=vec3f(0.0);
  let ownLower=cellCenter(cell)-0.5*cellWidths(cell);
  let ownUpper=cellCenter(cell)+0.5*cellWidths(cell);
  for(var incidence=incidenceBegin(cell);incidence<incidenceEnd(cell);incidence+=1u){
    let dynamicFace=compiledDynamicBoundaryFace(cell,incidence);
    if(dynamicFace!=0u){
      if(dynamicFace==1u){let side=(incidence-ta(5u))%6u;
        let area=cellVolume(cell)/cellWidths(cell)[side/2u];
        if((side&1u)!=0u){positive[side/2u]+=area;}else{negative[side/2u]+=area;}}
      continue;
    }
    let seam=compiledHostDynamicSeam(cell,incidence);
    if(seam.x!=INVALID){
      let axis=rowAxis(seam.x);let own=termCoefficient(seam.w);
      let area=cellVolume(cell)/cellWidths(cell)[axis];
      if(own<0.0){positive[axis]+=area;}else{negative[axis]+=area;}
      continue;
    }
    var row=incidenceRow(incidence);var term=incidenceTerm(incidence);
    if(cell<ta(2u)&&row>=ta(3u)){
      // Retirement selects the immutable exterior slot again. Canonical
      // incidence still points at the accepted dynamic seam until publication.
      // Use the compiled exterior only if the scheduled image selects it;
      // an active but unprepared neighbour must still fail certification.
      let exterior=atomicLoad(&topologyArena[IMMUTABLE_HOST_INCIDENCE_BASE+2u*incidence]);
      if(rowKind(exterior)==3u&&shadowRowScheduled(exterior)){
        row=exterior;term=atomicLoad(&topologyArena[IMMUTABLE_HOST_INCIDENCE_BASE+2u*incidence+1u]);
      }
    }
    if(!shadowRowScheduled(row)){continue;}
    let own=termCoefficient(term);let axis=rowAxis(row);
    let terms=rowTermRange(row);var area=0.0;
    // Match the production subface compiler's structural measure. Aperture
    // belongs to flux, so a fully closed tank face still closes this cell.
    if(terms.y-terms.x==1u){area=abs(own)*rowStaticDualWeight(row);}else{
      for(var term=terms.x;term<terms.y;term+=1u){
        if(own*termCoefficient(term)>=0.0){continue;}
        let other=termCell(term);
        let otherLower=cellCenter(other)-0.5*cellWidths(other);
        let otherUpper=cellCenter(other)+0.5*cellWidths(other);
        if(select(otherUpper[axis]!=ownLower[axis],ownUpper[axis]!=otherLower[axis],own<0.0)){continue;}
        let overlap=max(vec3f(0.0),min(ownUpper,otherUpper)-max(ownLower,otherLower));
        area+=overlap[(axis+1u)%3u]*overlap[(axis+2u)%3u];
      }
    }
    if(own<0.0){positive[axis]+=area;}else if(own>0.0){negative[axis]+=area;}
  }
  let expected=vec3f(cellVolume(cell))/cellWidths(cell);
  for(var axis=0u;axis<3u;axis+=1u){
    if(abs(negative[axis]-expected[axis])>9.5367431640625e-7*expected[axis]
      ||abs(positive[axis]-expected[axis])>9.5367431640625e-7*expected[axis]){return axis+1u;}
  }
  return 0u;
}
...
fn scheduleTopologyPreparation(@builtin(local_invocation_id)lid:vec3u){
  ...
  if(lane==0u){atomicStore(&conditioning[GEOMETRIC_TOPOLOGY_BACKING_MISSING],0);}
  ...
  for(var brick=lane;brick<count;brick+=64u){
    ...
    if(candidate>accepted||lifecycle||hardRegionSupport||coarseFirstEnabled()){
...
// The scheduler's candidate image is frozen between dispatches. Each brick
// uses a full workgroup to certify its cells. Missing compiled connectivity is
// terminal: never turn a structural failure into a resident-generation request.
@compute @workgroup_size(64)
fn certifyGeometricTopologyFaces(@builtin(workgroup_id)wid:vec3u,
  @builtin(local_invocation_id)lid:vec3u){
  let brick=wid.x;let lane=lid.x;
  if(lane==0u){
    geometricBrickCertificationEnabled=0u;
    if(brick<p.dispatch.w&&atomicLoad(&activity[16])!=0u){
      geometricBrickCertificationEnabled=select(0u,1u,scheduledBrickActive(brick)
        &&(brickActive(brick)||brick>=CM12_WDR_INITIAL_LEAVES));
    }
    atomicStore(&geometricBrickBackingMissing,0u);
  }
  if(workgroupUniformLoad(&geometricBrickCertificationEnabled)==0u){return;}
  // A new dry page has no material cells yet, but its seams must be certified
  ...
  for(var local=lane;local<range.y;local+=64u){
    ...
    if(!geometricScheduledCellHasMaterial(cell,brick)){continue;}
    let missingAxis=geometricScheduledCellMissingFace(cell);
    if(missingAxis!=0u){
      cm12RecordFailure(8u,cell,vec4u(brick,missingAxis-1u,
        acceptedBrickResolution(brick),scheduledBrickResolution(brick)));
      atomicStore(&geometricBrickBackingMissing,1u);
    }
  }
  workgroupBarrier();
  if(lane!=0u||atomicLoad(&geometricBrickBackingMissing)==0u){return;}
  atomicStore(&conditioning[GEOMETRIC_TOPOLOGY_BACKING_MISSING],1);
}

// All certification workgroups finish before this dispatch can refuse the
// candidate globally. The accepted topology remains the published image.
@compute @workgroup_size(64)
fn sealGeometricTopologyFaces(@builtin(global_invocation_id)gid:vec3u){
  if(atomicLoad(&conditioning[GEOMETRIC_TOPOLOGY_BACKING_MISSING])==0){return;}
  if(gid.x<p.dispatch.w){setTopologyPreparationScheduled(activityRecord(gid.x),false);}
  if(gid.x==0u){
    atomicStore(&activity[16],0u);
    atomicStore(&activity[18],atomicLoad(&activity[14])+atomicLoad(&activity[15]));
  }
}
...
fn allocateSparseWorldInteractionPages(@builtin(global_invocation_id)gid:vec3u){
  ...
  // The interaction grid assigns exactly one invocation to each coordinate.
  let leaf=cm12WorldAllocateUniqueExact(targetCoordinate,0u);
...
// Elect from the immutable activity snapshot, before any directory payload is
// published. Only one source may insert a target key, even when face, edge and
// corner sweeps request it together. This lets distinct keys safely traverse
// each other's hash reservations without postponing required transport support.
fn sparseWorldFrontierAllocationOwner(brick:u32,targetCoordinate:vec3i)->bool{
  for(var bit=0u;bit<27u;bit+=1u){
    if(bit==13u){continue;}
    let offset=vec3i(i32(bit%3u)-1,i32((bit/3u)%3u)-1,i32(bit/9u)-1);
    let sourceCoordinate=targetCoordinate-offset;
    let source=cm12WorldOwnerAt(sourceCoordinate);
    if(source==INVALID||source>=brick||!brickActive(source)){continue;}
    // A macro leaf can cover several queried coordinates, but its allocation
    // invocations are anchored at its actual origin, just like this caller.
    if(any(cm12WorldLeafCoordinate(source)!=sourceCoordinate)){continue;}
    let support=cm12BrickFrontierDemandMask(source);
    if((support&(1u<<bit))==0u){continue;}
    if(cm12FluidNeighborReachable(sourceCoordinate,offset)){return false;}
  }
  return true;
}
...
fn allocateSparseWorldFrontier(@builtin(global_invocation_id)gid:vec3u){
  ...
  if(brick==INVALID||!brickActive(brick)){return;}
  // Allocation binds an inactive identity to prepared multi-rung backing.
  // Admission selects its coarsest graded rung before publication.
  let output=activityRecord(brick);
  // The directional mask also carries every nonzero dilute mass donor.
  // The coarser occupied/surface classifier is not transport residency proof.
  // Grow from immediate occupied support, characteristic-swept support or the
  // geometric interface stencil - cm12BrickFrontierDemandMask is the single
  // predicate activation and retirement also read. Activity publishes the
  // complete 3^3 receiver stencil, including diagonal characteristic travel.
  // The old fixed leaf catalog could activate every one of those receivers;
  // allocate the same 26 possible neighbours in SparseWorld rather than
  // silently dropping edge/corner demand. Every allocation below, geometric or
  // swept, passes the identical page-pool budget and unique-exact world claim,
  // so a full pool simply stops growing the apron instead of faulting.
  let demandMask=cm12BrickFrontierDemandMask(brick);
  let supportBit=select(localNeighbor,localNeighbor+1u,localNeighbor>=13u);
  if((demandMask&(1u<<supportBit))==0u){return;}
  ...
  if(!sparseWorldFrontierAllocationOwner(brick,targetCoordinate)){return;}
  revokeCM12SourceTopologyLease();
  let leaf=cm12WorldAllocateUniqueExact(targetCoordinate,0u);
...
// Connectivity and rung storage were prepared at construction. Allocation
// binds a fresh directory identity and clears its physical fields only.
@compute @workgroup_size(64)
fn synthesizeSparseWorldFrontierPages(@builtin(local_invocation_index)lane:u32,
 @builtin(workgroup_id)wid:vec3u){
  ...
  for(var local=lane;local<select(0u,CM12_DYNAMIC_CELLS,synthesize);local+=64u){
    ...
    for(var bank=0u;bank<2u;bank+=1u){let at=select(p.stateOffsets1.x,p.stateOffsets1.y,bank==1u)+4u*cell;
      state[at]=0.0;state[at+1u]=0.0;state[at+2u]=0.0;state[at+3u]=0.0;}
    ...
    if(hasSolidBoundaries()){state[p.solidOffsets.x+cell]=1.0;}
    if(hasStaticSolidVoxels()){refreshSparseCM12SolidWorldCell(cell);}
  }
  for(var local=lane;local<select(0u,CM12_DYNAMIC_ROWS,synthesize);local+=64u){
    let row=ta(3u)+page*CM12_DYNAMIC_ROWS+local;
    state[p.stateOffsets1.z+row]=0.0;state[p.stateOffsets1.w+row]=0.0;
    if(hasSolidBoundaries()){
      state[p.solidOffsets.y+3u*row]=1.0;state[p.solidOffsets.y+3u*row+1u]=0.0;
      state[p.solidOffsets.y+3u*row+2u]=1.0;}
    if(hasStaticSolidVoxels()){refreshSparseCM12SolidWorldRow(row);}
  }
  workgroupBarrier();
  if(synthesize&&lane==0u){
    atomicStore(&topologyArena[pageBase+3u],0x8000001fu);
    atomicStore(&topologyArena[pageBase+1u],resolution);
    // Publish the complete page as inactive coarse frontier capacity. The
...
${SPARSE_CM12_DYNAMIC_SEAM_PUBLICATION_WGSL}
...
fn sparseWorldDynamicRowOwner(row:u32)->u32{
  let rows=CM12_DYNAMIC_ROWS;
  return CM12_WDR_INITIAL_LEAVES+(row-ta(3u))/rows;
}
fn dynamicNegativeRowSuperseded(row:u32)->bool{
  if(row<ta(3u)){return false;}
  let within=(row-ta(3u))%CM12_DYNAMIC_ROWS;let resolution=cm12DynamicRowRung(within);
  let leaf=sparseWorldDynamicRowOwner(row);
  if(scheduledBrickResolution(leaf)!=resolution){return true;}
  let local=within-cm12DynamicRowOffset(resolution);
  let axis=local/((resolution+1u)*resolution*resolution);let face=local%(resolution+1u);
  if(face!=0u&&face!=resolution){return false;}
  let side=2u*axis+select(0u,1u,face==resolution);
  return cm12ScheduledDynamicRowBinding(row).x!=row;
}

fn hostExteriorRowSupersededAt(row:u32,candidateImage:bool)->bool{
  ...
  if(!select(brickActive(hostBrick),scheduledBrickActive(hostBrick),candidateImage)){return false;}
  ...
  if(neighbour==INVALID||neighbour==hostBrick
    ||!select(brickActive(neighbour),scheduledBrickActive(neighbour),candidateImage)){return false;}
  if(neighbour>=CM12_WDR_INITIAL_LEAVES){
    if(candidateImage){
      let side=2u*axis+select(0u,1u,coefficient<0.0);
      let binding=cm12PreparedDynamicFace(hostBrick,cellResolution(hostCell),side,rowCenter(row));
      return cm12PreparedDynamicTermForCell(binding,hostCell)!=INVALID;
    }
    for(var incidence=incidenceBegin(hostCell);incidence<incidenceEnd(hostCell);incidence+=1u){
      if(ta(IMMUTABLE_HOST_INCIDENCE_BASE+2u*incidence)==row
        &&incidenceRow(incidence)!=row){return true;}}
    return false;
  }
  // Generation-zero sparse-air rows remain in the catalogue. An authored
  // neighbour supersedes them only when its physical footprint covers the
  // complete owner face AND a selected two-sided catalogue interface supplies
  // that face. This conservative full-face proof cannot erase an uncovered
  // part of a macro leaf or a partially covered exterior patch.
  let tangent0=(axis+1u)%3u;let tangent1=(axis+2u)%3u;
  let lower=cellCenter(hostCell)-0.5*cellWidths(hostCell);
  let upper=cellCenter(hostCell)+0.5*cellWidths(hostCell);
  let neighborLower=vec3f(cm12WorldLeafCoordinate(neighbour)*i32(BRICK_FINE_RESOLUTION));
  let neighborUpper=neighborLower+vec3f(f32(brickSpan(neighbour)*BRICK_FINE_RESOLUTION));
  if(neighborLower[tangent0]>lower[tangent0]||neighborUpper[tangent0]<upper[tangent0]
    ||neighborLower[tangent1]>lower[tangent1]||neighborUpper[tangent1]<upper[tangent1]){return false;}
  var replacementArea=0.0;
  for(var incidence=incidenceBegin(hostCell);incidence<incidenceEnd(hostCell);incidence+=1u){
    let otherRow=incidenceRow(incidence);
    if(otherRow==row||otherRow>=ta(3u)||rowAxis(otherRow)!=axis
      ||rowCenter(otherRow)[axis]!=rowCenter(row)[axis]){continue;}
    let otherTerms=rowTermRange(otherRow);if(otherTerms.y-otherTerms.x<2u){continue;}
    let ownCoefficient=termCoefficient(incidenceTerm(incidence));
    if(ownCoefficient*coefficient<=0.0){continue;}
    let requirements=rowRequirementOffset(otherRow);let count=ta(requirements);var selected=true;
    for(var at=0u;at<count;at+=1u){let metadata=ta(requirements+1u+at);
      let brick=metadata>>TEMPLATE_CELL_RESOLUTION_BITS;
      let resolution=metadata&TEMPLATE_CELL_RESOLUTION_MASK;
      selected=selected&&select(brickActive(brick),scheduledBrickActive(brick),candidateImage)
        &&select(acceptedBrickResolution(brick),scheduledBrickResolution(brick),candidateImage)==resolution;
    }
    if(selected){replacementArea+=abs(ownCoefficient)*rowStaticDualWeight(otherRow);}
  }
  let fullArea=cellWidths(hostCell)[tangent0]*cellWidths(hostCell)[tangent1];
  return abs(replacementArea-fullArea)<=9.5367431640625e-7*fullArea;
}
fn hostExteriorRowSuperseded(row:u32)->bool{return hostExteriorRowSupersededAt(row,true);}
...
// Compensated expansion of a parent amount and its stored child amounts.
// The low component is carried into allocation instead of being discarded.
fn transferAccumulateAmount(total:vec2f,value:f32)->vec2f{
  let high=total.x+value;let virtualValue=high-total.x;
  let error=(total.x-(high-virtualValue))+(value-virtualValue);
  let low=total.y+error;let normalized=high+low;
  let virtualLow=normalized-high;
  return vec2f(normalized,(high-(normalized-virtualLow))+(low-virtualLow));
}
fn transferCandidateCellsWork(lid:vec3u,brick:u32,validBrick:bool){
  ...
  if(lane==0u){
    atomicStore(&activity[output+23u],0u);
  ...
  // One invocation owns all child scalar slots of one parent. PLIC guesses
  // and deterministic residual allocation therefore share a single extensive
  // budget, rather than independently rounding unrelated child fractions.
  workgroupBarrier();
  if(acceptedActive&&candidateActive&&candidate>accepted){
    let factor=candidate/accepted;
    for(var parentLocal=lane;parentLocal<sourceCount;parentLocal+=64u){
      let parent=first+parentLocal;let parentBase=factor*transferLocalCoordinate(parentLocal,sourceDimensions);
      let parentVolume=state[destinationDensity()+parent]*cellVolume(parent);
      var capacitySum=vec2f(0.0);
      for(var child=0u;child<factor*factor*factor;child+=1u){
        ...
        capacitySum=transferAccumulateAmount(capacitySum,cellOpenVolume(cell));
      }
      ...
      for(var child=0u;child<factor*factor*factor;child+=1u){
        ...
        if(any(q>=candidateDimensions)){continue;}
        let local=transferLocalIndex(q,candidateDimensions);let cell=candidateRange.x+local;
        let volume=cellVolume(cell);let open=cellOpenFraction(cell);
        // Cut parents retain capacity-weighted conservative allocation over
      ...
      // Finite scalar storage can leave an unrepresentable final ULP after
      // bounded residual allocation. Audit it under the existing capacity-
      // scaled f32 budget; do not require bit-exact equality or keep adapting
      // forever toward an unrepresentable scalar. The transfer mass receipt
      // and full accepted-volume QA retain the resulting numerical error.
      let remainder=remaining.x+remaining.y;
      let roundoff=8.0*1.1920928955078125e-7*totalCapacity;
      if(!finiteTransferValue(remainder)||abs(remainder)>roundoff){
  ...
  for(var local=lane;local<sourceCount;local+=64u){let cell=first+local;
    ...
    // Retirement has no destination field. Require every source amount to be
    // zero independently: a signed sum can hide cancellation, and neither the
    // transfer tolerance nor an activity-density threshold permits deletion.
    // Ordinary nonzero leaves were retained by retirement planning; reaching
    // this guard means an inconsistent candidate transaction, not a policy veto.
    if(!candidateActive&&rho!=0.0){atomicStore(&activity[output+23u],2u);}
  ...
  for(var local=lane;local<candidateCount;local+=64u){
    ...
    if(!acceptedActive){gamma=1.0;
      ...
      // Restrict in short x rows, then y slices, then z. A serial sum of
      // all 512 fine children can lose enough low bits in gamma to reject
      // an otherwise conservative B8 -> B1 transfer. Keep the conservation
      // receipt tolerances; reduce the accumulation error instead.
      var totals=vec4f(0.0);var momentumSum=vec3f(0.0);
      var restrictedAmount=vec2f(0.0);
      for(var dz=0u;dz<factor;dz+=1u){
        var sliceTotals=vec4f(0.0);var sliceVelocity=vec3f(0.0);
        var sliceMomentum=vec3f(0.0);
        for(var dy=0u;dy<factor;dy+=1u){
          var rowTotals=vec4f(0.0);var rowVelocity=vec3f(0.0);
          var rowMomentum=vec3f(0.0);
          ...
          restrictedAmount=transferAccumulateAmount(restrictedAmount,sourceRho*volume);
          let velocityAt=destinationCellVelocity()+4u*cell;
          let sourceVelocity=vec3f(state[velocityAt],state[velocityAt+1u],
            state[velocityAt+2u]);
          rowTotals+=vec4f(volume,sourceRho*volume,
            state[destinationGamma()+cell]*volume,state[p.stateOffsets2.x+cell]*volume);
          rowVelocity+=sourceVelocity*volume;
          rowMomentum+=sourceRho*volume*sourceVelocity;
          }
          sliceTotals+=rowTotals;sliceVelocity+=rowVelocity;
          sliceMomentum+=rowMomentum;
        }
        totals+=sliceTotals;velocity+=sliceVelocity;momentumSum+=sliceMomentum;
      }
      if(totals.x>0.0){rho=(restrictedAmount.x+restrictedAmount.y)/totals.x;
        // Store the nearest available parent scalar and audit the compensated
        // extensive rounding error. Requiring an exactly representable sum
        // would reject ordinary mixed-cell coarsening indefinitely.
        let remainder=transferAccumulateAmount(restrictedAmount,
          -rho*candidateCellVolume(brick,candidate,local));
        let error=remainder.x+remainder.y;
        let roundoff=8.0*1.1920928955078125e-7*cellOpenVolume(candidateRange.x+local);
        if(!finiteTransferValue(error)||abs(error)>roundoff){
          atomicStore(&activity[output+23u],2u);
        }
        gamma=totals.z/totals.x;
        pressure=totals.w/totals.x;
        velocity=select(velocity/totals.x,momentumSum/totals.y,abs(totals.y)>1e-12);}
    }else{
      let factor=max(1u,candidate/accepted);
      let sourceLocal=transferLocalIndex(candidateCoordinate/factor,sourceDimensions);
      let cell=first+sourceLocal;let candidateCell=candidateRange.x+local;
  ...
  // Liquid volume is conserved through retirement as well as rerunging.
  // Empty retired cells may discard auxiliary gamma and velocity fields only.
  ...
  let valid=atomicLoad(&activity[output+23u])!=2u
    &&finiteTransferValue(transferMassBefore[0])
...
fn transferCandidateFaceSide(lid:vec3u,brick:u32,side:u32,validBrick:bool){
  ...
  if(workgroupUniformLoad(&candidateFaceActive)==0u){
    // An authored fixed-rung retirement may have no candidate slot. Its
    // inactive target has no face field; only the zero transfer receipt exists.
  ...
  for(var patchIndex=lane;patchIndex<candidate*candidate;patchIndex+=64u){
    ...
    for(var dv=0u;dv<sourceSpan;dv+=1u){for(var du=0u;du<sourceSpan;du+=1u){
      ...
      for(var at=0u;at<range.y;at+=1u){
        let row=candidatePhysicalFaceRow(range.x+at);if(!acceptedRowMember(row)){continue;}
        let center=rowCenter(row);
...
fn publishCandidateTopologyDeltaWork(lid:vec3u,brick:u32,validBrick:bool){
  ...
  // A promotion restarts the whole history. A demotion keeps the quiet and
  // proof epochs: the brick has already proven it is calm, and the next rung
  // must re-prove representability through its own fresh receipt words
  // (zeroed above), not wait another full settle ladder. Zeroing here made
  // B8->B1 cost 2 epochs per rung, so a calm pool never returned to coarse.
  let transitionHistory=atomicLoad(&activity[output+2u]);
  atomicStore(&activity[output+2u],
    select(0u,transitionHistory&0xffffff00u,candidate<accepted));
  atomicStore(&activity[output+11u],
...
// The shadow row cache follows candidate fields in transient storage. Build it
// before publication overwrites the old rung and its cell fields.
fn candidateFaceRemapIndex(row:u32)->u32{
  return topology[p.topologyOffsets2.w]
    *(6u*CANDIDATE_CELLS_PER_BRICK+6u*CANDIDATE_FACE_SAMPLES_PER_SIDE)+row;
}

// Lowest-order staggered reconstruction: normal velocity is linear between
// the two authoritative faces and constant tangentially within an old cell.
// Consequently splitting a cell preserves its integrated divergence, and
// averaging patches preserves normal flux. Collocated velocity is not an
// authority for this operation: averaging faces into cells and back filters
// even a perfectly representable staggered field on every topology update.
fn candidateAcceptedFaceSample(point:vec3f,axis:u32)->vec4f{
  let q=vec3i(floor(point));
  var cell=ownerCellAt(q);
  if(cell==INVALID){
    // A face on the positive edge of accepted coverage can use its negative
    // normal owner. Tangential coordinates must remain in the queried patch:
    // clamping them to the original tank returns an upper edge behind the
    // integration cursor and turns one dry rectangle into millions of probes.
    var lower=q;lower[axis]-=1;cell=ownerCellAt(lower);
  }
  if(cell==INVALID){return vec4f(0.0,vec3f(q+vec3i(1)));}
  let center=cellCenter(cell);let widths=cellWidths(cell);
  var velocity=vec2f(0.0);var weight=vec2f(0.0);
  for(var at=incidenceBegin(cell);at<incidenceEnd(cell);at+=1u){
    let row=incidenceRow(at);
    if(!acceptedRowMember(row)||rowAxis(row)!=axis){continue;}
    let side=select(0u,1u,rowCenter(row)[axis]>center[axis]);
    let w=abs(termCoefficient(incidenceTerm(at)))*rowStaticDualWeight(row);
    velocity[side]+=w*state[destinationFaceVelocity()+row];weight[side]+=w;
  }
  for(var side=0u;side<2u;side+=1u){
    velocity[side]=select(0.0,velocity[side]/max(weight[side],1e-20),weight[side]>0.0);
  }
  let fraction=clamp((point[axis]-center[axis])/widths[axis]+0.5,0.0,1.0);
  return vec4f(mix(velocity.x,velocity.y,fraction),center+0.5*widths);
}

fn candidateRemappedFaceVelocity(row:u32)->f32{
  let axis=rowAxis(row);let tangent0=select(0u,1u,axis==0u);
  let tangent1=select(2u,1u,axis==2u);var center=rowCenter(row);
  var widths=vec3f(0.0);var unchanged=true;
  ...
  if(binding.x!=INVALID){
    ...
    let range=rowTermRange(row);
    for(var term=range.x;term<range.y;term+=1u){widths=max(widths,cellWidths(termCell(term)));}
  }
  if(acceptedRowMember(row)&&unchanged){return state[destinationFaceVelocity()+row];}
  let lower=center-0.5*widths;let upper=center+0.5*widths;
  var integral=0.0;var area=0.0;var v=lower[tangent1];
  // Integrate whole accepted-cell rectangles, rather than finest voxels, so
  // a macro face costs only the donor patches it actually overlaps.
  loop{
    if(v>=upper[tangent1]-1e-5){break;}
    var u=lower[tangent0];var nextV=upper[tangent1];var strip=0.0;var stripWidth=0.0;
    loop{
      if(u>=upper[tangent0]-1e-5){break;}
      var point=center;point[tangent0]=u+1e-4;point[tangent1]=v+1e-4;
      let sample=candidateAcceptedFaceSample(point,axis);
      let nextU=min(upper[tangent0],max(u+1e-4,sample[1u+tangent0]));
      nextV=min(nextV,max(v+1e-4,sample[1u+tangent1]));
      strip+=(nextU-u)*sample.x;stripWidth+=nextU-u;u=nextU;
    }
    integral+=strip*(nextV-v);area+=stripWidth*(nextV-v);v=nextV;
  }
  return select(0.0,integral/max(area,1e-20),area>0.0);
}

// Independent preflip receipt and cache: conservatively remap the still
// authoritative accepted faces, but author no stable face until commit.
@compute @workgroup_size(64)
fn validateCandidateShadowFaces(@builtin(global_invocation_id)gid:vec3u){
  ...
  candidateState[candidateFaceRemapIndex(row)]=velocity;
...
fn publishCandidateShadowFaces(@builtin(global_invocation_id)gid:vec3u){
  ...
  let velocity=candidateState[candidateFaceRemapIndex(row)];
...
fn validateAndAuthorizeShadowTopology(){
  ...
  if(!valid){atomicStore(&activity[21],1u);
    atomicStore(&topologyArena[base+3u],3u);
    for(var index=0u;index<deltaCount;index+=1u){let leaf=topologyDeltaLeafInvocation(index);
      if(leaf>=CM12_WDR_INITIAL_LEAVES&&leaf!=INVALID){
        cm12RecordFailure(8u,leaf,vec4u(atomicLoad(&topologyArena[base+1u]),
          select(0u,1u,ptrReady),select(0u,1u,effectsReady),atomicLoad(&topologyArena[tfxReserved+11u])));break;}}
    return;
  }
...
// Dynamic pages now use the same certified transfer and publication as host
// leaves. This marker only maintains page recycling's lifecycle receipt.
@compute @workgroup_size(64)
fn publishSparseWorldFrontierAcceptance(@builtin(workgroup_id)wid:vec3u,
 @builtin(local_invocation_index)lane:u32){
  if(lane!=0u||wid.x>=ta(topologyWorklistBase()+27u)
    ||ta(topologyWorklistBase()+3u)!=2u){return;}
  let leaf=CM12_WDR_INITIAL_LEAVES+wid.x;
  if(cm12PreparedPageReady(leaf)){
    atomicStore(&topologyArena[candidateTopologyPageBase(wid.x)+3u],0x8000003fu);
  }
}
...
// One staging body for every request producer. Intent only: accepted
// membership, stable fields, cache ownership, receipts, counters and journals
// remain untouched until the shared flip.
...
// Coarsest rung a geometry-demanded page can adopt without violating the 2:1
// face invariant that closePlannedResolution enforces one dispatch later. This
// mirrors that closure's own face rule, so the page enters transport already
// graded. Staging static air at BRICK_FINE_RESOLUTION instead made the
// refine-only closure raise every liquid neighbour to half the page's rung: a
// calm B1 pool acquired B4 on the step its dry presentation-support apron was
// claimed. Air carries no scalar feature to resolve, so face grading is the
// whole of its rung.
fn cm12DemandedFrontierGradingRung(brick:u32)->u32{
  var rung=1u;
  for(var side=0u;side<6u*brickSpan(brick)*brickSpan(brick);side+=1u){
    let neighbor=cm12WorldOwnerAt(candidateFaceNeighborCoordinate(brick,side));
    if(neighbor==INVALID||neighbor==brick||!brickActive(neighbor)){continue;}
    let neighborOutput=activityRecord(neighbor);
    let neighborResolution=max(atomicLoad(&activity[neighborOutput+8u]),
      atomicLoad(&activity[neighborOutput+12u]));
    rung=max(rung,max(1u,neighborResolution*brickSpan(brick)
      /(2u*brickSpan(neighbor))));
  }
  return clamp(rung,1u,BRICK_FINE_RESOLUTION);
}

// A physical signal still claims the fine frontier floor on the staging frame
// itself: this page's own injection or swept/projected transport demand, or a
// face neighbour whose thin classification or measured travel proves material
// is about to cross the seam. Presentation support for a calm free surface is
// none of those. brickHasTransportDemand already covers injection, and the
// short-circuit keeps the mask read alone on the ordinary path.
fn cm12DemandedFrontierNeedsFineRung(brick:u32)->bool{
  if(brickHasTransportDemand(brick)){return true;}
  for(var side=0u;side<6u*brickSpan(brick)*brickSpan(brick);side+=1u){
    let neighbor=cm12WorldOwnerAt(candidateFaceNeighborCoordinate(brick,side));
    if(neighbor==INVALID||neighbor==brick||!brickActive(neighbor)){continue;}
    let neighborOutput=activityRecord(neighbor);
    if((atomicLoad(&activity[neighborOutput+1u])&256u)!=0u){return true;}
    if(velocityResolutionFloor(activityF32(neighborOutput+33u))>1u){return true;}
  }
  return false;
}

// Geometric demand claims a page for support, not for material. Under
// coarse-first it therefore enters at the coarsest rung its own faces permit;
// a page claimed by a B1 pool is created at B1 and cannot grade that pool up.
// Both face walks sit behind this one branch: a dynamic page pool leaf, a
// distance-mode scene, and any page carrying real transport all skip them, and
// no lane reaches either walk unless it is staging this exact page.
fn stageGeometricDemandFrontierPage(brick:u32){
  stageDemandedFrontierPage(brick);
}

// Activate a page when its own accepted-liquid receipt asks for it
// (brickTouchesAcceptedLiquid: word 38, published by classifyAcceptedLiquidFrontier
// from a neighbour's boundary-liquid-face bit, so a calm free surface keeps its
// dry presentation-support page without any velocity) or when a resident
// neighbour demands it through cm12BrickFrontierDemandMask, the union of the
// directional free-surface sweep and the geometric interface-support stencil.
// retireUnsupportedEmptyBricks tests the identical pair, so a claim is never
// handed straight back. Compare-exchange makes publication single-writer.
@compute @workgroup_size(64)
fn activateSweptFrontierPages(@builtin(workgroup_id)wid:vec3u,
 @builtin(local_invocation_index)lane:u32){
  ...
  if(eligible){
    ...
    if(lane<26u){
      ...
      if(neighbor!=INVALID&&neighbor!=brick&&brickActive(neighbor)){
        ...
        if((cm12BrickFrontierDemandMask(neighbor)
  ...
  if(lane==0u&&atomicLoad(&frontierDemanded)!=0u){
    stageGeometricDemandFrontierPage(brick);
  }
}

fn geometricTransportMaterialDemand(brick:u32)->bool{
  if(brick==INVALID||brick>=p.dispatch.w||!cm12WorldLeafAllocated(brick)){return false;}
  if((atomicLoad(&activity[activityRecord(brick)+9u])
    &ACTIVITY_GEOMETRIC_SWEEP_RECEIVER)!=0u){return true;}
  if(p.inflowVelocity.w>0.5&&p.inflowOutlet.w>0.0){
    let endpoint=p.inflowOutlet.xyz+2.0*p.inflowVelocity.xyz*p.frame.x;
    let padding=vec3f(p.inflowOutlet.w+1.0);
    let lower=min(p.inflowOutlet.xyz,endpoint)-padding;
    let upper=max(p.inflowOutlet.xyz,endpoint)+padding;
    let brickLower=vec3f(cm12WorldLeafCoordinate(brick))*f32(BRICK_FINE_RESOLUTION);
    let brickUpper=brickLower+vec3f(f32(BRICK_FINE_RESOLUTION*brickSpan(brick)));
    if(all(brickUpper>=lower)&&all(brickLower<=upper)){return true;}
  }
  return false;
}

// A directional sweep already includes both its material donor and every page
// intersected by the receiver volume. Transverse dry pages are not physical
// transport receivers; sparse-air rows provide their boundary samples, so the
// former reserveGeometricTransportFaceSupport reservation pass had an empty
// body and has been removed along with its two per-frame dispatches.
...
// A catalogue interface disappears when either authored owner retires. The
// remaining cell needs an independently selectable exterior row: construction
// does not automatically catalogue sparse-air variants of internal faces.
fn retirementHasExteriorFallback(cell:u32,sharedRow:u32,ownCoefficient:f32,
 retiringBrick:u32)->bool{
  // Refinement closure runs after retirement planning. A current-rung proof
  // cannot authorize removing support from a neighbour that may still rerung.
  if(brickCandidatePlanningEnabled(cellBrick(cell))){return false;}
  let axis=rowAxis(sharedRow);let widths=cellWidths(cell);
  let fullArea=widths[(axis+1u)%3u]*widths[(axis+2u)%3u];
  for(var at=incidenceBegin(cell);at<incidenceEnd(cell);at+=1u){
    let row=incidenceRow(at);
    if(row>=ta(3u)||rowKind(row)!=3u||rowAxis(row)!=axis
      ||rowCenter(row)[axis]!=rowCenter(sharedRow)[axis]){continue;}
    let terms=rowTermRange(row);
    if(terms.y-terms.x!=1u||termCell(terms.x)!=cell){continue;}
    let coefficient=termCoefficient(terms.x);
    if(coefficient*ownCoefficient<=0.0){continue;}
    // One full-cell exterior rectangle is sufficient. Do not infer coverage
    // from a sum of possibly overlapping partial catalogue rectangles.
    let area=abs(coefficient)*rowStaticDualWeight(row);
    if(abs(area-fullArea)>9.5367431640625e-7*fullArea){continue;}
    let requirements=rowRequirementOffset(row);let count=ta(requirements);
    var available=true;
    for(var item=0u;item<count;item+=1u){let metadata=ta(requirements+1u+item);
      let brick=metadata>>TEMPLATE_CELL_RESOLUTION_BITS;
      let resolution=metadata&TEMPLATE_CELL_RESOLUTION_MASK;
      available=available&&brick!=retiringBrick&&brickActive(brick)
        &&acceptedBrickResolution(brick)==resolution;
    }
    if(available){return true;}
  }
  return false;
}
fn retirementRemovesAuthoredFaceSupport(cell:u32,brick:u32)->bool{
  for(var at=incidenceBegin(cell);at<incidenceEnd(cell);at+=1u){
    let row=incidenceRow(at);
    // Synthesized dynamic rows own explicit exterior-restoration branches.
    if(row>=ta(3u)||!acceptedRowMember(row)||!rowAccepted(row)){continue;}
    let terms=rowTermRange(row);
    for(var term=terms.x;term<terms.y;term+=1u){let other=termCell(term);
      if(cellBrick(other)==brick||!cellActive(other)){continue;}
      if(!retirementHasExteriorFallback(other,row,termCoefficient(term),brick)){
        return true;
      }
    }
  }
...
// Retire only exactly empty non-feature bricks that nothing demands. Demand is
// the same pair activateSweptFrontierPages claims with: this brick's own
// brickTouchesAcceptedLiquid receipt, and a resident neighbour's
// cm12BrickFrontierDemandMask. Claim and retirement are therefore exactly
// symmetric, and the dry page above a calm free surface neither churns nor
// disappears. Activity thresholds are scheduling hints, never permission to
// discard any signed liquid amount. Dilute tails may therefore keep sparse
// storage resident indefinitely.
@compute @workgroup_size(64)
fn retireUnsupportedEmptyBricks(@builtin(workgroup_id)wid:vec3u,
 @builtin(local_invocation_index)lane:u32){
  ...
  if(eligible){
    ...
    // underflow cannot make a nonzero amount look empty. Negative roundoff
    // and non-finite values also retain their leaf; no cancellation is allowed.
    let acceptedRange=templateBrickCellRange(brick,acceptedBrickResolution(brick));
    for(var local=lane;local<acceptedRange.y;local+=64u){
      let cell=acceptedRange.x+local;
      if(state[destinationDensity()+cell]!=0.0
        ||retirementRemovesAuthoredFaceSupport(cell,brick)){
        // Preserve accepted support normally, without opening a doomed
        // candidate transaction or changing any pressure/volume audit.
        atomicStore(&frontierDemanded,1u);
      }
    }
    if(lane==0u&&(policyTileMembershipRequired(brick)
  ...
  // Intent only. The exact-zero check above leaves rejected retirements
  // active without creating a transaction failure. Publication invalidates
  // caches/journals and updates counters after the shared selector flip.
  let resolution=acceptedBrickResolution(brick);
  atomicStore(&activity[output+8u],resolution);
...
const PRESENTATION_FRAME_PLAN_CAUSES:u32=${
        (SPARSE_CM12_DIRTY_CAUSE_BIT.topologyCreated |
          ...
          SPARSE_CM12_DIRTY_CAUSE_BIT.presentationConfiguration) >>>
        0
      }u;
...
// Scalar dirty stamps already cover the changed brick's complete immediate
// shell. Geometry adds a normal-estimation ring and an extension ring. Read
// two more complete physical brick shells for presentation only; no velocity
// stamp, transport mask, or pressure worklist is expanded. Whole shells also
// include corner interpolation donors, unlike face-only incidence traversal.
fn geometricPresentationDirtyShell(brick:u32)->bool{
  if(incrementalActivityBrickDirty(brick)){return true;}
  let origin=cm12WorldLeafCoordinate(brick);let span=i32(brickSpan(brick));
  for(var z=-1;z<=span;z+=1){for(var y=-1;y<=span;y+=1){
    for(var x=-1;x<=span;x+=1){
      if(x>=0&&x<span&&y>=0&&y<span&&z>=0&&z<span){continue;}
      let other=brickDirectoryLookupAtSignedCoordinate(origin+vec3i(x,y,z));
      if(other!=INVALID&&incrementalActivityBrickDirty(other)){return true;}
    }
  }}
  return false;
}
fn geometricPresentationDependencyDirty(brick:u32)->bool{
  if(incrementalActivityBrickDirty(brick)){return true;}
  // Keep this predicate barrier-free: the existing FPL producer has
  // storage-dependent early exits. A clean census has no dependency source.
  if(atomicLoad(&activity[6])==0u){return false;}
  let origin=cm12WorldLeafCoordinate(brick);let span=i32(brickSpan(brick));
  for(var z=-1;z<=span;z+=1){for(var y=-1;y<=span;y+=1){
    for(var x=-1;x<=span;x+=1){
      if(x>=0&&x<span&&y>=0&&y<span&&z>=0&&z<span){continue;}
      let other=brickDirectoryLookupAtSignedCoordinate(origin+vec3i(x,y,z));
      if(other!=INVALID&&geometricPresentationDirtyShell(other)){return true;}
    }
  }}
  return false;
}
...
fn populateSparseCM12PresentationFramePlan(
 @builtin(workgroup_id)wid:vec3u,@builtin(local_invocation_index)lane:u32){
  ...
  let dynamicBrick=geometricPresentationDependencyDirty(brick);
  ...
  let forcePresentation=p.correctionReserved.z>0.5;
  let scheduled=bootstrap||injected||pageNeedsActivation||scalarChanged
    ||topologyChanged||dynamicBrick||heightChanged||forcePresentation;
  if(!scheduled){return;}
  ...
  if(forcePresentation){origin|=${SPARSE_CM12_DIRTY_CAUSE_BIT.presentationConfiguration}u;}
...
fn cm12PresentationPreparePage(brick:u32,page:u32,lane:u32,
 generation:u32,causeMask:u32)->u32{
  ...
  presentationStageTopology(brick,lane);
  if(lane==0u){
    ...
    // are farther apart, but a coarse macro rung still fits this same slab.
    // Bypassing it multiplied each page's restriction work by up to 40*P^3.
    if(scale>1u){
    ...
    // The published width tag prices the field, not the solver rung: a banded
    // brick carries phi at the finest lattice under a coarse accepted cell.
    cm12PresentationScale=cm12PhiPresentationScale(brick,span,resolution);
    cm12PresentationCacheFirst=cacheFirst;
    ...
    // The resolved surface apron still needs exact magnitudes: replacing its
    // submerged endpoint with -4h moves a crossing at the next page boundary.
...
// visible result cannot drift into different reconstruction policies.
@compute @workgroup_size(64)
fn publishSparseCM12SurfaceRepresentabilityReceipts(
 @builtin(workgroup_id)wid:vec3u,@builtin(local_invocation_index)lane:u32){
  ...
  if(lane==0u){
    ...
    let flags=cm12FppLoad(${
      framePlanPresentationLayout.baseWords +
      SPARSE_CM12_FRAME_PLAN_PRESENTATION_HEADER.flags
    }u);
    let acceptedGeneration=cm12FppLoad(${
      framePlanPresentationLayout.baseWords +
      SPARSE_CM12_FRAME_PLAN_PRESENTATION_HEADER.acceptedGeneration
    }u);
    let accepted=(flags&${SPARSE_CM12_FRAME_PLAN_PRESENTATION_FLAG.executionComplete}u)!=0u
      ...
      &&cm12FppLoad(${
        framePlanPresentationLayout.baseWords +
        SPARSE_CM12_FRAME_PLAN_PRESENTATION_HEADER.generationReceipt
      }u)
        ==acceptedGeneration
      &&cm12FppLoad(${
        framePlanPresentationLayout.baseWords +
        SPARSE_CM12_FRAME_PLAN_PRESENTATION_HEADER.topologyGeneration
      }u)
        ==atomicLoad(&activity[12]);
...
fn publishSparseLevelSet(@builtin(workgroup_id)wid:vec3u,
 @builtin(local_invocation_index)lane:u32){
  ...
  let resolution=acceptedBrickResolution(brick);
  let scale=BRICK_FINE_RESOLUTION*span/resolution;
  // The stencil cache is still keyed by the accepted cell rung; only the
  // published width tag follows the phi lattice.
  let phiScale=cm12PhiPresentationScale(brick,span,resolution);
  var patchFirst=vec3i(0);var patchDimensions=vec3u(1u);
  var cacheFirst=vec3i(0);var cacheDimensions=vec3u(1u);var cacheCount=0u;
  if(scale>1u){
  ...
  let activityReasons=select(0u,atomicLoad(&activity[activityRecord(brick)+1u]),
    brick<p.dispatch.w);
  let uniformBulkReady=atomicLoad(&activity[0])!=0u&&p.injectionCenter.w<=0.5;
  let wantsStencil=scale>1u&&cacheFits
    &&(!uniformBulkReady||(activityReasons&(1u|256u|512u))!=0u
  ...
  for(var localIndex=lane;localIndex<PRESENTATION_SAMPLES_PER_PAGE;localIndex+=64u){
    ...
    let flags=1u|select(0u,16u,phi<0.0)
      |((31u-countLeadingZeros(max(1u,phiScale)))<<8u);
