...
function membership(packet: Packet, candidate: boolean): Uint32Array {
  ...
  if (!Number.isSafeInteger(generation) || generation < 0 || generation > 0xffff_ffff
    ...
    throw new Error("Sparse Geometric (CM12) generation packet header is invalid");
  }
  const selected = new Set<number>();
  for (const cell of cells) {
    if (cell >= packet.cellCount || selected.has(cell)) {
      throw new Error("Sparse Geometric (CM12) generation has invalid or duplicate cell membership");
    }
    selected.add(cell);
  }
  ...
  for (const row of rows) {
    if (row >= packet.rowCount || selectedRows.has(row)) {
      throw new Error("Sparse Geometric (CM12) generation has invalid or duplicate row membership");
    }
    selectedRows.add(row);
    const bits = packet.words[packet.words[7]! + row];
    if (bits === undefined) throw new Error("Sparse Geometric (CM12) generation row is missing");
    const first = bits & 0x007f_ffff, count = bits >>> 23;
    if (count === 0 || first + count > packet.words[4]!) {
      throw new Error("Sparse Geometric (CM12) generation row terms are invalid");
    }
    for (let term = first; term < first + count; term++) {
      if (!selected.has(packet.words[packet.words[8]! + 2 * term]!)) {
        throw new Error("Sparse Geometric (CM12) generation row references an unselected cell");
      }
    }
  }
...
export class SparseCM12TopologyGenerationStore {
  ...
  private constructor(private readonly device: GPUDevice, readonly maximumBytes: number) {
    if (!Number.isSafeInteger(maximumBytes) || maximumBytes < 0) {
      throw new RangeError("Sparse Geometric (CM12) generation storage budget must be a nonnegative safe integer");
    }
  }
  ...
  static async create(device: GPUDevice, packet: Packet, maximumBytes: number) {
    ...
    if (!image) throw new RangeError("Sparse Geometric (CM12) accepted generation exceeds storage budget");
  ...
  private assertLive() {
    if (this.disposed) throw new Error("Sparse Geometric (CM12) generation store is destroyed");
  }
  ...
  private async allocate(topologyWords: Uint32Array, memberWords: Uint32Array,
    ...
    if ([topologyWords, memberWords].some((words) =>
      ...
      throw new RangeError("Sparse Geometric (CM12) generation exceeds the device storage binding limit");
    }
    ...
    try {
      for (const [label, words] of [["topology", topologyWords],
        ...
        const buffer = this.device.createBuffer({
          label: `Sparse Geometric (CM12) generation ${generation} ${label}`,
    ...
    try {
      ...
      if (error) throw new Error(`Sparse Geometric (CM12) generation allocation failed: ${error.message}`);
      this.assertLive();
  ...
  async prepare(packet: Packet): Promise<SparseCM12TopologyGenerationReservation> {
    this.assertLive();
    if (this.preparing || this.pending) throw new Error("Sparse Geometric (CM12) already has a pending generation");
    ...
    if (packet.acceptedGeneration !== generation || packet.candidateGeneration !== generation + 1) {
      throw new Error("Sparse Geometric (CM12) candidate generation is stale or discontinuous");
    }
  ...
  private requirePending(candidate: SparseCM12StagedTopologyGeneration): Image {
    this.assertLive();
    if (candidate !== this.pending) throw new Error("Sparse Geometric (CM12) candidate is not owned by this transaction");
    return this.pending;
  }
