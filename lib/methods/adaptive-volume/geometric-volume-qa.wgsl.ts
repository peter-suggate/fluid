...
fn reduceAcceptedGeometricVolumeQA(@builtin(workgroup_id)wid:vec3u,
 ...
 if(brickActive(brick)){
  ...
  for(var local=lane;local<cells.y;local+=64u){
   ...
   if(c<0.0||v< -margin||v>c+margin){counts.w+=1.0;}
   if(c==0.0){counts.y+=1.0;if(v!=0.0){sums.z+=1.0;}}
   else if(c>0.0){extrema.x=min(extrema.x,v/c);extrema.y=max(extrema.y,v/c);}
   extrema.z=max(extrema.z,max(-v,v-c));
   extrema.w=max(extrema.w,max(-rho,rho-open));
 ...
 if(lane==0u){geometricVolumeQA[4u*brick]=gvqaSum[0];
  geometricVolumeQA[4u*brick+1u]=gvqaCounts[0];
  geometricVolumeQA[4u*brick+2u]=gvqaExtrema[0];
  geometricVolumeQA[4u*brick+3u]=gvqaOutside[0];}
}
`;
