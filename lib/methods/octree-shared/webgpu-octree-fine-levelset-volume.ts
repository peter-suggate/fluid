...
import { fineLevelSetLinearWorkgroupWGSL, planFineLevelSetDispatch2D } from "../../core/webgpu-fine-levelset-dispatch";
...
export class WebGPUFineLevelSetVolumeCorrection {
  ...
  constructor(private readonly device: GPUDevice, readonly source: WebGPUFineLevelSetBrickSource,
    ...
    // The Losasso path corrects only pages that actually transported the main
    // moving surface. Historical wall films and sleeping detached fragments do
    // not receive volume lost by semi-Lagrangian transport elsewhere.
    u[5] = correctionMode === "moving-pages" ? 1 : 0;
...
const INVALID:u32=0xffffffffu;const VALID:u32=1u;const PAGE_ACTIVITY_MOVING:u32=32u;const PUBLISHED:u32=0x80000000u;const ERROR_COARSE:u32=1u;const ERROR_FINE:u32=2u;const ERROR_OWNER:u32=4u;const OWNER_FOUND:u32=0u;const OWNER_ABSENT:u32=1u;const OWNER_MALFORMED:u32=3u;const OWNER_OUTSIDE:u32=4u;
...
@compute @workgroup_size(256)fn finalizeCorrectedFineVolume(@builtin(local_invocation_index)lid:u32){finalizeCorrectedMeasurement(true,lid);if(lid==0u){let saturated=control.flags==PUBLISHED&&((c.pad0!=0u&&control.correction!=0.)||abs(control.correction)>=.5*p.fineCellWidth);let samples=min(worklist[1],p.pageCapacity)*p.samplesPerBrick;let groups=(samples+63u)/64u;let x=select(0u,min(groups,c.p2),saturated);residualDispatch[0]=x;residualDispatch[1]=select(1u,(groups+x-1u)/x,x>0u);residualDispatch[2]=1u;residualDispatch[3]=select(0u,1u,saturated);residualDispatch[4]=1u;residualDispatch[5]=1u;}}
