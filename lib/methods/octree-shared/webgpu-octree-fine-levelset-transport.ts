...
} from "./octree-air-velocity-support-abi";
import type { PassBroker } from "../../core/webgpu-pass-broker";
import type { SurfaceInflowState } from "./surface-state";
...
/**
 * The immutable catalog a characteristic samples through, and the topology
 * metrics that say which rows are live.
 *
 * Only what transport binds or uploads is named, so a coarse-dynamics backend's
 * own topology publication satisfies it without the shared stage naming that
 * backend. The four tetrahedron/sampler buffers stay optional exactly as the
 * publisher declares them; the constructor rejects a source missing any of
 * them rather than binding a sentinel.
 */
export interface StructuredFineTransportCatalogSource {
...
/**
 * The accepted packed A/B row-velocity publication every characteristic reads.
 *
 * `plan.offsets` is uploaded whole with `Object.values`, so what transport
 * depends on is the arena's offset words in the order the publisher declares
 * them -- the same order the shader's `Params` struct lists from word 28. It is
 * deliberately not narrowed to a field list here: naming the fields would imply
 * this stage addresses them individually, and it never does.
 */
export interface StructuredFineTransportVelocitySource {
  readonly plan: Readonly<{
    rowCapacity: number;
    maximumCaseSlots: number;
    slotCapacity: number;
    authorityWords: number;
...
export interface StructuredFineTransportResources {
  readonly structured: StructuredFineTransportVelocitySource;
  readonly topology: StructuredFineTransportCatalogSource;
