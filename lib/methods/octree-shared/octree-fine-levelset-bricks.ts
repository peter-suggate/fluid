...
import { packFineLevelSetSample } from "../../core/fine-levelset-packed-sample";

import {
  FINE_LEVELSET_BYTES_PER_SAMPLE,
  ...
  type FineLevelSetBrickPlanOptions,
  type FineLevelSetVec3,
} from "../../core/fine-levelset-brick-abi";

/**
 * The publication ABI is re-exported rather than re-declared: consumers of the
 * fine band outnumber its producers, and a second copy of a stride or a flag
 * bit is exactly the divergence `lib/core/fine-levelset-brick-abi.ts` exists to
 * prevent. Importing either path gets the same constant.
 */
export {
  FINE_LEVELSET_BYTES_PER_SAMPLE,
  FINE_LEVELSET_CHANNELS,
  FINE_LEVELSET_INVALID,
  FINE_LEVELSET_METADATA_WORDS,
  FINE_LEVELSET_SAMPLE_FLAGS,
  FINE_LEVELSET_WORKSET_HEADER,
  FINE_LEVELSET_WORKSET_HEADER_WORDS,
  readFineLevelSetWorksetHeader,
} from "../../core/fine-levelset-brick-abi";
export type {
  FineLevelSetBrickPlan,
  FineLevelSetBrickPlanOptions,
  FineLevelSetBrickResolution,
  FineLevelSetFactor,
  FineLevelSetVec3,
  FineLevelSetWorksetHeaderValues,
} from "../../core/fine-levelset-brick-abi";
...
export class FineLevelSetBrickOracle {
    ...
    for (let z = 0; z < this.plan.brickResolution; z += 1) {
      for (let y = 0; y < this.plan.brickResolution; y += 1) {
        for (let x = 0; x < this.plan.brickResolution; x += 1) {
          ...
          if (!Number.isFinite(value)) throw new RangeError("Coarse phi authority returned a non-finite value");
  ...
  sampleOrCoarse(position: FineLevelSetVec3, coarsePhi: FineLevelSetCoarsePhi): number {
    ...
    const coarseAuthority = coarsePhi(position);
    if (!Number.isFinite(coarseAuthority)) throw new RangeError("Coarse phi authority returned a non-finite value");
    return coarseAuthority;
  }
  ...
  exportGPUGeneration(): FineLevelSetGPUGenerationData {
    ...
    const morton = (key: number): bigint => {
      ...
      for (let bit = 0; bit < 21; bit += 1) {
        code |= BigInt((x >>> bit) & 1) << BigInt(3 * bit);
        code |= BigInt((y >>> bit) & 1) << BigInt(3 * bit + 1);
        code |= BigInt((z >>> bit) & 1) << BigInt(3 * bit + 2);
      }
      return code;
    };
    const ordered = [...this.pagesByKey.values()].sort((a, b) => {
