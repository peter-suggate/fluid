 ...
 *
 * This is no longer a user-facing choice: `losasso` and `power-liquids` are two
 * registered simulation methods, and each one names its backend as a literal.
 * What survives here is the engine-side policy the shared octree stack switches
 * on, and the lane key `lib/methods/index.ts` registers the two lanes under.
 */
...
export function resolveOctreeCoarseDynamics(
  ...
  const policy = octreeCoarseBackendPolicy(selection.backend);
