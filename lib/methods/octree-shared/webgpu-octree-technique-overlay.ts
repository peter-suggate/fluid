...
import { CAMERA_TAN_HALF_FOV } from "../../core/webgpu-camera";
...
} from "../../core/octree-technique-debug";
import type { OctreeTechniqueDebugSource } from "../../core/levelset-consumer-abi";
import {
  visualizationBindGroupEntries,
  visualizationBindingPreambleWGSL,
  type VisualizationProgram,
} from "../../core/visualization-bindings";
import { octreeTechniqueSharedWGSL } from "../../core/webgpu-octree-technique-shared";
import { describeFineFloodLadder } from "../../core/fine-flood-provenance";
import { WebGPUFluidBlastRadius } from "../../core/webgpu-fluid-blast-radius";
...
fn volumeTopology(uv:vec2f,mode:i32)->vec4f {
  ...
  for(var sample=0u;sample<512u;sample+=1u){if(sample>=steps||accum.a>0.985){break;}let t=interval.x+(f32(sample)+0.5)*dt;let point=ray.origin+ray.direction*t;let pointFine=worldToFine(point);let cell=vec3i(clamp(floor(pointFine),vec3f(0.0),u.gridInfo.xyz-vec3f(1.0)));let row=textureLoad(ownerRows,cell,0).x;
    ...
    var ink=site;let count=min(header.count,${OCTREE_CATALOG_EXTENTS.maximumTetrahedra}u);let transform=metric.transformAndFlags&63u;let width=max(baseWidth,t*1.44/max(u.viewport.y,1.0));
...
@fragment fn fragmentMain(input:VertexOutput)->@location(0) vec4f {
  ...
  if(row==INVALID){discard;}let fault=topologyFault(row,pointFine);if(fault.a>0.0){return sliceDisplay(fault.rgb,0.94);}
  ...
  if(!valid){return sliceDisplay(vec3f(1.0,0.01,0.06),0.96);}let header=tetraHeaders[metric.topologyCode];let transition=(header.flags&1u)==0u;
  ...
  if(mode==12){let base=select(vec3f(0.08,0.52,0.42),vec3f(0.42,0.14,0.70),transition);let color=mix(base,vec3f(1.0,0.73,0.12),siteInk);return sliceDisplay(color,max(0.38,0.92*siteInk));}
  if(mode==15){let selected=transition||boundary;let color=select(vec3f(0.03,0.09,0.18),select(vec3f(0.94,0.45,0.06),vec3f(0.96,0.08,0.40),boundary),selected);return sliceDisplay(color,select(0.18,0.88,selected));}
  if(mode!=14||!transition||header.first>arrayLength(&tetrahedra)||header.count>arrayLength(&tetrahedra)-header.first){discard;}
  var ink=siteInk;let count=min(header.count,${OCTREE_CATALOG_EXTENTS.maximumTetrahedra}u);let transform=metric.transformAndFlags&63u;
  for(var local=0u;local<count;local+=1u){let packed=tetrahedra[header.first+local];let selectors=vec3u(packed&255u,(packed>>8u)&255u,(packed>>16u)&255u);
    if(any(selectors>=vec3u(arrayLength(&tetraVertices)))){return sliceDisplay(vec3f(1.0,0.01,0.06),0.96);}let a=centerFine+f32(leaf.size)*inversePowerTransform(tetraVertices[selectors.x].value.xyz,transform);let b=centerFine+f32(leaf.size)*inversePowerTransform(tetraVertices[selectors.y].value.xyz,transform);let c=centerFine+f32(leaf.size)*inversePowerTransform(tetraVertices[selectors.z].value.xyz,transform);
  ...
  if(ink<0.02){discard;}return sliceDisplay(mix(vec3f(0.12,0.72,0.86),vec3f(1.0,0.72,0.12),siteInk),0.94*ink);
}`;
...
@fragment fn fragmentMain(input:VertexOutput)->@location(0) vec4f{let operatorView=i32(round(u.debug.w))==16;if(i32(round(u.debug.x))==4){return faceVolume(input.uv,operatorView);}let hit=sliceRay(input.uv);if(hit.w<=0.0){discard;}let minimum=vec3f(-0.5*u.container.x,0.0,-0.5*u.container.z);if(any(hit.xyz<minimum)||any(hit.xyz>minimum+u.container.xyz)){discard;}let row=rowAt(hit.xyz);if(row==INVALID){discard;}if(!rowValid(row,worldToFine(hit.xyz))){return sliceDisplay(vec3f(1.0,0.01,0.10),0.94);}let sample=faceSample(row,cameraRay(input.uv),vec2f(0.0),hit.xyz,max(hit.w*1.44/max(u.viewport.y,1.0),1e-5),false,operatorView);if(sample.a<=0.001){discard;}return sliceDisplay(sample.rgb,sample.a);}
`;
...
@fragment fn fragmentMain(input:VertexOutput)->@location(0) vec4f{let mode=i32(round(u.debug.w));if(i32(round(u.debug.x))==4){return structuredVolume(input.uv,mode);}let hit=sliceRay(input.uv);if(hit.w<=0.0){discard;}let minimum=vec3f(-0.5*u.container.x,0.0,-0.5*u.container.z);if(any(hit.xyz<minimum)||any(hit.xyz>minimum+u.container.xyz)){discard;}let row=rowAt(hit.xyz);if(row==INVALID){discard;}if(!rowValid(row,worldToFine(hit.xyz))){return sliceDisplay(vec3f(1.0,0.01,0.10),0.94);}let sample=rowSample(row,mode,false);return sliceDisplay(sample.rgb,sample.a);}
`;
...
@fragment fn fragmentMain(input:VertexOutput)->@location(0) vec4f{
  if(i32(round(u.debug.x))==4){return adaptiveVelocityVolume(input.uv);}let hit=sliceRay(input.uv);if(hit.w<=0.0){discard;}let minimum=vec3f(-0.5*u.container.x,0.0,-0.5*u.container.z);if(any(hit.xyz<minimum)||any(hit.xyz>minimum+u.container.xyz)){discard;}let row=rowAt(hit.xyz);if(row==INVALID||!leafAt(row,worldToFine(hit.xyz))){discard;}let arrow=arrowSample(row,cameraRay(input.uv),hit.w,false);if(arrow.a<=0.001){discard;}return sliceDisplay(arrow.rgb,arrow.a);
}`;
...
@fragment fn fragmentMain(input:VertexOutput)->@location(0) vec4f {
  if(i32(round(u.debug.x))==4){return lifecycleVolume(input.uv);}let hit=sliceRay(input.uv);if(hit.w<=0.0){discard;}let minimum=vec3f(-0.5*u.container.x,0.0,-0.5*u.container.z);let maximum=minimum+u.container.xyz;if(any(hit.xyz<minimum)||any(hit.xyz>maximum)){discard;}let sample=lifecycleAt(hit.xyz);if(sample.a<=0.001){discard;}return sliceDisplay(sample.rgb,sample.a);
}`;
...
@fragment fn fragmentMain(input:VertexOutput)->@location(0) vec4f {
  ...
  if(mode==31){let sample=floodProvenance(hit.xyz);if(sample.a<=0.001){discard;}return sliceDisplay(sample.rgb,sample.a);}
  if(mode==26){let sample=bandResidency(hit.xyz);if(sample.a<=0.001){discard;}return sliceDisplay(sample.rgb,sample.a);}
  if(mode==25){let sample=globalFinePhi(hit.xyz);if(sample.a<=0.001){discard;}return sliceDisplay(sample.rgb,sample.a);}let sample=fineState(hit.xyz);if(sample.alpha<=0.001){discard;}return sliceDisplay(sample.color,sample.alpha);
}`;
