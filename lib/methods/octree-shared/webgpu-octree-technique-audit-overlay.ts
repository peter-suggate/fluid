...
fn auditRow(row:u32,volume:bool)->AuditSample {
  ...
  var negative=0u;
  var invalid=0u;
  let count=min(header.count,${OCTREE_CATALOG_EXTENTS.maximumTetrahedra}u);
  ...
  if(header.count>${OCTREE_CATALOG_EXTENTS.maximumTetrahedra}u){invalid+=header.count-${OCTREE_CATALOG_EXTENTS.maximumTetrahedra}u;}
