...
/**
 * The strip's generic dial declaration, narrowed to this lane's keys.
 *
 * Only the key narrows: everything the panel renders is declared once in
 * `RuntimeDialSpec`, so the table below cannot describe a dial the strip has
 * no row for, and a dial cannot name a knob this lane does not resolve.
 */
export interface OctreeRuntimeDialSpec extends RuntimeDialSpec {
...
/**
 * Everything the live dial strip prints, derived where the semantics are.
 *
 * The panel holds all three inputs already; what it cannot hold is the rules.
 * A slider position is a request, and the number that request becomes is an
 * envelope clamp, a protection width, or a residual target — each of them a
 * Losasso coarse-band invariant that this module already owns. Deriving them
 * in a component meant the observatory restating solver arithmetic, and it
 * would have restated it for every method, including the ones that run none of
 * this machinery.
 */
export function octreeRuntimeDialReadouts(
  ...
  const builtEnvelope = info?.quadtreePressureIterationBudget ?? 40;
  // The authored band and grading are ordinary method params, so this can
  // resolve the same width the shader will compute. Leaf size 2 is the finest
  // merge candidate, i.e. the width that governs the cells around the surface
  // itself rather than the coarser rungs behind it.
  ...
  const iterationCap = octreeDialledIterationCap(builtEnvelope, dials);
  const executed = info?.quadtreePressureIterationsUsed;
  return {
    positions: Object.fromEntries(
      OCTREE_RUNTIME_DIALS.map((spec) => [spec.key, dials[spec.key]])),
    effective: {
      pressureToleranceDecades: octreeDialledRelativeTolerance(
        scene.numerics.pressureRelativeTolerance, dials).toExponential(1),
      pressureIterationCap: `${iterationCap}`,
      ...
      interfaceBandCells: `${octreeDialledSurfaceBand(
        authoredBandCells, authoredGradingLayers, authoredFineFactor, dials).widthCells}`,
    },
    executed: executed === undefined ? undefined : {
      text: `${executed} / ${iterationCap} PCG`,
      hint: "Iterations the residual gate actually executed on the last polled advance",
    },
  };
}
