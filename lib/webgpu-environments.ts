...
fn sampleEnvironmentProps(ro:vec3f,rd:vec3f)->EnvironmentPropHit{
  ...
  if(e==0){
    ...
    // water refracts furniture instead of a backplate.
    let floorY=environmentFloorY();let th=labTableHalf();let zb=labBenchZ();let ceilY=floorY+2.0*envRoomHalf().y;
    let steel=vec3f(.30,.32,.34);
    h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(0.0,-.021*s,0.0),vec3f(th.x,.019*s,th.y),vec3f(.35,.26,.17),0));
    h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(0.0,-.074*s,0.0),vec3f(th.x-.07*s,.034*s,th.y-.07*s),vec3f(.125,.135,.145),0));
    for(var i=-1;i<=1;i+=2){for(var j=-1;j<=1;j+=2){h=envNearest(h,envCylinderPrimitive(ro,rd,vec3f(f32(i)*(th.x-.10*s),floorY+.34*s,f32(j)*(th.y-.10*s)),.030*s,.34*s,steel,0));}}
    h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(0.0,floorY+.16*s,0.0),vec3f(th.x-.13*s,.013*s,th.y-.13*s),vec3f(.20,.21,.22),0));
    h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(-.35*th.x,floorY+.235*s,.15*th.y),vec3f(.15*s,.062*s,.115*s),vec3f(.10,.155,.165),0));
    ...
    h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(0.0,floorY+.42*s,zb),vec3f(1.72*s,.42*s,.30*s),vec3f(.165,.195,.215),0));
    h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(0.0,floorY+.862*s,zb),vec3f(1.80*s,.022*s,.34*s),vec3f(.54,.54,.52),0));
    h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(.95*s,floorY+.93*s,zb),vec3f(.05*s,.055*s,.05*s),vec3f(.06,.065,.07),0));
    h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(.95*s,floorY+1.17*s,zb+.05*s),vec3f(.30*s,.19*s,.014*s),vec3f(.030,.034,.040),0));
    h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(.95*s,floorY+1.17*s,zb+.068*s),vec3f(.265*s,.155*s,.004*s),vec3f(.25,.45,.58),1.0));
    h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(.95*s,floorY+.892*s,zb+.22*s),vec3f(.19*s,.007*s,.07*s),vec3f(.085,.09,.10),0));
    ...
    h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(-.20*s,floorY+1.62*s,zb-.05*s),vec3f(1.22*s,.016*s,.17*s),vec3f(.29,.30,.31),0));
    for(var i=0;i<3;i+=1){let bx=(-.92+.13*f32(i))*s;var bc=vec3f(.36,.14,.10);if(i==1){bc=vec3f(.10,.235,.255);}else if(i==2){bc=vec3f(.27,.27,.26);}h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(bx,floorY+1.751*s,zb-.05*s),vec3f(.050*s,.115*s,.135*s),bc,0));}
    for(var i=-1;i<=1;i+=2){for(var j=0;j<2;j+=1){let jz=select(-.30*s,.95*s,j==1);h=envNearest(h,envBoxPrimitive(ro,rd,vec3f(f32(i)*.95*s,ceilY-.035*s,jz),vec3f(.55*s,.012*s,.20*s),vec3f(.92,.93,.90),2.3));}}
  }else if(e==3){
...
fn labKeyLights(p:vec3f,n:vec3f,withShadow:bool)->vec3f{
  ...
  for(var i=-1;i<=1;i+=2){
    ...
    var vis=1.0;
    if(withShadow){let sh=sampleEnvironmentProps(p+w*.02*s,w);if(sh.t<dist-.08*s){vis=.16;}}
...
fn labRoomShade(albedo:vec3f,p:vec3f,n:vec3f,rd:vec3f)->vec3f{
  ...
  if(n.y>.7){
    let rr=reflect(rd,n);
    let hit=sampleEnvironmentProps(p+vec3f(0.0,.01*s,0.0),rr);
    var refl=environmentLight(rr);
    if(hit.t<1e19){refl=hit.color*(.20+hit.emission*2.0);}
