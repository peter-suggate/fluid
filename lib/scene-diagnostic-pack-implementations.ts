...
import { evaluateQuadtreeDamParityDiagnostic } from "./scene-quadtree-dam-parity-diagnostic";
...
const equilibrium = defineDiagnosticPackImplementation({
  ...
  evaluate: (context) => selected(context).flatMap(([method, diagnostics]) => {
    const info = solver(diagnostics), summary = finalSummary(diagnostics);
    const isQuadtree = method === "quadtree-tall-cell";
    const maximumDrift = parameterNumber(context,
      isQuadtree ? "maximumQuadtreeExactVolumeDrift" : "maximumTallCellExactVolumeDrift", isQuadtree ? 1e-3 : 0.01);
    const maximumComponents = parameterNumber(context,
      isQuadtree ? "maximumQuadtreeComponents" : "maximumTallCellComponents", 1);
    const findings: RuntimeDiagnosticFinding[] = [
    ...
    if (method === "tall-cell") findings.push(scalar({
      id: "stability-flags", method, actual: arrayPath(info, "stabilityFlags")?.length ?? 0,
      label: "equilibrium stability flag count", expected: { maximum: parameterNumber(context, "maximumTallCellStabilityFlags", 0) },
      pass: (value) => value <= parameterNumber(context, "maximumTallCellStabilityFlags", 0),
    }));
    if (isQuadtree) findings.push(scalar({
      id: "liquid-speed", method,
      actual: numberPath(diagnostics, "velocity", "summary", "liquidMaximum") ?? numberPath(diagnostics, "velocitySummary", "liquidMaximum") ?? numberPath(info, "maxSpeed_m_s"),
      label: "equilibrium liquid speed", expected: { maximum: parameterNumber(context, "maximumQuadtreeLiquidSpeed_m_s", 0.05) },
      pass: (value) => value <= parameterNumber(context, "maximumQuadtreeLiquidSpeed_m_s", 0.05),
    }));
    return findings;
  }),
});
...
const tallCellRestricted = defineDiagnosticPackImplementation({
  id: "tall-cell-restricted",
  requires: ["solver", "tall-cell topology", "field summary"],
  evaluate: (context) => selected(context).flatMap(([method, diagnostics]) => {
    const info = solver(diagnostics);
    const activity = recordPath(diagnostics, "field", "final", "tallCellActivity")
      ?? recordPath(diagnostics, "finalTallCellActivity");
    const gaps = recordPath(diagnostics, "field", "final", "tallVolumeGaps")
      ?? recordPath(diagnostics, "finalTallVolumeGaps");
    const maximumDrift = parameterNumber(context, "maximumExactVolumeDrift", 0.01);
    return [
      truth({ id: "grid-kind", method, actual: info.gridKind, label: "restricted tall-cell backend",
        expected: parameterString(context, "expectedGridKind", "restricted-tall-cell"),
        pass: (value) => value === parameterString(context, "expectedGridKind", "restricted-tall-cell") }),
      scalar({ id: "tall-columns", method, actual: numberPath(activity, "tallColumns"), label: "allocated tall columns",
        expected: { minimum: parameterNumber(context, "minimumTallColumns", 1) }, pass: (value) => value >= parameterNumber(context, "minimumTallColumns", 1) }),
      scalar({ id: "ordinary-columns", method, actual: numberPath(activity, "ordinaryColumns"), label: "ordinary column count",
        expected: { maximum: parameterNumber(context, "maximumOrdinaryColumns", 0) }, pass: (value) => value <= parameterNumber(context, "maximumOrdinaryColumns", 0) }),
      scalar({ id: "dry-tall-gaps", method, actual: numberPath(gaps, "dryTallWithWetRegularAbove"), label: "dry tall columns below wet cells",
        expected: { maximum: parameterNumber(context, "maximumDryTallWithWetRegularAbove", 0) }, pass: (value) => value <= parameterNumber(context, "maximumDryTallWithWetRegularAbove", 0) }),
      scalar({ id: "exact-volume-drift", method, actual: Math.abs(exactVolumeDrift(diagnostics) ?? Infinity), label: "restricted tall exact-volume drift",
        expected: { maximum: maximumDrift }, pass: (value) => value <= maximumDrift }),
    ];
  }),
});
...
const quadtreeDamParity = defineDiagnosticPackImplementation({
  ...
  requires: ["run", "solver", "field summary", "stability envelope"],
  evaluate: (context) => evaluateQuadtreeDamParityDiagnostic({
    scene: context.scene,
    evidence: context.evidence,
    parameters: {
      method: context.selectedMethods[0] ?? "quadtree-tall-cell",
      minimumSimulatedTime_s: parameterNumber(context, "minimumSimulatedTime_s", 0.2),
      maximumNeighborRatio: parameterNumber(context, "maximumNeighborRatio", 2),
      minimumLeafCount: parameterNumber(context, "minimumLeafCount", 1),
      minimumGhostFaceCount: parameterNumber(context, "minimumGhostFaceCount", 1),
      maximumPressureRelativeResidual: parameterNumber(context, "maximumPressureRelativeResidual", 1e-4),
      maximumPressureAbsoluteResidual: parameterNumber(context, "maximumPressureRmsResidual", 1e-5),
      maximumFluidScale: parameterNumber(context, "maximumFluidScale", 100),
      rebuildCadenceSteps: parameterNumber(context, "expectedRebuildCadenceSteps", 1),
      inlineRebuildCompletionFraction: parameterNumber(context, "inlineRebuildCompletionFraction", 0.9),
      maximumBlockedRebuildFrames: parameterNumber(context, "maximumBlockedRebuildFrames", 0),
      maximumWallToGpuRatio: parameterNumber(context, "maximumWallToGpuTimeRatio", 2),
      requireStabilitySampleEveryStep: parameterBoolean(context, "requireStabilitySampleEveryStep", true),
      maximumNonFiniteVelocityCount: parameterNumber(context, "maximumNonFiniteVelocitySamples", 0),
      maximumPeakLiquidSpeed_m_s: parameterNumber(context, "maximumPeakLiquidSpeed_m_s", 5),
      maximumPeakComponentCfl: parameterNumber(context, "maximumPeakComponentCfl", 1),
      maximumProjectionEnergyRatio: parameterNumber(context, "maximumProjectionEnergyRatio", 1.1),
      maximumEnvelopePressureRelativeResidual: parameterNumber(context, "maximumPressureRelativeResidual", 1e-4),
      maximumExactVolumeDrift: parameterNumber(context, "maximumExactVolumeDrift", 0.02),
      maximumCompressionRatio: parameterNumber(context, "maximumCompressionRatio", 0.25),
      minimumDominantComponentFraction: parameterNumber(context, "minimumDominantComponentFraction", 0.995),
      maximumFinalComponentCount: parameterNumber(context, "maximumFinalComponents", 10),
      minimumFront_m: parameterNumber(context, "minimumFront_m", -0.005),
      kineticGateAfter_s: parameterNumber(context, "kineticEnergyEvaluationStart_s", 0.5),
      minimumPeakKineticEnergyProxy: parameterNumber(context, "minimumPeakKineticEnergyProxy", 0.4),
      referenceMethod: context.selectedMethods.includes("uniform") ? "uniform" : undefined,
      minimumKineticEnergyRatioToReference: parameterNumber(context, "minimumUniformPeakKineticEnergyRatio", 0.8),
    },
  }),
});
...
export const sceneDiagnosticPackImplementations = Object.freeze({
  ...
  "tall-cell-restricted": tallCellRestricted,
