 ...
 * module back, which would close an import cycle. Two of the three views that
 * include this source — the cell trace and the blast-radius cone — are core
 * render passes, so the library belongs on their side of the method boundary
 * rather than on the solver's.
 */
