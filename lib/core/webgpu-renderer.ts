...
import {
  ...
  type AssembleDecorationsOptions,
...
export interface PixelTraceConfig {
   ...
   */
  readonly pinned?: boolean;
  /** A click waiting to pin must not sit behind the live-hover sampling cadence. */
  readonly urgent?: boolean;
  /** False while the pointer is still moving; an explicit pin request bypasses it. */
  readonly settled?: boolean;
}

/**
 * Hover inspection is perceptually continuous well below the presentation rate.
 * Limiting it to 12.5 probes/s avoids replaying the traced primary and copying
 * its fixed readback buffer for every one-pixel pointer move. A click waiting
 * to pin bypasses the cadence; a completed pin is frozen.
 */
export const SVO_PIXEL_TRACE_LIVE_PROBE_INTERVAL_MS = 80;

export function svoPixelTraceProbeDue(
  requestKey: string | undefined,
  config: Pick<PixelTraceConfig, "pinned" | "urgent" | "settled">,
  lastEncodedRequestKey: string,
  lastEncodedAt_ms: number,
  now_ms: number,
): boolean {
  if (requestKey === undefined || requestKey === lastEncodedRequestKey) return false;
  if (config.urgent) return true;
  // A pin is a frozen answer, including when an animated scene advances under
  // it. Staleness is UI state, not permission to start a readback every frame.
  if (config.pinned) return false;
  if (config.settled === false) return false;
...
export interface RendererFrameMetrics {
  ...
  /** GPU presentation span when timestamps are available; queue total otherwise. */
...
export class FluidLabRenderer {
  ...
  /** Last pixel/camera/scene request actually encoded, so a still pointer is free. */
  private pixelTraceEncodedRequestKey = "";
  /** Cadence boundary for transient hover probes; an outstanding pin bypasses it. */
  private pixelTraceEncodedAt_ms = Number.NEGATIVE_INFINITY;
  ...
  constructor(private readonly canvas: HTMLCanvasElement | OffscreenCanvas, private readonly statusCallback: (status: GPUStatus) => void, onGPUInfo?: (info: GPUEulerianInfo) => void, onGPURigidLoads?: (loads: GPURigidLoad[]) => void, onGPUAdvanceCompleted?: (time_s: number) => void, onEffectiveRendererStatus?: (status: EffectiveRendererStatus) => void, onGPUPressureJournal?: (journal: PressureJournal | undefined) => void, onGPUStageLens?: (receipt: StageLensReceipt | undefined, layers: readonly StageLensLayerReport[]) => void) { this.gpuInfoCallback = onGPUInfo; this.gpuPressureJournalCallback = onGPUPressureJournal; this.gpuStageLensCallback = onGPUStageLens; this.gpuRigidLoadCallback = onGPURigidLoads; this.gpuAdvanceCompletedCallback = onGPUAdvanceCompleted; this.effectiveRendererStatusCallback = onEffectiveRendererStatus; }

  /** Worker exceptions use the same terminal path as asynchronous GPU failures. */
  stopAfterFailure(error: unknown): void {
    this.onStatus({ state: "unavailable", label: error instanceof Error ? error.message : String(error), failure: error instanceof SimulationFailureError ? error.failure : undefined, resource: webGPUPlatformResourcePlugin });
  }

  /** First failure wins; no later asynchronous ready event can reopen admission. */
  private onStatus(status: GPUStatus): void {
    if (this.runtimeFailure) return;
    if (status.state === "lost" || status.state === "unavailable" || status.state === "blocked") {
      this.runtimeFailure = status.label;
      this.simulationRunning = false;
      this.gpuFluidInitializationAbort?.abort();
    }
  ...
  private ensureOptionalPipeline<T>(
    ...
    if (current || !device || this.disposed || this.runtimeFailure || this.deviceLost || this.failedOptionalPipelines.has(key) || this.optionalPipelineTasks.has(key)) return current;
    let candidate: T;
    try {
      ...
      this.stopAfterFailure(`Render pipeline ${key} failed: ${error instanceof Error ? error.message : String(error)}`);
      if (key === "svo-dry-scene") this.failPendingLiveSvoPresentation(error);
      return undefined;
    }
    const task = initialize(candidate).then(() => {
      if (this.disposed || this.runtimeFailure || this.deviceLost || this.device !== device) {
  ...
  async pickRigidBody(
    ...
    if(!fluid?.pickRigidBody||this.disposed||this.runtimeFailure||this.deviceLost)return undefined;
   ...
   *
   * A live trace clears this by itself on the next frame. A pinned one remains
   * a frozen snapshot and is labelled stale after the scene advances.
   */
  ...
  private pumpFluidCellTraceReadback(): void {
    ...
    void pipeline.read().then((trace) => {
      if (this.disposed || this.runtimeFailure || this.deviceLost || this.fluidCellTracePipeline !== pipeline || !trace) return;
  ...
  private pumpPixelTraceReadback(): void {
    ...
    void pipeline.readPixelTrace().then((trace) => {
      if (this.disposed || this.runtimeFailure || this.deviceLost || this.svoDryScenePipeline !== pipeline || !trace) return;
  ...
  private encodeDecorationOverlay(
    ...
    const assemblyOptions: AssembleDecorationsOptions = {
    ...
    // Resolve plug-in identity before running any builder. A pinned ray's
    // reveal and the camera still change frame uniforms, but its recorded work
    // does not; rebuilding all fifteen layer geometries merely to rediscover
    // the same key was the sustained CPU half of the inspection slowdown.
    const assemblyKey = decorationAssemblyKey(assemblyOptions);
    const geometryKey = `${vesselOutline?.key ?? "no-vessel"}|${assemblyKey}`;
    if (geometryKey !== this.decorationGeometryKey) {
      const assembled = assembleDecorations(assemblyOptions);
  ...
  initialize(): Promise<void> {
    ...
    if (this.runtimeFailure) return Promise.reject(new Error(this.runtimeFailure));
  ...
  private async initializeInternal(): Promise<void> {
    ...
    device.addEventListener("uncapturederror", (event) => {
      ...
      const label = `WebGPU validation failed: ${event.error.message}`;
      console.error(label);
      this.onStatus({ state: "unavailable", label, resource: webGPUPlatformResourcePlugin });
    });
    void device.lost.then((info) => {
      ...
      // Device loss is terminal. A new session requires an explicit reload.
    }).catch((error: unknown) => {
  ...
  injectLiquidBall(ball: InjectedLiquidBall): boolean {
    if (this.disposed || this.runtimeFailure || this.deviceLost
  ...
  private currentFrameMetrics(
    ...
    const presentation = instrumentation.enabled
      ...
      ? this.latestPresentationTrace
      : undefined;
    const presentationStages = presentation
      && this.latestPresentationStageTrace
      && this.latestPresentationStageTrace.sampleId === presentation.sampleId
      && this.latestPresentationStageTrace.context === presentation.context
      ? this.latestPresentationStageTrace
      : undefined;
    return {
      cpu,
  ...
  private beginGPUFluidInitialization(scene:SceneDescription,config:SimulationRunConfig,key:string,presentationMode:ScenePresentationMode){
    if(!this.device||this.disposed||this.runtimeFailure||this.deviceLost)return;
    ...
    const report=(progress:{phase:string;taskId?:string;label:string;completed:number;total:number})=>{if(this.disposed||this.runtimeFailure||this.deviceLost||generation!==this.gpuFluidRequestGeneration)return;reportedCompleted=progress.completed;reportedTotal=progress.total;this.onStatus(sparseWorldMethod
   ...
   */
  /** Preflight both consumers without submitting work or pausing the fluid clock. */
  validateLiveSolidEdit(scene: SceneDescription): void {
    const solver = this.gpuFluid;
    if (this.gpuFluidPending || this.simulationFault || this.runtimeFailure) {
      throw new Error("Wait for the scene to be ready before editing voxels.");
    }
  ...
  private settleInitialRasterPresentation(
    ...
    if (this.disposed || this.runtimeFailure || this.deviceLost || this.pendingInitialRasterPresentation !== pending
  ...
  private settleLiveSvoPresentation(pending: PendingLiveSvoPresentation) {
    if (this.disposed || this.runtimeFailure || this.deviceLost || this.pendingLiveSvoPresentation !== pending
      || this.gpuFluid !== pending.solver || this.gpuFluidGeneration !== pending.solverGeneration
  ...
  private retireGPUAdvances(completions: readonly PendingGPUAdvanceCompletion[]) {
    for (const completion of completions) {
      ...
      if (this.disposed || this.runtimeFailure || this.deviceLost || this.gpuFluid !== fluid || this.gpuFluidGeneration !== generation) continue;
  ...
  private submitPreparedGPUFluid(fluid: GPUSolverInstance, time_s: number, bodies: RigidBodyState[], maximumPendingAdvances = 1, resource?: ResourcePluginDefinition) {
    ...
    if (fluid.info.simulationPipelineError || fluid.info.topologyGenerationError) {
      this.onStatus({ state: "unavailable", label: fluid.info.simulationPipelineError ?? fluid.info.topologyGenerationError!, resource: resource ?? webGPUPlatformResourcePlugin });
      return fluid.info;
    }
  ...
  /** Keep a split pressure advance and its presentation in one worker request.
   * The accepted pressure receipt resumes this request immediately; waiting for
   * another animation frame leaves the GPU idle between simulation and rendering.
   * Scene replacement may arrive while the receipt is outstanding, so the worker
   * also supplies its retained-document validity check.
   */
  async drawWithDeferredPresentation(
    args: Parameters<FluidLabRenderer["draw"]>,
    isCurrent: () => boolean = () => true,
  ): Promise<RendererFrameMetrics> {
    const metrics = this.draw(...args);
    const solver = this.gpuFluid;
    if (metrics.presentationSubmitted || !solver?.deferredFramePublication
      || !solver.framePending || !solver.awaitFrameCompletion) return metrics;
    const generation = this.gpuFluidGeneration;
    try {
      await solver.awaitFrameCompletion();
    } catch (error) {
      if (this.disposed || this.gpuFluid !== solver || this.gpuFluidGeneration !== generation || !isCurrent()) return metrics;
      throw error;
    }
    if (this.disposed || this.runtimeFailure || this.simulationFault || this.deviceLost
      || this.gpuFluid !== solver || this.gpuFluidGeneration !== generation || !isCurrent()
      || !completedFrameAwaitsPresentation(solver, this.deferredPresentedTimes.get(solver) ?? 0)) return metrics;
  ...
  draw(time_s: number, scene: SceneDescription, camera: CameraState, bodies: RigidBodyState[], selectedBodyId: string | undefined, config: SimulationRunConfig, gridOverlay?: GridOverlayConfig, environmentId: EnvironmentId = defaultEnvironmentId, presentationMode: ScenePresentationMode = "full-scene", fluidSurfaceRenderMode: FluidSurfaceRenderMode = "shaded", svoLightingOptions: SvoLightingOptions = DEFAULT_SVO_LIGHTING_OPTIONS, svoDiagnostics: SvoRenderDiagnostics = DEFAULT_SVO_RENDER_DIAGNOSTICS, svoTuning: SvoRenderTuning = DEFAULT_SVO_RENDER_TUNING, pixelTrace?: PixelTraceConfig, fluidCellTrace?: FluidCellTraceConfig): RendererFrameMetrics {
    ...
    // Advance authored motion before sampling the scene epoch used by a trace
    // request. The old order keyed the pre-animation scene and encoded against
    // the post-animation arena one frame later.
    ...
    // A pinned ray stops probing. A live ray also stops when its target
    // pixel, camera and scene are unchanged: pointer events update a ref, but
    // presentation still runs every animation frame, and the old path turned a
    // stationary cursor into a continuous GPU readback/geometry rebuild loop.
    let pixelTraceRequestKey: string | undefined;
    let pixelTracePixel: readonly [number, number] | undefined;
    if (pixelTraceRequested && pixelTrace) {
      const width = this.presentationTexture.width, height = this.presentationTexture.height;
      pixelTracePixel = [
        Math.min(width - 1, Math.floor(Math.max(0, Math.min(1, pixelTrace.normalizedX)) * width)),
        Math.min(height - 1, Math.floor(Math.max(0, Math.min(1, pixelTrace.normalizedY)) * height)),
      ];
      pixelTraceRequestKey = `${width}x${height}:${pixelTracePixel[0]},${pixelTracePixel[1]}|${camera.azimuth_rad},${camera.elevation_rad},${camera.distance_m},${camera.target_m.x},${camera.target_m.y},${camera.target_m.z},${cameraTanHalfFov(camera)}|${this.pixelTraceSceneRevisionValue}`;
    }
    const pixelTraceNow_ms = performance.now();
    const pixelTraceProbing = pixelTraceRequestKey !== undefined && svoPixelTraceProbeDue(
      pixelTraceRequestKey,
      pixelTrace!,
      this.pixelTraceEncodedRequestKey,
      this.pixelTraceEncodedAt_ms,
      pixelTraceNow_ms,
    );
    if (pixelTraceProbing) {
      this.svoDryScenePipeline?.requestPixelTrace(
        pixelTracePixel![0], pixelTracePixel![1],
      );
    }
    ...
    if (!pixelTraceRequested) {
      ...
      this.pixelTraceEncodedRequestKey = "";
      this.pixelTraceEncodedAt_ms = Number.NEGATIVE_INFINITY;
    ...
    // Pass instrumentation forces Metal encoder breaks. Keep it out of ordinary
    // frames: one frame in twelve is enough for a stable live readout without
    // turning diagnostics into the sustained renderer workload.
    const presentationTraceSamplingFrame = measurementInstrumentationEnabled
      && this.presentationTraceFrameCounter++ % 12 === 0;
    const shouldTracePresentation = presentationTraceSamplingFrame
     ...
     *
     * The phase label is looked up from the stage id rather than written here,
     * so the encoder seam and the panel row cannot drift apart. Empty stages
     * share the next boundary and therefore measure as exact zeroes.
     */
    const closeStage = (stage: RenderFrameStageId) => {
      const phase = RENDER_FRAME_STAGE_PLUGINS[stage].phase;
      presentationTrace?.completePhase(phase);
    };
    ...
    const drySceneReplacement = (
      ...
      const replacementResult = this.svoDryScenePipeline?.encode(replacementEncoder, target, closeStage) ?? false;
    ...
    if (!disabledStages.has("fluid-coverage")) this.svoFluidCoverage?.encode(encoder);
    ...
    if (pixelTraceRequested && pixelTrace && !inspectionWithheld) {
      ...
      if (pixelTraceProbing && this.svoDryScenePipeline?.encodePixelTrace(encoder)) {
        ...
        this.pixelTraceEncodedRequestKey = pixelTraceRequestKey!;
        this.pixelTraceEncodedAt_ms = pixelTraceNow_ms;
      }
    }
    ...
    void presentationCompletion.then(async ()=>{
      if(this.disposed||this.simulationFault||this.runtimeFailure||this.deviceLost
    ...
    const presentationStageTraceRead = presentationTraceResolved && presentationTrace
      ...
      : undefined;
    const presentationTraceRead = presentationStageTraceRead
      ? presentationStageTraceRead.then((stages) => stages ?? presentationQueueTraceRead)
    ...
    if(initialLiveSvoSubmission||initialRasterSubmission){
      ...
      void presentationCompletion.then(async()=>{
        if(this.disposed||this.runtimeFailure||this.deviceLost||this.device!==presentationDevice)return;
