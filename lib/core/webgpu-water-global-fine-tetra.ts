...
${globalFineCubeContourWGSL}
fn domainCell()->vec3f{let dims=max(vec3f(p.sample.xyz),vec3f(1));return select(u.container.xyz/dims,p.sizing.xyz,all(p.sizing.xyz>vec3f(0)));}fn domainOrigin()->vec3f{return select(vec3f(-.5*u.container.x,0.,-.5*u.container.z),p.physical.xyz,all(p.sizing.xyz>vec3f(0)));}fn world(q:vec3f)->vec3f{let dims=vec3f(p.sample.xyz);let bounded=clamp(q,vec3f(.5),dims+vec3f(.5));return domainOrigin()+(bounded-vec3f(.5))*domainCell();}fn edge(x:vec3f,y:vec3f,a:f32,b:f32)->vec3f{return mix(x,y,clamp((.5-a)/(b-a),0.,1.));}fn countTet(a:f32,b:f32,c:f32,d:f32)->u32{let n=select(0u,1u,a>=.5)+select(0u,1u,b>=.5)+select(0u,1u,c>=.5)+select(0u,1u,d>=.5);if(n==0u||n==4u){return 0u;}return select(3u,6u,n==2u);}
