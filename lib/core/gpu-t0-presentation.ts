...
/** A method with a separately published sparse authority and adaptive raster
 * source must cross a presentation fence before the transport can unlock; one
 * that attaches its direct field textures atomically with the warmed solver
 * does not. The method declares which it is. */
export function requiresFencedInitialRasterPresentation(methodId: string): boolean {
  return getMethod(methodId).capabilities?.fencedInitialPresentation === true;
}
