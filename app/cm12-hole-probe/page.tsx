...
import {GLOBAL_FINE_SURFACE_EMIT_LANES,globalFineClassifiedEmitShader,globalFineClassifiedIndirectScanShader} from "../../lib/core/webgpu-water-global-fine-tetra";
...
const assert={ok:(value:unknown,message?:string)=>{if(!value)throw new Error(message??"assertion failed");}};
...
export default function Probe(){
 ...
 async function live(){
  setStatus("Creating live browser solver");const rows:unknown[]=[];let device:GPUDevice|undefined;let solver:WebGPUAdaptiveMassSolver|undefined;
  try{
   ...
   for(let step=1;step<=180;step++){
    ...
    const work=new Uint32Array(await read(device,source.worklist,source.worklist.size));
    const meta=new Uint32Array(await read(device,source.metadata,source.metadata.size));
    const samples=new Uint32Array(await read(device,source.samples,source.plan.payloadCapacityBytes));
    const phi=new Float32Array(32768).fill(NaN),width=new Uint8Array(32768);
    for(let i=0;i<work[1];i++){let page=work[7+i],key=meta[4*page+1],bx=(key&2047)-1024,by=((key>>>11)&1023)-512,bz=((key>>>21)&2047)-1024;
     for(let q=0;q<512;q++){let x=bx*8+q%8,y=by*8+Math.floor(q/8)%8,z=bz*8+Math.floor(q/64);if(x<0||x>=32||y<0||y>=32||z<0||z>=32)continue;let packed=samples[page*512+q];if(!(packed&65536))continue;let at=x+32*(y+32*z);phi[at]=unpackFineLevelSetPackedPhi(packed);width[at]=1<<((packed>>>24)&15);}
    }
    let invalidWet=0,positiveWet=0,worst=0,where=[0,0],lost=0;
    for(let z=0;z<32;z++)for(let x=0;x<32;x++){let h=0,top=0,previous=1,monotone=true;
     for(let y=0;y<32;y++){let at=x+32*(y+32*z),r=fields.density[at];h+=Math.max(0,Math.min(1,r));if(phi[at]<0)top=y+.5;if(r>previous+.02)monotone=false;previous=r;if(r>.999&&phi[at]>0)positiveWet++;if(r>.999&&!Number.isFinite(phi[at]))invalidWet++;}
 ...
 async function run(){
  setStatus("Requesting GPU");const rows:unknown[]=[];let device:GPUDevice|undefined;
  try{
   const adapter=await navigator.gpu.requestAdapter();if(!adapter)throw new Error("No adapter");
   device=await adapter.requestDevice({requiredLimits:requiredFluidDeviceLimits(adapter.limits)});
   device.addEventListener("uncapturederror",e=>{rows.push({error:e.error.message});setStatus(JSON.stringify(rows,null,2));});
   for(const step of [3,8,12,16,20,24,30,40,52,60]){
    const path=`/__cm12_holes/step-${step}/`;const plan=await(await fetch(path+"source.json")).json();
    const source:any={plan};for(const key of ["metadata","worklist","samples"]){const bytes=new Uint8Array(await(await fetch(path+key+".bin")).arrayBuffer());source[key]=buffer(device,key,bytes.byteLength,GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC,bytes);}
    for(const ratio of [0,1] as const){const result=await runField(device,`step-${step}`,ratio,()=>0,false,source);
     rows.push({step,ratio,adaptive:result.adaptive,triangles:result.metrics.triangleCount,open:result.metrics.interiorOpenEdgeCount,firstOpen:result.metrics.firstInteriorOpenEdge,topology:topology(result.mesh,result.metrics.vertexCount)});
     setStatus(JSON.stringify(rows,null,2));
    }
    for(const key of ["metadata","worklist","samples"])source[key].destroy();
   }
   setStatus("Complete\n"+JSON.stringify(rows,null,2));
  }catch(e){setStatus("Failed: "+String(e)+"\n"+JSON.stringify(rows,null,2));}finally{device?.destroy();}
 }
 return <main style={{padding:24,color:"white",background:"#111",minHeight:"100vh"}}><button onClick={live}>Run live browser mini32 diagnosis</button><button onClick={run}>Run saved mini32 mesh comparison</button><pre style={{whiteSpace:"pre-wrap"}}>{status}</pre></main>;
}
