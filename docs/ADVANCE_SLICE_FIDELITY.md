...
| Drawn readback | `advance-lab/lenses.test.ts` clips every accepted cut cell's published normal and offset against that cell's own square across four coarse-rung scenes, and checks each drawn normal points from the liquid towards the air | The published `interfaceOffset` is measured in finest cells about the cell centre, so a cell above the finest rung is more than one of them across. `buildSliceLattice` was clipping it against the unit square unscaled, which put an 8² cell's plane eight cell-widths outside its own box: a cell holding 36% liquid drew as empty, and every rung above the finest drew its surface somewhere it is not. Fixed by carrying the axis-scaled clip pair beside the unit normal; the area a cut cell draws now matches its own fraction to `6.8e-8` over 3,761 cut cells in all 82 catalog scenes. This is a readback and drawing claim, not a parity one — no transported quantity was involved |
