...
## C7. Fuse the Section 4.3 band apply into its band smooth — BUILT, NOT YET MEASURED

**What existed.** `Section43HybridPreconditioner.encodeBandSweep` encoded a
*pair*: `encodeMergedBandWorkset` (the `applyMergedBand` entry point, indirect
over the class-4 union workset, writing `operatorImage`) immediately followed by
`smoothAtoB`/`smoothBtoA`, which consumed `operatorImage[row]` for the same row.
15 pairs per correction x 11 corrections = 165 pairs = **330 encoded dispatches
per advance**, each pair carrying an inter-dispatch barrier and a round trip of
the image through device memory. Captured on the mini lane with
`--pass-timestamps --isolate-pass-labels`: `SPGrid Section 6.3 ·
destination-owned merged band` 2.719 ms/advance over **22 brackets** (0.124 ms
mean). The 22 is 2 per correction, and it brackets the smooths too — the label
opens on the apply and `runBand` passes no descriptor, so a whole run of pairs
is one bracket. That number is the pair, not the apply.

**What it is now.** `smoothAtoB`/`smoothBtoA` call `applyRowImage` inline and
the separate apply dispatch is deleted. Per correction: **30 -> 15** encoded
dispatches for the shell sweeps; `encodedCorrectionDispatchCount` 5 + 2(2k-1) +
A + V -> 5 + (2k-1) + A + V (52 -> 37 at k=8 with the production A/V). Pass
transitions are unchanged — the pair never fenced.

**Why it is sound.** Damped Jacobi reads only the source state and writes only
the destination state, so a row taking `A x` of the same source vector it
smooths from is not a hazard: no invocation reads a word another invocation
writes. Three things make it Gate A rather than a rewrite:

- **One operator, not two.** `applyRow` was split into `applyRowImage` (returns
  a `RowImage`) plus a one-line store, and everything it needs was lifted into
  `octreeSection63RowAddressWGSL` + `octreeSection63RowImageWGSL`, which BOTH
  shaders now interpolate. The shell's own prefix-loop transcription of the
  Section 6.3 addressing is deleted with it, so the band dilation and the
  operator finally walk one implementation. Verify by rendering
  `octreeSPGridAccurateOperatorShader` before and after: the only differences
  are `reportAt`->`reportRowAt`, `inputVector[i]`->`operandAt(i)`,
  `arrayLength(&inputVector)`->`operandCount()`, and the early returns yielding
  an invalid `RowImage`.
- **Same rows.** The apply ran `workRow(item,4u)`; the sweep ran `bandRow(item)`;
  both are indirect over the same union record at the same offset, and
  `bandRow`-valid already implied `workRow`-valid (the count cannot exceed the
  capacity word `compactBandIntersections` fails closed against). The fused
  `bandRow` carries both guard sets anyway.
- **Nothing else read the staged image.** `operatorImage` is private to the
  shell; its only other reader is `formInnerResidual`, which runs immediately
  after the *full* A2 apply over the accepted union. `rowClass` sends every
  published row to exactly one of four class lists, so that apply refreshes
  every row `formInnerResidual` reads; and any row `applyRow` leaves unwritten
  reported an error, which stops `formInnerResidual` outright. No stale image
  is ever consumed.

**What it cost.** `hybridA`/`hybridB` are now one buffer, A at row 0 and B at
row `rowCapacity` — the fused sweep needs the operator's topology/state/geometry
/metric ABI and lands on exactly **10 storage bindings**, the portable ceiling
`tests/webgpu-octree-section43-persistent-correction.test.ts` asserts. The
`bandB[row] == 0u` early-out in `smoothValue` is deleted: the union record
admits a row only when `bandB[row] != 0u` and `bandB` is not written between
publication and sweep, so it was never taken, and reading it would have been an
eleventh binding. Binding 7 is retired.

**MEASURE IT BEFORE BELIEVING IT.** This was built with the GPU held by another
process, so there is no wall for it. Two readings of the same 2.719 ms are
consistent with the evidence in this document and they disagree by ~5x:
item 9 says a dispatch launch is free (231 removed measured as a *regression*),
which caps the win near zero; the Instruments capture says the solve runs at
1.05 % occupancy with every limiter <= 4 %, i.e. a chain of ~24-workgroup
dispatches that drain between links, which would put the win near the smooth's
share of the bracket. Run the paired interleaved protocol on `mini churn`. If it
does not move, this belongs in "things that look like wins and are not" — the
single-copy operator and the deleted addressing transcription are worth keeping
regardless, but the fusion is then a refactor, not a speedup.
