...
| D9 | Density and velocity characteristics both use substepped midpoint/RK2 integration. Velocity traces target at most 1.5 cells per substep; when the final departure lies inside a rigid/spherical solid, the complete chord is clipped by bisection. | Declared tracing variant | This remains semi-Lagrangian transport but is more local than CM11b's single long characteristic. A chord that crosses and exits a thin solid is not detected, and terrain is not part of the velocity chord clip. |
...
| D12 | CM12's velocity-advection gap is exposed as selectable midpoint semi-Lagrangian or bounded MacCormack. An experimental liquid-only control can prevent the exterior transport field from supplying liquid momentum. | Deliberate deviation with runtime A/B | Every scene—including CM12 figure cases—defaults to unrestricted paper-style sampling. The live method toggle instead keeps the extension as the characteristic map while gathering the transported value only from `velocityIn` faces adjacent to prior liquid; no scene profile overrides the default. |
...
| # | Reference algorithm | Status | Corresponding code | Differences and gotchas |
|---:|---|---|---|---|
| 31 | Velocity advection (CM12 specification gap) | Deliberate variant | options in [`lib/methods/uniform.ts`](../../lib/methods/uniform.ts); semi-Lagrangian dispatch at [`semiLagrangianAdvection`](../../lib/webgpu-uniform-reference.wgsl.ts); phase-masked gather at [`samplePhysicalVelocityComponent`](../../lib/webgpu-uniform-reference.wgsl.ts) | Default remains semi-Lagrangian with unrestricted paper-style sampling. Enabling the experimental liquid-only toggle makes the extension guide the characteristic while interpolating the transported component only from authoritative prior-liquid faces. Stored positive/embedded wall faces and the separate negative-domain boundary values remain authoritative boundary-condition donors; a failed trace searches its chord and otherwise resolves to zero. |
...
| Profile | Code | Notes |
|---|---|---|
| midpoint semi-Lagrangian | [`semiLagrangianAdvection`](../../lib/webgpu-uniform-reference.wgsl.ts) | default; one backward characteristic per MAC component, integrated in at most 1.5-cell RK2 substeps |
...
Both apply forces exactly once. Liquid-only velocity advection defaults off for every scene, including the CM12 figure catalog, so the extension supplies the unrestricted paper-style semi-Lagrangian sample. Enabling the live uniform-method toggle instead uses the extension only for the characteristic and gathers the transported momentum value from the authoritative prior velocity field with a prior-liquid phase mask. Scenes and method profiles do not set the toggle.

Additional behavior beyond the minimal reference is:
...
- a failed liquid-only trace searches its characteristic chord for a prior-liquid `velocityIn` stencil and returns zero if none exists, rather than sampling the exterior transport value;
- closed-domain RK2 stages are clamped before every velocity lookup, and prescribed wall-face values remain in the stencil without being confused with extrapolated air;
