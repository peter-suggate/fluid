...
| `collocateAndDiagnose` + divergence partial reduction **(implemented 2026-09-01)** | The collocation invocation owns the cell divergence it immediately contributes. The diagnostic reduction is `max(abs(divergence))`; accepted-cell/workgroup mapping and the mixed-row predicate are preserved. The final singleton reduction remains separate. |
