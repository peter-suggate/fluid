# SVO shadow-field CPU oracle

The snapshot shadow-field oracle is a conservative accelerator layered on the resident
...
| --- | --- | --- | --- |
| 0 | 0 | dirty/unavailable | exact current-frame snapshot trace |
| 1 | 0 | every receiver in the page is proven clear of snapshot geometry | skip the snapshot trace |
| 0 | 1 | every receiver in the page is proven blocked by snapshot geometry | reject the light |
| 1 | 1 | mixed/uncertifiable | exact current-frame snapshot trace |
...
snapshot SVO. It may publish `occluded` only after proving every such segment is
...
`SvoShadowFieldCpuOracleCache.dirtyWork()` exposes that deterministic work list and
...
1. Allocate one storage buffer of `atlasCapacity * 8` bytes and clear it when
   the snapshot SVO generation or authored-light revision changes.
