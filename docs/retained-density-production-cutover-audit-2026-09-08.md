...
Two relevant research directions are available, without selecting either for production yet. Feng and Shang construct volume-preserving integrators by composing essentially two-dimensional Hamiltonian subflows of a divergence-free vector field. This requires an appropriate continuous divergence-free reconstruction; native pressure residuals alone do not provide it. [Feng and Shang (1995)](https://link.springer.com/article/10.1007/s002110050153).

The recent Characteristic Bending preprint instead projects a near-identity departure map using its determinant error and an additional Poisson solve. Its determinant correction is linearized, and the authors identify the global elliptic solve as the most expensive component. It is therefore a candidate to investigate for characteristic error, not evidence of exact discrete mass conservation or a free replacement for geometric integration. The reported formulation also has interpolation error. Our acceptance would still need independently integrated local/global amounts and a map residual bound. [Blomquist, Gaudreault and Theillard (2025), sections 2.4 and 3](https://arxiv.org/html/2512.06455v1).
