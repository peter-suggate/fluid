...
dawnTest("gravity changes reclassify cached pressure rows even with unchanged density and topology", async () => {
  ...
  try {
    ...
    const production = source.match(/fn compileCanonicalPressureRows\([\s\S]*?\n}/)?.[0];
...
fn main(@builtin(workgroup_id)wid:vec3u,@builtin(num_workgroups)nwg:vec3u,
 ...
 compileCanonicalPressureRows(wid,nwg,lane);
}` });
    ...
    assert.deepEqual([...new Uint32Array(readback.getMappedRange())], [0xffffffff, 0xffffffff, 0, 0]);
