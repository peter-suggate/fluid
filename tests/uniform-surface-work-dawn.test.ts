...
(modulePath?test:test.skip)("bounded correction and cached targets match original work on identical inputs through 64³ impact",{timeout:1200000},async()=>{
 await acquireWebGPUExclusiveLock("dawn-test","Uniform bounded surface correction");
 ...
 try {
  ...
  for(const surfaceCorrectionForQA of ["dense",undefined] as const){
   const solver=await WebGPUUniformReferenceSolver.createAsync(device,scene,"balanced",undefined,{
    ...uniformGeometricSolverOptions({},scene),surfaceCorrectionForQA,
    targetInterpolationForQA:surfaceCorrectionForQA === "dense" ? "uncached" : undefined,
    pressureCycleBudget:"fixed",
   },()=>{});
   solvers.push(solver);
   assert.equal(solver.info.nx,64);
  }
  assert.equal(solvers[0]!.surfaceCorrectionWorkSourceForQA,undefined);
  assert.ok(solvers[1]!.surfaceCorrectionWorkSourceForQA);
  ...
  for(let frame=1;frame<=40;frame++){
   ...
   for(const field of fields){
    ...
    // Compact reduction and shader contraction can change FP32 rounding.
   ...
   // isolates the correction operator from amplification of f32 roundoff by
