...
test("octree owner page planning halves the full-capacity owner payload and obeys byte ceilings", () => {
  ...
  const twoPageBytes = (16 + 512 + 2 * (1 + 512)) * 4;
...
function ownerLookupFixture() {
  ...
  arena[plan.pageTableOffsetWords + logical(left[0])] = 1;
  arena[plan.pageTableOffsetWords + logical(right[0])] = 2;
  arena[payload(0, left)] = packOctreeOwnerPageWord(left, [0, 0, 0], 32);
  arena[payload(1, right)] = packOctreeOwnerPageWord(right, [0, 0, 0], 32);
  arena[plan.pageTableOffsetWords + logical(24)] = 0xffff_ffff;
  arena[plan.pageTableOffsetWords + logical(32)] = plan.capacity + 1;
  arena[plan.pageTableOffsetWords + logical(40)] = 3;
  arena[payload(2, [40, 7, 7])] = (OCTREE_OWNER_PAGE_WORD_VALID | (6 << 18)) >>> 0;
  arena[plan.pageTableOffsetWords + logical(48)] = 4; // zero payload is ordinary missing air
  arena[plan.pageTableOffsetWords + logical(56)] = 6; // declared slot exists, backing words do not
