...
test("production fused transport maps exact owners through the direct identity table", () => {
  ...
  assert.match(owner,
    ...
    "the owner-page payload must provide the exact row origin and size");
  ...
  assert.doesNotMatch(shader, /fn directFind|fn directMorton|loadDirectDirectory/,
    "the fused recurring transport must not retain its obsolete binary direct-row directory");

  const neighbor = wgslFunction(shader, "directNeighbor");
  assert.match(neighbor, /directIdentityRow\(/,
    "Delaunay neighbor vectors must share the exact direct identity path");
  const descriptor = wgslFunction(shader, "sampleDirectDescriptor");
  assert.match(descriptor, /letnr=directIdentityRow\(/,
    "uniform-cube corners must share the exact direct identity path");
});

test("direct identity transport fails closed across owner and row publication clocks", () => {
  const shader = makeFineLevelSetProductionFusedTransportWGSL();
  const authority = wgslFunction(shader, "directIdentityAuthorityValid");
  assert.match(authority,
    /dp\.generation==p\.powerGeneration.*all\(dp\.dimensions==p\.dims\).*dp\.maximumLeafSize==p\.maximumLeaf.*pagedOwnerPublicationValid\(\).*owners\[7u\]==p\.powerGeneration.*bandControl\(0u\)==0u.*bandControl\(6u\)==VALID.*fusedBandGenerationValid\(\).*transitionWord\(5u\)==VALID.*transitionWord\(6u\)==VALID/s,
    "direct mapping must couple the owner, power-row, face-band, and transition authorities");
  const ownerPage = wgslFunction(shader, "ownerAtCached");
  assert.match(ownerPage, /if\(\(\*cache\)\.y==key\).*else\{.*while\(low<high\)/s,
    "the sparse owner-page ABI may search only when the trajectory enters a new 8-cubed page");
});
