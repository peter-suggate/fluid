...
(modulePath ? test : test.skip)("pressure safety rejects corrupt cycles before velocity projection", { timeout: 180_000 }, async t => {
  ...
  try {
    ...
    for (const kind of ["finite", "nan", "infinity", "after-accepted-cycle"] as const) await t.test(kind, async () => {
      fault = kind === "finite" ? "1e20" : kind === "nan" ? "bitcast<f32>(0x7fc00000u+atomicLoad(&mgConvergence[17]))" : kind === "infinity" ? "bitcast<f32>(0x7f800000u+atomicLoad(&mgConvergence[17]))"
        : "select(mgP(id)+textureLoad(mgResidualIn,id,0).x,1e20,atomicLoad(&mgConvergence[17])>0u)";
      const before = hits;
