...
test("resident sharpening consumes producer-authored air-side packets with full receiver closure", () => {
  ...
  const stageBegin = source.indexOf('stage("surface-sharpening"');
  const stageEnd = source.indexOf('stage("symmetry-authority"', stageBegin);
  ...
  assert.doesNotMatch(stage, /pressure(?:Cell|Row|Membership)/,
    "sharpening authority must never narrow or consult pressure membership");
});

test("resident face preparation preserves the dry extrapolated velocity band", () => {
  const begin = webgpuSparseCM12ResidentWGSL.indexOf("fn prepareTransportFaceRow");
  const end = webgpuSparseCM12ResidentWGSL.indexOf(
    "fn publishSparseCM12MovingSolidVelocityRoots", begin);
  assert.ok(begin >= 0 && end > begin, "face preparation must be inspectable");
  const kernel = webgpuSparseCM12ResidentWGSL.slice(begin, end);
  assert.match(kernel,
    /touchesExtendedVelocity=negative\.extended\|\|positive\.extended/,
    "the skip predicate must follow extrapolated-velocity validity into dry receivers");
  assert.doesNotMatch(kernel, /touchesTransport|rho>CM12_DRY_CELL_THRESHOLD|rowTerm|termCell/,
...
test("Sparse CM12 presentation can collapse resolved bulk to a brick-uniform sign", () => {
  const source = readFileSync(new URL(
    "../lib/methods/adaptive-mass/webgpu-sparse-cm12-resident.wgsl.ts",
    import.meta.url,
  ), "utf8");
  const begin = source.indexOf("fn cm12PresentationPreparePage");
  const end = source.indexOf(
    "fn cm12PresentationCandidateBase", begin,
  );
  assert.ok(begin >= 0 && end > begin, "presentation transaction must be inspectable");
  const transaction = source.slice(begin, end);
  assert.match(source,
    /const EXP_PRESENTATION_UNIFORM_BULK:bool=\$\{!\[[\s\S]*?structure-presentation-uniform-legacy/,
    "resolved-bulk publication must be production with an explicit legacy arm");
  assert.match(transaction, /activityReasons&\(1u\|256u\|512u\)/,
    "surface, thin-fluid, and cut-boundary bricks must retain exact samples");
  assert.match(transaction,
    /activityF32\(activityOutput\+4u\)>=CM12_LIQUID_ISOVALUE/,
    "feature-free bulk sign must come from the existing brick density reduction");
  assert.match(transaction,
    /cm12PresentationResolvedFeature!=0u&&cm12PresentationScale>1u/,
    "resolved bulk must bypass the coarse reconstruction cache");
  assert.match(transaction, /var phi=cm12PresentationUniformPhi/);
  assert.match(transaction,
    /else if\(cm12PresentationResolvedFeature!=0u&&cm12PresentationWet!=0u/,
    "only feature-bearing wet bricks may enter per-sample reconstruction");
});
