...
(process.env.WEBGPU_NODE_MODULE?test:test.skip)("GPU Dual Marching Cubes reconstructs closed surfaces and a wall missed by primal corner signs",async()=>{
 ...
 try{
  ...
  @compute @workgroup_size(1) fn emit(@builtin(global_invocation_id) id:vec3u){meshDmcExtract(vec3u(0),1u,0u,id.x,16u);}
  `;
  ...
  for(const kind of [0,1,2,3,4]){
   device.queue.writeBuffer(shape,0,new Float32Array([kind,0,0,0]));device.queue.writeBuffer(state,0,new Uint32Array(64));device.queue.writeBuffer(state,47*4,new Uint32Array([0x80000001]));
   ...
   const edges=new Map<string,number>();const winding=new Map<string,number>();let error=0,sharp=0,volume=0;
