...
(modulePath?test:test.skip)("pressure mask, balance tree and compact extension preserve fields through terrain, edits and moving bodies",{timeout:1200000},async()=>{
 await acquireWebGPUExclusiveLock("dawn-test","Uniform system and extension work");
 ...
 try {
  ...
  for(const systemBuildForQA of ["baseline",undefined] as const){
   const solver=await WebGPUUniformReferenceSolver.createAsync(device,scene,"balanced",undefined,{
    ...uniformGeometricSolverOptions({},scene),systemBuildForQA, extensionWorkForQA:systemBuildForQA,
  ...
  assert.equal((solvers[0] as unknown as {velocityExtrapolator:{compactShell:boolean}}).velocityExtrapolator.compactShell,false);
  assert.equal((solvers[1] as unknown as {velocityExtrapolator:{compactShell:boolean}}).velocityExtrapolator.compactShell,true);
  assert.equal((solvers[0] as unknown as {balanceChunks:number}).balanceChunks,0);
  assert.ok((solvers[1] as unknown as {balanceChunks:number}).balanceChunks>0,"parallel balance reduction is exercised");
  ...
  for(let frame=1;frame<=12;frame++){
   ...
   for(const field of fields){
    ...
    // on identical physical input, comparing original and reduced system/extension work.
   ...
   // isolates system and extension changes from amplification of f32 roundoff by
