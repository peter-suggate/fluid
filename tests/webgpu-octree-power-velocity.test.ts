...
test("power velocity shader contains area-weighted normal equations and guarded publication", () => {
  ...
  assert.match(octreePowerVelocityShader, /rowStatus\[row\]=ILL_CONDITIONED_FIT/);
  assert.match(octreePowerVelocityPublishShader,
    /status==ILL_CONDITIONED_FIT\)\{illConditionedCount\+=1u;errors\|=ILL_CONDITIONED;firstError=min\(firstError,row\)/,
  ...
  const encodePasses = String((WebGPUOctreePowerVelocity.prototype as unknown as {
    encodePasses: (...args: never[]) => unknown;
  }).encodePasses).replace(/\s+/g, "");
  assert.doesNotMatch(encodePasses, /clearBuffer|dispatchWorkgroups\(Math\.ceil|plan\.rowCapacity/,
    "Stage A must not clear or dispatch immutable row capacity");
  assert.match(encodePasses,
    /updateIndirectBuffer\(this\.rowDispatchControl,0,this\.rowDispatch,0,12\)[\s\S]*dispatchWorkgroupsIndirect\(this\.rowDispatch,0\)/,
    "the validated device-published live row count is the sole reconstruction launch authority");
  assert.match(octreePowerVelocityPrepareFromFaceControlShader,
    /beginDispatch\(\)[\s\S]*publishDispatch\(params\.rowCount\)/,
    "invalid prepare transactions must retain zero work while valid ones publish the compact live prefix");
  assert.match(octreePowerVelocityShader, /velocities\[row\]=vec4f\(0\.0\);rowStatus\[row\]=0u/,
    "every live row must overwrite its own payload and status without a capacity-wide clear");
  assert.doesNotMatch(encodePasses, /newPassBroker|broker\.fence/,
    "Stage A must continue in the caller-owned broker without a local-broker fallback");
...
test("Dawn reconstructs covered power velocities and rejects a query outside every catalog simplex", {
  ...
  for (const code of [octreePowerVelocityPrepareFromFaceControlShader,
    octreePowerVelocityShader, octreePowerVelocityPublishShader]) {
  ...
  const run = async (rowCount = 3) => {
    const faceControlWords = new Uint32Array(12);
    faceControlWords.set([rowCount, rows.length, rows.length]); faceControlWords[7] = 19;
    ...
    velocity.encodeFromFaceControl(broker, input, faceControl, {
      maximumIncidencePerRow: 6, generation: 19,
    });
    broker.fence("power velocity test readback");
  ...
  assert.deepEqual(firstControl, {
    ...
    incidenceCount: 12, reconstructedCount: 0, illConditionedCount: 1, generation: 19,
  });
  ...
  assert.deepEqual([...values.slice(8, 12)], [0, 0, 0, 0], "rank-deficient fit must reject the complete authority");
  ...
  assert.deepEqual(validationErrors, []);
  velocity.destroy(); faceControl.destroy(); faces.destroy(); faceNormals.destroy(); incidenceRows.destroy(); incidences.destroy(); device.destroy();
});
