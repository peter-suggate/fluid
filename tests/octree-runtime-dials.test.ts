...
test("one number drives both terms of the surface band thickness", () => {
  const at = (interfaceBandCells: number) => resolveOctreeRuntimeDials({ interfaceBandCells });
  ...
  assert.equal(coarse(4, 3, 6).gradingLayers, 2);
  assert.equal(coarse(4, 3, 7).gradingLayers, 3);
  ...
  assert.equal(octreeDialledSurfaceBand(4, 3, 4, at(3)).widthCells, 3);
  assert.equal(octreeDialledSurfaceBand(4, 3, 4, at(3)).gradingLayers, 3);
  assert.throws(() => octreeDialledSurfaceBand(Number.NaN, 1, 1, at(3)), RangeError);
  assert.throws(() => octreeDialledSurfaceBand(4, 0, 1, at(3)), RangeError);
});
