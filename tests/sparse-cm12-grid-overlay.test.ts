...
test("the structure overlay outlines support leaves without drawing their fine cells", () => {
  assert.match(gridOverlayShader,
  ...
  assert.match(gridOverlayShader,
    /if\(sparseStructureHalo\)\{[\s\S]*line=gridLinePaint\(abs\(distance-2\.5\),0\.8\);lineStrength=0\.45;/);
});
