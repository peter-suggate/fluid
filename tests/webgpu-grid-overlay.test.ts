...
test("default adaptive structure mode has no retired sparse-band overlay", () => {
  assert.doesNotMatch(gridOverlayShader,
    /SparseSurfaceParams|sparseSurfaceAvailable|sparseSurfacePayload|sparseSurfacePageState|sparseSurfaceCoreSample|octreeSurface/,
    "the grid overlay consumes only the solver's current dense presentation textures");
});
