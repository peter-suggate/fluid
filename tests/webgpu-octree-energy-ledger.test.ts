...
test("fine ledger brackets transport, topology, redistance, and volume measurement", () => {
  ...
  assert.match(source,
    /preFineTransport[\s\S]*transport\.encode[\s\S]*postFineTransport[\s\S]*publicationTopology\.encode[\s\S]*postFineTopology[\s\S]*publicationRedistance\.encode[\s\S]*postFineRedistance[\s\S]*publicationVolume\?\.encode[\s\S]*postFineVolumeMeasurement/);
  assert.match(source,
