...
async function runField(device: GPUDevice, name: string, ratio: 0|2|4,
...
for(const fullPool of (process.env.FLUID_FULL_POOL ? [true] : [false,true])) dawnTest(
  ...
  try {
    ...
    try {
      ...
      solver.applyRuntimeValues({...values,surfaceMeshRefinement:"4"});
      assert.equal(solver.globalFineLevelSetSource.surfaceMeshRefinement,4);
