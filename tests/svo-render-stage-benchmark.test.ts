...
test("render-stage plan rotates fixed-resolution GLOBAL lighting isolation variants", () => {
  ...
  assert.deepEqual(plan.runs.map(({ variant }) => variant), [
    "production", "primary-only",
    "primary-only", "production",
    "production", "primary-only",
  ]);
  ...
  assert.doesNotMatch(plan.runs[0].url, /svoTemporal/);
  assert.equal(plan.runs[1].expectedPerformanceContextFragment, "shadow-off:ao-off:gi:smooth");
