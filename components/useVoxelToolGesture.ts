...
export function useVoxelToolGesture(ray: (event: PointerEvent<HTMLCanvasElement>) => EditorRay,
  ...
  const active = useRef<{ id: number; transaction: Transaction; queued?: EditorRay;
    busy: boolean; ending?: boolean; cancelled: boolean; frame?: number; element: HTMLCanvasElement } | undefined>(undefined);
  ...
  const pump = async () => {
    ...
    try {
      ...
      if (sample && !stroke.cancelled) {
        ...
        if (result && !stroke.cancelled) showPreview(result);
      }
  ...
  useEffect(() => {
    ...
    if (active.current) endFromEffect(false); else clearFromEffect();
  }, [armed, viewportMode]);
  useEffect(() => {
    ...
    return () => {
      ...
      if (stroke) {
        ...
        void stroke.transaction.finish(stroke.cancelled).catch(noticeFromEffect);
      }
    };
  }, [session]);
  return {
    down(event: PointerEvent<HTMLCanvasElement>): boolean {
      ...
      try {
        ...
        if (transaction) {
          ...
          active.current = { id: event.pointerId, transaction, queued: initial, busy: false, cancelled: false, element: event.currentTarget };
          schedule();
        }
