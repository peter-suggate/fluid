 ...
 * turning this off" literally asks for. It is a difference of two averaged
 * totals, so it also catches whatever downstream got cheaper alongside, which
 * the node's own phase sum cannot.
 */
 ...
 */
function useStageAblation(signature: string, total_ms: number, measured: boolean): ReadonlyMap<string, number> {
  const samples = useRef(new Map<string, AblationSample>());
  const previous = useRef<{ signature: string; at_ms: number } | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    if (!last || last.signature !== signature) {
      ...
      previous.current = { signature, at_ms: now_ms };
      setDeltas(new Map());
      return;
    }
    ...
    for (const [id, on] of parseNodeStates(signature)) {
      const sample = { ...samples.current.get(id), [on ? "on_ms" : "off_ms"]: total_ms };
      samples.current.set(id, sample);
      if (sample.on_ms === undefined || sample.off_ms === undefined) continue;
      next.set(id, (on ? sample.off_ms : sample.on_ms) - (on ? sample.on_ms : sample.off_ms));
    }
    setDeltas((current) => (current.size === next.size
      && [...next].every(([id, value]) => current.get(id) === value) ? current : next));
  }, [signature, total_ms, measured]);
...
export function VisualPanel() {
  ...
  // work the product did not ask for. The panel opts in for as long as it is
  // open and hands the setting back exactly as it found it — a panel that left
  // measurement running after it closed would charge every later frame for a
  // readout nobody is looking at. It never overrides an explicit choice made in
  // the performance panel, which is why the guard is on `off` rather than a
  ...
  const timingLabel = !liveTiming ? "timing off"
    ...
    : `${total_ms.toFixed(2)} ms / frame${hardwarePerPassSplit ? "" : " · total only"}`;
  const timingHint = !liveTiming
    ...
    : !measured
      ...
      : hardwarePerPassSplit
        ? `Hardware GPU timestamps, ${TRACE_WINDOW}-frame mean. Compute-node costs are summed from their passes; Metal render-pass tiler windows are excluded.`
  ...
  const deltas = useStageAblation(pipelineSignature, total_ms, measured);

  const controls: Readonly<Record<string, ReactNode>> = {
    "sparse-world-build": <div className="pipe-fields">
      <PipeRange
        ...
        hint={sceneIsDry
          ...
          : `Zero on this scene whatever the slider says: the fluid solver claims every brick of the container and a solver brick pins its node, so the leaf stays the ${trimmed(finestCellSize_m * 1000)} mm lattice. Turn water off under Scene configuration to move on this environment-only ladder.`} />
    ...
    "primary-traversal": <div className="pipe-fields">
    ...
    "gi-composition": <div className="pipe-fields" data-testid="gi-composition-controls"
  ...
  return <aside id="render-panel" className="right-panel panel-scroll performance-panel performance-v2 visual-panel"
    aria-label="Render pipeline" data-testid="visual-panel">
    <header className="performance-panel-header render-panel-header">
      <div><span>RENDER OBSERVATORY</span><h2>Frame pipeline</h2></div>
    ...
    {/* Profile and live measurement are the two questions asked before any node
        is opened — "how expensive" and "am I looking at real numbers" — so they
        stay above the graph rather than inside a node that can be collapsed
        over them. */}
    ...
    <div className="render-preset-strip render-live-strip" role="group" aria-label="Live per-pass timing">
      <PipeToggle label="LIVE ms" checked={liveTiming} onChange={setLiveTiming}
        hint={`Continuously records GPU pass timestamps with one readback in flight, then reads each trustworthy node cost from them. Measurement is real work, so it stops when this is off and when the panel closes.\n\n${measured
    ...
    <RenderPipeline
      context={context}
      durations={durations}
      total_ms={stageTotal_ms}
      measured={perPassSplit}
      exhaustive={stageTrace?.measurementSource !== "gpu-pass-timestamp"}
      stageView={svoStageView}
      onToggle={toggleNode}
      onTap={(view: SvoRenderStageView) => setSvoStageView(view)}
      controls={controls}
      deltas={deltas}
    />

    <div className="render-preset-strip render-probe-strip" role="group" aria-label="Pointer probes">
      <span>PROBES</span>
      <PipeToggle label="Ray" checked={pixelTraceEnabled} onChange={setPixelTraceEnabled}
        hint="Draws what the frame actually did to produce one pixel: the octree walk, the shadow rays and the cones. Click the viewport to freeze a pixel and orbit its work; click again to follow the pointer." />
      {/* The viewport owns which pixel and which view a pin records, so this
          asks for one rather than declaring one from out here. */}
      <PipeToggle label="Pin ray" checked={pixelTracePinned} disabled={!pixelTraceEnabled}
        onChange={(on) => (on ? requestPixelTracePin() : setPixelTracePinned(false))} />
      <PipeToggle label="Cell" checked={fluidCellTraceEnabled} onChange={setFluidCellTraceEnabled}
        hint="Names the pressure cell behind the hovered pixel and reads its row, stencil and power neighbours from what the frame published." />
      <PipeToggle label="Pin cell" checked={fluidCellTracePinned} disabled={!fluidCellTraceEnabled}
