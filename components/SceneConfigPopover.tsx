    ...
    <section className="scene-popover" role="dialog" aria-label="Scene configuration" data-testid="scene-settings-modal" style={{ transform: `translate(${offset.x}px, ${offset.y}px)` }}>
      ...
      <div className="popover-body">
        ...
        <section>
          ...
          <div className="field-grid">
            <NumberField label="Fixed dt" unit="s" value={scene.numerics.fixedDt_s} step={0.001} min={0.0005} onChange={(value) => patchNumerics({ fixedDt_s: Math.max(0.0005, value) })} />
            <NumberField label="Max dt" unit="s" value={scene.numerics.maxDt_s} step={0.001} min={0.0005} onChange={(value) => patchNumerics({ maxDt_s: Math.max(0.0005, value) })} />
          ...
          <small className="control-hint">Fixed dt drives the rigid clock and CPU oracle; max dt caps each GPU advance. The PCG budget and relative tolerance ({scene.numerics.pressureRelativeTolerance.toExponential(0)}) bound the CPU reference projection; GPU pressure effort is set per method.</small>
        </section>
      </div>
      <footer>
