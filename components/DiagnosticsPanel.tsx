  ...
  return (
    <aside className="right-panel panel-scroll diagnostics-panel">
      ...
      <section className="metric-grid panel-section">
        ...
        <MetricCard label={gpuInfo?.gridKind === "quadtree-tall-cell" ? "GPU quadtree tall cells" : gpuInfo?.gridKind === "uniform" ? "GPU uniform grid" : "GPU tall grid"} value={gpuInfo ? `${gpuInfo.nx} × ${gpuInfo.storedNy} × ${gpuInfo.nz}` : "initializing"} unit={gpuInfo ? `${gpuInfo.ny} cubic-equivalent Y · ${((gpuInfo.activeCompressionRatio ?? gpuInfo.compressionRatio) * 100).toFixed(0)}% active` : undefined} tone={backend === "webgpu" ? "good" : "neutral"} />
        ...
        <MetricCard label="Global correction" value={gpuInfo?.volumeControl ? `${gpuInfo.volumeCorrectionNormalSpeed_cells_s?.toFixed(2) ?? "0.00"}` : "None"} unit={gpuInfo?.volumeControl ? `cells/s normal speed · ${gpuInfo.phiInterfaceCellCount?.toFixed(0) ?? "—"} interface cells` : "pairwise conservative face flux"} tone={gpuInfo?.volumeControl ? "neutral" : "good"} />
