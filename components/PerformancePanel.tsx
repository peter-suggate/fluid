...
/** Live CPU/GPU profiler rendered in the shared right-side utility panel. */
export function PerformancePanel() {
  ...
  // Only signed-distance transport is subdivided; pressure and coupling run
  // once for the full frame delta.
  const substeps = contextMatches && method.backend === "webgpu" ? gpuInfo?.lastSubsteps ?? 1 : 1;
  const substepLabel = `${substeps}× phi · sampled`;
  ...
  const rebuildSparseGPU_ms = contextMatches ? gpuInfo?.quadtreeGPUSparsePack_ms ?? 0 : 0;
