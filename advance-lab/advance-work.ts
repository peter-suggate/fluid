...
export const ADVANCE_DISPATCH_KINDS = {
  ...
  member: { label: "cell membership words", workgroups: m => groups(m.cells / 32) },
  rowMember: { label: "row membership words", workgroups: m => groups(m.rows / 32) },
...
export interface AdvanceKernel {
  ...
  /** Indirect, and zeroed after the first limiter pass of each microstep. */
  readonly microstep?: boolean;
    ...
    seams: [
      {
        ...
        kernels: [
          ...
          kernel("initializeContinuousGeometricSource", "cell", { gate: "inflow" }),
  ...
  "pressure-topology": {
    ...
    seams: [
      ...
      },
      {
        ...
        note: "Classify every capacity word, prefix its live bits, and publish stable ascending pressure-cell ids.",
        kernels: [
          kernel("classifyFullPressureCellWords", "member"),
          kernel("scanFullPressureCellWords", "one"),
          kernel("publishFullPressureCellIds", "member"),
        ],
      },
      {
        id: "pcm-row-publication",
        note: "Classify and prefix every row-membership word for the same complete image.",
        kernels: [
          kernel("classifyFullPressureRowWords", "rowMember"),
          kernel("scanFullPressureRowWords", "one"),
        ],
      },
      {
        id: "pca-fine-publication",
        note: "Publish pressure coefficients once over the accepted-cell worklist.",
        kernels: [
          kernel("publishFullPressureCoefficients", "cell"),
        ],
      },
      {
      ...
      {
        id: "pei-publication",
        note: "Seal the complete pressure image, then publish its compact cell count to the indirect argument buffer.",
        kernels: [
          kernel("sealFullPressureImage", "one"),
          bufferCopy("pressure-image cell indirect → pressure arguments"),
        ],
      },
      {
        id: "ptr-commit-and-prepare-pressure",
        note: "Close the journal against the full image, reopen it for later topology producers, and build per-cell pressure terms.",
        kernels: [
          kernel("finalizeFullPressureTopologyJournal", "one"),
          kernel("beginSparseCM12PressureTopologyRepair", "one"),
    ...
    seams: [
      {
        ...
        kernels: [
          ...
          kernel("compileSparseCM12VelocityExtensionSchedule", "leaf"),
          kernel("sealSparseCM12VelocityExtensionSchedule", "one"),
          bufferCopy("projected packet schedule → transport indirect arguments"),
          kernel("initializeVelocityExtensionPackets", "vex"),
          kernel("advanceVelocityExtensionPackets", "vex", { repeats: 8 }),
        ],
      },
    ],
  },
...
export function advanceSeamCost(
  ...
  return seam.kernels.reduce((total, entry) => {
    ...
    /* commit and first-pass kernels are encoded in every packet; indirect gates
       execute them once per microstep. */
    const workgroups = stage.loop === "transport" && (entry.commit || entry.microstep)
