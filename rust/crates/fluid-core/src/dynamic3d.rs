//! Exact 3-D moving capacity, face geometry and inflow source plan.
//!
//! Static restriction follows `refreshSparseCM12SolidWorldCell/Row` and the
//! moving/source terms follow `geometric-solid-motion.wgsl.ts` and
//! `geometric-source.wgsl.ts`. The GPU row ABI describes a square face by its
//! static area, so its `round(sqrt(area))` sampling is retained here, including
//! on a clipped row; changing that requires a coordinated topology ABI change.
use crate::rigid::RigidBodyState;
use crate::scene_model::{FluidInflow, PhysicalScene, SceneModelError, Vec3};
...
#[derive(Clone, Debug)]
pub struct DynamicGeometry3dInput<'a> {
...
pub struct DynamicGeometry3d {
    ...
    pub inflow_velocity_fine: [f32; 3],
...
impl From<DynamicGeometry3d> for crate::dynamic::DynamicGeometry {
    fn from(value: DynamicGeometry3d) -> Self {
        Self {
            capacity: value.capacity,
            capacity_before: value.capacity_before,
            capacity_after: value.capacity_after,
            capacity_rate: value.capacity_rate,
            source_rate: value.source_rate,
            open_fraction: value.open_fraction,
            open_fraction_before: value.open_fraction_before,
            open_fraction_after: value.open_fraction_after,
            mean_open_fraction: value.mean_open_fraction,
            solid_velocity: value.solid_velocity,
            inflow_coverage: value.inflow_coverage,
            inflow_velocity_fine: [value.inflow_velocity_fine[0], value.inflow_velocity_fine[1]],
            requested_source_area_fine: value.requested_source_area_fine,
            source_rate_area_fine: value.source_rate_area_fine,
            source_available_area_fine: value.source_available_area_fine,
            source_factor: value.source_factor,
            source_component_count: value.source_component_count,
            source_anchored_component_count: value.source_anchored_component_count,
        }
    }
}
...
fn world_point(frame: VolumeFrame, p: [f32; 3]) -> Vec3 {
    Vec3 {
        x: frame.origin_m[0] + p[0] as f64 * frame.cell_size_m,
        y: frame.origin_m[1] + p[1] as f64 * frame.cell_size_m,
        z: frame.origin_m[2] + p[2] as f64 * frame.cell_size_m,
    }
}
fn static_fraction(graph: &Graph, frame: VolumeFrame, q: [i32; 3]) -> f32 {
    let [nx, ny, _nz] = frame.dimensions;
    if (0..3).any(|a| q[a] < 0 || q[a] >= frame.dimensions[a] as i32) {
        return 0.0;
    }
    let i = q[0] as usize + nx as usize * (q[1] as usize + ny as usize * q[2] as usize);
...
fn compact_capacity(
    ...
    for c in &graph.cells {
        let mut sum = 0.0;
        let mut count = 0;
        for z in (c.minimum[2].floor() as i32)..(c.maximum[2].ceil() as i32) {
            for y in (c.minimum[1].floor() as i32)..(c.maximum[1].ceil() as i32) {
                for x in (c.minimum[0].floor() as i32)..(c.maximum[0].ceil() as i32) {
                    let center = Vec3 {
                        x: frame.origin_m[0] + (x as f64 + 0.5) * h,
                        y: frame.origin_m[1] + (y as f64 + 0.5) * h,
                        z: frame.origin_m[2] + (z as f64 + 0.5) * h,
                    };
                    ...
                    sum +=
                        (1.0 - static_fraction(graph, frame, [x, y, z]) as f64) * (1.0 - maximum);
                    count += 1
                }
            }
        }
...
fn static_row_open(
    ...
    let tangent = [0, 1, 2]
        ...
        .filter(|&a| a != axis)
        .collect::<Vec<_>>();
    let side = (area as f64).sqrt().round().max(1.0) as i32;
    let starts =
        [tangent[0], tangent[1]].map(|a| (center[a] as f64 - 0.5 * side as f64).round() as i32);
    let mut open = 0.0;
    for v in starts[1]..starts[1] + side {
        for u in starts[0]..starts[0] + side {
            let mut q = [0; 3];
            q[axis] = plane - 1;
            q[tangent[0]] = u;
            q[tangent[1]] = v;
            let a = static_fraction(graph, frame, q);
            q[axis] = plane;
            let b = static_fraction(graph, frame, q);
            open += 1.0 - a.max(b) as f64
        }
    }
    open / (side * side) as f64
}
fn dynamic_rows(
    graph: &Graph,
    frame: VolumeFrame,
    ...
    for row in &graph.rows {
        ...
        let tangent = row.measure.sqrt() as f64 * h;
        let tangents = [0, 1, 2]
            .into_iter()
            .filter(|&a| a != row.axis as usize)
            .collect::<Vec<_>>();
        let (mut covered, mut speed) = (0.0, 0.0);
        for sv in [-0.25, 0.25] {
            for su in [-0.25, 0.25] {
                let mut xyz = [center.x, center.y, center.z];
                xyz[tangents[0]] += su * tangent;
                xyz[tangents[1]] += sv * tangent;
                let p = Vec3 {
                    x: xyz[0],
                    y: xyz[1],
                    z: xyz[2],
                ...
                if let Some(b) = bodies.iter().find(|b| contains(b, p).unwrap_or(false)) {
                    ...
                    speed += 0.25 * [v.x, v.y, v.z][row.axis as usize] / h
                }
            }
        }
            ...
            'outer: for ns in [-0.4, 0.4] {
                for tv in [-0.4, 0.4] {
                    for tu in [-0.4, 0.4] {
                        ...
                        let mut xyz = [center.x, center.y, center.z];
                        xyz[row.axis as usize] += ns * normal;
                        xyz[tangents[0]] += tu * tangent;
                        xyz[tangents[1]] += tv * tangent;
                        let p = Vec3 {
                            x: xyz[0],
                            y: xyz[1],
                            z: xyz[2],
        ...
        velocity[row.id as usize] = (if covered > 0.0 {
            ...
            [v.x, v.y, v.z][row.axis as usize] / h
        } else {
            0.0
        }) as f32
    }
...
pub(crate) fn average_strength(i: &FluidInflow, a: f64, b: f64) -> f64 {
    if b <= a {
        return 0.0;
    }
    let mut p = vec![
...
pub fn compute_dynamic_geometry_3d(
    input: &DynamicGeometry3dInput<'_>,
) -> Result<DynamicGeometry3d, SceneModelError> {
    ...
    out.inflow_velocity_fine = [
        ...
        (inflow.velocity_m_s.z * strength as f64 / h) as f32,
    ];
    out.requested_source_area_fine =
        (speed * std::f64::consts::PI * inflow.radius_m.powi(2) * strength as f64 / h.powi(3))
            as f32;
    ...
    for row in &input.graph.rows {
        if row.axis as usize != axis || mean[row.id as usize] <= 0.0 {
            continue;
        }
    ...
    for c in &input.graph.cells {
        ...
        let radial = (r.x - axial * h * direction.x)
            ...
            / h;
        let minw = c.widths[0].min(c.widths[1]).min(c.widths[2]) as f64;
...
mod tests {
    ...
    #[test]
    fn clipped_row_retains_gpu_square_area_sampling() {
        let mut graph = Graph::default();
        graph.dimension = 3;
        graph.dimensions = [2.0, 3.0, 1.0];
        graph.solid_voxel_fraction = vec![1.0; 6];
        let frame = VolumeFrame {
            origin_m: [0.0; 3],
            cell_size_m: 1.0,
            dimensions: [2, 3, 1],
        };
        // The GPU ABI carries area but no two tangent widths. Its source
        // rounds sqrt(3) to 2x2; one stencil row lies beyond clipped z.
        assert_eq!(
            static_row_open(&graph, frame, 0, [1.0, 1.5, 0.5], 3.0, 1.0),
            0.5
        );
    }
}
